...
This is the project's single north star (re-anointed 2026-06-28 around the **type-priority principle** — `02_foundations/FOUND_TYPE_PRIORITY_PRINCIPLE.md`, FTD-0339, `[SYNTHESIS]` / adopted commitment: *a context — a type — is prior to, and the precondition for, the value of any content — a token; content cannot bootstrap context*). The earlier wording — *"derive everything we can from a discrete ontology, and rigorously establish what we cannot"* — is **preserved in full**, now read through that principle: *deriving* is building the content a set of types makes meaningful; *establishing the boundary* is marking, exactly, which types the ontology cannot set for itself. The algebraic spine, the engine, the pre-registered tests, the LEDGER, and the manuscripts each serve this goal or are subordinate to it.
