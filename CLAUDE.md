...
This is the project's single north star, and it rests on one principle — **type-priority** (`02_foundations/FOUND_TYPE_PRIORITY_PRINCIPLE.md`, FTD-0339): a *context* (a type) is prior to, and the precondition for, the *value* of any *content* (a token); content cannot bootstrap context. The goal has three inseparable faces (amendment of record 2026-07-12, FTD-0383 — the consumption drive added to the two 2026-07-05 faces). To **derive** is to build the content a set of types makes meaningful — a strict chain from the postulates to a `[THEOREM]` or `[DERIVED]` result. To **establish the boundary** is to mark — and to *price* — rigorously, which types the discrete ontology cannot set for itself and must instead import: to name each imported type, count it in a common currency, and attach its falsifier (the priced-import ledger, `01_reference/SPEC_IMPORT_LEDGER.md`, FTD-0371). To **drive** is to treat every priced line as a standing work item, never a resting state: its falsifier gets *run*, its retirement path gets *attacked* under a fresh pre-registered lock, or its unreachability gets *proven* at the tag the proof earns — and where retirement provably fails, the deliberate ontology-extension search (`01_reference/SCOPE_CONSUMPTION_PROGRAM.md`, the FC-W process template) may adopt the next honest type at a minimal, falsifiable, priced cost; an adoption is registered as an adoption, never presented as a derivation. The algebraic spine, the engine, the pre-registered tests, the LEDGER, and the manuscripts each serve this goal or are subordinate to it.
...
- **"A discrete ontology"** — FTD's five postulates: an **uncontained, undefined-boundary discrete lattice** (neither a bounded container nor a completed-infinity totality; no primitive continuum); discrete time; ternary states {−1, 0, +1}; local Moore-neighbour causality; determinism. These, with the Framework Commitments (FC-0…FC-W), are the **types** — the chosen context, *set first*, that the dynamics presuppose. The ternary cubic lattice is the current concrete model; discreteness and uncontained undefined-boundary adjacency are non-negotiable. **Finitude belongs to specified or realized configurations and computations — the cloud/star-like extent or support of what is present — not to a wall around the substrate.**
...
- **"Drive every priced line / search for the next honest type"** — the consumption face (amendment of record 2026-07-12; program charter `01_reference/SCOPE_CONSUMPTION_PROGRAM.md`, FTD-0383). Consumption is a **direction, not an achievability claim** — the doctrine ledger's own note stands ("closure may not be achievable without ontology extension beyond the 5 axioms"), and the adversarial audit of record found the strong "zero-import theory" reading **unestablished either way** (FTD-0339 is an adopted commitment, not an impossibility theorem; no known program has derived categorical physics from logical necessity alone). The coherent maximal form is: minimize and price toward an **audited floor relative to the declared currency**, conditional on named external results (Chudnovsky 1976 proven; E1/E2 open). A priced line that is never fired at is the F10 failure mode ("a falsifier attached is not a falsifier run") — the drive face exists to prevent exactly that.
...
**Operational test** for any claim, paper, or experiment: does it *set a type* honestly (a declared, minimal, falsifiable commitment), *build content forward* from the set types (`[THEOREM]`/`[DERIVED]`), *mark and price a boundary* (a type the ontology must import, counted in the ledger's currency with its falsifier), or *drive a priced line* (run its falsifier, attack its retirement path under a fresh lock, prove its no-go, or convert it via a declared minimal adoption) — or is it a match/import still awaiting one of those verdicts? The Epistemic Discipline rules and tag system (below) are *how* the goal is pursued; this goal is *what* is pursued. The standing discipline admits no exceptions: `x₊=1/α` is a `[STRONGLY MOTIVATED CONJECTURE]`, MC-T4.3 is a `[FOUNDATIONAL OBSTRUCTION]`, and the type-priority principle that anchors this goal is itself an *adopted commitment open to external critique*, not a theorem.
...
Full rules live in the **`doc-cleanup` skill** (`.claude/skills/doc-cleanup/SKILL.md`) — invoke it before any archive, move, rename, or status-change of a theory document. Two rules bind at all times, skill loaded or not:

- **Preserve provenance; move, do not erase.** Archive superseded/retracted/closed-negative docs with `git mv`, never delete, unless the user explicitly asks for deletion.
- **Do not promote claims during cleanup.** Cleanup may clarify status, archive provenance, and align links; it must never upgrade an epistemic tag or introduce a new derivation without a separate audit.
...
**Five postulates:** Discrete space (an uncontained 3D cubic lattice with no defined boundary — at every specified position, axis-adjacent sites exist; neither a bounded container nor a completed-infinity ℤ³ totality, per `AUDIT_INFINITY_REFRAME.md`), discrete time (ticks), ternary states, local causality (26-neighbor Moore), determinism.

**Foundational commitment:** FTD uses **uncontained, undefined-boundary** lattice ontology, not a bounded container and not a completed-infinity totality. Realized structures and computations may have finite extent or support. Finite periodic boxes are computational quotients/probes, not the ontology. Claims of the form "in the L → ∞ limit" are not well-posed without explicit finite-support, decay, or ε-L restatement. See `docs/theory/07_assessment/AUDIT_INFINITY_REFRAME.md` for the full triage of which claims survive, which need restatement, and which need re-derivation.

**Key results** (within framework assumptions):
- Fine structure constant: master quadratic `x² − 16G*²x + 16G*³ = 0` has x₊ = 137.036 matching 1/α to **1.26 ppm** at tree level (pure algebra [THEOREM]; physical identification of `x₊  1/α` [STRONGLY MOTIVATED CONJECTURE] per `AUDIT_MASTER_QUADRATIC.md`; **there is no structural-uniqueness evidence — WITHDRAWN 2026-08-03, FTD-0791**. The FTD-0319 adversarial look-elsewhere scan (distinct from FTD-0189, the graviton-audit id) was cited here as showing the master quadratic to be the unique dual-matcher across 2.65M degree-2 polynomials. It was audited refute-by-default and **sits at the chance base rate**: the null expects **1.42–1.67** matchers at the registered gate and it found **1**, so uniqueness is the expected outcome under no structure; at that gate the second leg eliminates nothing. Retagged `[MEASURED]` → `[SELECTION]`; **the scan-rigid count is ZERO**. Never cite 137.036 as evidence for the framework without the base rate attached). The polynomial's smaller root `x₋ ≈ 3.024` is a mathematical artifact of the quadratic; the **identification `x₋  N_c` is RETIRED** per FTD/FQCR Cleanup Taxonomy v1.4 §5 (FTD-0014 was removed from the LEDGER in commit `ca7eb61`). `N_c = 3` comes from independent structural sources — see the Moore Layer Theorem and `DERIV_NC_FROM_TOPOLOGY.md` below. The 7-term series matching CODATA to 24 digits is a post-hoc fit [CONJECTURE] beyond experimental precision (CODATA 2022 has ~11 digits), not a "< 0.001 ppt derivation"
...
- Confinement from area-law Wilson loops at x₋ (σ = 0.209) — claim of record: LEDGER FTD-0025, [MEASURED at an inserted coupling [SELECTION]]; [THEOREM-within-compact-U(1)-LGT framework, PARAMETRIC at FTD-substrate level] (compact-U(1) gauge framework imported, β = x₋ inserted). The engine Wilson-loop battery `benchmark_wilson_loops.cpp` was DELETED 2026-05-03 (`e8eb8e82`, superseded confinement physics) — historical only
...
**Honest accounting:** **18** derived/theorem-grade claims, **133** parametric insertions (FTD values in standard QFT formulas), **11** imposed/selected — reconciled 2026-07-26 against the catalog's own category rows, which now sum to their TOTAL (18+133+11 = 162). The earlier "~21 / ~131" here and "~23 / ~129" at the catalog pointer below were two incompatible spellings of the same table. ~50+ external physics adopted. 50 physics tests pass across three tiers (see clarification above — reproduction-of-fit, not independent prediction). Test suite (denominators re-measured 2026-07-26): **544 CTest targets registered** (`ctest -N`), **263 Python tests collected** (`pytest --collect-only`), **54/54 master verification pass**. ⚠ The CTest and pytest figures are REGISTRATION counts, not pass counts — the suites were not run for this line, and at least one target (`gauss_law_fidelity`) is known RED at HEAD, asserting a sign the Term-2 amendment `748256f1` already corrected. The retired "211/211 CTest pass" understated the denominator by ~2.6x and asserted a pass rate nobody had measured. See [EPISTEMIC_AUDIT.md](docs/theory/07_assessment/AUDIT_EPISTEMIC_AUDIT.md) and [CATALOG_PARAMETRIC_INSERTIONS.md](docs/theory/07_assessment/CATALOG_PARAMETRIC_INSERTIONS.md).

**Engine-theory bridge:** 20-benchmark suite connects engine output to theory. Coulomb 1/r^2 converges (B+), hydrogen 1/n^2 < 0.001% (A+; **classical Kepler check, NOT a quantum eigenvalue derivation** — generic to any 1/r force; see `AUDIT_ATOMIC_DYNAMICS_STATUS.md`), color forces correct (A+), Higgs threshold exact (A+), Bell S=2.000 (**N/A as a QM grade, re-graded 2026-07-01** — this is the *classical* local-hidden-variable bound, measured by a standalone LHV toy that never touches the lattice engine; it confirms the substrate is local/classical, the opposite of a QM confirmation), Born lattice bias 10x (A-). EFT reconstruction: alpha = G_C^2 (an algebraic identity by construction — a consistency check, not a derivation of alpha; see `constants.h` HONEST FRAMING note). Wilson loops (historical — the 12/17 flux-tube battery `benchmark_wilson_loops.cpp` asserted superseded confinement physics and was DELETED 2026-05-03, `e8eb8e82`; confinement of record is LEDGER FTD-0025), gluon dynamics (historical — the 7/11 linear-E(r) battery `campaign_gluon_dynamics.cpp` was consolidated into `campaign_qcd_forces.cpp` (`d1e4a056`) then DELETED 2026-05-03, `e8eb8e82`; its checks presumed a derived α_s — of record: LEDGER FTD-0020 [STRUCTURALLY MOTIVATED PARAMETRIC] + FTD-0025), budget equation (0.2% at r=6). LATENCY FIX unlocked GR: time dilation 0.004% match, BH gravitational wells L_peak=0.62. Three theorem papers: continuum limit -> QED, singlet from void event, N_c from topology. WASM rebuilt and deployed. Scientific status: C+ -> B+.
...
| N_c (colors) | 3 | Moore Layer Theorem + `DERIV_NC_FROM_TOPOLOGY.md` (4 independent routes). **NOT** the master-quadratic root x₋ — that identification is RETIRED (FTD-0014 removed in `ca7eb61`) |
...
| C_SPEED | 1/√3 | **[SELECTION]** (FTD-0407) — not CFL-forced; the production 18-point stencil permits c ≤ √3/2 ≈ 0.866 |
...
- ** Doctrine ledger (single-page status map, FTD-0145 [SYNTHESIS])**: `docs/theory/01_reference/SPEC_DOCTRINE_LEDGER.md` — 14 sections + non-circularity audit + compressed roadmap. Roll-up across LEDGER + TRACKER_ONTIC_TRUTH + SPEC_ALGEBRAIC_SPINE + SPEC_FQCR + SPEC_OPEN_MATH_BY_SECTOR. **Introduces no new theorems**; every claim points at a canonical source. Ledger-to-canonical tag map in §0.2. Read BEFORE planning hardening arcs or answering "what is the status of claim X" questions.
- **Priced-import ledger (FTD-0371 [SYNTHESIS])**: `docs/theory/01_reference/SPEC_IMPORT_LEDGER.md` (rendered) + `docs/theory/01_reference/import_ledger.json` (data) + `scripts/proofs/proof_import_ledger.py` (verifier 8/8). The Number-One-Goal "mark the boundary" face made **quantitative**: every imported type priced in a common currency with a falsifier each — **1 adopted bit** (FC-W / the δ branch), **4 selected types** (D=3, singlet, ℭ generator-set, and IMP-S4's gauge-connection carrier choice — global `A_μ=𝒫_TJ_μ` or the FTD-0417 independent local-link branch), **4 named results** (Chudnovsky proven; CM-h=1 / E1 / E\*/E\*\* open), **3 calibrations** (a_phys / t_phys / K_B), the **empirical bridges** (x₊=1/α [SMC] + ~131 [PARAMETRIC] + ~50 external), **2 declined** (M via FC-1, reversibility via FC-2). IMP-S4 was minted 2026-07-12 and broadened 2026-07-22 without changing the count. The import surface = the *argument* half of the modulus/argument frontier (FTD-0336); the self-set column = the *modulus* half. ⚠ **The "1 adopted bit" is the α-sector branch choice ONLY — never cite it as FTD's total physics import** (the reading guard). Cite for "what does FTD import / what's the price of claim X". RF-1 (D=3 status drift) **RESOLVED 2026-07-05** — constitution §1.4 + §3.2 read [SELECTION — declared] (FTD-0355); verifier C7 asserts the resolved state. Introduces no theorem; promotes nothing.
- **Consumption Program charter (FTD-0383 [SCOPE / PROGRAM CHARTER])**: `docs/theory/01_reference/SCOPE_CONSUMPTION_PROGRAM.md` — the Number-One Goal's **drive face** operationalized: Front A (retirement campaign — fire at every priced line via its own falsifier, fresh locks only), Front B (P6C-\* ontology-extension search on the FC-W pipeline; FC-1/FC-2 routed here — bets stand unless a candidate earns reversal), Front C (forcing-proof + semantics: [SELECTION]→[THEOREM] attempts + MFO-1..4 formalization), Front D (sector-consumption of the ~131 [PARAMETRIC] rows — native functional form, priced no-go, or sharpened falsifier). First scheduled deliverable: **the δ-IND v2 nonlinear pre-registration (authorized 2026-07-12)**. ⚠ Binding: **consumption is a direction, not an achievability claim**; wall grades cited at their tags of record (charter §0); an adoption is never a derivation.
- **Unified Axiom Register (FTD-0386 [SYNTHESIS], Unification Annex Stage U1)**: `docs/theory/01_reference/SPEC_UNIFIED_AXIOM_REGISTER.md` (rendered) + `unified_axiom_register.json` (data) + `scripts/proofs/proof_unified_register.py` (verifier 11/11) — the axiom system in force assembled once (5 postulates · 5 FCs · 4 selected types · 3 calibrations · 4 named results · 4 dynamics posits ACT-1/CLK-1/CLK-2/MDL-1) + the Unified Conditional Statement U-1..U-12 (what the register licenses, per-row at tags of record; the α row carries the [CONDITIONAL THEOREM given W] + [SMC] both-stand split; Newton g₀₀-only, g_rr RETRACTED per FTD-0361). ⚠ **FTD-U names the Stage-U3 deliverable only — zero P6C purchases adopted; the register licenses no GR, no QCD dynamics, no Born, no cosmology.** Cite for "what axioms is FTD actually running on / what does the axiom system license".
- **Adoption Pricing Rules (FTD-0387 [SCOPE / METHODOLOGY — DRAFT], Unification Annex Stage U0)**: `docs/theory/01_reference/SPEC_ADOPTION_PRICING_RULES.md` (rendered) + `adoption_pricing.json` (data) + `scripts/proofs/proof_adoption_pricing.py` (verifier 11/11) — the rules that gate every Stage-U2 purchase, **awaiting owner ratification at D5/D6/D7**: D5 currency (stratified vector; bit-equivalent for the predicate only; calibration floor excluded), D6 compression predicate (adopt A only if y>c in distinct weighted gap-classes; [PARAMETRIC] weight 0; y/c is a declared heuristic, not an info-theoretic ratio), D7 FC-W calibration (c=1/y=1 marginal point anchors the floor — calibration not adjudication, moves no tag). ⚠ **Ratifies nothing, adopts nothing**; the illustrative candidate ordering is non-binding (surfaces the yield-individuation + sector-scope design questions the owner must settle at D6). Cite for "how is an axiom priced / what would a purchase have to clear to be adopted".
...
- **Parametric insertions catalog**: `docs/theory/07_assessment/CATALOG_PARAMETRIC_INSERTIONS.md` (162 rows enumerated: 18 [DERIVED]/[THEOREM], 133 [PARAMETRIC], 11 [IMPOSED]/[SELECTION])
...
- **Master claim ledger**: `docs/theory/07_assessment/core_ledgers/LEDGER.md` — **747 claim rows** (re-counted 2026-08-04; highest id FTD-0801) with tag history, dependencies, reframe status. The retired "52 load-bearing claims" invited readers to treat the single-source-of-truth ledger as a short table. **Single source of truth for claim status** — papers cite tags from here; if they disagree, the ledger wins. Rows FTD-0050/0051/0052 (Link 8 closure + Langevin infrastructure + deferred s-Metropolis).
- **Ledger index (read this to search the ledger)**: `docs/theory/07_assessment/core_ledgers/LEDGER_INDEX.md` — generated, one line per claim, all 747 rows grouped into 19 research programmes with their epistemic tags. LEDGER.md is ~1.9 MB and cannot be read whole; this index can, so **check it before opening a question, to see whether it has already been investigated**. Navigation aid only — introduces no claim and moves no tag; where it and the LEDGER disagree, the LEDGER wins. Regenerate with `python scripts/theory/build_ledger_index.py`; `scripts/tests/test_ledger_index.py` fails if it drifts.
...
- **Wilson loops / confinement (of record)**: LEDGER row FTD-0025 (`docs/theory/07_assessment/core_ledgers/LEDGER.md`) — σ = 0.209 at [MEASURED at an inserted coupling [SELECTION]], [THEOREM-within-compact-U(1)-LGT framework, PARAMETRIC at FTD-substrate level]. The engine battery `benchmark_wilson_loops.cpp` (12/17, flux tube) was DELETED 2026-05-03 (`e8eb8e82`, superseded confinement physics) — historical only
- **Gluon dynamics (historical)**: the engine battery `campaign_gluon_dynamics.cpp` (7/11 pass, linear E(r)) was consolidated into `campaign_qcd_forces.cpp` (`d1e4a056`) and DELETED 2026-05-03 (`e8eb8e82`) — its checks presumed a derived α_s (of record: LEDGER FTD-0020, α_s = 7/59 [STRUCTURALLY MOTIVATED PARAMETRIC]) and a substrate confinement signal (CLOSED NEGATIVE per LEDGER FTD-0025)
...
- 20-benchmark scorecard (re-graded 2026-07-01, see `analyze_convergence.py`): grades listed under "Engine-theory bridge" above
