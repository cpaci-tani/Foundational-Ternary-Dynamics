...
int main() {
    ...
    ftd::native_desktop::UiSnapshot first;
    first.seq = 1;
    ...
    ftd::native_desktop::UiSnapshot second = *held;
    second.seq = 2;
    second.frame.tick = 4;
    ...
    ftd::native_desktop::SnapshotPublisher stress;
    std::atomic<bool> running{true};
    std::shared_ptr<const ftd::native_desktop::UiSnapshot> retained;
    std::uint64_t retained_checksum = 0;
    std::thread writer([&] {
        for (std::uint64_t i = 1; i <= 2000; ++i) {
            ftd::native_desktop::UiSnapshot snap;
            snap.seq = i;
