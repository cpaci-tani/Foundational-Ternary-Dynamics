...
int main() {
    ...
    const auto s1 = queue.push(ftd::native_desktop::SetToggle{"symplectic_leapfrog", true});
    const auto s2 = queue.push(ftd::native_desktop::SetDt{0.5});
    const auto s3 = queue.push(ftd::native_desktop::SetToggle{"symplectic_leapfrog", false});
    const auto s4 = queue.push(ftd::native_desktop::SetDt{0.7});
    ...
    ftd::test::check("first toggle is on",
                     std::get<ftd::native_desktop::SetToggle>(items[0].command).value);
    ftd::test::check("first dt is 0.5",
                     std::get<ftd::native_desktop::SetDt>(items[1].command).dt == 0.5);
    ftd::test::check("second toggle is off",
                     !std::get<ftd::native_desktop::SetToggle>(items[2].command).value);
    ftd::test::check("second dt is 0.7",
                     std::get<ftd::native_desktop::SetDt>(items[3].command).dt == 0.7);

    ftd::test::section("view/request commands keep last write at last position");
    ftd::native_desktop::CommandQueue view;
    view.push(ftd::native_desktop::InspectVoxel{1, 2, 3});
    view.push(ftd::native_desktop::SetToggle{"damping", true});
    view.push(ftd::native_desktop::InspectVoxel{4, 5, 6});
    view.push(ftd::native_desktop::RequestField{ftd::VisualFieldKind::State, 2});
    view.push(ftd::native_desktop::RequestField{ftd::VisualFieldKind::FluxVector, 4});
    ...
    ftd::test::check("toggle remains between coalesced view commands",
                     std::holds_alternative<ftd::native_desktop::SetToggle>(
                         coalesced[0].command));
    const auto voxel = std::get<ftd::native_desktop::InspectVoxel>(coalesced[1].command);
    ftd::test::check("last InspectVoxel is retained at its original later position",
                     voxel.x == 4 && voxel.y == 5 && voxel.z == 6
                     && coalesced[1].seq == 3);
    const auto field = std::get<ftd::native_desktop::RequestField>(coalesced[2].command);
