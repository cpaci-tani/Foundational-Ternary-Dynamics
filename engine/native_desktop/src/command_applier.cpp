...
void process_ui_boundary(ftd::RenderBridge& bridge, NativeEngineSession* session,
    ...
    bridge.bind_sim_thread();
    const int apply_tick = bridge.current_tick();
    ...
    for (const auto& item : items) {
        ...
        const auto applied =
            apply_mutation_on_bridge(bridge, session, item, *journal, apply_tick,
                                     state.loop);
        if (applied.ok) state.last_applied_seq = item.seq;
    }
    if (session) session->set_loop_control(state.loop);

    bridge.backend().flush_host_mutations();
    ...
    for (const auto& item : observations) {
        const auto observed = observe_on_bridge(bridge, item.command, snapshot, state);
    ...
    if (state.did_tick && state.scheduler) {
        state.scheduler->on_tick_complete(bridge);
        (void)state.scheduler->pump(bridge);
    }
    ...
    build_snapshot(bridge, state.scheduler ? &cached : nullptr, state.demand, snapshot);
