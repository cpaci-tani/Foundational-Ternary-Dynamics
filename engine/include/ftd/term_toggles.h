...
inline constexpr ToggleSpec TOGGLE_SPECS[] = {
    ...
    {"de_broglie_clock",   &TermToggles::de_broglie_clock,   false, false, "",                 "",                 "", ToggleBackend::CPU, "[IMPOSED] de Broglie internal clock: Klein-Gordon mass term -omega0^2*J at manifested voxels (FTD-0271). CPU-only: the GPU phase_read kernel omits the -omega0^2*J KG term, so the clock never fires on native CUDA (mirrors db_clock_coulomb). Independent of wave_propagation: with the wave term the full KG dispersion omega^2=c^2 k^2 + omega0^2 acts; alone, each manifested voxel is the k=0 rest-frame clock oscillating at omega0."},
