...
struct Voxel {
  ...
  // Persistent particle identity (monotonically increasing, assigned at genesis)
  // Transferred during movement, cleared on evaporation/annihilation.
  // -1 = no particle.
  ...
  // DEPRECATED: always zero in logic-first engine (v2.0).
  // Noetic fields reserved for Phase 3 consciousness extension.
  // Kept for Qt GUI binary compatibility.
  ...
  double gamma_ftd() const {
    double bw = bandwidth_used();
    if (bw >= 1.0)
      return 1e30; // bandwidth overflow
