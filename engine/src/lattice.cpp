...
Lattice::Lattice(int size) : size_(size), total_(static_cast<int64_t>(size) * size * size) {
...
int Lattice::index(int x, int y, int z) const {
    return static_cast<int>(static_cast<int64_t>(wrap(x)) * size_ * size_
                            + wrap(y) * size_ + wrap(z));
}
...
void Lattice::build_neighbor_tables() {
    ...
    for (int64_t idx = 0; idx < total_; ++idx) {
        auto [x, y, z] = coord(static_cast<int>(idx));
