...
constexpr std::size_t N = CHARGED_TRIMER_SIZE;
...
bool site_projection_valid(const ChargedTrimerState& state) {
...
bool charges_valid(const ChargedTrimerState& state) {
  int positive = 0;
  int negative = 0;
  int sum = 0;
  for (int charge : state.charges) {
    if (charge == +1) ++positive;
    else if (charge == -1) ++negative;
    else return false;
    sum += charge;
  }
...
bool state_valid(const ChargedTrimerState& state,
                 const std::vector<double>& stationary_density,
                 const ChargedTrimerOptions& options) {
  const std::size_t expected = volume(state.electric.L);
  ...
  if (!std::all_of(stationary_density.begin(), stationary_density.end(),
...
std::array<Vec3, N> positions(const ChargedTrimerState& state) {
...
std::array<Vec3, N> binding_impulses(
    const std::array<Vec3, N>& x0,
    const std::array<Vec3, N>& x1,
    const ChargedTrimerOptions& options) {
  std::array<Vec3, N> impulses{};
  for (std::size_t a = 0; a < N; ++a) {
    for (std::size_t b = a + 1; b < N; ++b) {
...
PreparedForwardFields prepare_forward_fields(
    const ChargedTrimerState& earlier, double lambda) {
...
void gather_impulses(Candidate& candidate,
  ...
  const auto x0 = positions(candidate.earlier);
  const auto x1 = positions(candidate.later);
  candidate.binding_impulses = binding_impulses(x0, x1, options);
  for (std::size_t a = 0; a < N; ++a) {
...
Candidate evaluate_forward(const ChargedTrimerState& earlier,
                           const ChargedTrimerOptions& options,
  ...
  for (std::size_t a = 0; a < N; ++a) {
    ...
    const Vec3 x0 = effective_position(earlier.constituents[a]);
    candidate.later.constituents[a] = point_at(
        x0 + candidate.velocities[a] * options.dt, L, p1[a]);
  }
...
Candidate evaluate_reverse(const ChargedTrimerState& later,
                           const ChargedTrimerOptions& options,
                           double interaction_scale,
                           const VectorN& unknown) {
  ...
  for (std::size_t a = 0; a < N; ++a) {
    ...
    candidate.velocities[a] = (p0[a] + p1)
        * (C_SPEED * C_SPEED / denominator);
    const Vec3 x1 = effective_position(later.constituents[a]);
    candidate.earlier.constituents[a] = point_at(
        x1 - candidate.velocities[a] * options.dt, L, p0[a]);
  }
  if (!make_segments(candidate)) return candidate;
...
struct RootResult {
  Candidate candidate;
  ChargedTrimerSolveDiagnostics diagnostics{};

  explicit RootResult(int L) : candidate(L) {}
};

RootResult solve_root(
    int L, const VectorN& initial, const ChargedTrimerOptions& options,
...
double aggregate_continuity_residual(
  ...
  const std::size_t count = volume(L);
  std::vector<double> rho0(count, 0.0);
  std::vector<double> rho1(count, 0.0);
...
std::vector<double> aggregate_density(
    const std::array<QuadraticCoatFaceCurrent, N>& segments,
    const std::vector<double>& stationary, bool later) {
  std::vector<double> result = stationary;
  for (const auto& segment : segments)
    add_density(result, later ? segment.rho_after : segment.rho_before);
  return result;
}

Vec3 sum_momentum(const ChargedTrimerState& state) {
  Vec3 result{};
  for (const auto& constituent : state.constituents)
    result += constituent.momentum;
  return result;
}

void measure_pair_distances(ChargedTrimerStepResult& result) {
  result.minimum_pair_distance = INFINITY;
  result.maximum_pair_distance = 0.0;
  for (const auto* state : {&result.earlier, &result.later}) {
    const auto x = positions(*state);
    for (std::size_t a = 0; a < N; ++a)
      for (std::size_t b = a + 1; b < N; ++b) {
        const double distance = (x[a] - x[b]).mag();
        result.minimum_pair_distance = std::min(
            result.minimum_pair_distance, distance);
        result.maximum_pair_distance = std::max(
            result.maximum_pair_distance, distance);
      }
  }
}

ChargedTrimerStepResult finalize(const RootResult& root, bool forward,
                                 const std::vector<double>& stationary,
                                 const ChargedTrimerOptions& options,
                                 const FaceFluxNormalization& normalization) {
  ChargedTrimerStepResult result;
  ...
  const auto density0 = aggregate_density(result.segments, stationary, false);
  const auto density1 = aggregate_density(result.segments, stationary, true);
  ...
  result.matter_momentum_before = sum_momentum(result.earlier);
  result.matter_momentum_after = sum_momentum(result.later);
  ...
  result.field_pseudomomentum_after = matched_local_translation_momentum(
      ...
      * result.interaction_scale;
  result.pseudomomentum_defect = result.matter_momentum_after
  ...
  const bool finite_values = std::isfinite(result.root_residual)
      ...
      && std::isfinite(result.minimum_pair_distance)
      && std::isfinite(result.maximum_pair_distance);
  result.valid = result.solve.converged && finite_values
      && result.site_projection_valid;
...
ChargedTrimerStepResult solve_charged_trimer_forward(
  ...
  if (!normalization.valid || !(normalization.mapped_field_work_coefficient > 0.0)
      || !state_valid(earlier, stationary_density, options))
    return ChargedTrimerStepResult(0);
  const double lambda = options.wave_speed * options.dt;
  ...
  return finalize(root, true, stationary_density, options, normalization);
}
...
ChargedTrimerStepResult solve_charged_trimer_reverse(
  ...
  const FaceFluxNormalization normalization = measure_face_flux_normalization();
  if (!normalization.valid || !(normalization.mapped_field_work_coefficient > 0.0)
      || !state_valid(later, stationary_density, options))
    return ChargedTrimerStepResult(0);
  const auto evaluate = [&](const VectorN& unknown) {
  ...
  return finalize(root, false, stationary_density, options, normalization);
}
