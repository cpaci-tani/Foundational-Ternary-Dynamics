...
NineSourceOrbitCoherenceResult analyze_nine_source_orbit_coherence() {
  NineSourceOrbitCoherenceResult result;
  ...
  for (const RemovalTimeOrbitCoherenceVolume& spectral : parent.volumes) {
    NineSourceOrbitCoherenceVolume volume;
    ...
    for (int removed = 0; removed <= 9; ++removed) {
      const long double remaining =
          ...
          * std::sqrt(static_cast<long double>(9 - removed));
    ...
    volume.nine_source_orbit_bound = static_cast<double>(maximum_bound);
    volume.nine_source_margin = K_GENESIS - volume.nine_source_orbit_bound;
    volume.all_partition_bounds_finite = finite;
    volume.nine_source_closed = maximum_bound
        < static_cast<long double>(K_GENESIS);
    volume.valid = spectral.valid && finite
        && maximizing_removed >= 0 && maximizing_removed <= 9
        && std::isfinite(volume.nine_source_margin);
    all_finite = all_finite && finite;
    all_closed = all_closed && volume.nine_source_closed;
    result.volumes.push_back(volume);
    ++result.spectral_volume_count;
  }

  result.all_partition_bounds_finite = all_finite;
  result.arbitrary_removal_n_le_nine_closed =
      result.parent_orbit_analysis_valid && all_finite && all_closed;
  result.nine_source_bound_inconclusive =
      result.parent_orbit_analysis_valid && all_finite && !all_closed;
