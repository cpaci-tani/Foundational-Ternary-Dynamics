...
P1 Observables now declares Scenario 1 explicitly inapplicable. The selection-
intent boundary stops its 4 Hz coordinator before the replacement engine
generation commits, unmounts all eight experiment cards and their local state,
and makes public/manual updates inert before any bridge, particle-list, or
field-volume access. The focused nonempty → Empty → nonempty race/restoration
test passes. Its isolated worker-backed `L = 97` campaign records 60.00 FPS,
p95/p99 16.67 ms, zero periodic callbacks, zero DOM/canvas work, and zero
collapsed work. The static boundary states that Empty prepares none of the
sources or experiments required by these observables and is not evidence for
physical vacuum, zero-point, or Standard Model observables.

Knots now declares Scenario 1 explicitly inapplicable rather than reporting a
fabricated zero-detector result. Selection intent synchronously stops panel and
contribution coordinators, clears detector/energy histories, disables all
field extraction and streamline jobs through a runtime-owned applicability
predicate, and keeps the retained tracking and knot-box preferences separate
from their effective state. This boundary remains fail-closed with the panel
absent. Scenario reset clears the actual renderer mesh and draw range, qualified
nonempty restoration reapplies only the effective retained request, disabling
tracking suppresses boxes and scheduler work without erasing that request, and
an all-empty live detector frame clears the final rendered box when the last
detected clump disappears. Dock, tab, floating-window, collapse, and re-dock
transitions revoke contribution reads synchronously; in-flight completions
recheck both applicability and live contribution demand before reading or
publishing. The focused transition/visibility/renderer test and all three knot
scheduler regressions pass. The final isolated worker-backed `L = 97` campaign
records 60.00 FPS with p95/p99 16.67 ms, zero periodic callbacks, zero DOM/canvas
...
3. generic Time, Thermodynamics, and Dispersion surfaces need explicit `empty`
   applicability states. P1 Observables and Knots are resolved: each now exposes
   one static inapplicable boundary and performs no Empty scientific work;
