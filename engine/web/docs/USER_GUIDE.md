...
- Nuclear/isotope scenarios use **composite dynamical nuclei** (single particle with total Z and approximate A·m_p mass) rather than locked multi-nucleon sculptures. Nuclear binding is **not** emergent at this tier — color-singlet hadrons do not feel the strong force between each other.
- `pe-w-pair` enables relativistic-Verlet momentum integration plus the full force stack where toggled.

**Forces in the browser.** The JS ParticleEngine now mirrors the C++ force kernel: **Coulomb**, **Newtonian gravity** (G_PE), **Pauli exchange**, **strong/Yukawa** (colored quarks only), **magnetic dipole**, **spin–orbit**, **Lorentz**, **radiation reaction**, and a coarse **isotropic relativistic correction** — all toggle-gated. `peGetBackendCapabilities().advancedForces === true`. Initial orbit speeds in atom/pair scenarios are computed from **instantaneous force balance** at t=0 (`peApplyEquilibriumOrbit`); multi-body systems (helium, positronium, Δ⁺⁺, micro-BH ring) use a **batch pass** after all bodies are placed so every particle sees the full N-body Coulomb/gravity field. Scattering scenarios still set explicit approach velocities as boundary conditions. Scale 1 remains **classical N-body** dynamics, not quantum Bohr eigenstates.

**Particle appearance (Scale 1).** Fixed **boundary volume** per species (mass + `r_eff`). Shader-driven **void ↔ manifest** blink (~40% bright / 60% ghost, constant balance). **Orbital rotation**: halo slots sweep around **L = r × v** (classical ω = |L|/mr²). **Intrinsic spin** (catalog ±½ → engine |S|=1) biases manifestation phase and shows a **purple spin-axis arrow** whose direction **precesses** when **Mag. Dipole** or **Lorentz** is on (`dS/dt = (q/m) S×B` from partner dipoles). **Magnetic dipole** (`μ = (q/m)S`) and **spin–orbit** (L·S) forces are toggle-gated; try scenario **Hydrogen (spin + dipole demo)** for a preset with tilted spins. **Color charge** tints quarks. Enable **Velocities** / **System** overlays for **v** and total **L** vectors. Not QM |ψ|².

**Particle masses.** Scale 1 masses are the **measured (PDG)** rest masses (the electron uses the FTD anchor m_e = 0.511 MeV). FTD's *predicted* mass formulae and their accuracy live in the **Particle Zoo** (FTD-Formula / Acc. columns); they are motivating matches, not the simulated mass and not first-principles derivations. Lepton ratios are FTD-0015/0016-family strongly-motivated conjectures; neutrinos and most hadrons/bosons are parametric.
