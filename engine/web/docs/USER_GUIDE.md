...
**Forces available in the browser.** The shipped in-browser (JS) ParticleEngine computes only **Coulomb**, **Newtonian gravity**, and **velocity damping**. The advanced force terms — exchange, strong/Yukawa, magnetic-dipole, spin–orbit, Lorentz, radiation reaction, and the relativistic correction — exist only in the **C++/WASM** backend; in the pure-JS dashboard they are inert no-ops (`peGetBackendCapabilities().advancedForces === false`). A scenario that names one of them (e.g. `pe-w-pair`'s relativistic correction) therefore runs as a Coulomb + gravity demo in the browser. Where the relativistic correction *is* available (WASM/native), it is a coarse **isotropic 1/γ force rescaling**, not a covariant integrator — use the relativistic-Verlet path for momentum-correct dynamics.
