...
| `s0-cell-*` | 12 | Flux cells (bounded field reservoirs): a Gauss-charged plate capacitor, an azimuthal ring reservoir with reversed / sign-scrambled / open-box / walled-box controls, a three-axis standing-arm cell, and a membrane family (a locked shell made opaque by the de Broglie clock's mass gap, with a scheduled discharge port, a two-cell transfer through a shared port, and every-tick and resonant flux-pump charge cycles). Storage, hold, pump work, and port flux are measured (`test_flux_cell_scenario_physics`), never assumed |
...
- **`s0-cell-torus`** — A ring of azimuthal flux released on the bare periodic wave map with zero canonical momentum. Watch the Poynting overlay: the electric channel (`E = -wave_vel`) starts at zero and fills from the magnetic channel (`B = curl J`) while the kick-drift Hamiltonian stays constant. Compare with `s0-cell-torus-open` (Dispersal boundary: the energy leaves the box) and `s0-cell-torus-walled` (Reflective boundary: it stays). The bare wave map has no confinement term, so the ring disperses; the retention numbers in the scenario description are measurements, not a battery claim.
- **`s0-cell-capacitor`** — Two locked plates of opposite polarity with no seeded field. Tick once: the engine's Gauss law `div(J) = s` builds the gap flux by itself. The stored channel is `U_J` (the flux-potential channel), not `½|E|²`.
- **`s0-cell-triad`** — Three orthogonal standing arms through the centre. Net Poynting flow and net flux vector are zero by construction and the flux dyad is isotropic; this is a zero-momentum reservoir, not a matter clock.
- **`s0-cell-torus-membrane`** — The same ring inside a locked shell of matter with the de Broglie clock on. The clock gives the shell a mass gap, so the ring’s sub-gap modes cannot propagate through it: the inner energy is retained (1.00 of its start after 300 ticks versus 0.36 for the bare ring). Set `omega0` to 0.05 in the config knobs and the same shell turns transparent. `s0-cell-torus-membrane-gated` opens a hole in the +x wall at tick 150 and the Flux Cell panel shows the energy leaving through it (`Port work W_out`); `s0-cell-membrane-pumped` starts empty and the flux pump fills it over 20 ticks (`Pump work W_in`), then disconnects.
- **`s0-cell-membrane-transfer`** — Two walled cells touching along x; the left one holds the ring. At tick 100 a hole opens through both walls at the contact point and the Flux Cell panel (registered on the right-hand cell) shows energy arriving. `s0-cell-membrane-pumped-resonant` spaces the pump’s 20 increments 8 ticks apart, the constructive spacing found by scanning the booked work; spacing them 3 ticks apart delivers a tenth as much.
...
### Scale 1 — particle context and effective references (2026-09-01 revision)

Scale 1 is a registry-driven particle context backed by the native C++/WASM `ParticleEngine`. One scenario selector exposes all 34 executable rows; there is no second workspace dropdown. Selecting a scenario automatically activates its declared read-only native observer, effective particle engine, or parametric catalog owner. Non-executable roadmap placeholders have been removed from the selector. Runtime scale handoff is retired; use the Scale Context sidepanel for pedagogical comparison.

Runnable native observations preserve their Scale-0 provenance and do not manufacture a Standard Model species, physical mass, spin, or particle-statistics claim. Effective point-particle scenes and Particle Zoo records are explicitly imposed or **[PARAMETRIC]** comparison instruments; they are not promoted lattice objects and are not evidence that their laws or catalog identities emerged from the substrate.

In the **Particle Zoo**, each Category or Generation heading is a collapsible disclosure section. Its open/closed state survives searches, filters, and grouping-mode changes for the current dashboard session. The catalog fills the available dock or detached-window height and scrolls inside its own contained viewport.

**QED reference scenarios.** Seven executable scenes expose the parts the current effective engine can actually render: static Coulomb response, Møller-style and Bhabha-style Coulomb deflection, imposed magnetic-dipole interaction, Lorentz response to a dipole field, imposed spin-orbit coupling, and an incomplete radiation-reaction sink. These are classical/effective QED-facing forms. The scenario contract card names the expected observable and the prohibited inference for every scene. Compton scattering, electron-positron annihilation, pair production, vacuum polarization, and loop observables such as g-2 and the Lamb shift are not registered scenarios because Scale 1 has no photon state, scattering-amplitude owner, field-to-pair transaction, renormalized propagator, or loop-functional owner. Existing g-2 and Lamb-shift values are **[PARAMETRIC]** insertions into standard QED formulas, not Scale-1 derivations.

**Quantum reference scenarios.** Twelve executable controls isolate the current exchange, spin-orbit, magnetic-dipole, Lorentz, radiation, relativistic-integrator, and three-color demonstration terms. Matched positive/null or orientation-reversal scenes make selector and sign behavior visible. These tests qualify the implemented effective kernels only: they do not supply a wavefunction, exchange statistics, quantized spin, photon/amplitude state, QFT, or QCD. The three-color scene uses neutral integer-charge records because the current particle record cannot represent fractional quark charge.

**Physics controls.** Scale 1 exposes the eleven available native-registry physics rows directly in the **Controls** panel beside numerical parameters. The retired isotropic rescale is retained only as a fail-closed native registry record and is not rendered. Native-matter observations remain read-only. The **Scenario**, **Verified**, and **All applicable** profile buttons restore the registered scenario mask, select only the two verified-baseline rows, or enable all eleven implemented kernels respectively. “All applicable” includes explicitly imposed/quarantined effective terms and never enables the retired rescale. A visible profile chip marks manual and non-scenario changes. The selected scenario's behavior, validation evidence, source/artifact revision, object provenance, and prohibited inference live in the collapsed **Scenario details** disclosure in Controls rather than in Diagnostics. Diagnostics is reserved for snapshot and time-series telemetry: static provenance rows are omitted, while active unavailable-claim telemetry remains beside the state-energy coverage ledger. Every diagnostics row exposes a hover explanation of its meaning, source, and history columns.

**Interaction Hierarchy and Particle Log.** Scale 1 exposes these as separate, full-height sidepanels. **Interaction Hierarchy** lists every currently published particle record beneath its instantaneous cluster and reports the energy-weighted center plus the member nearest that center (the “energy anchor”) for each cluster and the full scene. The display weight uses current kinetic energy plus the magnitude of net force over a local interaction length; a completely dormant scene falls back to mass weighting and says so. These links are observer-derived presentation relationships and never feed the force solver. **Particle Log** is the chronological event ledger for seeds/spawns, unexplained removals, native selected contact removals, significant particle and system energy changes, close approaches, cluster splits/merges, anchor changes, scenario/backend/coverage changes, and state-energy completeness transitions. Lifecycle, energy, hierarchy, interaction, and environment categories can be hidden independently. The ledger retains the latest 4096 significant records until **Clear log** is pressed.

**Viewport overlays.** The Scale-1 overlay is a presentation-only layer selector with bounded scrolling, equal-width controls, an active-layer count, and explicit field/force/anatomy groups. Force decomposition now visualizes every native channel published by the engine: Coulomb, gravity, Lorentz, exchange, strong, magnetic dipole, spin-orbit, radiation reaction, and net force. Their distinct colors and arrows qualify renderer wiring only; they do not upgrade the underlying effective term's epistemic status.

**Forces.** Available native kernels include softened Coulomb, Newtonian gravity, damping, Lorentz response, selected exchange/strong demonstration terms, magnetic dipole, spin-orbit, radiation reaction, contact-event selection, and the momentum-form relativistic Verlet integrator. Each scenario applies an exact registered profile; the old isotropic non-covariant relativistic rescale is retired and cannot be enabled. Quarantined terms expose incomplete energy coverage instead of reporting a false conservation result. The r=35 sphere is a visual reference shell, not a boundary wall.
...
**Particle masses.** Qualified native observation records expose mass only when their source artifact owns a valid mass observer; the current live/native candidate boundary does not. Zoo particles carry **measured (PDG)** rest masses (electron = FTD anchor m_e = 0.511 MeV); FTD mass-formula matches shown with them retain their ledger status and are not promoted to derivations.
