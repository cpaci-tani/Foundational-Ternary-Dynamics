...
test('source contract pins token fences, lifecycle observability, and cache generations', () => {
    ...
    assert.match(loader, /wasm-bridge-proxy\.js\?v=\d+/);
    assert.match(controller, /scenario-loader\.js\?v=\d+/);
    assert.match(controller, /bindings\.js\?v=\d+/);
    assert.match(controller, /limits\.js\?v=\d+/);
    assert.match(controller, /gravity-panel\.js\?v=\d+/);
    assert.match(app, /viewport\.js\?v=\d+/);
    assert.match(app, /scale0\/controller\.js\?v=\d+/);
    assert.match(app, /gravity-panel\.js\?v=\d+/);
    assert.match(app, /app-shell\.js\?v=\d+/);
    assert.match(app, /bridge-boot\.js\?v=\d+/);
    assert.match(bridgeBoot, /bridge-init\.js\?v=\d+/);
    assert.match(bridgeBoot, /ws-bridge\.js\?v=\d+/);
    assert.match(bridgeInit, /ws-bridge\.js\?v=\d+/);
    assert.match(gravityPanel, /gravity-analysis\.js\?v=\d+/);
    assert.match(viewport, /flux-renderer\.js\?v=\d+/);
    assert.match(index, /app\.js\?v=\d+/);
    assert.match(index, /capabilities\/scale0\.js\?v=\d+/);
    assert.match(index, /sampler-want-set\.js\?v=\d+/);
    assert.match(index, /wasm-bridge\.js\?v=\d+/);
    assert.match(index, /ws-bridge\.js\?v=\d+/);
    assert.match(index, /state\/store\.js\?v=\d+/);
});
