...
export class PETelemetryPanel {
    ...
    _updateConservation(diag) {
        ...
        this._els.energy.textContent = fmt(E);
        this._els.momentum.textContent = fmt(pMag);
        this._els.angmom.textContent = fmt(lMag);
    ...
    _updateSystemProps(diag, ext) {
        ...
        this._els.ke.textContent = fmt(diag.totalKE);
        this._els.pe.textContent = fmt(diag.totalPE);
        if (this._els.peCoulomb) this._els.peCoulomb.textContent = fmt(diag.coulombPE || 0);
        if (this._els.peGravity) this._els.peGravity.textContent = fmt(diag.gravityPE || 0);
        ...
        this._els.temp.textContent = fmtShort(temp);
        ...
        this._els.vrms.textContent = fmtShort(Math.sqrt(v2sum / ext.count));
        ...
        this._els.com.textContent = `(${cmx.toFixed(1)}, ${cmy.toFixed(1)}, ${cmz.toFixed(1)})`;
        ...
        this._els.radius.textContent = fmtShort(maxR);
    }
    ...
    _updateOrbitalMechanics(ext) {
        ...
        this._els.orbR.textContent = fmtShort(r);
        this._els.orbMu.textContent = fmtShort(mu);
        this._els.orbH.textContent = fmtShort(h);
        this._els.orbA.textContent = fmtShort(a);
        this._els.orbE.textContent = fmtShort(ecc);
        this._els.orbT.textContent = fmtShort(T);
