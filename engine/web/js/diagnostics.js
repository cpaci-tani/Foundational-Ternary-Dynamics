...
export class DiagnosticsPanel {
    ...
    update(diag) {
        ...
        this.els.flux.textContent = fmt(diag.totalFlux);
        this.els.energy.textContent = fmt(diag.totalEnergy);
        this.els.entropy.textContent = fmt(diag.entropy || 0);
    ...
    updateEnergyAudit(ea) {
        if (!ea) return;
        this.els.fieldEnergy.textContent = fmt(ea.fieldEnergy);
        this.els.waveEnergy.textContent = fmt(ea.waveEnergy);
        this.els.coulombPE.textContent = fmt(ea.coulombPE);
        this.els.gaussViolation.textContent = fmtSci(ea.gaussViolation);
    }
