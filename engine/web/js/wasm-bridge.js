...
export class MockBridge {
    ...
    tick() {
        ...
        if (doForces && ps.length > 1) {
            for (let i = 0; i < ps.length; i++) {
                ...
                for (let j = 0; j < ps.length; j++) {
                    ...
                    // Periodic wrapping for minimum image
                    ...
                    if (r2raw > 2500) continue; // skip pairs beyond r=50 (force < 0.04% of r=1)
                    const r2 = r2raw + soft;
                    const r = Math.sqrt(r2);
                    const invR2 = 1 / r2;
                    const invR = 1 / r;
                    // Coulomb: F = -alpha * qi * qj / (4pi * r^2) * rhat
                    const qi = pi.state, qj = pj.state;
                    const fCoul = -alpha4pi * qi * qj * invR2 * invR;
                    fx += fCoul * dx;
                    fy += fCoul * dy;
                    fz += fCoul * dz;
                    // Gravity: F = +G_N * mi * mj / r^2 * rhat (attractive)
                    if (doGravity) {
        ...
        // Integrate positions + boundary + speed limit
        ...
        this._particles = ps.filter(p => p.state !== 0 && p.density > this._params.kb * 0.01);
    }
    ...
    getParticleData() {
        ...
        for (let i = 0; i < count; i++) {
            ...
            sizes[i] = p.state !== 0 ? 12.0 : 4.0 + p.density * 8.0;
        }
    ...
    getLagrangian() {
        ...
        return {
            bornInfeld: fieldEnergy,       // ½|J|² (Born-Infeld / field energy density)
            coupling: 0,                    // g_c·s·∇·J (zero without particles)
            velocity: waveEnergy,           // ½|∂J/∂t|² (kinetic / wave energy)
            gauss: 0,                       // Gauss constraint (zero in free wave)
            dissipation,                    // γ·½|J|²
            total: fieldEnergy + waveEnergy,
            hamiltonian: fieldEnergy + waveEnergy,
    ...
    initPE() {
        this._pe = {
            particles: [], nextId: 0, tick: 0, dt: 1.0, soft: 0.1, damping: false, gravity: false
        };
    ...
    _peComputeForces() {
        ...
        for (let i = 0; i < n; i++) {
            for (let j = i + 1; j < n; j++) {
                ...
                const fc = -ALPHA * ps[i].charge * ps[j].charge * (invR2 / (4 * Math.PI));
    ...
    peGetDiagnostics() {
        ...
        for (let i = 0; i < ps.length; i++) {
            for (let j = i + 1; j < ps.length; j++) {
                ...
                pe_coulomb += ALPHA * ps[i].charge * ps[j].charge / (4 * Math.PI * r);
                if (this._pe.gravity) {
    ...
    peGetExtendedData() {
        ...
        for (let i = 0; i < N; i++) {
            ...
            for (let j = 0; j < N; j++) {
                ...
                const fc = -ALPHA * p.charge * q.charge / (4 * Math.PI * r2s);
    ...
    aeGetAtomData() {
        if (!this._ae) return { positions: new Float32Array(0), colors: new Float32Array(0), sizes: new Float32Array(0), atomicNums: new Int32Array(0), charges: new Int32Array(0), ids: new Int32Array(0), bonds: new Int32Array(0), bondCount: 0, count: 0 };
        const atoms = this._ae.atoms;
        ...
        return { positions, colors, sizes, atomicNums, charges, ids, bonds, bondCount, count };
    }
    ...
    getLagrangian() {
        if (!this._module || !this._bridge)
            return {
                ...
                total: 0, hamiltonian: 0, gaussViolation: 0, maxGaussError: 0,
