...
export const FAQ_SECTIONS = Object.freeze([
    {
        ...
        entries: [
            ...
            },
            {
                ...
                ftdAngle: [
                    { tag: 'CONJECTURE', text: 'The master quadratic \\(x^2 - 16G^{*2}x + 16G^{*3} = 0\\) is a [THEOREM]; its larger root \\(x_+ \\approx 137.036\\) matches \\(1/\\alpha\\) to 1.26 ppm at tree level. The physical identification \\(x_+ = 1/\\alpha\\) is a [STRONGLY MOTIVATED CONJECTURE]: no derivation chain from the axioms to \\(\\alpha\\) exists. The numerical-uniqueness support once cited here (a look-elsewhere / structural-uniqueness scan quoted at \\(\\sim 4\\times 10^5\\):1) was withdrawn in 2026 — the scan sits at its own chance base rate and is not evidence for the identification. The 1.26 ppm is arithmetic agreement, not a derivation, and the engine runs on the CODATA value of \\(\\alpha\\); the derived root feeds no force path.' },
                    ...
                    { tag: 'PARAMETRIC', text: 'Masses like \\(m_e\\) and \\(m_H\\) are computed by inserting the engine’s CODATA-calibrated \\(\\alpha\\) and FTD integers into adopted formulas; agreements are parametric, not first-principles predictions.' },
                ],
                stillOpen: [
