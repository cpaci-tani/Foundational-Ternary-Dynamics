...
const SELECTOR_TOOLTIPS = [
    ...
    ['#planetary-scenario-select', 'Choose the J2000 Solar System, an effective-physics laboratory, or a catalog-seeded comparison system.'],
    ['label[for="planetary-scenario-select"]', 'Scenario selector for the Scale 4 Solar System engine.'],
    ['#planetary-gravity-mode', 'Use period-faithful astronomical gravity or the deliberately slow lattice-G comparison gauge.'],
    ['label[for="planetary-gravity-mode"]', 'Dynamics and gravitational-unit selector for Scale 4.'],
    ...
    ['#status-energy', 'Current total energy readout. Scale 0 reports the whole-box dynamic sum in engine sim units (rest mass excluded); the MeV value is the electron-primary calibration (1 sim = 3 MeV) [CALIBRATION].'],
...
const SCALE1_DIAGNOSTIC_TOOLTIPS = {
    ...
    'Retired Relativistic Rescale': 'Unavailable non-covariant isotropic force rescale. The native registry rejects attempts to enable it; use the momentum-form Verlet integrator for the retained numerical profile.',
    ...
    'Kinetic Energy':   'Integrated-state kinetic energy. The verified relativistic-momentum profile uses sum of (gamma - 1)m c^2; the classical profile uses sum of 1/2 m|v|^2.',
    ...
    'State Energy':     'The state-energy sum for represented active channels. Read State Energy Complete and Missing Coverage Mask before interpreting it as a total or using a drift claim.',
    'State Energy Complete': 'True only when every active force or sink that contributes to state evolution has a represented energy/accounting channel.',
    'Missing Coverage Mask': 'Bit mask of active state-energy channels that the current snapshot cannot account for. A nonzero mask disables conservative drift claims.',
    'Drift Eligible':   'True only for a complete state-energy ledger with no active non-conservative source, sink, projection, or selected removal event.',
    'Conservative Drift': 'Relative state-energy drift from the current baseline, shown only when the native coverage contract marks the run eligible.',
    ...
    // ─── Conservation ──────────────────────────────────────────────────
    'Momentum':         'Total native linear momentum vector. In the relativistic-momentum profile this is sum of gamma m v, not a JavaScript reconstruction of m v.',
...
function annotateStatusItems(root) {
    const groups = [
        ...
        ['#status-ptime', 'Current physical or presentation time reported by the active scale.'],
        ['#status-particles', 'Current manifested or simulated particle count for the active scale.'],
        ['#status-energy', 'Current total energy reported by the active simulation. Scale 0: whole-box dynamic sum in engine sim units (rest mass excluded); MeV via the electron-primary calibration (1 sim = 3 MeV) [CALIBRATION].'],
        ['#status-fps', 'Approximate UI frame rate in frames per second.'],
