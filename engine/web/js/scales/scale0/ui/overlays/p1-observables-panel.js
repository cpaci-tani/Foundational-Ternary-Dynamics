...
const HYDROGEN_SCENARIOS = new Set(['s0-seed-hydrogen', 's0-seed-helium', 's0-seed-h2-molecule']);
...
export function mountP1ObservablesPanel(host, getBridge, { dockMode = false } = {}) {
    ...
    function update() {
        ...
        if (HYDROGEN_SCENARIOS.has(scenarioId)) {
            ...
            hydrogenBody.innerHTML = '<div style="font-style:italic;color:var(--text-muted);">Load <code>s0-seed-hydrogen</code>, <code>s0-seed-helium</code>, or <code>s0-seed-h2-molecule</code> to see the predicted level diagram.</div>';
