...
function buildPanel() {
    ...
    const SECTION_HELP = {
        ...
        f: 'Field-wide τ/dτ-dt/φ aggregates (2026-09-03), reduced by the canonical telemetry collector over sampled manifested voxels rather than the single centre voxel Card E reads. Counts and effective strides describe the sampled support. WASM-only (get_tau_sampled/get_lapse_sampled/get_phase_sampled, ftd_wasm.cpp); the Time panel and ptime.* telemetry-grid channels consume the same source observation.',
    };
    root.innerHTML = `
...
export function mountTimePanel(host, getBridge) {
    ...
    // One-click enable affordance: latency_field has a production checkbox and
    // requires the gravity term. de_broglie_clock is scenario-owned and
    // deliberately has no checkbox. Drive the real prerequisite first, then
    // its dependent, so the engine never observes an invalid intermediate
    // toggle profile.
    cardF.addEventListener('click', (e) => {
        if (!e.target.closest(`#${PANEL_ID}-enable-ptime`)) return;
        const gravity = document.getElementById('t-gravity');
        const latency = document.getElementById('t-latency-field');
        if (!gravity || !latency) return;
        for (const checkbox of [gravity, latency]) {
    ...
    let twinHistoryGeneration = 0;
    const twinHistoryBuffer = {
        get count() { return twin.history.length; },
        get total() { return twin.history.length; },
        get generation() { return twinHistoryGeneration; },
        get: (index) => twin.history[index],
        getTick: (index) => twin.historyTicks[index],
    };
    let lastTick = -1;
    ...
    function visibleTwin() {
        ...
        if (historyControl.isAll && count >= 2) {
            const projection = projectHistoryIndices([twinHistoryBuffer], count, 232);
            if (projection) return {
                ...twin,
                history: projection.map(index => twin.history[index]),
                historyTicks: projection.map(index => twin.historyTicks[index]),
            };
        }
    ...
    function renderCardF(container, ptime, toggles) {
        const readout = beginTimeReadout(container);
        const renderRow = bindTimeRow(readout);
        const { gravity, latencyField, deBroglieClock } = toggles;
        const latencyReady = gravity && latencyField;
        const needsEnable = !latencyReady && !deBroglieClock;
        let html = `<div class="time-provenance" title="get_tau_sampled/get_lapse_sampled/get_phase_sampled (ftd_wasm.cpp) accumulate only at manifested voxels. latency_field requires gravity; a scenario-owned de_broglie_clock is the alternate clock source.">τ needs <code>gravity + latency_field</code> or a scenario that owns <code>de_broglie_clock</code>; φ additionally requires that clocked scenario (dφ=ω₀·dτ).</div>`;
        if (needsEnable) {
            html += `<button type="button" class="time-enable-ptime-btn" id="${PANEL_ID}-enable-ptime" style="margin:4px 0 8px;padding:4px 10px;font-size:16px;background:var(--accent,#7dd3fc);color:#04111a;border:none;border-radius:4px;cursor:pointer;">Enable gravity + latency_field</button>`;
        }
        html += renderRow('gravity', gravity ? 'ON' : 'OFF', 'M', gravity ? 'var(--positive-text)' : 'var(--text-muted)', 'Required engine prerequisite for latency_field.');
        html += renderRow('latency_field', latencyField ? (gravity ? 'ON' : 'ON · waiting for gravity') : 'OFF', 'M', latencyReady ? 'var(--positive-text)' : 'var(--text-muted)', 'Engine toggle gating accumulate_proper_time (transmutation_phases.cpp); requires gravity.');
    ...
    function update() {
        ...
        if (!diag || tick === null || !Number.isFinite(physicalTime)) {
            ...
            renderCardF(cardF, { hasField: false, tauCount: 0, lapseCount: 0, phaseCount: 0,
                ...
                { gravity: readEngineToggle(b, 'gravity'), latencyField: readEngineToggle(b, 'latency_field'), deBroglieClock: readEngineToggle(b, 'de_broglie_clock') });
            return;
        }
        ...
        // Card F consumes the canonical proper-time telemetry observation used
        // by the Grid. Acquisition and tau/lapse/phase sampler demand belong to
        // telemetry/demand.js, so opening this panel cannot create a second read
        // or relabel a held sample with the current diagnostics clock.
        const ptimeMeta = telemetryHub.getScale0TelemetryMeta?.('properTime') ?? null;
        const ptimePublished = telemetryHub.s0?.properTime ?? null;
        const ptime = ptimeMeta && ptimeMeta.stale !== true
            && Number.isFinite(ptimeMeta.tick) && ptimePublished?.hasField === true
            ? ptimePublished
            : { hasField: false, tauCount: 0, lapseCount: 0, phaseCount: 0,
                properTimeMean: Number.NaN, properTimeMin: Number.NaN, properTimeMax: Number.NaN,
                lapseMean: Number.NaN, dbPhaseMean: Number.NaN, dbPhaseCircVar: Number.NaN };
        renderCardF(cardF, ptime, {
            gravity: readEngineToggle(b, 'gravity'),
