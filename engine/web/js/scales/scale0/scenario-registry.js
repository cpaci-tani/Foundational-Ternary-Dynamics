...
import { SCALE0_SCENARIO_PRESENTATION } from './scenario-presentation.js';
import { getScale0ScenarioCategory, compareScale0ScenarioCategories } from './scenario-categories.js';
export { SCALE0_SCENARIO_VALIDATION } from './scenario-validation.js';
export { SCALE0_SCENARIO_CATEGORIES } from './scenario-categories.js';
function makeScenario(evidenceCategory, id, title, tags = [], epistemicStatus = '[OPEN]') {
    ...
    const presentation = SCALE0_SCENARIO_PRESENTATION[id];
    const scientificTitle = presentation?.title || title;
    // Retain the existing model marker for rejected historical identities.
    // The complete status and validation remain in the description panel.
    ...
    return {
        ...
        title: identityRejected ? `${scientificTitle} (model)` : scientificTitle,
        category: getScale0ScenarioCategory(id),
        evidenceCategory,
        tags,
        defaultParams: {},
...
export function populateScale0ScenarioSelect(select, selectedId = 'flux-pulse') {
    ...
    // change only the live value instead of tearing down/recreating every option.
    ...
    const currentGroups = Array.from(select.children);
    ...
    const alreadyPopulated = currentIds.length === expectedIds.length
        ...
        && currentGroups.length === expectedGroups.length
        && currentGroups.every((group, index) => {
            const [category, scenarios] = expectedGroups[index];
            return group.tagName === 'OPTGROUP'
                && group.label === category
                && group.children.length === scenarios.length
                && Array.from(group.children).every((option, optionIndex) =>
                    option.tagName === 'OPTION'
                    && option.value === scenarios[optionIndex].id
                    && option.textContent === scenarios[optionIndex].title
                    && option.dataset.uiTooltip === scenarios[optionIndex].intent);
        });
    if (alreadyPopulated) {
    ...
    for (const [category, scenarios] of groups) {
        ...
        for (const scenario of scenarios) {
            ...
            option.dataset.uiTooltip = scenario.intent;
...
export function validateScale0ScenarioRegistry() {
    ...
    for (const scenario of SCALE0_SCENARIOS) {
        ...
        if (!SCALE0_SCENARIO_PRESENTATION[scenario.id]?.title || !scenario.intent) {
            errors.push(`presentation:${scenario.id}`);
        }
