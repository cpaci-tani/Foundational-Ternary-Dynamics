...
import { createBridge, MockBridge } from './wasm-bridge.js?v=20260304q';
import { Viewport } from './viewport.js?v=20260304s';
...
import { expandAEToOrbitalCloud } from './orbitals.js?v=20260304q';
import { atomicEnergy, periodicTableTotalEnergy, formatEnergy } from './atomic-energy.js?v=20260304q';
...
console.log('[FTD] App version 20260304b loaded (cache-busted)');
...
const _dom = {
    ...
    statusFps: null, dtDisplay: null, aeLegend: null,
...
function _cacheDOM() {
    ...
    _dom.dtDisplay = document.getElementById('dt-display');
...
function animateLattice(now) {
    ...
    if (running) {
        if (logTimeMode) {
            const T = bridge.getPhysicalTime();
            const newDt = 1.0 + Math.log10(1.0 + T / 100.0);
            bridge.setDt(newDt);
            _dom.dtDisplay.textContent = newDt.toFixed(1);
        }
    ...
    if (frameCount % 3 === 0) {
        // Use _fluxMock diagnostics when available (has live flux field data)
        const diag = _fluxMock ? _fluxMock.getDiagnostics() : bridge.getDiagnostics();
        // Keep tick counter from the main bridge
        diag.tick = bridge.getDiagnostics().tick;
        ...
        _dom.statusEnergy.textContent = diag.totalEnergy.toFixed(4);
...
function animateAE(now) {
    ...
    // Update bond lines (Scale 3 / molecules only)
    if (engineMode === 'molecules' && _showBonds && atomData.bondCount > 0) {
    ...
    if (frameCount % 3 === 0) {
        ...
        _dom.aeDiagKe.textContent = diag.totalKE.toFixed(6);
        _dom.aeDiagEtotal.textContent = diag.totalEnergy.toFixed(6);
        _dom.aeDiagPeIonic.textContent = diag.totalPEIonic.toFixed(6);
        _dom.aeDiagPeVdw.textContent = diag.totalPEVdw.toFixed(6);
        _dom.aeDiagPeBond.textContent = diag.totalPEBond.toFixed(6);
        _dom.aeDiagTemp.textContent = diag.temperature.toFixed(4);
        const pMag = Math.sqrt(diag.momentumX ** 2 + diag.momentumY ** 2 + diag.momentumZ ** 2);
        _dom.aeDiagMomentum.textContent = pMag.toFixed(6);
...
function updateAtomicEnergyDisplay(atomData) {
    ...
    if (atomData.count === 1 && atomData.atomicNums) {
        ...
        _dom.aeDiagMass.textContent = formatEnergy(e.massEnergy);
        _dom.aeDiagNbe.textContent = formatEnergy(e.bindingEnergy);
        _dom.aeDiagBa.textContent = e.bindingPerNucleon.toFixed(4);
        _dom.aeDiagEbe.textContent = (e.electronBinding / 1000).toFixed(2) + ' keV';
        _dom.aeDiagMassKb.textContent = formatSI(e.massInKB);
        ...
        _dom.aeDiagMass.textContent = formatEnergy(totalMass);
        _dom.aeDiagNbe.textContent = formatEnergy(totalBE);
        _dom.aeDiagBa.textContent = avgBA.toFixed(4);
...
function wireToolbar() {
    ...
    // Ticks per frame slider
    const slider = document.getElementById('ticks-per-frame');
    const display = document.getElementById('tpf-display');
    slider.addEventListener('input', () => {
        ticksPerFrame = parseFloat(slider.value);
        _tickAccumulator = 0;
        display.textContent = ticksPerFrame < 1 ? ticksPerFrame.toFixed(1) : Math.round(ticksPerFrame);
    });

    // Log time toggle
    document.getElementById('log-time').addEventListener('change', (e) => {
        logTimeMode = e.target.checked;
        if (!logTimeMode) {
            bridge.setDt(1.0);
            _dom.dtDisplay.textContent = '1.0';
        }
    });
...
function switchEngineMode(mode) {
    ...
    // Reset field visualization state on mode switch
    _showPEEField = false;
    _showPEPotential = false;
    _showPEGravField = false;
    _showPEForces = false;
    _showAEField = false;
    if (mode !== 'lattice') _fluxMock = null; // free JS flux sim when leaving Scale 0
    _fieldGrid = null;
    // Deactivate all PE overlay buttons
    for (const id of ['toggle-pe-efield', 'toggle-pe-potential', 'toggle-pe-gravity-field', 'toggle-pe-forces']) {
        const btn = document.getElementById(id);
    ...
    // Clear element labels when leaving atoms/molecules mode
    if (mode !== 'atoms' && mode !== 'molecules' && viewport) {
    ...
    // Initialize the appropriate engine (init calls are inside load functions)
...
function loadPEScenario(name) {
    ...
    _trailHistory.clear(); // clear orbit trails
    _fieldGrid = null; // invalidate field grid for new scenario
    if (viewport) viewport.clearTrails();
