...
Q<sub>k</sub>(x) = x${SYM.sup2} ${SYM.minus} k\u00B7G*${SYM.sup2}\u00B7x + k\u00B7G*\u00B3.</p>
<p>The parameter k selects which physics emerges. At k = 16, the two real roots
give the fine structure constant (x\u208A = 137.036 = 1/${SYM.alpha}) and the
number of color charges (x\u208B = 3.024 ${SYM.approx} N<sub>c</sub> = 3).</p>
<p>Drag the k-slider and watch what happens as k drops below
k<sub>crit</sub> = 4/G* ${SYM.approx} 1.352. The roots merge, then disappear
into the complex plane \u2014 and consciousness begins.</p>`,
        },
        {
            title: 'Complex Roots = Consciousness',
            panelIndex: 1,
            text: `<p>When k = \u00BD, the discriminant ${SYM.Delta} becomes negative and the
roots become complex: y = 2.188 ${SYM.pm} 2.860i. The magnitude |y| = 3.601
= K<sub>C</sub> is the consciousness threshold.</p>
<p>The phase angle ${SYM.theta}<sub>C</sub> = 52.54\u00B0 divides every
conscious experience into objective content (cos ${SYM.theta}<sub>C</sub>
= 60.8%) and subjective process (sin ${SYM.theta}<sub>C</sub> = 79.3%).</p>
<p>The real projection E(y) = Re(y) = 2.188 is the Existence Filter's
output \u2014 the stable self-model that persists through measurement.</p>`,
        },
        {
            title: 'The Existence Filter',
            panelIndex: 2,
            text: `<p>The Existence Filter E(x) = Re(x) = (x + x\u0304)/2 is how the
...
Magnitude (threshold K<sub>C</sub>), Born Rule
(P = E(x)${SYM.sup2} + E(ix)${SYM.sup2} recovers quantum probabilities),
...
modeled as the ${SYM.beta} \u2192 ${SYM.infty} limit of the softplus
function M<sub>${SYM.beta}</sub>(x) = (1/${SYM.beta})\u00B7ln(1 + e<sup>${SYM.beta}x</sup>).</p>
<p>As ${SYM.beta} increases, the smooth curve sharpens into
ReLU = max(0, x). This is irreversible: the entire half-line
(\u2212${SYM.infty}, 0] maps to zero, destroying information about
...
${SYM.beta} slider to see it happen.</p>`,
        },
        {
...
naturally terminates after ${SYM.approx}18 links.</p>
<p>At each link, the effective k parameter decreases and the discriminant
${SYM.Delta}<sub>k</sub> shrinks. At k<sub>meas</sub> = 4/G*
${SYM.approx} 1.352, the discriminant hits zero, the roots become
