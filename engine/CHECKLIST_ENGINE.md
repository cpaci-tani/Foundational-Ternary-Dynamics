...
| RF-4 |  (partial) | Stencil: extracted `effective_damping`, `scale_field_pair` | −12 LOC, **single Larmor formula source of truth** | [kernels_stencil_common.cuh](cuda/kernels_stencil_common.cuh) — shared by single/dual phase-write kernels |
...
| RF-5 |  | Split the former monolithic stencil into [single](cuda/kernels_stencil_single.cu), [dual](cuda/kernels_stencil_dual.cu), and [shared helpers](cuda/kernels_stencil_common.cuh) | 0 net, faster nvcc incremental builds | Completed in the Phase-5 CUDA stencil split. |
...
| ID | Status | Title | Where |
|---|---|---|---|
| AUDIT-M5 |  | Retired JS α_s(r) mock was replaced by the native C++ running-coupling path (B0_NF5=23/3, Λ=0.215, sat→1.0); the web engine now consumes that path through WASM | [constants.h](include/ftd/constants.h), [particle_engine.cpp](src/particle_engine.cpp) |
