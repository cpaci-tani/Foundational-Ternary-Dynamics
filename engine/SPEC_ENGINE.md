...
- Larmor radiation: acceleration-dependent damping (v2.11; law corrected 2026-09-16 in `b7ea1023` — the earlier form could only reduce dissipation — and numerically inert at every realized |a|, see `TRACKER_OPEN_ITEMS.md` §1.14)
...
RenderBridge::tick()
  ...
  5c.  absorbing/flux boundary passes [absorbing_boundary unless Dispersal / flux_boundary]
...
| `apply_absorbing_boundary` | `absorbing_boundary` | Applies an imposed D-deep quadratic damping sponge after movement. It is not a derived reflection-free radiation condition. It is ignored when `flux_boundary=Dispersal`; the complete Dispersal deletion surface never composes with an interior sponge. |
| boundary preparation / settled pass | `flux_boundary` | `Dispersal` keeps every record on all six outer faces exact zero both before stencil reads and after the last local writer, including observable, dual, strong, and weak fields, manifested identity/motion, potentials, and diagnostics. Boundary-adjacent wave targets use a virtual first-order one-way sample, `J_virtual(o) = J_target - |o|*wave_vel_target/(C_WAVE*M_out)`, where `M_out = sum(w_o*|o|)` over the selected stencil links that reach the outer shell. This target-local normalization gives face, edge, and corner targets the same impedance impulse without materializing a shared energy-bearing ghost. Thus no opposite-face value is read, no strictly interior cell is graded, and no face value survives publication or rendering. A manifested record is retired on its first attempted hop into a face. `Reflective` refreshes its six-face interior Neumann mirror shell, and `Periodic` identifies all three opposite face pairs through the lattice neighbour tables without a boundary pass. `periodic_axis` is orientation/provenance metadata only. These are imposed computational finite-box laws, not ontological boundaries or an exact all-angle transparent-boundary theorem. |
...
| `K_LARMOR` | `4*N_EFF/(3*K_B)` ≈ 33.9 [IMPOSED] | Radiation-reaction gain: `gain = min(1 + K_LARMOR·a², 256)`, `eff = damping_factor^gain` (`ftd/larmor_damping.h`, since `b7ea1023`). Inert at realized |a| ≲ 7e-4 (`K·a² ≲ 2e-5`); the scale is an open owner decision (`TRACKER_OPEN_ITEMS.md` §1.14) |
| `LARMOR_FLOOR` | 0.01 | Legacy: the a=0 floor of the pre-`b7ea1023` inverted law. No longer enters the law (gain ≥ 1 gives at least the baseline damping by construction); retained for the constant pins (BREM-6) and provenance |
...
- `larmor` -- Acceleration-dependent damping (gain excess proportional to a^2, LAM-3; LAM-6b pins ON == OFF at realized |a|)
...
2. **Damping hierarchy**: Default: uniform flux decay at rate alpha. With `selective_damping`: only near-particle sites damp. With `larmor_radiation` (requires `selective_damping`): `eff = damping_factor^(1 + K_LARMOR·a²)` (`ftd/larmor_damping.h`, corrected 2026-09-16 — the earlier `1 − DAMPING·min(1, LARMOR_FLOOR + K_LARMOR·a²)` could only reduce dissipation). Correct a² shape, but numerically inert at every acceleration the engine realizes (`K_LARMOR·a² ≲ 2e-5`); `K_LARMOR`'s scale is an open owner decision (`TRACKER_OPEN_ITEMS.md` §1.14).
...
**Scale 2 (AtomEngine):** 150 selector entries: 33 curated contracts plus 117 generated single-element references (hydrogen is the curated composite reference). Curated groups include noble-gas clusters, ionic fragments, effective covalent capture, water networks, VSEPR relaxation, thermal/collision dynamics, metallic packing, seven validation laboratories, five nuclear-reaction/transport initial-condition presets, periodic-table atlas, and custom mode. Every curated contract owns a deterministic seed, complete 11-term physics profile, numeric parameters, visual profile, expected counts, evidence, and epistemic status.

**Scale 3 (Molecule Engine):** 35 contracts: 25 declared reference structures, eight interactive molecular experiments, a finite NaCl crystal fragment, and a custom sandbox. Catalog identity comes from explicit atom-index topology and integer effective bond orders, never runtime distance inference. Experiments cover vibration, rotation, dissociation, capture, hydrogen bonding, dipole alignment, molecular collision, and a heat–quench–release protocol. Live diagnostics decompose classical kinetic energy into component translation, rigid rotation, and residual internal/vibrational motion; track graph components and topology changes; and report bond geometry, dipole magnitude, radius of gyration, energy accounting, momentum, and stability. These are `[IMPOSED]`/`[PARAMETRIC]` effective classical models, not ab-initio chemistry or substrate-QM recovery. Dedicated scenario-contract, topology-transition, UI-isolation, moving-selection, and Scale-2/3 regression tests prevent profile leakage. The opt-in `scale3-molecule-release-gate.spec.js` rejects software WebGL and holds six canonical workloads plus a 192-atom/64-water all-effective-physics stress fixture for at least 600 frames each. The 2026-09-04 RTX 5090 ANGLE/D3D11 record sustained at least 59.73 effective FPS and 59.74 molecule ticks/s, with p99 at 16.67 ms and no intervals above 33.4 ms or Long Tasks.
