...
**Test count:** 114 test files (70 unit + 44 campaigns), 114 CTests. 10-Phase Proof-Out: 125+ checks, all PASS. GPU conditional on `FTD_ENABLE_CUDA`.
...
engine/
  ...
  include/ftd/
    ontic.h                   # Ontic derivation chain (9+ layers), D=3 + varpi -> all constants (914L)
    constants.h               # Re-exports ontic + engine-specific constants (218L)
    voxel.h                   # Vec3, ForceDiag, Voxel struct (175L)
    lattice.h                 # Lattice class -- 3D cubic grid with periodic boundaries (59L)
    render_bridge.h           # RenderBridge -- main engine API, tick(), diagnostics() (214L)
    lagrangian.h              # 4-term Lagrangian + Rayleigh dissipation (137L)
    term_toggles.h            # 19 runtime toggles for pedagogy system (59L)
  ...
  src/
    render_bridge.cpp         # Logic-first engine -- 6-phase tick cycle (1321L)
  ...
  tests/
    114 test files            # 70 unit + 44 campaigns (all registered as CTests)
  wasm/
    ftd_wasm.cpp              # Emscripten Embind bindings -- full engine API (1388L)
    CMakeLists.txt            # WASM build rules (Emscripten-only)
  web/
    index.html                # Browser dashboard (HTML + CSS + tab system)
    js/                       # 21 JS modules (~20K lines total)
      app.js                  # Main controller: WASM loading, frame loop, 3 scale modes
      viewport.js             # Three.js 3D: particles, bonds, orbitals, field overlays
      ...
      orbitals.js             # Electron orbital cloud generation + nuclear structure
...
| Component | Lines |
|-----------|-------|
| Headers (`include/ftd/*.h`) | 3,564 |
| Sources (`src/*.cpp`) | 3,242 |
| CUDA (`cuda/*.cu + CMakeLists`) | 3,213 |
| WASM bindings | 1,388 |
| Web frontend (HTML + JS) | ~20,000 |
| **Total engine C++** | **~11,400** |
...
The `TermToggles` struct provides **19 runtime booleans** for the pedagogy system. Core rules default ON; extensions default OFF.
...
|----------|-------|--------|
| Unit tests (test_*) | 70 | ~400+ |
| Campaign tests (campaign_*) | 44 | ~350+ |
| **Total** | **114** | **750+** |

All 114 tests are registered as CTests. GPU tests (3 files) are conditional on `FTD_ENABLE_CUDA`.
...
| `toggles` | Public `TermToggles` struct (19 booleans) |
