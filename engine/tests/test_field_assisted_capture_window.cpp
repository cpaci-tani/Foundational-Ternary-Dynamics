// FTD-0723: locked incident-momentum capture-window campaign.
...
void write_records(
  ...
  std::ofstream csv(directory /
      "ftd_0723_field_assisted_capture_window_v1.csv");
  ...
  std::ofstream json(directory /
      "ftd_0723_field_assisted_capture_window_v1.json");
...
int main() {
  ...
  const bool predicted_window = summaries[0].captured == 52
      && summaries[1].captured == 52
      && summaries[3].captured == 0
      && summaries[4].captured == 0;

  std::string verdict;
  if (!algebra)
    verdict = "CAPTURE_WINDOW_TRANSACTION_UNRESOLVED";
  else if (bound_pass != 52)
    verdict = "DERIVED_PAIR_BOUND_STATE_UNSTABLE_IN_WINDOW_CAMPAIGN";
  else if (!monotone_capture_fraction)
    verdict = "NONMONOTONE_CAPTURE_RESPONSE";
  else if (predicted_window)
    verdict = "PREDICTED_CAPTURE_WINDOW_CONFIRMED";
  else if (captured > 0)
    verdict = "CAPTURE_WINDOW_SHIFTED_OR_DIRECTIONAL";
  else if (negative > 0)
    verdict = "NEGATIVE_SECTOR_WITHOUT_DETACHED_FIELD";
  else
    verdict = "NO_CAPTURE_WINDOW_OBSERVED_LOCKED_V1";

  write_records(arms, summaries, verdict, scalar_spread,
                monotone_capture_fraction);
  std::cout << "FTD-0723 " << verdict << " arms=" << arms.size()
