// ============================================================================
// test_universal_freefall_engine_align.cpp
// ----------------------------------------------------------------------------
// FTD-1014 / PREREG_UNIVERSAL_FREEFALL_ENGINE_ALIGN_v1.md
// Lock prefix SHA256:
//   5A99CA4FD0EB3EACA1FE9712688E0D8D118F9756CB53EC86DD860C8FCAAA3E9F
// Anchor: anchored-late until git tag
//   preregister-universal-freefall-engine-align-v1 resolves.
//
// Dual-path observer. No production gravity rewrite. No golden-tick contact.
// Protocol gates are CTest assertions. Physics gate A1 classifies FOUND vs
// CLOSED-NEGATIVE and does not fail CTest.
// ============================================================================
...
void configure_path_f(RenderBridge& rb) {
...
// Public TU split of private RenderBridge::phase_forces(). The lock forbids
// tick() (Poisson overwrite / movement); it does not require calling a
// private method. Order matches render_bridge.cpp.
...
PathResult run_path_f(int edge) {
    RenderBridge rb(kL);
    rb.force_cpu();
    rb.seed_rng(20260819);
    configure_path_f(rb);
...
void test_universal_freefall_engine_align() {
    section("FTD-1014 dual-path UFF engine alignment (CPU observer)");
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        F[i] = run_path_f(edges[i]);
        std::printf("    N=%2d  g_ext=%.6e  r_G=%.6e  r_F=%.6e  a_F=%.6e  |f_g|=%.6e\n",
                    G[i].N, G[i].g_ext, G[i].r, F[i].r, F[i].a_com, F[i].max_f_grav);
    }

    bool p1 = true, p2 = true, p3 = true, p4 = true, p5 = true, a1 = true;
    double rGmin = G[0].r, rGmax = G[0].r, rGsum = 0.0;
    for (int i = 0; i < 3; ++i) {
        p1 = p1 && (std::abs(G[i].g_ext) > kGextFloor);
        p2 = p2 && (std::abs(G[i].r - 1.0) < kPathGTol) && G[i].rigid;
        p4 = p4 && (F[i].max_extra < kExtraForce);
        p5 = p5 && F[i].gravity_written && F[i].rigid;
        a1 = a1 && (std::abs(F[i].r - 1.0) < kA1Tol);
        rGmin = std::min(rGmin, G[i].r);
        rGmax = std::max(rGmax, G[i].r);
        rGsum += G[i].r;
        const int expected_n = edges[i] * edges[i] * edges[i];
        check("registered N matches edge^3", G[i].N == expected_n && F[i].N == expected_n);
    }
    ...
    check("P4: Path F extra-force channels < 1e-12", p4);
    check("P5: Path F gravity diagnostic written", p5);

    const bool protocol = p1 && p2 && p3 && p4 && p5;
    const char* verdict = "UNDERDETERMINED";
    if (protocol && a1) verdict = "FOUND";
    else if (protocol && !a1) verdict = "CLOSED-NEGATIVE";
    std::printf("    VERDICT %s  (A1 live r_F~1: %s)\n",
                verdict, a1 ? "yes" : "no");
    check("protocol complete (physics verdict printed, not a CTest fail)", protocol);
}
...
int main() {
    ftd::test::init("test_universal_freefall_engine_align");
    ftd::test::test_universal_freefall_engine_align();
