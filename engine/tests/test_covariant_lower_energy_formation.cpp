// FTD-0726: locked covariant lower-energy formation campaign.
...
void write_records(
  ...
  std::ofstream csv(directory /
      "ftd_0726_covariant_lower_energy_formation_v1.csv");
  ...
  std::ofstream json(directory /
      "ftd_0726_covariant_lower_energy_formation_v1.json");
...
int main() {
  ...
  const bool expected_pattern = summaries[0].negative_sector == 52
      && summaries[1].negative_sector == 52
      && summaries[2].negative_sector == 52
      && summaries[3].negative_sector == 52
      && summaries[4].negative_sector == 0;

  std::string verdict;
  if (!algebra)
    verdict = "TIGHT_ROOT_FORMATION_TRANSACTION_UNRESOLVED";
  else if (bound_pass != 52)
    verdict = "DERIVED_PAIR_BOUND_STATE_UNSTABLE_TIGHT_ROOT";
  else if (!monotone_negative_fraction || !monotone_capture_fraction)
    verdict = "NONMONOTONE_COVARIANT_FORMATION_RESPONSE";
  else if (expected_pattern && captured == 208)
    verdict = "COVARIANT_LOWER_ENERGY_CAPTURE_CONSTRUCTIVE";
  else if (expected_pattern && captured > 0)
    verdict = "COVARIANT_CAPTURE_MORPHOLOGY_DIRECTIONAL";
  else if (expected_pattern)
    verdict = "COVARIANT_ENERGETIC_TRAPPING_WITHOUT_DETACHED_FIELD";
  else if (negative > 0)
    verdict = "COVARIANT_TRAPPING_BOUNDARY_SHIFTED";
  else
    verdict = "NO_COVARIANT_LOWER_ENERGY_BASIN_OBSERVED";

  write_records(arms, summaries, verdict, scalar_spread,
                monotone_negative_fraction, monotone_capture_fraction);
  std::cout << "FTD-0726 " << verdict << " arms=" << arms.size()
