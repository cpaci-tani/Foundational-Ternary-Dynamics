// FTD-0286 v2: alpha estimator validation — half-energy gate pairing.
//
// FTD-0286 v1 returned ENERGY_FUNCTIONAL_MISMATCH because the frozen gate
// alpha_r = 2 r G_L(r) pairs with SUM |J|^2, while energy_audit().field_energy
// uses 1/2 SUM |J|^2 since 2026-04-27. This v2 campaign re-tests with the
// paired gate alpha_r = r G_L(r). Not an alpha recovery attempt.

#include "ftd/eft/lattice_coulomb_gate.h"
...
using ftd::eft::FieldEnergyConvention;
using ftd::eft::phase_g_alpha_r;
...
ModeResult run_mode(const char* label, bool matched) {
    ...
    for (std::size_t i = 0; i < R_VALUES.size(); ++i) {
        ...
        const double phase_g =
            phase_g_alpha_r(L, r, FieldEnergyConvention::HalfSumJ2);
...
int main() {
    std::printf("FTD-0286 alpha estimator validation v2\n");
    ...
    std::printf("energy_convention,half_sum_j2\n");
    std::printf("gate,alpha_r_expected,r_G_L_r\n");
    ...
    if (!production.absolute_gate && matched.absolute_gate) {
        ...
        verdict = "BOTH_GATES_CONFIRMED";
    } else if (!production.absolute_gate && !matched.absolute_gate &&
               matched.projection_converged) {
        verdict = "STILL_MISMATCH";
