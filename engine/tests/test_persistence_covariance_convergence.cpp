// FTD-0728: locked persistence covariance convergence campaign.
...
constexpr double kParentScalarSpread = 1.1065308669344631e-9;
...
HistorySpread history_spread(const std::vector<ArmResult>& arms) {
  HistorySpread result;
  for (const auto& reference : arms) {
    ...
    for (const auto& arm : arms) {
      ...
      for (std::size_t i = 0; i < arm.separation_history.size(); ++i) {
        const std::array<std::pair<double, const char*>, 3> defects{{
            {std::abs(arm.separation_history[i]
                - reference.separation_history[i]), "separation"},
            {std::abs(arm.internal_history[i]
                - reference.internal_history[i]), "pair_internal"},
            {std::abs(arm.field_history[i]
                - reference.field_history[i]), "field_energy"}}};
        for (const auto& defect : defects)
          if (defect.first > result.maximum) {
            result.maximum = defect.first;
            result.family = arm.family;
            result.momentum = arm.momentum;
            result.direction = arm.direction;
            result.polarity = arm.polarity;
            result.translation = arm.translation;
            result.tick = static_cast<int>(i);
            result.component = defect.second;
          }
      }
    }
  }
...
std::array<MomentumSummary, kUnboundMomenta.size()> summarize_momenta(
  ...
  for (const auto& arm : arms) {
    ...
    summary.tail_persistent += arm.tail_persistent ? 1 : 0;
    summary.localized_dressing_96 += arm.localized_dressing_96 ? 1 : 0;
    summary.escape_control_pass += arm.escape_control_pass ? 1 : 0;
    ...
    summary.minimum_median_radius2_48 = std::min(
        summary.minimum_median_radius2_48,
        arm.dynamic_median_radius2_48);
    summary.maximum_median_radius2_48 = std::max(
        summary.maximum_median_radius2_48,
        arm.dynamic_median_radius2_48);
  }
  return summaries;
}
...
void write_records(
    ...
    const std::string& verdict, const HistorySpread& spread) {
  const auto directory = std::filesystem::path(__FILE__).parent_path()
      .parent_path() / "results" / "ftd_0728";
  std::filesystem::create_directories(directory);
  int persistent_parent = 0, localized_parent = 0, escape_controls = 0,
      negative = 0, bound = 0, identities = 0, inverses = 0, recoils = 0,
      executed = 0;
  double worst_common = 0.0, worst_recoil = 0.0,
      worst_inverse = 0.0, worst_balance = 0.0;
  for (const auto& arm : arms) {
    const bool parent_trapped = arm.family == "unbound"
        && arm.momentum < 0.0120;
    persistent_parent += parent_trapped && arm.tail_persistent ? 1 : 0;
    localized_parent += parent_trapped
        && arm.localized_dressing_96 ? 1 : 0;
    escape_controls += arm.escape_control_pass ? 1 : 0;
  ...
  std::ofstream csv(directory /
      "ftd_0728_persistence_covariance_convergence_v1.csv");
  ...
  for (const auto& arm : arms)
    csv << arm.family << ',' << arm.momentum << ',' << arm.direction << ','
        ...
        << arm.escape_control_pass << ',' << arm.graph_transitions << ','
        << arm.graph_active_ticks << ','
  ...
  json << std::setprecision(17)
       ...
       << "  \"persistent_parent_arms\": " << persistent_parent << ",\n"
       << "  \"localized_parent_arms\": " << localized_parent << ",\n"
       << "  \"escape_control_pass_arms\": " << escape_controls << ",\n"
       ...
       << "  \"parent_scalar_history_spread\": "
       << kParentScalarSpread << ",\n"
       << "  \"maximum_scalar_history_spread\": " << spread.maximum << ",\n"
       << "  \"tight_to_parent_scalar_ratio\": "
       << spread.maximum / kParentScalarSpread << ",\n"
       << "  \"worst_covariance_family\": \"" << spread.family << "\",\n"
       << "  \"worst_covariance_momentum\": " << spread.momentum << ",\n"
       << "  \"worst_covariance_direction\": \""
       << spread.direction << "\",\n"
       << "  \"worst_covariance_polarity\": \""
       << spread.polarity << "\",\n"
       << "  \"worst_covariance_translation\": \""
       << spread.translation << "\",\n"
       << "  \"worst_covariance_tick\": " << spread.tick << ",\n"
       << "  \"worst_covariance_component\": \""
       << spread.component << "\",\n"
       << "  \"momentum_summaries\": [\n";
  for (std::size_t i = 0; i < summaries.size(); ++i) {
    ...
    json << "    {\"momentum\": " << summary.momentum
         ...
         << ", \"tail_persistent\": " << summary.tail_persistent
         << ", \"localized_dressing_96\": "
         << summary.localized_dressing_96
         << ", \"escape_control_pass\": "
         << summary.escape_control_pass
         ...
         << ", \"minimum_median_radius2_48\": "
         << summary.minimum_median_radius2_48
         << ", \"maximum_median_radius2_48\": "
         << summary.maximum_median_radius2_48 << "}"
         << (i + 1 == summaries.size() ? "\n" : ",\n");
  }
  json << "  ]\n}\n";
...
int main() {
  ...
  const auto spread = history_spread(arms);
  const double scalar_spread = spread.maximum;
  const auto summaries = summarize_momenta(arms);
  const int persistent_parent = static_cast<int>(std::count_if(
      arms.begin(), arms.end(), [](const ArmResult& arm) {
        return arm.family == "unbound" && arm.momentum < 0.0120
            && arm.tail_persistent;
      }));
  const int localized_parent = static_cast<int>(std::count_if(
      arms.begin(), arms.end(), [](const ArmResult& arm) {
        return arm.family == "unbound" && arm.momentum < 0.0120
            && arm.localized_dressing_96;
      }));
  const int escape_pass = static_cast<int>(std::count_if(
      arms.begin(), arms.end(), [](const ArmResult& arm) {
        return arm.escape_control_pass;
      }));
  ...
  const bool executed = normalization.valid && arms.size() == 208
      && std::all_of(arms.begin(), arms.end(),
          [](const ArmResult& arm) { return arm.initialized && arm.executed; });
  const bool algebra = executed
      && std::all_of(arms.begin(), arms.end(), [](const ArmResult& arm) {
        return arm.identity_pass && arm.inverse_pass && arm.recoil_pass;
      });
  const bool classes = persistent_parent == 104
      && localized_parent == 0 && escape_pass == 0 && negative == 116
      && bound_pass == 52
      && summaries[0].tail_persistent == 52
      && summaries[0].localized_dressing_96 == 0
      && summaries[0].graph_transitions == 52
      && summaries[1].tail_persistent == 52
      && summaries[1].localized_dressing_96 == 0
      && summaries[1].graph_transitions == 52
      && summaries[2].tail_persistent == 0
      && summaries[2].negative_sector == 12
      && summaries[2].escape_control_pass == 0
      && summaries[2].graph_transitions == 156;
  const double convergence_ratio = scalar_spread / kParentScalarSpread;
  std::string verdict;
  if (!algebra)
    verdict = "PERSISTENCE_TIGHT_ROOT_TRANSACTION_UNRESOLVED";
  else if (!classes)
    verdict = "PERSISTENCE_CLASS_SOLVER_SENSITIVE";
  else if (scalar_spread > 1e-9)
    verdict = "PERSISTENCE_COVARIANCE_DEFECT_PERSISTS";
  else if (convergence_ratio > 0.2)
    verdict = "PERSISTENCE_COVARIANCE_PASSES_WITH_INCOMPLETE_CONVERGENCE";
  else
    verdict = "FINITE_VOLUME_PERSISTENCE_WITH_EXTENDED_FIELD_AND_RECURRENCE_QUALIFIED";

  write_records(arms, summaries, verdict, spread);
  std::cout << "FTD-0728 " << verdict << " arms=" << arms.size()
            << " persistent=" << persistent_parent << "/104"
            << " localized=" << localized_parent << "/104"
            << " escape=" << escape_pass << "/52"
            << " negative=" << negative << "/156"
            << " bound=" << bound_pass << "/52"
            << " persistence_counts=";
  for (std::size_t i = 0; i < summaries.size(); ++i)
    std::cout << (i == 0 ? "" : "/") << summaries[i].tail_persistent;
  std::cout << " localized_counts=";
  for (std::size_t i = 0; i < summaries.size(); ++i)
    std::cout << (i == 0 ? "" : "/")
              << summaries[i].localized_dressing_96;
  std::cout << " history_spread=" << std::setprecision(17)
            << scalar_spread << " ratio=" << convergence_ratio
            << " worst=" << spread.family << '/' << spread.momentum << '/'
            << spread.direction << '/' << spread.polarity << '/'
            << spread.translation << "/tick" << spread.tick << '/'
            << spread.component << '\n';
  return executed ? 0 : 1;
}
