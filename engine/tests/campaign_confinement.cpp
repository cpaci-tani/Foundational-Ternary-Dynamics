 ...
 *
 * FTD Status: The current engine implements pairwise color forces with
 * running α_s(r) but does NOT have non-Abelian flux self-interaction
 * (3-gluon vertex). Therefore:
 *   - Short-range: Expect Coulomb-like 1/r from α_s(r)/r² force [EMERGENT]
 *   - Long-range: Expect α_s saturates → 1/r potential [LIMITATION]
 *   - Linear confinement (σ·r) is NOT expected without flux tubes [HONEST]
 *
 * Protocol:
 ...
 *
 * Checks:
 *   CON1: Color force is stronger at short distance (running coupling)
 *   CON2: Force follows approximate 1/r² at moderate distance
 *   CON3: Running coupling α_s decreases with decreasing r
 *   CON4: Force at large r does not vanish (coupling saturates)
 */
...
int main() {
    ...
    // CON1: Force is stronger at short distance (running coupling)
    check("CON1: Force at r=3 > force at r=11 (running coupling effect)",
          forces[0] > forces[N_sep-1]);

    // CON2: Force approximately follows 1/r² (F×r² roughly constant at moderate r)
    // Compare F×r² at r=5 and r=9 — should be within factor of 3
    // (not exact 1/r² because running coupling changes with r)
    double fr2_5 = forces[1] * 25.0;
    double fr2_9 = forces[3] * 81.0;
    double ratio_fr2 = (fr2_5 > fr2_9) ? fr2_5 / fr2_9 : fr2_9 / fr2_5;
    std::cout << "  F×r² at r=5: " << fr2_5 << "\n";
    std::cout << "  F×r² at r=9: " << fr2_9 << "\n";
    std::cout << "  Ratio:       " << ratio_fr2 << "\n";
    check("CON2: F×r² varies by less than 3× (approximate 1/r² with running)",
          ratio_fr2 < 3.0);
    ...
    std::cout << "  RESULT: " << (failures == 0 ? "ALL PASSED" : "FAILURES DETECTED")
              << " (" << failures << " failures)\n";
    std::cout << "  NOTE: The current engine implements Coulomb-like 1/r² color\n";
    std::cout << "  force with running α_s(r). Linear confinement (V~σr) would\n";
    std::cout << "  require non-Abelian flux tube dynamics [NOT YET IMPLEMENTED].\n";
    std::cout << "  Running coupling is [IMPOSED] from QCD beta function.\n";
    std::cout << "  α_s decrease at short r (asymptotic freedom) is [EMERGENT]\n";
    std::cout << "  from the running coupling functional form.\n";
    std::cout << "================================================================\n";
    return failures;
}
