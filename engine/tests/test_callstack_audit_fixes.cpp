...
int main() {
    ...
    {
        RenderBridge rb(16);
        rb.force_cpu();  // F11.A-5: this test verifies the CPU pair_production_cpu port (F2 finding).
                         // GPU pair_production has independent issues tracked under W3.x; isolating
                         // CPU here keeps the canary scoped to its original purpose.
        rb.toggles.wave_propagation = false;
        rb.toggles.coupling = false;
        rb.toggles.damping = false;
        rb.toggles.gauss_projection = false;
        rb.toggles.genesis = false;  // pair_production is an INDEPENDENT
                                     // code path — phase_write::genesis and
                                     // pair_production_cpu (Rule 2b) share
                                     // K_GENESIS as their flux threshold but
                                     // operate independently. F11.A-5 audit
                                     // (2026-05-09) confirmed + removed the
                                     // artificial "requires genesis" toggle
                                     // dependency in TOGGLE_SPECS.
    ...
    {
        RenderBridge rb(16);
        rb.force_cpu();  // F11.A-5: isolate CPU path
        rb.toggles.wave_propagation = false;
        rb.toggles.coupling = false;
        rb.toggles.damping = false;
        rb.toggles.gauss_projection = false;
