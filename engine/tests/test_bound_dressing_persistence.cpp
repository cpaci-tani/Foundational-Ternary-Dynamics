// FTD-0727: locked finite-volume bound-dressing persistence campaign.
...
void write_records(
  ...
  std::ofstream csv(directory /
      "ftd_0727_bound_dressing_persistence_v1.csv");
  ...
  std::ofstream json(directory /
      "ftd_0727_bound_dressing_persistence_v1.json");
...
int main() {
  ...
  options.solve_tolerance = 2e-12;
  options.max_iterations = 96;
  ...
  if (!algebra)
    ...
    verdict = "DERIVED_PAIR_BOUND_CONTROL_UNSTABLE_BY_96_TICKS";
  else if (escape_pass != 52)
    verdict = "FINITE_VOLUME_OR_DYNAMICAL_RECURRENCE_CONTAMINATES_ESCAPE_CONTROL";
  else if (persistent_parent == 104 && localized_parent == 104)
    verdict = "FINITE_VOLUME_BOUND_DRESSING_PERSISTS_TO_96_TICKS";
  else if (persistent_parent == 104)
    verdict = "FINITE_VOLUME_TRAPPING_PERSISTS_WITH_EXTENDED_FIELD";
  else if (persistent_parent > 0)
    verdict = "DIRECTIONAL_OR_POLARITY_PERSISTENCE_SPLIT";
  else
    verdict = "TRANSIENT_TRAPPING_REEMITS_BY_96_TICKS";

  write_records(arms, summaries, verdict, scalar_spread);
  std::cout << "FTD-0727 " << verdict << " arms=" << arms.size()
