...
int main() {
    ...
    std::cout << "\n-- CONT-4: Post-Genesis Charge Conservation --\n";
    {
        ...
        // Now disable genesis — subsequent dynamics must conserve charge exactly
        rb.toggles.genesis = false;
        ...
        for (int step = 0; step < 5; ++step) {
            rb.run(100);
            int Q = total_charge(rb);
            if (Q != Q_post_genesis) all_conserved = false;
        }

        int n_final = manifested_count(rb);
        std::cout << "    Post-genesis Q = " << Q_post_genesis
                  << ", particles = " << n_post << "\n";
        std::cout << "    After 500 more ticks (no genesis): conserved = "
                  << (all_conserved ? "YES" : "NO")
                  << ", particles = " << n_final << "\n";

        check("CONT-4: Post-genesis Q exactly conserved over 500 ticks (genesis OFF)",
              all_conserved && n_post > 0);
    }
