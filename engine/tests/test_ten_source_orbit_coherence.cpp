...
int main() {
  const ftd::eft::TenSourceOrbitCoherenceResult result =
      ftd::eft::analyze_ten_source_orbit_coherence();

  std::cout << std::setprecision(17);
  std::cout << "FTD-0593 ten-source cubic-orbit coherence v1\n";
  ...
  std::cout
      ...
      << ",n10_inconclusive," << result.ten_source_bound_inconclusive
  ...
  if (result.arbitrary_removal_n_le_ten_closed) {
    std::cout
        << "verdict,ARBITRARY_REMOVAL_N_LE_10_CLOSED_BY_ORBIT_COHERENCE\n";
  } else if (result.ten_source_bound_inconclusive) {
    std::cout << "verdict,TEN_SOURCE_ORBIT_BOUND_INCONCLUSIVE\n";
  } else {
