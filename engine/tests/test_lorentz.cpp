...
int main() {
    ...
    // Case 8: Voxel gamma_ftd method (latency-aware formula)
    // With L>0: gamma = sqrt(f) / sqrt(f² - v²) where f = 1 - L²
    ftd::Voxel vox;
    vox.velocity = {0.3, 0.4, 0.0};  // |v| = 0.5
    vox.latency = 0.3;
    {
        double f = 1.0 - 0.3 * 0.3;  // f = 1 - 0.09 = 0.91
        double v2 = 0.25;  // |v|² = 0.25
    ...
    // Case 9: Born-Infeld core (latency-aware formula)
    // With L>0: core = -K_B * sqrt(f² - v²) / sqrt(f)
    {
        double f = 1.0 - 0.3 * 0.3;  // f = 0.91
        double v2 = 0.25;
    ...
    // Case 10: Voxel gamma_ftd with L=0 matches standard SR
    {
        ftd::Voxel v0;
        ...
        check_close("Voxel gamma_ftd v=0.5 L=0 matches SR", g_vox, g_sr, 1e-12);
    }
