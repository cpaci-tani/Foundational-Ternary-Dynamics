 ...
 *
 * Follow-up to FTD-0293. The source-free tick preserves the modified global
 * energy
 *
...
void observe(BalanceSet& set,
    ...
    for (std::size_t k = 0; k < balls.size(); ++k) {
        ...
        const long double rel =
            std::abs(balance) / std::max(std::abs(dE) + std::abs(flux), 1e-300L);
        ...
        const long double vals[] = {e0, e1, dE, flux, balance, rel};
    ...
    for (const auto& m : set.metrics) {
        ...
        const long double mean_abs = m.steps > 0
                                         ? m.mean_abs_balance_accum /
                                               static_cast<long double>(m.steps)
                                         : 0.0L;
        std::printf("balance,radius,%d,sites,%d,steps,%d,max_abs_dE,%.21Lg,max_abs_flux,%.21Lg,max_outward_flux,%.21Lg,max_inward_flux,%.21Lg,max_abs_balance,%.21Lg,rms_balance,%.21Lg,mean_abs_balance,%.21Lg,max_rel_balance,%.21Lg,max_abs_energy,%.21Lg,finite,%s\n",
                    m.radius, m.sites, m.steps, m.max_abs_dE, m.max_abs_flux,
                    m.max_outward_flux, m.max_inward_flux, m.max_abs_balance,
                    rms, mean_abs, m.max_rel_balance, m.max_abs_energy,
                    m.finite ? "true" : "false");
    }
    std::printf("balance_summary,max_abs_dE,%.21Lg,max_abs_flux,%.21Lg,max_outward_flux,%.21Lg,max_inward_flux,%.21Lg,max_abs_balance,%.21Lg,rms_balance,%.21Lg,max_rel_balance,%.21Lg,finite,%s\n",
                set.max_abs_dE, set.max_abs_flux, set.max_outward_flux,
                set.max_inward_flux, set.max_abs_balance, set.rms_balance,
                set.max_rel_balance, set.finite ? "true" : "false");
}

}  // namespace

int main() {
    std::printf("FTD-0294 source-free discrete tick local continuity v1\n");
    std::printf("protocol,L,%d,ticks,%d,mode_n,%d,amp,%.17g,c_wave,%.17g,alpha,%.17g,balance_abs_gate,%.21Lg,balance_rel_gate,%.21Lg\n",
    ...
    std::printf("gates,local_continuity,%s\n",
