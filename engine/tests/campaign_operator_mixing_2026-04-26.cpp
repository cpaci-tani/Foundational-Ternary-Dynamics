...
int main(int argc, char** argv) {
    ...
    // ── Pre-registered ensemble parameters (PROTOCOL §3) ─────────────
    ...
    std::vector<OpVec> coarse_samples;
    ...
    for (int s = 0; s < N_SEEDS; ++s) {
        ...
        for (int k = 0; k < N_SAMPLES; ++k) {
            ...
            const int q_coarse = ftd::eft::total_source(coarse);
            if (q_fine != q_coarse) {
            ...
            const double res_coarse = ftd::eft::max_gauss_residual(coarse);
            if (!std::isfinite(res_fine) || !std::isfinite(res_coarse) ||
                res_fine > 1.0 || res_coarse > 1.0) {
                ++snapshots_dropped;
                continue;
            }
            ...
            coarse_samples.push_back(mean_operators(coarse));
        }
    }
    ...
    // ── Output artifacts ─────────────────────────────────────────────
    fs::path out_dir = fs::path("engine/results/operator_mixing_2026-04-26");
    std::error_code ec;
