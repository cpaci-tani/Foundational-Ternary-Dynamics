/** FTD-0758: fixed-chart fresh held-out M3 validation. */

#define FTD_0754_MAIN_NAME ftd_0758_global_observer_main
...
namespace ftd0758_embedded {

#include "campaign_m3_support_invariant_validation_cuda.cpp"

namespace {

constexpr char kM3FixedValidationProtocolSha256[]=
    "16289A34CC23ED39A7EEC2E9479E8C7BC6666ABE284FB08BF69FA58931F17765";
...
M3ParentCheckpoint ftd0758_build_checkpoint(
...
M3FibreComparison ftd0758_run_fibre(
    int L,const Direction& direction,
    const ConnectedMooreBlockOptions& options,double interaction_scale) {
  M3FibreComparison result;
  result.volume=L;
  auto parent=ftd0758_build_checkpoint(L,direction,options,interaction_scale);
  ...
  result.baseline=ftd0758_run_history(L,direction,"fibre_baseline",
      parent.state,options,interaction_scale,kM3FibreTicks,true,24);
  result.remote=ftd0758_run_history(L,direction,"fibre_remote",
...
void ftd0758_write_candidate(
  ...
  const auto directory=std::filesystem::path(__FILE__).parent_path()
      .parent_path()/"results"/"ftd_0758";
  std::filesystem::create_directories(directory);
  const auto stem="ftd_0758_m3_candidate_v1_"+small.direction+"_"
      +small.variant;
  ...
  json<<std::setprecision(17)
      <<"{\n  \"ftd_id\": \"FTD-0758\",\n"
      <<"  \"protocol_sha256\": \""<<kM3FixedValidationProtocolSha256
...
void ftd0758_write_fibre(const std::string& direction,
                         const std::vector<M3FibreComparison>& values) {
  const auto directory=std::filesystem::path(__FILE__).parent_path()
      .parent_path()/"results"/"ftd_0758";
  std::filesystem::create_directories(directory);
  const auto stem="ftd_0758_m3_causal_fibre_v1_"+direction;
  std::ofstream csv(directory/(stem+".csv"));
...
int run(int argc,char** argv) {
  ...
  if(!qualification&&!candidates&&!fibre) {
    std::cout<<"FTD-0758 runner: --qualify face 1; registered "
  ...
  if((candidates||fibre)
      &&std::string(kM3FixedValidationProtocolSha256)=="UNLOCKED") {
    std::cerr<<"FTD-0758 registered execution refused before protocol lock\n";
    return 3;
  }
  ...
  if(candidates) {
    ...
    for(std::size_t volume_index=0;volume_index<kM3Volumes.size();++volume_index) {
      ...
      for(std::size_t variant_index=0;variant_index<variants.size();++variant_index) {
        ...
        M3History history;
        if(initial.valid)
          history=ftd0758_run_history(L,direction,variant,
    ...
    for(std::size_t variant_index=0;variant_index<variants.size();++variant_index) {
      ...
      ftd0758_write_candidate(histories[variant_index][0],
    ...
    std::cout<<"FTD-0758 candidates direction="<<slug<<" pass="<<pass<<'\n';
    return pass?0:1;
  }
  std::vector<M3FibreComparison> values;
  values.reserve(2);
  for(int L:kM3Volumes)
    values.push_back(ftd0758_run_fibre(
        L,direction,options,interaction_scale));
  ftd0758_write_fibre(direction.label,values);
  const bool pass=std::all_of(values.begin(),values.end(),
      [](const auto& value) { return value.valid; });
  std::cout<<"FTD-0758 fibre direction="<<slug<<" pass="<<pass<<'\n';
  return pass?0:1;
}
...
int main(int argc,char** argv) {
  return ftd0758_embedded::run(argc,argv);
}
