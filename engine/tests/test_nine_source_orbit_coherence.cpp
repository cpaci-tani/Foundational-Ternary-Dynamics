#include "ftd/eft/nine_source_orbit_coherence.h"

#include <iomanip>
#include <iostream>

int main() {
  const ftd::eft::NineSourceOrbitCoherenceResult result =
      ftd::eft::analyze_nine_source_orbit_coherence();

  std::cout << std::setprecision(17);
  std::cout << "FTD-0592 nine-source cubic-orbit coherence v1\n";
  for (const auto& volume : result.volumes) {
    ...
    std::cout
        ...
        << ",closed," << volume.nine_source_closed
        << ",valid," << volume.valid << '\n';
  }
  std::cout
      ...
      << ",n9_closed," << result.arbitrary_removal_n_le_nine_closed
      << ",n9_inconclusive," << result.nine_source_bound_inconclusive
  ...
  if (result.arbitrary_removal_n_le_nine_closed) {
    std::cout
        << "verdict,ARBITRARY_REMOVAL_N_LE_9_CLOSED_BY_ORBIT_COHERENCE\n";
  } else if (result.nine_source_bound_inconclusive) {
    std::cout << "verdict,NINE_SOURCE_ORBIT_BOUND_INCONCLUSIVE\n";
  } else {
