...
| **M7** | **The UI is [RmlUi](https://github.com/mikke89/RmlUi) — RML markup + RCSS (a CSS subset: flexbox, box model, positioning, transitions, animations, media queries) — rendered through our own Direct3D 12 render interface** (decision 2026-08-20). No Dear ImGui, no embedded browser. | Owner wants CSS-grade flexibility, natively. RmlUi hands us geometry (vertices/indices + textures + scissor); we draw it with the engine's D3D12 device **in the presenter's command list**, so the *one-device / no-compositing-boundary* property still holds — now with CSS. Panels and shell are authored in RML/RCSS; live values (telemetry, toggle state) bind through RmlUi **data models** fed by `UiSnapshot`. Docking we still build ourselves. **Vendored: RmlUi + FreeType** (its font engine). The presenter's opaque `OverlayRecorder` seam stays, implemented as an RmlUi→D3D12 render pass. `SPEC_UI_V2` drops to a *requirements* reference (which panels, what IA). Dear ImGui/ImPlot are gone from `engine/native` (kept for `native_desktop` until retired). |
