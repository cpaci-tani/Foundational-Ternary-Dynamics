...
>
> **Approval record.** Source/architecture audit baseline: `d468585510b7`. Owner decisions on 2026-08-19: adopt the 0A→0B→1 split; use mutex-protected immutable snapshots; move the swapchain in Phase 1; provide demand-gated synchronized GPU ledger data. Same-day implementation split: Phase 1 is executed as **1a** (vendor + headless ImGui, swapchain stays on the child HWND) then **1b** (scene rectangle, HWND move, SRV heap, Win32/DX12 backends). Revision 3 is the implementation-governing text; revision history remains in git.
...
| D12 | The swapchain moves to the top-level HWND in **Phase 1b**, after an explicit viewport/scissor/input contract exists | Moving it in a no-UI gate phase left scene placement, camera aspect, control-strip occlusion and pointer transforms unspecified. Phase **1a** vendors ImGui/ImPlot and lands L1 headless coverage without moving the child swapchain. |
...
⚠ `src/d3d12_presenter.cpp` is compiled into the desktop app and every native_desktop device test that lists this file. Adding `#include <imgui.h>` there forces all of them to link Dear ImGui. Keep the presenter ImGui-free: it takes an opaque overlay recorder receiving the active `ID3D12GraphicsCommandList*` and render-target metadata (§3.5).
...
struct InjectWavepacket { int x, y, z; int state; };          // RenderBridge::inject_wavepacket
struct InjectFluxAdd    { int x, y, z; double fx, fy, fz; };  // inject_flux_add
struct CreateEntangledPair { int x, y, z; double fx, fy, fz; };
struct ClearField { };
struct SeedRandomFlux { };  // engine RNG; journalled, not replayed
...
- **SRV heap is 256.** Named reserved slots: **index 0 = the interop `StructuredBuffer` SRV; index 1 = the overlay font atlas.** Slots 2+ are the free-list. Post-1.91.5 backends require app-supplied alloc/free callbacks (`PresenterUiContext`).
- **The swapchain is on the top-level HWND** (Phase 1b). The presenter accepts an explicit scene rectangle covering viewport and scissor origin/extent, camera aspect from the scene extent, client-to-scene pointer transformation, and rejection of input in the Win32 control strip. Depth-target extent remains the full backbuffer; the region outside the scene is cleared with the lattice background and then covered by the Win32 strip.
...
| **Setup** (left, ~336 DIP) | dock | Vertical stacked tabs: Scenarios · Run config · Substrate |
| **Viewport** (centre) | dockspace central node | 3D scene + HUD: axis gizmo, field legend, hover/selection readout |
| **Play bar** | viewport chrome | Floating capsule over the central node: Play/Pause · Step · Reset · ticks/second. Always visible, including Presentation. Not a dock. |
...
The three docks are movable, resizable, closable, and restored from the active workspace. The menu bar, status bar, and play bar are **shell chrome** — `ImGuiWindowFlags_MenuBar` on the dockspace host, `BeginViewportSideBar`, and an undocked `AlwaysAutoResize` overlay pinned to the central node — and **no workspace may hide the status bar** (§2.5). The play bar can be toggled from the View menu for the current session; workspace switches restore it.

All region dimensions are **DIPs at 100% scaling** and are multiplied by `PanelContext::dpi_scale`; 336 DIP is 672 physical pixels at 200%.
...
- **Presentation** — all three docks hidden; full-bleed viewport with legend, menu bar, **status bar**, and **play bar**. This is how "demoable second" is satisfied without compromising the dense default, and without breaking §2.5.
...
**DockBuilder recipe** — run when no `.ini` exists, after an explicit workspace reset, or when the saved `.ini` has no `Setup` host (stacked-tab migration). Guarded by `layout_dirty_` plus `ImGui::DockBuilderGetNode(dockspace_id) == nullptr`. Ratios are fractions of the parent, computed against the §4.4 reference `DisplaySize` of 1920×1080 DIP:

```
DockBuilderRemoveNode(root); DockBuilderAddNode(root, Dockspace); DockBuilderSetNodeSize(root, {1920,1080})
  split root Left   0.175  → Setup        (336/1920)
...
Setup:        one host (`Setup###setup`) with a vertical stacked tab rail
              (Scenarios · Run config · Substrate); one pane visible
...
- Live substring + fuzzy search over `SCENARIO_META` / `scale0_scenario_ids()` (**130** ids). Grouped by the five honest menu classes; the list shows `title`, not the legacy id. Selected row shows id, epistemic tag, admission status, and description (empty ⇒ *"No description authored yet"*).
- `engine/include/ftd/scenario_meta.h` is the native descriptor table. A set-equality test (`test_scenario_meta`) covers `scale0_scenario_ids()` explicitly. ⚠ `engine/web/tests/scenario-parity.spec.js` still compares JS `case` labels against C++ `name ==` branches and **does not** read `scale0_scenario_ids()`; the native gate now does. Rich descriptions remain unauthored (`description = ""`). `min_lattice` ships as 0 (unconstrained) until owner-authored.
  This is Direction B of `engine/web/docs/adr/0002-scenario-architecture.md` for the native shell: one descriptor table plus the C++ seed bodies. The JS registry is still hand-maintained; it is the copy source for titles/categories, not a second SoT for native.
...
⚠ **`.gitignore` currently ignores `engine/thirdparty/*` wholesale** (`.gitignore:179`) — the existing `glad` loader is untracked. Vendoring there as-is would make the dependency invisible to git and break every fresh clone. Add explicit re-inclusion exceptions (`!engine/thirdparty/imgui/` plus `**`, same for implot), following the existing precedent for `!engine/web/js/vendor/three/build/` (`.gitignore:13`). The ignore line must be `engine/thirdparty/*` (contents) rather than `engine/thirdparty/` (the directory): git cannot re-include a file whose parent directory is excluded.
...
Forbid local edits to the vendored trees, enforced by a **hash-manifest lint block in `FtdSourceLint.cmake`** so `ctest -R source_lint` already covers it. It lands in **Phase 1a** with the vendoring.
...
| Regression bisection difficulty | Low | `--no-ui` runtime flag (**Phase 1a**, when the flag exists; ImGui draw is skipped once 1b wires the overlay): skips all ImGui init and draw; renderer, sim loop and status logging unchanged. ⚠ After Phase 4 it yields an app with no controls; it is a bisection tool, not a supported mode. |
...
| **1a · Vendor + headless ImGui** | Pin/vendor Dear ImGui (docking ≥ 1.91.5) + ImPlot; add `ftd_imconfig.h` assertion dispatch and the embedded Inter font; hash-manifest lint; L1 headless tests that apply before panels exist (`omp_get_max_threads`, draw-data invariants, deterministic re-draw, `###` window names, DPI matrix); add `--no-ui`. **Does not move the swapchain, grow the SRV heap, or compile Win32/DX12 backends.** | `ftd_imgui` builds; `source_lint` hashes match; `ui_imgui_headless` and `native_desktop_cli` pass; the app still launches with the child HWND and Win32 strip |
| **1b · Renderer host + first pixel** | Define bootstrap, overlay-record, capture-record and post-fence completion seams; add explicit scene rectangle; move swapchain to top-level HWND; grow SRV heap; wire ImGui Win32/DX12 and DPI atlas rebuild; honour `--no-ui` by skipping ImGui init/draw; render one debug window while preserving Win32 controls | ImGui renders over the lattice inside the explicit scene rect; viewport/scissor/aspect/input-arbitration, DPI/font rebuild, `--no-ui`, OpenMP-neutral initialization, one-render-call-per-frame/frame-in-flight and capture lifecycle tests pass; both CPU-expanded and interop particle paths remain visible; info queue is clean on the owner rig |
