...
| The 13 specialized Scale-0 analysis panels | ◐ **4/13** | Reusable pattern = a collapsible instrument section demanding its telemetry group only when open. Done: **gravity** (`756f05ac`, L→f→γ→dilation), **time** (`995f306d`, causal clock dτ/dt=√(1−L²)), **thermo** (`93aee95a`, bath T / T_kin / entropy), **spectrum** (`da86205c`, adapter-side 3D FFT E(k) + `<ftd-chart>`; native/spectrum.{h,cpp} unit-tested, Parseval-exact). Remaining: flux-slice, wave-lab, p1-observables, **dispersion** (temporal FFT ω(k) — reuses native/spectrum.h, needs field-mode time-buffering + a 2D ω-k view), knots (reuses the FFT infra), scale-context, symmetry, genesis, conservation |
