...
| The 13 specialized Scale-0 analysis panels | ◐ **3/13** | Reusable pattern = a collapsible instrument section demanding its telemetry group only when open. Done: **gravity** (`756f05ac`, GravityMetricAgg — L→f→γ→dilation), **time** (`995f306d`, causal clock dτ/dt=√(1−L²)), **thermo** (`93aee95a`, bath T / T_kin=E_wave/(1.5·L³) / entropy). Remaining: flux-slice, wave-lab, p1-observables, **spectrum**, **dispersion**, knots, scale-context, symmetry, genesis, conservation. **spectrum/dispersion/knots need an FFT/field-spectrum subsystem** (no engine E(k)/ω(k) telemetry, no L³ field grid in the native frame) — the next lift |
