/**
 * @file engine/archive/phase_b_2026-04/campaign_s_eff_nonlinear_wilson_2026-06-04.cpp
 * @purpose Archived unregistered nonlinear S_eff campaign variant with Wilson-coefficient CLI hooks.
 * @consumers None while archived; restore and re-register deliberately before use.
 * @contract engine/archive/README.md
 */
/**
...
struct Args {
    ...
    ftd::TermToggles wilson_toggles{}; // Store parsed wilson coefficients
};
...
static bool parse_args(int argc, char** argv, Args& a, std::string& err) {
    for (int i = 1; i < argc; ++i) {
        ...
        if (eq_split("--scenario=", val)) {
        ...
        } else if (s.find("--wilson-coefficient=") == 0) {
            std::string kv = s.substr(21);
            size_t colon = kv.find(':');
            if (colon != std::string::npos) {
                std::string name = kv.substr(0, colon);
                double val_d = std::stod(kv.substr(colon + 1));
                if (name == "g_JJ") a.wilson_toggles.g_JJ = val_d;
                else if (name == "g_J4") a.wilson_toggles.g_J4 = val_d;
                else if (name == "g_divJ2") a.wilson_toggles.g_divJ2 = val_d;
                else if (name == "g_stateSq") a.wilson_toggles.g_stateSq = val_d;
                else if (name == "g_reactionDensity") a.wilson_toggles.g_reactionDensity = val_d;
                else if (name == "g_genesisFlux") a.wilson_toggles.g_genesisFlux = val_d;
                else if (name == "g_JdotDeltaS") a.wilson_toggles.g_JdotDeltaS = val_d;
                else {
                    err = "Unknown wilson coefficient: " + name;
                    return false;
                }
            }
        } else if (s == "--smoke") {
            ...
            std::cout << "campaign_s_eff_nonlinear --scenario={langevin-pure,genesis-rich,pair-rich,mixed-balanced}\n"
                      << "  --L=N --N-seeds=N --N-samples=N --N-burn=N --b4 --smoke --output-dir=PATH --wilson-coefficient=name:val\n";
            std::exit(0);
        } else {
            err = "unknown arg: " + s;
            return false;
...
int main(int argc, char** argv) {
    ...
    for (int s = 0; s < args.N_seeds; ++s) {
        ...
        // Apply parsed wilson coefficients
        if (args.wilson_toggles.g_JJ != 0.0) rb.toggles.g_JJ = args.wilson_toggles.g_JJ;
        if (args.wilson_toggles.g_J4 != 0.0) rb.toggles.g_J4 = args.wilson_toggles.g_J4;
        if (args.wilson_toggles.g_divJ2 != 0.0) rb.toggles.g_divJ2 = args.wilson_toggles.g_divJ2;
        if (args.wilson_toggles.g_stateSq != 0.0) rb.toggles.g_stateSq = args.wilson_toggles.g_stateSq;
        if (args.wilson_toggles.g_reactionDensity != 0.0) rb.toggles.g_reactionDensity = args.wilson_toggles.g_reactionDensity;
        if (args.wilson_toggles.g_genesisFlux != 0.0) rb.toggles.g_genesisFlux = args.wilson_toggles.g_genesisFlux;
        if (args.wilson_toggles.g_JdotDeltaS != 0.0) rb.toggles.g_JdotDeltaS = args.wilson_toggles.g_JdotDeltaS;
