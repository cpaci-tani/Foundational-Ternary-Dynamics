...
| `web_scale0_scenario_mirror_2026-08-27/scenarios/` | `engine/web/js/bridge/scenarios/` | Retired JS Scale-0 seed dispatcher and group implementations. C++ `ftd::dispatch_scenario` is the sole live owner; parity tests now compare the UI registry directly with C++. The live wave-spectrum utility and genesis-panel term profile were extracted into `engine/web/js/scales/scale0/`. |
| `web_ui_scaffolding_2026-08-27/` | Six no-op wrappers under `engine/web/js/ui/panels/` | Unimported migration scaffolds that only set a `data-component` attribute. Real inspector/zoo/ontic/physics/planetary/cosmic panels are owned by their existing runtime modules. |
