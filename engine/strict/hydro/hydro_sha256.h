#pragma once
// Self-contained SHA-256 (FIPS 180-4). No external dependency: used to verify the
// collision-table blob against tables::TABLE_HASH before it is trusted by the runtime.
#include <array>
#include <cstddef>
#include <cstdint>
#include <vector>
...
inline std::array<unsigned char, 32> sha256(const std::uint8_t* data, std::size_t length) {
    ...
    // Padded message: original bytes + 0x80 + zero pad + 64-bit big-endian bit length,
    ...
    for (int i = 7; i >= 0; --i) msg.push_back(std::uint8_t(bit_length >> (8 * i)));
