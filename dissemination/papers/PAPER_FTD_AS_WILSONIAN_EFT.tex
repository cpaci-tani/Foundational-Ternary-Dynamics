...
asymptotic-freedom direction of textbook QED but with magnitude roughly
$160\times$ the one-loop QED prediction --- a new quantitative
...
[\textsc{imposed}] in the parametric-insertions catalogue. The 1/L²
extrapolation of $\alpha_{\mathrm{eff}}(L)$ from $L \in
\{32,48,64\}$ gives $\alphaLargeL = 0.021$, a factor of $2.9$ above the
...
reproduces the static V(r) $\alpha$-extraction exactly at small impact
parameter, confirming the 5$\times$ ratio to $\alpha_{\text{ref}}$ as
genuine engine physics rather than a methodology artefact.
...
the honest interpretation. The measured $\alpha_r(r, L)$ is **not a
coupling constant at all}: the engine's emergent-mode Gauss law is
...
\SI{1}{\percent} precision. The $L = 48$ outlier destabilises the fit;
...
quantitatively matches textbook QED at the pre-registered tolerances.
...
two-point fits that converge, and a 1/L² fit value that trends toward
$\alphaREF$ as $L$ grows through the tested range --- establishes that
the five-pillar checklist is not empty on this lattice. The quantitative
gaps are attributed to
(i) a 6-iteration SOR projection that limits gauge-invariance fidelity,
(ii) a screening-length $\lambda \sim 10a$ that obscures the Coulomb-tail
regime at the measurement scales tested, and (iii) a
...
textbook QED --- or it does not, and the prediction stands as a
