...
--- the lowest ratio measured in the entire programme, and strong
evidence that the continuum limit is $\alpha_{\text{ref}}$ itself.
\end{abstract}
...
emerging as we probe farther from the pair. The scale dependence of
$\alpha(r_{\max})$ across L = 64, 128, 256 (4.1, 3.8, 1.4) supports
the Phase-4C extrapolation conjecture that the continuum limit is
$\alpha_{\mathrm{ref}}$. A larger-L measurement --- feasible in minutes
on GPU once the CUDA build is unblocked --- would confirm or refute.

The headline ``5$\times$ gap to $\alpha_{\mathrm{ref}}$'' characterising
the engine at moderate L is therefore \textit{not} a persistent engine
offset but a finite-size effect that shrinks monotonically. At
$L = 256$ we are already at 1.4$\times$.
