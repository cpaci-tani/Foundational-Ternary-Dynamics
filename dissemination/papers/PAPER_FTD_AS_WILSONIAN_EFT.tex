...
consistent conditions (ticks = 150, same r/L $\approx 0.31$ slice).
The result retracts an earlier 3-point estimate ($\alpha_\infty = 1.23
\alpha_{\text{ref}}$ was based on under-equilibrated CPU data) and
a line-by-line audit of the V(r) extraction pipeline
(\texttt{AUDIT\_ALPHA\_EXTRACTION.md}) establishes the honest range.
The 4-point extrapolation is a \textbf{plateau, not convergence to
CODATA}, at
$\alpha_\infty \approx 1.8\text{--}3.6 \times \alpha_{\text{ref}}$,
where the factor-of-two is an explicit energy-accumulator convention
(engine uses $\sum|J|^2$ without the classical $\tfrac{1}{2}$) and the
residual $\sim 1.8\times$ after convention correction is an open
quantitative question (real physics vs.\ kinetic-normalization choice
vs.\ lattice-Green's-function artefact). The engine's Coulomb-tail
coupling is demonstrably \emph{not} $\alpha_{\text{ref}}$; whether the
true classical-convention plateau is a genuine FTD prediction or a
resolvable systematic is explicitly flagged [OPEN].
\end{abstract}
...
\paragraph{What remains open after the audit.}
The engine does not reproduce $\alpha_{\text{ref}}$ at the Coulomb
tail under any convention. After absorbing the $\times 2$ convention
factor, the residual $\sim 1.8\times$ admits at least three
interpretations, and the Phase-F data do not discriminate between
them:
\begin{enumerate}
\item \textbf{Real physics} --- bare lattice dynamics produce
$\alpha \ne \alpha_{\text{ref}}$; the definitional identity
$\alpha_{\text{EFT}} = G_C^2 = \alpha$ in \verb|constants.h| is
a target, not a guaranteed outcome of the dynamics.
\item \textbf{Kinetic normalization} --- the engine's Lagrangian
uses $|\partial J|^2$ rather than the classical $\tfrac{1}{2}
|\partial J|^2$; field redefinition $J \to J/\sqrt{2}$ absorbs
the factor but changes the coupling's interpretation.
\item \textbf{Lattice-Green's-function artefact} --- the 7-point
Laplacian has a different discrete Green's function than the
continuum $1/r$; staggered vs.\ cell-centered placement of $J$
shifts the effective Coulomb coefficient.
\end{enumerate}
Matched-stencil Poisson (Phase 2 Ticket A) closes some of the Ward
floor but not by a factor of 2. Any of (1)--(3) would qualitatively
close the residual; isolating them is an explicit [OPEN] ticket
and the appropriate next-phase target.
