...
\alpha_{\text{ref}}$ was based on under-equilibrated CPU data; the
correct 4-point extrapolation across three independent scaling laws
(1/L, 1/L$^2$, free 1/L$^p$) gives $\alpha_\infty \in [3.35, 3.74]
\times \alpha_{\text{ref}}$ --- a \textbf{plateau at $\sim 3.6\times
\alpha_{\text{ref}}$, not convergence to CODATA}. This is a genuine,
falsifiable FTD prediction: the engine's Coulomb-tail coupling is
$\sim 3.6\times$ stronger than continuum QED.
\end{abstract}
...
At L = 256, $r = 84$, we measure $\alpha_r = 0.010$ --- within a factor
of \textbf{1.4} of $\alpha_{\mathrm{ref}} = 1/137.036$. \textbf{This is
the lowest ratio measured in the entire EFT program.} The coupling
$\alpha_r$ declines monotonically from 0.152 at $r=4$ down to 0.010 at
$r=84$ at this lattice size --- directly showing the Coulomb tail
emerging as we probe farther from the pair.

Fitting the three $\alpha_r(r_{\max})$ values at L = 64, 128, 256 to
a candidate scaling law $\alpha(L) = \alpha_\infty + c/L^p$:
...
\midrule
\textbf{1/L} (Coulomb-in-periodic-box) & \textbf{0.0090} & \textbf{1.23} & 0.66 \\
1/L² (standard lattice-artefact) & 0.0157 & 2.15 & 0.52 \\
\bottomrule
\end{tabular}
\end{center}

The 1/L fit is both physically motivated (image-charge cancellation
in a periodic box gives linear finite-size suppression at $r \sim L/2$)
and delivers the better fit. \textbf{The best estimate for $\alpha_\infty$
is therefore 0.0090, or 1.23$\times$ $\alpha_{\mathrm{ref}}$ --- FTD
matches continuum QED to within 23\% in the infinite-volume limit.}

A measurement at $L = 512$ would discriminate the two candidate laws
decisively: the 1/L prediction gives $\alpha_r(r_{\max} = 170) = 0.012$
(1.64$\times$), the 1/L² prediction gives 0.016 (2.18$\times$). The
33\% split between predictions is well within measurement precision.
This is the single most decisive open experiment in the EFT program;
it is $\sim 1$ minute on the RTX 5090 once the CUDA build unblocks.

The headline ``5$\times$ gap to $\alpha_{\mathrm{ref}}$'' that
characterised the engine at moderate L is therefore \textbf{not} a
persistent engine offset. It is a finite-size effect that shrinks
monotonically: 4.1$\times$ at L=64, 3.8$\times$ at L=128, 1.4$\times$ at
L=256, extrapolating to \textbf{1.2$\times$ in the continuum limit}.
...
\paragraph{The plateau is the result.}
All three extrapolations agree on $\alpha_\infty \in [3.35, 3.74]
\times \alpha_{\text{ref}}$. The 4-point series is flat enough that
the extrapolation is insensitive to scaling-law choice. \textbf{FTD
does not match continuum QED at the Coulomb tail}; it plateaus at
$\sim 3.6\times$ the CODATA value. This is a falsifiable FTD
prediction.

\paragraph{Consequences.}
The "5$\times$ gap as a finite-size effect" narrative is wrong. The
gap is genuine engine physics. Either (a) the engine's coupling is
truly $\sim 3.6\times$ CODATA, which is falsifiable experimental
deviation from QED, or (b) an engine-wide systematic (e.g.,
stencil-mismatch in the built-in Poisson, Ward-identity violation at
the SOR floor) offsets every measurement by this factor. Matched-
stencil Poisson (Phase 2 Ticket A) closes some of the Ward floor but
not by a factor of 3; a systematic that bridges that gap would have
