...
\subsection{Cross-check with the linear-level prediction}
...
\begin{tabular}{c c c}
\toprule
$L$ & mean $N(\text{ic1})$ & deviation from 25 \\
\midrule
32  & $\sim 25$ & $\leq 0\%$ \\
64  & 26.0 & $+4\%$ \\
128 & 27.0 & $+8\%$ \\
...
Agreement is within the engine's independently-measured $k(A)$ drift
envelope ($k = 0.239 \pm 0.018$ across $A \in [10, 50]\cdot\KGen$
per the FTD-0110 LEDGER row). The slight upward drift in mean
$N$ with $L$ ($25 \to 26 \to 27$) is consistent with random walk in
the seed ensemble; the standard error across the 5 seeds at $L=128$
is 1.4 voxels.
...
volume range.} The bridge between FTD's algebraic spine and engine
phenomenology, [DERIVED at linear level] on 2026-04-28
(\S\ref{sec:bridge}), is now empirically anchored across $\{32, 64,
128\}$ with no finite-L correction required at this resolution.
