% ============================================================================
% PAPER A · A pi-free generator of the lemniscatic field, with applications
%           to a closed-form quadratic for the fine-structure constant
% ============================================================================
% Target venue: Letters in Mathematical Physics (~10 pages).
% Strategy:     docs/theory/STRATEGY_PAPER_SPLIT_2026-04-30.md
% Created:      2026-05-01 evening (post physics-bridge crystallization).
% Citation targets:
%   - docs/theory/01_reference/SPEC_ALGEBRAIC_SPINE.md (Theorems 1, 2, 8, 9)
%   - docs/theory/01_reference/SPEC_PHYSICS_BRIDGE.md  (uniqueness scans)
%   - docs/theory/03_derivations/THEOREM_HARMONIC_INVARIANT_TOWER.md (Theorem 8)
%   - docs/theory/07_assessment/sm_constants_mass_flavour/AUDIT_LOOK_ELSEWHERE_RESULTS.md     (FTD-0097)
%   - docs/theory/09_mathematical/EXPLR_TOWER_MULTIPLIER_UNIQUENESS.md
%   - docs/theory/09_mathematical/EXPLR_POLYNOMIAL_LOOK_ELSEWHERE.md
% Build:   cd dissemination/papers && pdflatex PAPER_A_PI_FREE_GENERATOR.tex
% Status:  DRAFT v2 (2026-05-02). v2 update: §6.2 polynomial-shape scan
%          extended from 147,456 to 2,871,576 polynomials/multipliers
%          (~19.5× original) under pre-registration tag
%          `preregister-polynomial-scan-extended-v1`; 0 dual-matchers in
%          Eisenstein-integer multiplier family confirms (1+i, k=4)
%          Gaussian-integer choice is structurally distinguished.
%          Look-elsewhere trials factor (scan-size prior-odds heuristic, NOT a
%          calibrated Bayes factor) on the order of 4x10^5; tolerance-conditioned.
%          (2026-06-24 spine-audit reconciliation: see FTD-0319/0320, AUDIT_RIGIDITY_CATALOG_v1.)
% CORRECTION (2026-08-05, FTD-0802): the v2 headline above is WRONG in two ways.
%          (1) The cubic-embedding claim ("every cubic dual-matcher factors as
%          master x linear") was a hardcoded literal in the runner, never a
%          computed test, and is FALSE: all four cubic dual-matchers are
%          x*P(x) - a_0 with a_0 in {1, G*, 2, 3}, nonzero remainder. The true
%          count is 5 distinct dual-matchers, 4 beyond the master quadratic.
%          (2) The pre-registered base-rate control (PREREG_OT33_BASERATE_v1,
%          verify_ot33_baserate.py) returned Outcome B: N_null = 0.0014, i.e.
%          finding zero others is what chance predicts, so the count carries no
%          evidential weight; the x_- gate eliminates nothing at the registered
%          tolerances. Body text corrected in-place below (S6.2, Summary).
% ============================================================================

\documentclass[11pt,reqno]{amsart}

\usepackage[margin=1in]{geometry}
\usepackage{amsmath,amssymb,amsthm,mathtools}
\usepackage{booktabs}
\usepackage{hyperref}
\usepackage{xcolor}
\usepackage[numbers,sort&compress]{natbib}
\usepackage{enumitem}

\hypersetup{colorlinks=true,
  linkcolor=blue!50!black,
  citecolor=blue!50!black,
  urlcolor=blue!50!black}

\theoremstyle{plain}
\newtheorem{theorem}{Theorem}[section]
\newtheorem{lemma}[theorem]{Lemma}
\newtheorem{proposition}[theorem]{Proposition}
\newtheorem{corollary}[theorem]{Corollary}
\theoremstyle{definition}
\newtheorem{definition}[theorem]{Definition}
\theoremstyle{remark}
\newtheorem{remark}[theorem]{Remark}

\numberwithin{equation}{section}

\newcommand{\Gstar}{G_{\!*}}
\newcommand{\Aut}{\operatorname{Aut}}
\newcommand{\disc}{\operatorname{disc}}

\title[A $\pi$-free generator of the lemniscatic field]
{A $\pi$-free generator of the lemniscatic field,\\
 with applications to a closed-form quadratic\\
 for the fine-structure constant}

\author{William J.\ Steinmetz III}
\email{williamjsteinmetziii@gmail.com}

\author{C.\ Pacitanimulli}
\email{cpacitanimulli@gmail.com}

\date{\today}

\subjclass[2020]{Primary 11J81, 11G15;
                 Secondary 11M06, 33B15, 81V05}
\keywords{Lemniscatic constant; complex multiplication; class-number-one;
  Chowla--Selberg; algebraic independence; Chudnovsky's theorem;
  fine-structure constant.}

\begin{document}

\begin{abstract}
Let $\Gstar := \Gamma(1/4)/\Gamma(3/4)$. We record four theorems about
$\Gstar$ and a one-parameter polynomial tower built from it. The
quadratic $P(x) = x^{2} - 16\,\Gstar^{2}\,x + 16\,\Gstar^{3}$ has
closed-form roots $x_{\pm} = 8\Gstar^{2} \pm 4\Gstar\sqrt{4\Gstar^{2}
- \Gstar}$. Embedded in the level-indexed family $M_{k}(x) := x^{2} -
2^{k}\Gstar^{k-2}\,x + 2^{k}\Gstar^{k-1}$ for $k \geq 3$, the
normalised roots $y_{\pm} := x_{\pm}/\Gstar$ satisfy the harmonic
invariant $1/y_{+} + 1/y_{-} = 1$ at every level, and the level-$k$
discriminant correction $A_{k} := 2^{k-2}\Gstar^{k-3} - 1$ is
transcendental over $\mathbb{Q}$ for $k \geq 4$ (rational at $k=3$).
\emph{Conditional on Chudnovsky's 1976 algebraic-independence theorem
for $\pi$ and $\Gamma(1/4)$,} we show that $\mathbb{Q}(\Gstar)$ is a
$\pi$-free subfield of $\mathbb{Q}(\pi, \Gamma(1/4))$, in the precise
sense that $\mathbb{Q}(\Gstar) \cap \mathbb{Q}(\pi) = \mathbb{Q}$.
The numerical proximity $x_{+} \approx 137.036$ to the reciprocal
fine-structure constant $1/\alpha$ (1.26 ppm to CODATA 2022) and
$x_{-} \approx 3.024$ to the QCD colour number $N_{c}=3$ (0.80\%) is
recorded as a strongly motivated empirical conjecture, not a
derivation; the conditional closed form
$\alpha = 1/(2\Gstar) - \sqrt{4\Gstar - 1}/(4\,\Gstar^{3/2})$ follows
algebraically from $P(x_{+}) = 0$ together with the identification
$x_{+} = 1/\alpha$. We assess the strength of this empirical
identification via two pre-registered structural-uniqueness scans
within natural polynomial families generated by $\Gstar$ and small
integers (a tower-multiplier scan over Gaussian-integer norms, and a
combined polynomial-shape scan over $\sim 2.87 \times 10^{6}$
polynomials/multipliers including rational-coefficient,
cubic-embedding, and Eisenstein-integer multiplier extensions), and
via a prior monomial-level look-elsewhere audit which is over-rich
and therefore cited as a methodological caveat rather than as
evidence. The Eisenstein-integer multiplier extension returns zero
dual-matchers, indicating that the $(1+i, k=4)$ Gaussian-integer
choice that defines the master quadratic is structurally
distinguished rather than generic.
\end{abstract}

\maketitle

% ============================================================================
\section{Introduction}
\label{sec:intro}

The constant
\begin{equation}
\label{eq:gstar-def}
  \Gstar \;:=\; \frac{\Gamma(1/4)}{\Gamma(3/4)} \;=\; 2.95867512\ldots
\end{equation}
arises in three classical settings: as the value of the lemniscate
period for the curve $E\colon y^{2} = x^{3} - x$, as a closed-form
evaluation of an $L$-series for the imaginary quadratic field
$\mathbb{Q}(i)$ via Chowla--Selberg, and as the closed-form value of
Watson's three-dimensional body-centred-cubic integral
$W_{3} = \Gstar^{2}/(2\pi)$ \cite{watson1939, glasser-zucker}. The
present paper concerns four mathematical statements about $\Gstar$
and a closed-form algebraic application.

The mathematical content is summarised in
\S\S\ref{sec:gstar}--\ref{sec:qg}: an algebraic identity for $\Gstar$
(Theorem~\ref{thm:gstar}), a one-parameter polynomial family in
which $\Gstar$ enters with integer multipliers
(Theorems~\ref{thm:masterq} and \ref{thm:tower}), and a
field-theoretic statement that
$\mathbb{Q}(\Gstar)$ is $\pi$-free in the field
$\mathbb{Q}(\pi, \Gamma(1/4))$ \emph{conditional on the Chudnovsky
algebraic-independence theorem} (Theorem~\ref{thm:qgstar}).

The application appears in \S\ref{sec:closed-form}. The smallest
member of the polynomial family --- the quadratic
\begin{equation}
\label{eq:masterq-intro}
  P(x) \;=\; x^{2} - 16\,\Gstar^{2}\,x + 16\,\Gstar^{3}
\end{equation}
--- has two real roots, the larger of which has been observed
empirically to coincide with the reciprocal fine-structure constant
$1/\alpha$ to 1.26 parts per million on CODATA 2022 data. We take this
empirical observation as motivation but \emph{not} as derivation;
\S\ref{sec:closed-form} states the consequence as a corollary
conditional on the identification $x_{+} = 1/\alpha$, and reports the
algebraic closed form
\[
  \alpha \;=\; \frac{1}{2\Gstar} \;-\; \frac{\sqrt{4\Gstar - 1}}{4\,\Gstar^{3/2}}
\]
in that conditional sense. The smaller root $x_{-} \approx 3.024$ is
observed to coincide with the QCD colour number $N_{c} = 3$ at
0.80\%, a fact we record but do not lean on.

The empirical identifications $x_{+} = 1/\alpha$ and $x_{-} = N_{c}$
are external to the mathematical content of the paper. Their
plausibility within natural polynomial complexity classes generated
by $\Gstar$ and small integers is the subject of
\S\ref{sec:uniqueness-and-caveat}, where we report two
structural-uniqueness scans (the master quadratic is rank-1 in a
58-element tower-style family with a five-orders-of-magnitude gap to
the runner-up; it is the unique simultaneous dual-matcher among
$147{,}456$ polynomials of analogous shape). We also cite a prior
monomial-level look-elsewhere audit which establishes that catalogues
of single-monomial fits at parts-per-million precision are over-rich
under any natural null model; the master quadratic's standing is
therefore argued at \emph{polynomial} level rather than at
monomial level.

A note on terminology. The constant $\Gstar = \Gamma(1/4)/\Gamma(3/4)
\approx 2.9587$ is sometimes called ``the lemniscate constant'' in
informal contexts; it is to be distinguished from the
Bernoulli--Gauss lemniscate constant $\varpi = \Gamma(1/4)^{2}/(2\sqrt{2\pi})
\approx 2.6221$. In this paper we reserve $\Gstar$ for the ratio
$\Gamma(1/4)/\Gamma(3/4)$ throughout, and write $\varpi$ explicitly
when the Bernoulli value is meant. The two are linked by
$\Gstar = 2\varpi/\sqrt{\pi}$.

\paragraph{Notation.} Throughout, $\mathbb{Q}$ denotes the rationals,
$\overline{\mathbb{Q}}$ their algebraic closure, and $\Gamma$ Euler's
gamma function. Numerical values quoted to many digits are computed
from \eqref{eq:gstar-def} and the elementary identity
$\Gamma(1/4)\,\Gamma(3/4) = \pi\sqrt{2}$.

% ============================================================================
\section{The constant $\Gstar$}
\label{sec:gstar}

\begin{theorem}[Algebraic identity for $\Gstar$]
\label{thm:gstar}
With $\Gstar$ defined by \eqref{eq:gstar-def}, the equivalent forms
\begin{equation}
\label{eq:gstar-forms}
  \Gstar \;=\; \frac{\Gamma(1/4)}{\Gamma(3/4)}
        \;=\; \frac{\Gamma(1/4)^{2}}{\pi\sqrt{2}}
        \;=\; \frac{2\,\varpi}{\sqrt{\pi}}
\end{equation}
hold, where $\varpi := \Gamma(1/4)^{2}/(2\sqrt{2\pi})$ is the
Bernoulli--Gauss lemniscate constant. Numerically
$\Gstar = 2.95867512\ldots$ and $16\,\Gstar^{2} = 140.060\ldots$.
\end{theorem}

\begin{proof}
The Euler reflection formula gives
$\Gamma(1/4)\,\Gamma(3/4) = \pi/\sin(\pi/4) = \pi\sqrt{2}$. Dividing
$\Gamma(1/4)^{2}$ through by this identity yields the second form;
the third follows from the definition of $\varpi$.
\end{proof}

The two further classical evaluations of $\Gstar$ --- via the
Chowla--Selberg formula applied to the elliptic curve
$E\colon y^{2} = x^{3} - x$, and via Watson's $W_{3}$ identity
\cite{watson1939} --- are consequences of $\eqref{eq:gstar-forms}$
together with standard facts about the lemniscatic period
\cite{chowla-selberg, glasser-zucker}; we cite them but do not
re-prove them here. They serve to anchor $\Gstar$ as a constant of
established mathematical literature, not a contrivance for this
paper.

% ============================================================================
\section{The master quadratic}
\label{sec:masterq}

\begin{theorem}[Master quadratic and roots]
\label{thm:masterq}
Let $P(x) := x^{2} - 16\,\Gstar^{2}\,x + 16\,\Gstar^{3}$. The
discriminant
\begin{equation}
\label{eq:disc}
  \disc(P) \;=\; 256\,\Gstar^{4} - 64\,\Gstar^{3}
            \;=\; 64\,\Gstar^{3}(4\,\Gstar - 1)
\end{equation}
is positive since $\Gstar > 1/4$. Hence $P$ has two distinct real
roots
\begin{equation}
\label{eq:roots}
  x_{\pm} \;=\; 8\,\Gstar^{2} \;\pm\; 4\,\Gstar\,\sqrt{4\,\Gstar^{2} - \Gstar}.
\end{equation}
Numerically $x_{+} = 137.036171\ldots$ and $x_{-} = 3.023964\ldots$.
\end{theorem}

\begin{proof}
Direct application of the quadratic formula and computation of the
discriminant. Positivity of $\disc(P)$ uses only $\Gstar > 1/4$,
which is immediate from \eqref{eq:gstar-forms} and
$\Gamma(1/4) > \Gamma(3/4) > 0$.
\end{proof}

The polynomial $P$ has integer-and-$\Gstar$-power coefficients, with
both coefficients sharing the integer factor 16. We do not assign a
mathematical meaning to the value 16 in this paper beyond ``the
smallest integer for which the resulting two real roots are positive
and admit a clean closed form''. (The factor 16 also appears as
$|\Aut(E)|^{2}$ for $E\colon y^{2} = x^{3}-x$, which has automorphism
group of order 4 over $\overline{\mathbb{Q}}$; this connection is not
used in any argument below and is mentioned only to fix the reader's
intuition.)

% ============================================================================
\section{The harmonic-invariant tower}
\label{sec:tower}

The master quadratic is the level-$k=4$ instance of a one-parameter
family.

\begin{definition}[The level-indexed family]
For each integer $k \geq 3$, define
\begin{equation}
\label{eq:tower-def}
  M_{k}(x) \;:=\; x^{2} \;-\; 2^{k}\,\Gstar^{k-2}\,x \;+\; 2^{k}\,\Gstar^{k-1}.
\end{equation}
\end{definition}

The $k=4$ instance is the master quadratic of
Theorem~\ref{thm:masterq}, since $2^{4} = 16$. Each $M_{k}$ has two
real roots $x_{\pm}^{(k)}$; we write $y_{\pm}^{(k)} :=
x_{\pm}^{(k)}/\Gstar$ for the normalised roots.

\begin{theorem}[Harmonic invariant; level-$k$ discriminant transcendence]
\label{thm:tower}
For every $k \geq 3$:
\begin{enumerate}[label=\textup{(\roman*)}, leftmargin=*]
\item \emph{Harmonic invariant.} $1/y_{+}^{(k)} + 1/y_{-}^{(k)} = 1$.
\item \emph{Discriminant factorisation.}
$\disc(M_{k}) = 2^{k+2}\,\Gstar^{k-1}\,A_{k}$, where
\begin{equation}
\label{eq:Ak}
  A_{k} \;:=\; 2^{k-2}\,\Gstar^{k-3} - 1.
\end{equation}
\item \emph{Discriminant transcendence.} $A_{3} = 1 \in \mathbb{Q}$,
and $A_{k} \in \mathbb{R} \setminus \overline{\mathbb{Q}}$ for every
$k \geq 4$ \emph{conditional on the Schneider--Chudnovsky theorem
\cite{chudnovsky1976}.}
\end{enumerate}
\end{theorem}

\begin{proof}
\textit{(i)} By Vieta's relations applied to $M_{k}$,
$x_{+}^{(k)} + x_{-}^{(k)} = 2^{k}\,\Gstar^{k-2}$ and
$x_{+}^{(k)}\,x_{-}^{(k)} = 2^{k}\,\Gstar^{k-1}$. Hence
\[
  \frac{1}{x_{+}^{(k)}} + \frac{1}{x_{-}^{(k)}}
  \;=\; \frac{x_{+}^{(k)} + x_{-}^{(k)}}{x_{+}^{(k)}\,x_{-}^{(k)}}
  \;=\; \frac{2^{k}\,\Gstar^{k-2}}{2^{k}\,\Gstar^{k-1}}
  \;=\; \frac{1}{\Gstar}.
\]
Multiplying through by $\Gstar$ gives
$1/y_{+}^{(k)} + 1/y_{-}^{(k)} = 1$.

\textit{(ii)} Direct expansion:
\[
  \disc(M_{k}) \;=\; (2^{k}\Gstar^{k-2})^{2} - 4 \cdot 2^{k}\Gstar^{k-1}
              \;=\; 2^{2k}\Gstar^{2k-4} - 2^{k+2}\Gstar^{k-1}
              \;=\; 2^{k+2}\Gstar^{k-1}\bigl(2^{k-2}\Gstar^{k-3} - 1\bigr).
\]

\textit{(iii)} At $k=3$, $A_{3} = 2^{1}\Gstar^{0} - 1 = 1$. For
$k \geq 4$, $A_{k} = 2^{k-2}\Gstar^{k-3} - 1$ is a polynomial in
$\Gstar$ with rational coefficients, of degree $k - 3 \geq 1$, with
non-zero leading coefficient $2^{k-2}$. By the Chudnovsky
theorem \cite{chudnovsky1976}, $\pi$ and $\Gamma(1/4)$ are
\emph{algebraically independent} over $\mathbb{Q}$ --- a statement
strictly stronger than the transcendence of $\Gamma(1/4)$ alone
(Schneider, 1948), and the one needed here; together with the algebraic
relation $\Gstar\cdot\pi\sqrt{2} = \Gamma(1/4)^{2}$ from
\eqref{eq:gstar-forms}, this forces $\Gstar$ to be transcendental over
$\mathbb{Q}$. A
non-rational polynomial in a transcendental over $\mathbb{Q}$ with
rational coefficients takes only transcendental values
\cite[\S 1.4]{waldschmidt2000}. Hence $A_{k} \notin \overline{\mathbb{Q}}$.
\end{proof}

\begin{remark}
The $k=3$ instance $M_{3}(x) = x^{2} - 8\,x + 8\,\Gstar$ has roots
$x_{\pm}^{(3)} = 4 \pm \sqrt{16 - 8\Gstar}$. Since
$16 - 8\Gstar = 8(2 - \Gstar) < 0$ for our $\Gstar \approx 2.96$, the
$k=3$ roots are complex conjugates, and the harmonic invariant
$1/y_{+} + 1/y_{-} = 1$ holds in the complex sense. The first
\emph{real-rooted} level is $k = 4$, the master quadratic.
\end{remark}

The harmonic invariant $1/y_{+} + 1/y_{-} = 1$ characterises the
level-indexed family in the following sense. Among one-parameter
families of monic quadratics
$\{Q(x) = x^{2} - bx + c\}$ with $b, c \in \mathbb{R}_{>0}$, the
identity $1/y_{+} + 1/y_{-} = 1$ (for $y = x/\Gstar$) holds precisely
when $c = \Gstar \cdot b$. The family $\{M_{k}\}_{k \geq 3}$ is a
particular sub-family of this constraint, parameterised by the
integer multiplier $2^{k}$.

% ============================================================================
\section{$\mathbb{Q}(\Gstar)$ is $\pi$-free in $\mathbb{Q}(\pi, \Gamma(1/4))$}
\label{sec:qg}

\begin{theorem}[Conditional on Chudnovsky 1976]
\label{thm:qgstar}
\emph{Conditional on the Chudnovsky algebraic-independence theorem
for $\pi$ and $\Gamma(1/4)$ \cite{chudnovsky1976},}
$\mathbb{Q}(\Gstar)$ is a subfield of $\mathbb{Q}(\pi, \Gamma(1/4))$
satisfying
\begin{equation}
\label{eq:qg-trivial-int}
  \mathbb{Q}(\Gstar) \;\cap\; \mathbb{Q}(\pi) \;=\; \mathbb{Q}.
\end{equation}
That is, $\Gstar$ generates a subfield of
$\mathbb{Q}(\pi, \Gamma(1/4))$ whose intersection with
$\mathbb{Q}(\pi)$ is trivial.
\end{theorem}

\begin{proof}
\emph{Containment.} By \eqref{eq:gstar-forms},
$\Gstar = \Gamma(1/4)^{2}/(\pi\sqrt{2})$, a rational function in
$\{\Gamma(1/4), \pi\}$ over $\mathbb{Q}(\sqrt{2})$, hence over
$\mathbb{Q}$ after adjoining the algebraic element $\sqrt{2}$. Every
element of $\mathbb{Q}(\Gstar)$ is therefore a rational function in
$\{\Gamma(1/4), \pi\}$, i.e.\ lies in $\mathbb{Q}(\pi, \Gamma(1/4))$
after the algebraic adjunction (which we absorb into
$\overline{\mathbb{Q}}$ for the purposes of intersection with
$\mathbb{Q}(\pi)$).

\emph{Trivial intersection.} Suppose $\beta \in \mathbb{Q}(\Gstar) \cap
\mathbb{Q}(\pi)$. Then $\beta = p(\Gstar)/q(\Gstar)$ for some
$p, q \in \mathbb{Q}[T]$ with $q(\Gstar) \neq 0$, and simultaneously
$\beta = f(\pi)/g(\pi)$ for some $f, g \in \mathbb{Q}[T]$ with
$g(\pi) \neq 0$. Cross-multiplying yields
\begin{equation}
\label{eq:cross-mult}
  p(\Gstar)\,g(\pi) \;=\; q(\Gstar)\,f(\pi),
\end{equation}
a polynomial relation in $\Gstar$ and $\pi$ with rational coefficients.

We claim $\Gstar$ and $\pi$ are algebraically independent over
$\mathbb{Q}$. By Chudnovsky's 1976 theorem,
$\{\pi, \Gamma(1/4)\}$ is algebraically independent over $\mathbb{Q}$.
Suppose for contradiction there is a non-trivial
$P \in \mathbb{Q}[X, Y]$ with $P(\Gstar, \pi) = 0$. Substituting
$\Gstar = \Gamma(1/4)^{2}/(\pi\sqrt{2})$ into $P$ and clearing
denominators (multiplying by an appropriate power of $\pi$) yields a
polynomial $\widetilde{P} \in \mathbb{Q}(\sqrt{2})[X, Y]$ with
$\widetilde{P}(\Gamma(1/4), \pi) = 0$. Since $\sqrt{2}$ is algebraic
over $\mathbb{Q}$, $\widetilde{P}$ has coefficients in
$\overline{\mathbb{Q}}$, contradicting the algebraic independence of
$\{\pi, \Gamma(1/4)\}$ over $\mathbb{Q}$ (which holds equally over
$\overline{\mathbb{Q}}$ since the property is preserved under
algebraic extensions). Hence $\Gstar$ and $\pi$ are algebraically
independent over $\mathbb{Q}$.

Returning to \eqref{eq:cross-mult}: the only polynomial relation
$P(\Gstar, \pi) = 0$ consistent with algebraic independence is the
trivial one in which both sides reduce identically. This forces
$\beta$ to be a constant, hence $\beta \in \mathbb{Q}$. \qedhere
\end{proof}

\begin{remark}[On the conditionality]
The theorem rests on Chudnovsky's 1976 result \cite{chudnovsky1976}, a
standard result of contemporary transcendence theory consolidated in
\cite[\S 1.4]{waldschmidt2000}. ``Conditional'' here means
``depends on this established theorem'', not ``depends on a
conjecture''. The conditionality is recorded in the abstract, the
theorem statement, and the conclusion (\S\ref{sec:discussion}) in
keeping with the convention that any result whose proof appeals to a
named external theorem in transcendence theory should bear the cite
visibly.
\end{remark}

\begin{remark}[What the theorem does not claim]
Theorem~\ref{thm:qgstar} does \emph{not} claim that
$\mathbb{Q}(\Gstar)$ is a maximal $\pi$-free subfield of
$\mathbb{Q}(\pi, \Gamma(1/4))$ in any extremal sense. Larger
$\pi$-free subfields may exist; the result only establishes
$\mathbb{Q}$-triviality of the intersection with $\mathbb{Q}(\pi)$.
\end{remark}

% ============================================================================
\section{Closed form for $\alpha$ conditional on the empirical match}
\label{sec:closed-form}

The numerical value $x_{+} = 137.036171\ldots$ from
Theorem~\ref{thm:masterq} matches the reciprocal fine-structure
constant $1/\alpha$ to 1.26 parts per million on CODATA 2022 data.
Specifically, the CODATA 2022 recommended value is
$\alpha^{-1} = 137.035999177(21)$ with quoted relative uncertainty
$\approx 1.5 \times 10^{-10}$ \cite{codata2022}. The discrepancy
\[
  |x_{+} - \alpha^{-1}| / \alpha^{-1} \;=\; 1.26 \times 10^{-6}
\]
exceeds CODATA precision by four orders of magnitude; the match is
empirical, not exact.

We do not derive $\alpha = 1/x_{+}$ from any of the mathematical
statements above. The empirical proximity is recorded as a strongly
motivated conjecture; in this paper it serves only as motivation.

\begin{corollary}[Closed form, conditional]
\label{cor:closed-form}
If the empirical identification $x_{+} = 1/\alpha$ holds exactly,
then
\begin{equation}
\label{eq:closed-form-alpha}
  \alpha \;=\; \frac{1}{2\,\Gstar} \;-\; \frac{\sqrt{4\,\Gstar - 1}}{4\,\Gstar^{3/2}}.
\end{equation}
\end{corollary}

\begin{proof}
By Theorem~\ref{thm:masterq}, $x_{+} = 8\,\Gstar^{2} +
4\,\Gstar\sqrt{4\,\Gstar^{2} - \Gstar}$. Substituting
$\alpha = 1/x_{+}$ and rationalising gives
\[
  \alpha \;=\; \frac{1}{8\,\Gstar^{2} + 4\,\Gstar\,\sqrt{4\,\Gstar^{2} - \Gstar}}
        \;=\; \frac{8\,\Gstar^{2} - 4\,\Gstar\,\sqrt{4\,\Gstar^{2} - \Gstar}}
                   {(8\Gstar^{2})^{2} - 16\,\Gstar^{2}(4\,\Gstar^{2} - \Gstar)}
        \;=\; \frac{8\,\Gstar^{2} - 4\,\Gstar\,\sqrt{4\,\Gstar^{2} - \Gstar}}
                   {16\,\Gstar^{3}}.
\]
Splitting the fraction and simplifying $4\,\Gstar^{2} - \Gstar
= \Gstar(4\,\Gstar - 1)$ gives \eqref{eq:closed-form-alpha}.
\end{proof}

\begin{remark}
Corollary~\ref{cor:closed-form} is purely algebraic given the
identification. It does \emph{not} establish that $\alpha = 1/x_{+}$;
that identification is the conjecture, and the corollary is its
consequence. The role of \eqref{eq:closed-form-alpha} in the paper is
to make explicit that \emph{if} the conjecture holds, then $\alpha$
admits a closed form in $\Gstar$ alone --- equivalently, in
$\Gamma(1/4)$ and $\pi$ via \eqref{eq:gstar-forms}. The smaller root
$x_{-} \approx 3.024$ is observed to coincide with $N_{c} = 3$ at
0.80\%; the corresponding identification $x_{-} = N_{c}$ is recorded
but not used.
\end{remark}

% ============================================================================
\section{On the empirical identification: structural-uniqueness within natural polynomial families, and a methodological caveat}
\label{sec:uniqueness-and-caveat}

The corollary of \S\ref{sec:closed-form} is conditional on the
identification $x_{+} = 1/\alpha$. Independent of any physical or
philosophical reading of that identification, one may ask: among
polynomials of comparable algebraic complexity built from $\Gstar$
and small integers, how distinguished is the master quadratic by the
joint property of having one root near $1/\alpha$ and the other near
$N_{c} = 3$? We report two scans whose results bear on this
question, and one prior scan whose negative result we cite as
methodological caveat.

\subsection{Tower-multiplier scan}
\label{sec:tower-scan}

In the level-indexed family of \S\ref{sec:tower}, the multiplier
$2^{k}$ may be replaced by a free Gaussian-integer norm $m^{k}$:
\begin{equation}
\label{eq:family-tower}
\begin{aligned}
  M_{m,k}(x) \;&:=\; x^{2} - m^{k}\,\Gstar^{k-2}\,x + m^{k}\,\Gstar^{k-1}, \\
  m \;&\in\; \{1, 2, 4, 5, 8, 9, 10, 13, 16, 17, 18, 20\}, \quad
  k \;\in\; \{3, 4, 5, 6, 7\}.
\end{aligned}
\end{equation}
The set of $m$-values consists of the twelve smallest distinct norms
$a^{2}+b^{2}$ of Gaussian integers $(a + bi) \neq 0$, mirroring the
Gaussian-integer-tower structure of $\mathbb{Z}[i]$. Of the $58$ pairs
$(m, k)$ in this scan that produce real roots, exactly one has
$x_{+}$ within 1\% of CODATA $\alpha^{-1} = 137.036$: the master
quadratic $(m=2, k=4)$, with relative discrepancy $1.26 \times
10^{-6}$. The runner-up, $(m=4, k=3)$, has $x_{+} = 186.35$, a
relative discrepancy of $0.36$. The gap from rank 1 to rank 2 spans
five orders of magnitude in relative error.

We note that, restricting to $k = 4$, the polynomials $M_{1, 4}$
(roots complex), $M_{4, 4}$, $M_{5, 4}$, $M_{8, 4}$, and $M_{9, 4}$
all have $x_{+}$ values $\geq 2238$, far from $137$. Restricting
instead to $m = 2$, the polynomials $M_{2, 3}$, $M_{2, 5}$, etc.\
have $x_{+}$ values of order $10^{1}$ or $10^{3}$. The pair $(m=2,
k=4)$ is genuinely isolated within the family
\eqref{eq:family-tower}.

\subsection{Polynomial-shape scan (extended)}
\label{sec:polynomial-scan}

A finer scan considers the four-parameter family of degree-2
polynomials with integer multipliers and small integer $\Gstar$-powers
in both coefficients independently:
\begin{equation}
\label{eq:family-polynomial}
  M_{n, p, m, q}(x) \;:=\; x^{2} - n\,\Gstar^{p}\,x + m\,\Gstar^{q},
  \qquad n, m \in \{1, \ldots, 64\},
  \quad p, q \in \{0, 1, 2, 3, 4, 5\}.
\end{equation}
The base family has cardinality $64^{2} \cdot 6^{2} = 147{,}456$
polynomials; the master quadratic $P$ corresponds to
$(n, p, m, q) = (16, 2, 16, 3)$. We extended this base family along
three independent directions, producing a combined search space of
$2{,}871{,}576$ polynomials/multipliers (a $\approx 19.5\times$
enlargement). The extension and its tolerances were committed under
the pre-registration tag
\texttt{preregister\allowbreak-polynomial\allowbreak-scan\allowbreak-extended\allowbreak-v1}
prior to running the scan; the verification script and exact
numerical thresholds are reproducible from that commit.
\begin{enumerate}[label=\textup{(\Alph*)}, leftmargin=*]
\item \emph{Rational-coefficient extension.} Replace the integer
multipliers $n, m$ in \eqref{eq:family-polynomial} by reduced
fractions $n/d_{n}$, $m/d_{m}$ with denominators $d_{n}, d_{m}
\leq 4$, scanning $\sim 2.36\times 10^{6}$ polynomials. After
removing fraction-equivalent representations of the master quadratic
itself, this extension produces a single genuine dual-matcher: the
master quadratic at $(n, p, m, q) = (16, 2, 16, 3)$.
\item \emph{Cubic embeddings.} Test cubic polynomials
$Q(x) = x^{3} - a_{2}\,x^{2} + a_{1}\,x - a_{0}$ with coefficients of
the same $\Gstar$-power-with-integer-multiplier shape.
\textbf{[Corrected 2026-08-05, FTD-0802.]} An earlier version of this
item asserted that every cubic dual-matcher factors exactly as
$Q(x) = P(x)\cdot(x - r)$ and that the count of independent cubic
dual-matchers is therefore zero. That assertion was transcribed from a
runner line that was a hardcoded literal, never a computed test, and it
is \emph{false}: synthetic division shows all four cubic dual-matchers
leave a nonzero remainder under $P$. They are
$Q(x) = x\,P(x) - a_{0}$ for $a_{0} \in \{1, \Gstar, 2, 3\}$ ---
genuinely distinct members of the declared family whose root pairs land
inside the registered gates (largest $x_{+}$ deviation $1.63\times
10^{-4}$ against the $1.73\times10^{-4}$ gate). The exact factorisation
is impossible on its face: matching the constant term requires
$r = a_{0}/(16\Gstar^{3})$ while the $x^{2}$ term requires $r = 0$,
jointly satisfiable only at $a_{0} = 0$, outside the scanned range. The
count of independent cubic dual-matchers is therefore \emph{four}, not
zero.
\item \emph{Eisenstein-integer multiplier family.} Replace the
Gaussian-integer norms of the tower-multiplier scan
\S\ref{sec:tower-scan} by Eisenstein-integer norms
$N(a + b\omega) = a^{2} - ab + b^{2}$ for $\omega = e^{2\pi i / 3}$,
sweeping $(m, k)$ pairs at the same range as
\eqref{eq:family-tower}. The scan returns zero dual-matchers under
the master-quadratic tolerances. The same scan run over an extended
Gaussian-integer multiplier family (beyond the base set of
\eqref{eq:family-tower}) likewise returns zero non-master-quadratic
dual-matchers.
\end{enumerate}
Adopting the master-quadratic empirical precisions as tolerances ---
$1.26$ ppm absolute on $x_{+}$ and $0.80\%$ absolute on $x_{-}$,
both relative to the targets $1/\alpha = 137.035999\ldots$ and
$N_{c} = 3$ --- \textbf{five} distinct polynomials across the combined
extended space satisfy both constraints: the master quadratic and the
four cubics $x\,P(x) - a_{0}$ of item (B). (An earlier version stated
``exactly one''; corrected 2026-08-05 per FTD-0802.)

The result holds at the precise tolerances above. Relaxing the
$x_{+}$ tolerance from 1.26 ppm to $10^{-3}$ admits a single-digit
number of additional polynomials whose $x_{+}$ values are within
$10^{-3}$ of $\alpha^{-1}$, but none of these simultaneously
satisfies the $x_{-}$ constraint. The dual-matching property is
master-quadratic-specific within the extended family at master-quadratic
tolerances.

The Eisenstein-family null result of (C) is structurally informative:
it shows that the empirical match is not produced generically by any
``ring-of-integers tower'' construction. The $(1+i, k=4)$
Gaussian-integer choice that defines the master quadratic is
distinguished, not interchangeable with the analogous Eisenstein
choice. The robust statement is a \emph{numerical fact}: across the extended
family, the master quadratic is the unique \emph{quadratic}
dual-matcher at master-quadratic tolerance, and the four cubic
matchers of item (B) are the only others. Two further facts bound what
this count can evidence (FTD-0802, 2026-08-05): a pre-registered
base-rate control returned $N_{\mathrm{null}} = 0.0014$ expected
dual-matchers under displaced targets --- so a zero-others count is
the \emph{chance} expectation for this family, not a signal --- and at
the registered tolerances the $x_{-}$ gate eliminates no candidate
that passes the $x_{+}$ gate (16 pass each), so ``dual-match'' and
``match'' coincide here and the second target does no evidential work.
One may summarise the scan size as a
\emph{look-elsewhere trials factor} --- a uniform-prior, one-match
heuristic over the $147{,}456$ base family (order $10^{4}$), widened by
the present extension's scan-size factor of $\approx 19.5$ to order
$4 \times 10^{5}$. We stress that this is a trials-factor / prior-odds
heuristic read off the scan size, \emph{not} a calibrated posterior
Bayes factor: it carries no explicit likelihood model, and the
dual-match count is tolerance-conditioned --- the $x_{+}$ and $x_{-}$
acceptance windows are not symmetric, and relaxing to a common relative
tolerance admits additional matchers drawn from other constants. The
defensible conclusion --- that the master quadratic is distinguished
within the natural polynomial complexity classes generated by $\Gstar$,
small integers, and small Gaussian-integer norms, \emph{at the stated
tolerance} --- is unaltered by either reading.

\subsection{Caveat: monomial-level look-elsewhere}
\label{sec:lookelsewhere-caveat}

A prior pre-registered audit of the program's catalogue of
monomial-level fits (single-monomial expressions of the form
$c \cdot \Gstar^{a} \cdot \pi^{b}$ matched against dimensionless
physics constants at $\varepsilon = 10^{-4}$) reported a count of $62$
raw / $11$ deduplicated hits against a Poisson null expectation of
$\lambda = 4$, with $\chi^{2}_{19} = 470$ raw / $38$ deduplicated; the
catalogue is over-rich at monomial level under any natural null model
\cite{lookelsewhere}. We cite this audit as a methodological caveat:
parts-per-million matches at monomial level are not in themselves
evidence of structural privilege, because catalogues of monomials in
$\Gstar$ and $\pi$ are sufficiently dense to produce ppm matches by
chance.

The polynomial-level results of \S\S\ref{sec:tower-scan}--%
\ref{sec:polynomial-scan} are not reducible to monomial-level
coincidences, because the dual-matching condition (one polynomial,
two roots, two distinct targets) constrains the polynomial's
coefficients jointly. The monomial caveat sets the appropriate prior
against which the polynomial-level uniqueness should be read; the
master quadratic's standing is argued at polynomial level, where the
catalogue is sparse, not at monomial level, where it is over-rich.

\subsection{Summary}

The two structural-uniqueness scans of \S\S\ref{sec:tower-scan}--%
\ref{sec:polynomial-scan} record observational facts about natural
polynomial families generated by $\Gstar$ and small integers; they
neither establish nor refute any physical identification. They place
the empirical identification $x_{+} = 1/\alpha$, $x_{-} = N_{c}$
within a methodologically explicit context: the master quadratic is
the unique \emph{quadratic} of its complexity class to dual-match these
two targets at the empirical precisions stated above, joined by four
cubic relatives $x\,P(x) - a_{0}$; the pre-registered base-rate control
(FTD-0802) shows a zero-others count would in any case have been the
chance expectation for this family, and the $x_{-}$ gate is inert at
the registered tolerances. We leave the question of how to weigh this
evidence against discipline-specific priors to the reader.

% ============================================================================
\section{Discussion}
\label{sec:discussion}

The four mathematical statements (Theorems \ref{thm:gstar},
\ref{thm:masterq}, \ref{thm:tower}, \ref{thm:qgstar}) hold
unconditionally except for the third part of Theorem~\ref{thm:tower}
(transcendence of $A_{k}$ for $k \geq 4$) and Theorem~\ref{thm:qgstar}
(triviality of $\mathbb{Q}(\Gstar) \cap \mathbb{Q}(\pi)$), both of
which are conditional on Chudnovsky's 1976 algebraic-independence
theorem \cite{chudnovsky1976}. The conditionality is named in each
statement, repeated in the abstract, and re-iterated here: the four
theorems together form an unconditional core (Theorems \ref{thm:gstar},
\ref{thm:masterq}, \ref{thm:tower}(i)--(ii)) and a Chudnovsky-conditional
extension (Theorems \ref{thm:tower}(iii) and \ref{thm:qgstar}).

The closed-form expression for $\alpha$ in
Corollary~\ref{cor:closed-form} is conditional on the identification
$x_{+} = 1/\alpha$, which is empirical and at present unmotivated by
any derivation we know of. Its mathematical content is the algebraic
inversion of $P$ at the larger root; its physical content is exactly
what one chooses to make of the 1.26-ppm empirical match.

We close with three observations on possible directions.

\paragraph{(1) Are there larger $\pi$-free subfields of $\mathbb{Q}(\pi, \Gamma(1/4))$?}
Theorem~\ref{thm:qgstar} establishes that $\mathbb{Q}(\Gstar)$ is one
$\pi$-free subfield, with trivial intersection in the sense of
\eqref{eq:qg-trivial-int}. It does not characterise all such
subfields, nor does it claim maximality. A natural question is
whether $\mathbb{Q}(\Gstar)$ is maximal among $\pi$-free subfields
under appropriate conditions; we leave this open.

\paragraph{(2) Polynomial families beyond \eqref{eq:family-tower}, \eqref{eq:family-polynomial}.}
The two structural-uniqueness scans of \S\ref{sec:uniqueness-and-caveat}
restrict to two natural families. Broader families --- e.g.\ allowing
rational coefficients, allowing $\pi$ to enter the coefficients,
allowing degree-$\geq 3$ polynomials --- would either weaken or
strengthen the master quadratic's standing. We did not pursue these
broader scans here.

\paragraph{(3) Tower levels $k > 4$.}
The harmonic invariant of Theorem~\ref{thm:tower}(i) holds at every
level $k \geq 3$. Levels $k = 4$ and $k = 5$ produce real roots; we
have not investigated whether any higher level produces roots whose
numerical proximity to other dimensionless constants of physics is
similarly tight. Should such a level be identified, the corresponding
discriminant correction $A_{k}$ would by Theorem~\ref{thm:tower}(iii)
be transcendental for $k \geq 4$, and the analogous closed form would
follow from the same Vieta inversion as Corollary~\ref{cor:closed-form}.

\paragraph{(4) On the Eisenstein-family null result.}
The null result of \S\ref{sec:polynomial-scan}(C) admits a candidate
structural reading: the unit groups $\mathbb{Z}[i]^{\times}$ and
$\mathbb{Z}[\omega]^{\times}$ have orders 4 and 6 respectively, and
several theorem-level features of the present family (the level
$k = 4$ at which the master quadratic appears; the integer
coefficient $16 = |\Aut(E)|^{2}$ for $E\colon y^{2} = x^{3}-x$; the
discriminant transcendence threshold $k \geq 4$ of
Theorem~\ref{thm:tower}(iii)) all reference the integer 4 in
arguments that, separately, point at $\mathbb{Z}[i]$ rather than at
$\mathbb{Z}[\omega]$. Whether these three roles for the integer 4
share a common structural origin via a homomorphism from
$\mathbb{Z}[i]^{\times}$ to a relevant ambient group is, to the best
of our knowledge, an open algebraic question. We do not claim such a
homomorphism in this paper; we record the question as one that the
Eisenstein null result motivates.

% ============================================================================
\section*{Acknowledgements}

The Chowla--Selberg evaluation underlying $\Gstar$, Watson's $W_{3}$
identity, and Chudnovsky's algebraic-independence theorem are
classical results of analytic number theory and transcendence theory;
we have used them freely. Computational verification of the two
structural-uniqueness scans of \S\ref{sec:uniqueness-and-caveat} was
carried out in Python with arbitrary-precision arithmetic
(\texttt{mpmath}). The polynomial-shape scan of
\S\ref{sec:polynomial-scan} was committed under the pre-registration
tag \texttt{preregister\allowbreak-polynomial\allowbreak-scan\allowbreak-extended\allowbreak-v1}
prior to execution; scripts, the pre-registration tag, and
verification output are available at the project repository. The
tower-multiplier scan of \S\ref{sec:tower-scan} was likewise
committed under
\texttt{preregister\allowbreak-tower\allowbreak-level\allowbreak-scan\allowbreak-v1}.

% ============================================================================
\begin{thebibliography}{99}

\bibitem{chowla-selberg}
S.\ Chowla and A.\ Selberg, \textit{On Epstein's zeta-function}, J.\
Reine Angew.\ Math.\ \textbf{227} (1967), 86--110.

\bibitem{chudnovsky1976}
D.\ V.\ Chudnovsky, \textit{Algebraic independence of values of
exponential and hypergeometric functions}, in: \textit{Number Theory}
(Proc.\ Conf., Carbondale, IL, 1979), Lecture Notes in Math.\
\textbf{751}, Springer, Berlin, 1979, 45--69; result originally
announced 1976.

\bibitem{codata2022}
E.\ Tiesinga, P.\ J.\ Mohr, D.\ B.\ Newell, and B.\ N.\ Taylor,
\textit{CODATA recommended values of the fundamental physical
constants: 2022}, Rev.\ Mod.\ Phys.\ \textbf{96} (2024), 025002.

\bibitem{glasser-zucker}
M.\ L.\ Glasser and I.\ J.\ Zucker, \textit{Lattice sums}, in:
\textit{Theoretical Chemistry: Advances and Perspectives}, vol.\ 5,
Academic Press, 1980, 67--139.

\bibitem{lookelsewhere}
W.\ J.\ Steinmetz III and C.\ Pacitanimulli, \textit{Look-elsewhere
audit of monomial-level fits in the FTD constant catalogue}, project
report (2026), pre-registration commit hash and verification scripts
available at the project repository.

\bibitem{waldschmidt2000}
M.\ Waldschmidt, \textit{Diophantine Approximation on Linear
Algebraic Groups: Transcendence Properties of the Exponential
Function in Several Variables}, Grundlehren der mathematischen
Wissenschaften \textbf{326}, Springer, Berlin, 2000.

\bibitem{watson1939}
G.\ N.\ Watson, \textit{Three triple integrals}, Quart.\ J.\ Math.\
Oxford Ser.\ \textbf{10} (1939), 266--276.

\end{thebibliography}

\end{document}
