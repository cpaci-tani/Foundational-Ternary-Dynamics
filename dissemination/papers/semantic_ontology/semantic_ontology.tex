...
the other adds a higher one --- and $\Gstar$ is the \emph{double} limit
$\mu\to0$, $A\to0$. Neither limit alone suffices, which is a sharper
statement of the price than either drift gives on its own.
...
$(0,0)$ --- the double limit $\mu\to0$, $A\to0$. Contours are exact
quadrature; the corner value agrees with $\sqrt\pi\,\Gstar$ to
$10^{-9}$.}
\label{fig:collapse}
\end{figure}
...
\centering
\begin{tikzpicture}
\node[sel]   (osc) at (0,0)     {marginal oscillator\\[-1pt]\tiny $V=\lambda x^4$ at $\mu\approx0$};
\node[thm]   (ta)  at (3.6,0)   {measured $(T,A)$\\[-1pt]\tiny one ringdown};
\node[thm]   (kap) at (7.2,0)   {modulus $\kappa$\\[-1pt]\tiny via \eqref{eq:collapse}};
\node[sel]   (mu)  at (10.5,0)  {detuning $\mu$\\[-1pt]\tiny via \eqref{eq:modulus}};
\node[openb] (act) at (5.4,-1.9) {actuator: what pays the maintenance?\\[-1pt]\tiny open problem, Section~\ref{sec:open}};
\draw[con] (osc) -- (ta);
\draw[con] (ta) -- (kap);
\draw[con] (kap) -- (mu);
...
statics rather than assumed. Figure~\ref{fig:epsilon} states the
resulting economy as the falsifiable commitment it is.

\begin{figure}[t]
\centering
\begin{tikzpicture}
\node[thm] (eps) at (0,0) {$\varepsilon$\\[-1pt]\tiny one energy scale, set once};
\node[sel] (bind) at (-4.5,-1.8) {binding\\[-1pt]\tiny register barrier $\Delta E=\varepsilon$\\[-2pt]\tiny (Prop.~\ref{prop:registerbarrier})};
\node[sel] (clk)  at (0,-1.8)    {clock rate\\[-1pt]\tiny stiffness $\lambda\sim\varepsilon$ in \eqref{eq:periodlaw}};
\node[sel] (ret)  at (4.5,-1.8)  {retention\\[-1pt]\tiny $\tau_{\rm flip}\,\nu_0\sim e^{\varepsilon/T}$};
\draw[con] (eps) -- (bind);
\draw[con] (eps) -- (clk);
\draw[con] (eps) -- (ret);
\end{tikzpicture}
...
\centering
\begin{tikzpicture}
\node[thm] (stats) at (0,0.85)    {reproduce quantum statistics\\[-1pt]\tiny hypothesis (i)};
\node[thm] (ord)   at (0,-0.85)   {order-independence \eqref{eq:R3}\\[-1pt]\tiny hypothesis (ii), standard QFT};
\node[thm] (hid)   at (5.1,0)     {foliation operationally hidden\\[-1pt]\tiny Prop.~\ref{prop:hiding}};
\node[sel] (samp)  at (0,-2.55)   {one sample space,\\[-1pt]\tiny setting-independent measure};
\node[thm] (fine)  at (5.1,-2.55) {$|S|\le2$ (Fine)\\[-1pt]\tiny vs.\ attained $2\sqrt2$};
\node[openb] (bar) at (9.6,-1.3)  {the barrier\\[-1pt]\tiny Bell-theoretic,\\[-2pt]\tiny not covariance};
\draw[con] (stats) -- (hid);
\draw[con] (ord)   -- (hid);
...
So the architecture composes, on one instrument, with no constant added
beyond those already booked. What the demonstrator cannot do is say
what the composition \emph{means} --- which pillar supplies which layer,
and which layer nobody supplies. That is the statement the next section
owes.
