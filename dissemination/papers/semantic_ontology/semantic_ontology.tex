...
numerics at all}. Only the $N=3$ set can threaten the bound, and a
Lipschitz-certified scan clears it there by between $0.55$ and
$1.5\,\varepsilon$ (at $s=1.3$ the set is empty). The value
$-2\varepsilon$ is attained: in-plane the body can sit at distance
exactly $r_0$ from two anchors, its distance to the third being
$s\sqrt3/2+\sqrt{1-s^2/4}\in[1.67,1.89]$, which \emph{exceeds the support
radius} $3/2$, so the third bond contributes exactly zero. With
$E_{\rm ground}=-3\varepsilon$ this gives $\Delta E\ge\varepsilon$, and
the hinge path gives $\Delta E\le\varepsilon$.
\end{proof}
