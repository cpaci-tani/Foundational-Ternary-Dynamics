...
The barrier between them is not a fitted quantity. An explicit hinge path
--- break one bond, swing on the remaining two --- costs exactly
$1.000\,\varepsilon$, which \emph{proves} the upper bound. That no
cheaper channel exists is a separate question, and it is answered
numerically rather than analytically: a watershed computation over the
entire configuration space, union-find over energy-sorted cells, returns
$1.016$--$1.021\,\varepsilon$ across $s\in[0.9,1.3]$, with the excess
attributable to grid resolution and the third-bond tail. The direct route
through the centre costs about $30\varepsilon$. So

\begin{proposition}[Register barrier]
\label{prop:registerbarrier}
The register's barrier is at most one bond's dissociation depth,
essentially independent of the geometry: $\Delta E \le \varepsilon$, by
the explicit hinge path.
\end{proposition}

The matching lower bound is \emph{not} proven here. A watershed on a
finite grid cannot exclude a channel narrower than its cell, so
$\Delta E = \varepsilon$ is an upper bound proven plus a lower bound
corroborated at the resolution searched --- not an equality established.
Every use of ``$\Delta E = \varepsilon$'' below carries that scope.
...
\node[sel] (clk)  at (0,-1.8)    {clock rate\\[-1pt]\figsub stiffness $\lambda\sim\varepsilon$ in \eqref{eq:periodlaw}};
\node[sel] (ret)  at (4.5,-1.8)  {retention\\[-1pt]\figsub $\tau_{\rm flip}\,\nu_0\sim e^{\varepsilon/T}$};
\draw[con] (eps) -- (bind);
\draw[con] (eps) -- (clk);
\draw[con] (eps) -- (ret);
\end{tikzpicture}
\caption{One constant, three bills. The same $\varepsilon$ prices the
matter's binding, the clock's rate, and the memory's lifetime, in fixed
proportion --- there is no separate constant for any of the three. This
is a structural commitment rather than an economy of notation: any
measurement that moved one of the three without the others would break
the architecture, and Section~\ref{sec:toy} exhibits all three roles on
one instrument.}
\label{fig:epsilon}
\end{figure}
...
The weight layer is now complete, at a cost stated line by line. But
nothing in it says \emph{when} a weight becomes a frequency. That is not a
...
The link the demonstrator leaves unshown is a scaling, not an identity.
Section~\ref{sec:mvc} gives the minimum viable clock an effective
stiffness $\lambda_{\rm eff} = 8k_1k_2/(k_1+3k_2)$ --- a function of the
bond stiffnesses, which a single-scale potential ties to $\varepsilon$ by
dimensional analysis but which is not the same number as the well depth.
\textbf{Whether one $\varepsilon$ genuinely fixes all three, and in what
proportion, is open} --- and it is the sharpest falsifiable commitment in
the architecture, since any measurement moving one of the three without
the others would break it.
...
no-go theorem \\
Register barrier $\le\varepsilon$ (Prop.~\ref{prop:registerbarrier}) &
Upper bound exact (explicit hinge path); the matching \emph{lower} bound
is finite-grid numerical only ($1.016$--$1.021\,\varepsilon$), so
$\Delta E=\varepsilon$ is corroborated at the resolution searched, not
proven \\
...
\item \textbf{One constant, or three?} The architecture commits to a
single $\varepsilon$ pricing binding, clock rate and retention. Two of
the three are exhibited together in Section~\ref{sec:toy}; the link from
the well depth to the clock's effective stiffness $\lambda_{\rm eff}$ is
a dimensional scaling, not an identity, and no instrument here moves
$\varepsilon$ and reads all three responses at once. Build one.
...
avoided.
\item \textbf{The register's lower bound.} Prove
$\Delta E \ge \varepsilon$ analytically, or exhibit a cheaper channel.
The upper bound is an explicit path; the lower bound is at present a
finite-grid search (Prop.~\ref{prop:registerbarrier}).
