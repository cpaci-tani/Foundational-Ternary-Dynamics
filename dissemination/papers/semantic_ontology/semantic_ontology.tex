...
%  Figures: figures/make_figures.py generates six vector PDFs; ten
...
accompanying script. The measurements of Sections~\ref{sec:clock}
and~\ref{sec:born} are transcribed from the locked pre-registration
records listed in Appendix~\ref{app:instruments}; the quantitative
results of Section~\ref{sec:relativity} are exploratory computations,
not pre-registered, and are marked as such where they appear.}}
\date{}
...
and a timescale. Two contributions are new. First, a \emph{clock}: the period constant of
any mechanical oscillator factors into a universal quadrature root
$\sqrt\pi$ and a degree ratio, with the harmonic case uniquely
...
into three exhaustive cases; the substrate natively realizes only the
harmonic one, and the quartic clock is available only at an itemized
price --- four bodies and one additional interaction range, plus a
threshold that must be actively held. Its period law is exact and its
constant is recovered from simulation with no fitted scale. Second, a reinterpretation of Born weighting: in a
...
Two postulates are declared, in Sections~\ref{sec:ontology}
and~\ref{sec:operational}. The imports are itemized rather than summarized,
...
this framework by choice. The full accounting is
...
% --- nodes ----------------------------------------------------------
\node[thm]   (H)   at (1.35,-1.0)  {$H_n$\\[-1pt]\figsub actual history};
...
\node[openb] (S)   at (10.0,1.55)  {$\Sigma$\\[-1pt]\figsub actualization};
\node[thm]   (e)   at (10.0,-1.0)  {$e_{n+1}$\\[-1pt]\figsub one event};
...
bookkeeping. The recursion this architecture will run on is indexed by a
stage, and if the index is to mean elapsed time rather than list
position, something physical has to carry it. A clock is therefore not a
component of the architecture among others; it is a precondition of the
architecture's own description of itself. That thread is picked up in
Section~\ref{sec:qg}, once there is a recursion on the page to be
indexed.
...
\label{eq:quadrature}
\end{equation}
Everything specific to the clock is in the $I(n)$ of \eqref{eq:quadrature},
and it factors --- in a
...
correction is worth more than the coincidence was. The
twelve-degree-of-freedom simulation of Section~\ref{sec:mvc} holds
...
oscillation at every detuning lies on this one curve: the harmonic side
...
the one modulus that is its own complement. That is the same fact
\eqref{eq:qrdr} reports as $n = 2$ being self-reproducing, seen through
the elliptic parametrisation rather than the Gamma one.
...
oscillation of the normal form at every detuning lies on the single curve
...
\emph{And the residual is computable, which is better than calling it
small.} Relaxing the chain at fixed $U$ gives a reduced system that is
not the ideal normal form in two respects. Its potential carries a sixth
order term, $E_{\rm eff}(U) = \lambda_{\rm eff}U^4 +
16k_1k_2(k_1{-}k_2)(k_1{+}3k_2)^{-2}U^6 + \cdots$, which vanishes
...
Quadrature on the reduced system reproduces the simulated drift
...
fixed. Whether that is a mechanism for holding it, or merely the
statement that the error signal exists, is exactly the open problem of
Section~\ref{sec:open}. Figure~\ref{fig:sensorloop} draws the loop with
its one missing edge.
...
\node[openb] (act) at (5.4,-1.9) {actuator: what pays the maintenance?\\[-1pt]\figsub open problem, Section~\ref{sec:open}};
\draw[con] (osc) -- (ta);
\draw[con] (ta) -- (kap);
\draw[con] (kap) -- (mu);
...
\end{tikzpicture}
\caption{The sensor inversion, and the missing edge. A marginal clock's
...
What would consume it is the double-ruled block: nothing in this paper
constructs an actuator, and that absence is the open problem the
maintenance cost points at.}
...
and the transition state is the in-plane two-bond configuration.
\end{proposition}
...
the number of bonds inside the attractive window $(3/4,\,3/2)$. The bond
law satisfies $V\ge-\varepsilon$ everywhere, $V\ge0$ for $q\le3/4$, and
--- decisively --- $V\equiv0$ for $q\ge3/2$. So $E\ge-\varepsilon N$
pointwise, and wherever $N\le2$ we have $E\ge-2\varepsilon$ \emph{with no
numerics at all}. Only the set where $N$ can reach $3$ needs certifying,
and it is certified by interval arithmetic over boxes in $(x,y,s)$
jointly --- every bond distance bracketed over the whole box, a cell
discharged either because at most two bonds \emph{can} be attractive or
because its interval lower bound already clears $-2\varepsilon$. Over
$2.3\times10^{7}$ boxes covering the continuum $s\in[0.9,1.3]$ no cell is
left uncertified, and the worst bound where $N$ can reach $3$ is
$-1.51\,\varepsilon$. The value $-2\varepsilon$ is attained: in-plane the
body sits at distance exactly $r_0$ from two anchors, its distance to the
third being $s\sqrt3/2+\sqrt{1-s^2/4}\in[1.67,1.89]$ throughout, which
\emph{exceeds the support radius} $3/2$, so the third bond contributes
exactly zero. With $E_{\rm ground}=-3\varepsilon$ this gives
...
interval, and it decided window-membership from cell \emph{centres} ---
so at $s=1.3$, where the centroid sits at $0.75056$ and clears the window
edge by $6\times10^{-4}$, it reported the three-bond set \emph{empty}
when that set is a disc of radius $\sim\!5\times10^{-4}$. Interval
arithmetic removes both: a set smaller than the grid can no longer hide
inside it.
...
the full configuration space finds nothing cheaper at the resolution
searched. (b) The
...
familiar thermodynamic shape. The model's statics supply the barrier
value; the exponential form is imported Kramers rate theory under the
imposed bath, and inherits its premises. Figure~\ref{fig:epsilon} states the
resulting economy as the falsifiable commitment it is.
...
of the three bills, and $\Gstar$ itself is independent of $\rho$.}
\label{fig:epsilon}
\end{figure}
...
The weight layer is now assembled, at a cost stated line by line --- and
assembled \emph{finite-dimensionally}, which is less than complete: the
covariant setting of Section~\ref{sec:relativity} uses local algebras
that are not of type~I and admit no density matrix, and nothing here
bridges the two. That bridge is imported wholesale and stands among the
open problems. But even granting it, nothing in this layer says
...
% =====================================================================
\section{Occupation weighting as a regime}
\label{sec:born}
...
and the two faces a noisy threshold multiplies, presentations counted at
a low level and amplitude gating at a high one.
...
product they multiply to --- presentations $\times$ success --- is
occupation. This figure is an illustration of the counting logic on one
...
slow-channel shares at $32$ seeds on one instrument; the statement is an
observation about that class, not a theorem about crossing statistics in
general.
\end{proposition}
...
(Figure~\ref{fig:bellangles}). Records are required for the readout and not for the
...
classical substrate is the lower path: a single setting-independent
sample space caps CHSH at $2$ against the attained $2\sqrt2$, which
breaks hypothesis~(i) directly. The open obligation (double-ruled) is
therefore Bell-theoretic and would exist in a non-relativistic world.}
...
\paragraph{Radiative stability, reduced.}
\label{sec:radiative}
...
\label{prop:gate}
Every \emph{operational two-frame comparison} is a functional of at least
two proper-time parametrizations --- time dilation compares $\dd\tau_1$
with $\dd\tau_2$; velocity composition relates two frames; geodesic
deviation is defined by the separation of two worldlines, a single freely
falling body detecting nothing by the equivalence principle. A theory
that has not constructed a physical clock cannot pose \emph{those} tests:
their observables are undefined in it, not merely unverified.
\end{proposition}

\noindent The scope word is load-bearing and an earlier draft omitted it.
Not every boost-\emph{sensitive} quantity needs two clocks: the
dispersion test of Section~\ref{sec:massivecone} reads exactness of
per-species dilation off $\Omega_{\rm proper}(k)$ with no constructed
clock at all, and it is a one-body measurement. What is clock-gated is
the operational comparison, not the boost sensitivity --- which is why
part of the two-body bill turned out to be a calculation rather than an
obligation.
...
find. This is the second time the same object has gated something:
Section~\ref{sec:succession} argued that if the stage index is to mean
elapsed time, something physical must carry it, and Section~\ref{sec:qg}
will pay that argument out in full. The same instrument is the
precondition of the boost question.
...
$2.7\times10^{-17}$ at $t=16$, $2.0\times10^{-26}$ at $t=24$ --- falling
below double precision's resolution \emph{relative to the peak}
(machine epsilon, $2.2\times10^{-16}$) at about sixteen ticks. With the
...
Six line-items follow. Three are computed and close at their stated
scope --- the free massive modes, the composite inheritance, and the
one-energy dilation; one is resolved in principle and priced but not
built --- the cross-sector cone; and two are diagnoses that relocate the
remaining work --- what a potential is, and what the substitution shows.
Each names the object whose absence blocks what is still open.
...
decompositions at every length down to \emph{four}, certified to
$5.3\times10^{-15}$ on four thousand fresh momenta, and none at three ---
where three is also the rigorous floor, since the symbol's Hessian at a
zero has rank three and $n$ squares can produce at most rank $n$. The
four is genuinely non-covariant, so four and seven are the honest
bracket. Seven costs spinor dimension eight, four costs four.
...
the unrestricted four uses the face and edge shells. So the shortest
carrier buys one square by leaving the body centres, and the
body-centre-only reading costs one square of length. Either way the
carrier does not want a finer cubic lattice --- every shell in play is a
half-offset of the existing one --- and the lattice spacing, on which
every dimensional prediction depends, does not move. (The stall at four
is search evidence of the same kind that previously mis-reported the
minimum, and is offered with exactly that confidence; the rank bound
$n\ge3$ is the only proof in this paragraph.)
...
Two consequences deserve stating, one structural and one from Clifford
counting. Structurally, this is the third independent route by which the
...
production stencil is orthogonal to that lattice. The counting: a mass
term must anticommute with every kinetic structure, so it costs one
further structure, and saturation alternates down the ladder --- four
squares at dimension four leave \emph{one structure spare}, so the
minimal carrier \emph{admits a mass at ordinary Dirac dimension}, while
the covariant seven saturates dimension eight exactly, so cubic
covariance together with a mass forces sixteen. Covariance, not mass, is
what costs spinor dimension here. (An earlier draft, built on the
...
$8$; and a search over the nearest-neighbour class finds a \emph{four}
at dimension $4$ --- ordinary Dirac, with one anticommuting structure to
spare for a mass --- which is genuinely non-covariant. What multiplies the
spinor here is covariance, not the symbol itself.}
...
\subsubsection{One energy, and the universality that follows}
\label{sec:oneenergy}
...
(b) The claim, directly, over one full run: released in phase, the two
traces drift apart, and by tick $430$ the moving clock has fallen $2.2$
cycles behind. The roughness on the boosted trace is real --- a boosted
lattice kink radiates. (c) Universality as convergence. Two-category carriers fan
apart, one of them running \emph{faster} when moving; measured as a
spread in the \emph{frequency ratio} $\Omega/\Omega_0$ at $\gamma =
1.22$, they span $0.283$ against the one-energy carriers' $0.006$, a
factor of $46$. (The body quotes the same comparison as a spread in the
\emph{fitted exponent}, $1.76$ against $0.043$; the two pairs measure
different things and neither is a restatement of the other.) The
one-energy curves collapse onto each other and lie a little below
$1/\gamma$, the residual discussed in the text. (d) Why this is
...
That closes the loop the construction opened. A loop that closes on paper
may still not run, though, and what has not been shown is that these
pieces coexist in one system rather than in eleven separate arguments.
A model can settle that; prose cannot.
...
This section answers that question with a single computable artifact in
...
\emph{``Compose'' here means coexistence under shared constants, and
nothing stronger.} The five pieces run as five independent computations
in one program: no module consumes another's state, and there is no
feedback anywhere in the artifact. What it demonstrates is that the
claims can be instantiated together, at one set of constants, without
contradiction --- which is worth checking and is not the same as a
coupled dynamics. A genuinely coupled model, in which the clock's phase
gates the register and the register's state back-reacts on the carrier,
does not exist and is named among the open problems.
...
$G^*$ to $1.6\times10^{-13}$, with nothing fitted --- \emph{reproducing}
the law of Section~\ref{sec:clock} on an instrument that shares no code
path with it, not deriving it a second time. (d) The register's
...
stiffness is $\lambda_{\rm eff}=8k_1k_2/(k_1+3k_2)$, and the curvatures
are fixed by the depths of the two potentials: $k_1 = 24\varepsilon$ for
the unit bond and $k_2 = \tfrac83\varepsilon_2$ for the purchased closure
species, whose depth $\varepsilon_2$ is its own. Writing
$\rho=\varepsilon_2/\varepsilon$,
\begin{equation}
\lambda_{\rm eff} = \frac{64\,\varepsilon\,\rho}{3+\rho},
\qquad
\Delta E = \varepsilon,
\qquad
\ln(\tau_{\rm flip}\nu_0) = \varepsilon/T .
...
What survives is the part that carries the paper. $\Gstar$ does not
depend on $\rho$: integrating the reduced mode across
$\varepsilon\in[\tfrac12,2]$ and $\rho\in[\tfrac14,4]$ recovers
$T\!\cdot\!A\sqrt{2\lambda_{\rm eff}/m}/\sqrt\pi = \Gstar$ to
$6\times10^{-14}$ throughout. The second energy scale buys the clock's
\emph{rate} and cannot touch the clock's \emph{constant}. The economy
claim fails where it priced the rate and holds where it priced the
number.
...
strictly subluminal, so no \emph{propagating mode} outruns $C$ --- making
it the effective cone, not the strict front, which is the reach boundary.
...
no propagating mode outruns the cone. This does \emph{not} make $C$ the
...
So the architecture's pieces coexist, on one instrument, with no constant
added beyond those already booked and the clock's free units --- which
the period law's scale invariance makes immaterial. What the demonstrator
cannot do is couple them, and it cannot say what the composition
\emph{means} --- which pillar supplies which layer, and which layer
...
\node[sel]  (rel)  at (0,-0.75) {relativity\\[-1pt]\figsub covariance of the selector};
...
\node[thm]   (W) at (6.6,1.45)  {\textbf{the weight layer}\\[-1pt]\figsub
   structured potentiality;\\[-2pt]\figsub evolution; composition};
\node[openb] (S) at (6.6,-1.05) {\textbf{the selector} $\Sigma$\\[-1pt]\figsub
   one outcome; covariant form;\\[-2pt]\figsub the Born pushforward};
...
% the clock is not only supplied: it supplies the index the whole
% recursion is written in, which is the loop the paper closes
\draw[wk]  (C) to[out=15,in=-15,looseness=1.5]
  node[lbl,pos=0.5,right=1pt] {supplies the index} (S);
...
verified $n = 2,3,4,5,6,8$. The $\sqrt\pi$ is kinematic and common to
every mechanical clock; the $\Gamma$ ratio is the potential's
contribution. $n=2$ is uniquely self-reproducing, which is why the
harmonic constant is a power of $\pi$ \\
Threshold forcing (Prop.~\ref{prop:threshold}) & Exact \emph{within its
stated hypotheses} (H1--H4) and \emph{in the limit $A\to0$}. Forces the
quartic given a threshold, a reflection symmetry and $\lambda\neq0$. It
does \textbf{not} force any substrate to sit at a threshold, and $\mu=0$
is codimension one \\
Finite-amplitude drift \eqref{eq:sexticdrift} & Exact. $\Gstar$ is the
$A\to0$ endpoint only: with $V = \lambda q^4+\nu q^6$ the constant sweeps
continuously in $r = \nu A^2/\lambda$, and every member has
$\lambda\neq0$. The measured $0.33\%$ across a sixfold amplitude range is
this term, at $r = 0.0056$ \\
\textbf{$\Gstar$ is chosen, not derived} & \textbf{No valid carrier has
produced it.} The native single-scale screen returned zero; the four-body
carrier that produces it requires a purchased interaction species and is
Galilean, hence disqualified as a relativistic instrument; the one
carrier here that demonstrably dilates is isochronous. $\Gstar$ enters
this framework by choice \\
...
$2\times10^{-6}$ in the $A\to0$ extrapolation. \textbf{Galilean
invariant, and therefore disqualified as a relativity test} \\
No native quartic clock & Negative at screened scope only; not a
no-go theorem \\
Register barrier $=\varepsilon$ (Prop.~\ref{prop:registerbarrier}) &
\textbf{Proven}, both bounds, for $s\in[0.9,1.3]$: hinge path above,
plane-crossing plus $E\ge-\varepsilon N$ below, with only the $N=3$ set
needing certification. Conditional on the bond law's \emph{compact
support} --- a Morse or LJ tail puts the barrier strictly below
$\varepsilon$ \\
One constant, three roles & \textbf{Refuted as stated.} With the closure
species carrying its own depth, $\rho=\varepsilon_2/\varepsilon$ moves
the clock rate alone \eqref{eq:economy} and the three exponents are
$(+1,+1,-\tfrac12)$, not a fixed proportion. $\Gstar$ is independent of
$\rho$ to $6\times10^{-14}$: the second scale buys the rate, not the
constant \\
...
offsets, exactly seven of them covariantly \eqref{eq:seven} and four at
the searched minimum (rigorous floor three, from the Hessian rank), so
the price is spinor dimension between $4$ and $8$; the four admits a mass
at Dirac dimension but is non-covariant and leaves the body-centre shell.
An earlier draft reported the minimum as five from an
under-dispersed search; corrected \\
...
\item \textbf{What holds the threshold.} Proposition~\ref{prop:threshold}
forces the quartic \emph{at} $\mu = 0$, and $\mu = 0$ is a
codimension-one surface: the generic configuration is not on it, and an
unmaintained system falls off it and becomes a harmonic clock. So the
lemniscatic clock's real price is a maintenance cost, and this paper does
not identify anything that pays it. What physical mechanism holds
$\mu = 0$ against detuning? Without an answer, the proposition describes
a clock nothing is shown to be able to keep.
...
\item \textbf{A coupled model.} Every demonstrator in this paper runs its
modules independently. Exhibit a system in which the clock's phase gates
the register and the register's state back-reacts on the carrier, and
check whether the architecture survives the feedback it has so far
avoided.
...
\item \textbf{From finite dimensions to local algebras.} The
reconstruction of Section~\ref{sec:operational} is finite-dimensional, while
the covariant setting of Section~\ref{sec:relativity} uses local algebras
that are not of type~I and admit no density matrix. The bridge is
imported wholesale and never constructed.
\end{enumerate}
...
Every measurement \emph{in the series catalogued below} was
pre-registered: outcomes, gates and
...
pre-registrations. Separately and more seriously: the committed kink
runner does \emph{not} reproduce the parameter set quoted for the kink
dilation in Section~\ref{sec:oneenergy} --- it carries a superseded
...
Barrier $=\varepsilon$, proven both ways \\
\bottomrule
\end{tabular}
...
Two results in this paper are established by argument and verified by
computation rather than measured, so they carry no pre-registration and
belong outside the series above. Both run in seconds and print their own
