...
Section~\ref{sec:register} gives $\tau_{\rm flip}\sim\nu_0^{-1}
...
\emph{input} to \eqref{eq:compweight}'s counterpart at the correlation
level, $S^2 = 4 + \lVert[A_0,A_1]\rVert\lVert[B_0,B_1]\rVert$; $S$ is the
\emph{readout}. Records are required for the readout and not for the
...
\\
Trace rule and the Born square & External theorem, conditional on the
quantum effect space \emph{and Postulate~\ref{post:B}}~\cite{busch} \\
...
no-go theorem \\
Register barrier $=\varepsilon$ & Exact statics within the declared model
\\
...
(Prop.~\ref{prop:hiding}); implied by reproducing quantum statistics \\
...
$C_{\rm eff}/C = 1 + M^2/12$; intra-species dilation exact to
$O(\beta^4)$, error $M^2\beta^4/4$; proton--electron mismatch
$1.6\times10^{-40}$ \\
...
\item \textbf{Shielded actualization.} The purity requirement demands
that the slow channel be nearly exclusive at the threshold. Can any
physical manifestation rule be shielded from its own field's fast noise?
