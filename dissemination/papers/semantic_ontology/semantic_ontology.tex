...
\end{tikzpicture}
\caption{The recursion~\eqref{eq:recursion}. Structure is encoded in
shape and rule, never in colour: solid boxes are theorem-grade within
their stated premises, thin boxes are selections or imported
representations, and the double-ruled box is the paper's one open object.
Actual history generates structured potentiality through a state-assignment
map $\Gamma$; a chosen context resolves it into weighted alternatives; the
actualization block selects one; the record extends and the cycle repeats.
Everything above the lower band is potential; only the events in it are
actual.}
\label{fig:ontology}
\end{figure}

% =====================================================================
\section{The operational layer, and what recovers quantum theory}
...
Everything above concerns weights. Nothing in it contains a clock. We now
turn to the paper's first constructive contribution, and for this we need
a concrete model rather than an operational abstraction.
...
period constant is $\pi$. The quartic case is the anti-pendulum: its
frequency grows with amplitude, and its period constant is the
lemniscatic
\begin{equation}
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{fig2_trichotomy.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{fig3_clock.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{fig7_register.pdf}
...
The unitary evolution of Section~\ref{sec:operational} preserves
information; the transition from potential to actual does not. In the
concrete model this is not an interpretive overlay but a measured
...
\paragraph{And a timescale condition, which is the paper's second new
result.} The thermodynamic quantity that turns out to control whether
actualization statistics are \emph{quantum} is not an energy but a
\emph{ratio of timescales}: the mode frequency against the noise
correlation time. That is the subject of the next section, and it is the
point at which the thermodynamic pillar stops being connective tissue and
starts doing work.
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{fig5_bornregime.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{fig4_bandmap.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.62\textwidth]{fig6_purity.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{fig12_tiers.pdf}
...
\begin{figure}[p]
\centering
\includegraphics[width=\textwidth]{fig13_bellangles.pdf}
...
to $\gamma$. This is vacuous. That clock is a mechanical framework,
$m\ddot q = -\nabla V(|q_i - q_j|)$, and such an equation is \emph{Galilean}
invariant: boosting it leaves the period exactly unchanged for any
velocity. It would report zero time dilation whatever the substrate did.
Dilation in a field substrate comes from the binding being mediated at the
substrate's own speed --- Lorentz's electron-theory reasoning, and the
light-clock argument in modern dress --- so the object to interrogate is
the \emph{dispersion of a massive mode}, which needs no clock at all.
...
\begin{figure}[p]
\centering
\includegraphics[width=\textwidth]{fig16_intersector.pdf}
...
\begin{figure}[p]
\centering
\includegraphics[width=\textwidth]{fig14_carrierarc.pdf}
...
The programme has long recorded that the harmonic clock is native and the
lemniscatic one must be purchased. It could say what the price was and not
what it bought, which is an unstable thing to leave in a ledger. The
structure above settles it.

Take $V = \lambda q^n$ with $n$ even. Only $n=2$ has nonzero second-order
stiffness; every $n \ge 4$ is second-order \emph{free} and still
oscillates. Ordinarily those properties exclude one another --- a stiff
mode oscillates but resists being moved, a soft mode moves freely but does
not oscillate --- and the quartic is the \emph{minimal} escape, requiring
exactly one vanishing Taylor coefficient where $n=6$ requires two. $G^*$
is the period constant of that minimal degeneracy:
$\int_0^1(1-u^4)^{-1/2}du = \varpi/2 = \sqrt\pi\,G^*/4$, a period of the
lemniscatic curve.
...
\begin{figure}[p]
\centering
\includegraphics[width=0.95\textwidth]{fig15_clockgeometry.pdf}
...
The genericity has to be located carefully, and it is easy to misplace.
For a reflection-symmetric system the normal form is
$V = \tfrac12\mu x^2 + \lambda x^4 + O(x^6)$, and \emph{at} $\mu = 0$ the
quartic is generic: the cubic is forbidden by symmetry rather than removed
by tuning, so the quartic clock is the normal form of a perfect Euler
column at critical load and of any mean-field symmetry-breaking order
parameter. What is non-generic is not the shape of the potential but the
\emph{location} --- $\mu = 0$ is a codimension-one surface, and staying on
it against detuning is the whole difficulty.
...
This interprets $G^*$ and does not recover it. No valid carrier has
produced it: the mechanical framework that did is Galilean and
disqualified, and the one carrier that dilates is isochronous. Nor does
the argument reach agency --- it is a selection statement about efficient
design, and marginal stability, while a hallmark of adaptive systems, is
equally a property of sandpiles and buckling columns.

Two caveats belong beside the dilation result. The lattice is not exactly
...
\begin{figure}[p]
\centering
\includegraphics[width=\textwidth]{fig11_conearc.pdf}
...
\begin{figure}[p]
\centering
\includegraphics[width=\textwidth]{fig9_toymodel.pdf}
...
\begin{figure}[p]
\centering
\includegraphics[width=\textwidth]{fig10_causal3d.pdf}
