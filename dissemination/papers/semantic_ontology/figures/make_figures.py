...
Reproducible from scratch: geometry and curves are generated here, none is
traced or hand-drawn. Two explicitly labelled clock-surrogate reference
lines transcribe the exploratory runner identified in the paper appendix.
  ...
  bornregime   occupation-fraction vs Omega*tau across three instruments
  purity       occupation-fraction vs slow-noise share (purity diagnostic)
...
Five conceptual diagrams are TikZ, compiled in-document; nine additional
included figures come from scripts/experiments/temporal_interior/. All
fifteen external figures share the style module imported below, so the
paper reads as one artifact rather than two sets of conventions.

Data provenance: the Section-8 instrument values are transcribed verbatim
from the protocol records named in Appendix A, and check_transcription()
keeps those mappings honest. The two clock-surrogate reference values are
exploratory, not pre-registered, and their separate runner/hash mapping is
also disclosed there. The nine sibling figure scripts self-verify.
...
TAU_LAT = 317.9                              # rounded latency Stage A
SLOWNESS_RATIO = 390.9                       # locked record, not rounded quotient
TAU_FLUX = TAU_LAT / SLOWNESS_RATIO          # ratio-consistent estimate (~0.8133)
...
def check_transcription():
    ...
    assert abs(TAU_LAT / TAU_FLUX - SLOWNESS_RATIO) < 1e-12, "latency ratio"
    ...
    assert all(a < b for a, b in zip(b2, b2[1:])), "OF not monotone"
...
def fig_trichotomy():
    ...
    ax2.loglog(A, np.full_like(A, 2 * np.pi), color=C1, lw=1.8,
               label=r"$n=2$:  $T$ independent of $A$  ($2\pi$)")
    ax2.loglog(A, np.sqrt(np.pi) * G_STAR / A, color=C2, lw=1.8,
               label=r"$n=4$:  $T\propto A^{-1}$  ($\sqrt{\pi}G^{*}$)")
...
def fig_clock():
    ...
    ys = np.zeros(4)
    labels = ["$A^{+}$", "$B^{-}$", "$C^{+}$", "$D^{-}$"]
    for i in range(3):                   # positive signs in stress certificate
        ax1.plot(xs[i:i + 2], ys[i:i + 2], color=C1, lw=2.6, zorder=1)
        ax1.text(xs[i] + 0.5, 0.085, r"$\omega_e=+1$", ha="center", color=C1,
                 fontsize=8)
    arc = np.linspace(0, np.pi, 200)       # negative sign in certificate
    ax1.plot(1.5 + 1.5 * np.cos(arc), -0.42 * np.sin(arc), color=C2,
             lw=2.6, zorder=1)
    ax1.text(1.5, -0.60, r"$\omega_e=-1$   (closure, $\ell=3r_0$)",
    ...
    ax1.axis("off")
    ax1.set_title(r"(a) self-stress certificate (zero actual force)")
    ...
    ax2.axhline(2e-6, color=C3, lw=1.4, ls="--",
                label=r"12-DOF harmonic surrogate, $A\!\to\!0$: $2\times10^{-6}$")
    ax2.axhline(3.3e-3, color=C4, lw=1.0, ls="-.",
                label=r"harmonic surrogate, $T\!\cdot\!A$ spread over $6\times$ range")
    ...
    ax2.legend(frameon=False, loc="upper left", fontsize=fs.FS_LEG)
...
def fig_bornregime():
    ...
    ax.set_ylabel(r"occupation fraction  $\mathrm{OF}$")
    ...
    ax.annotate("amplitude weighting", xy=(0.13, 0.055), fontsize=7.6,
                color=CK)
    ax.annotate("action / occupation weighting", xy=(30, 0.93), fontsize=7.6,
                color=CK, ha="center")
    save(fig, "bornregime")
...
def fig_purity():
    ...
    of = np.array([0.0406, 0.0530, 0.0462, 0.0907, 0.9361])
    ...
    ax.fill_between([-0.05, 0.82], -0.06, 0.16, color=C2, alpha=0.10, lw=0,
                    zorder=0)
    ax.errorbar(f, of, yerr=[of - lo, hi - of], fmt="o-", color=C3,
                ms=6, lw=1.7, capsize=3, zorder=3)
    ax.axhline(0.5, color=CK, ls=":", lw=1.0, zorder=1)
    ax.text(1.03, 0.52, "occupation\nendpoint", fontsize=7.2, color=CK,
            ha="right", va="bottom")
...
def fig_register():
    ...
    # (a) actual s=1 hinge path. Two bonds stay at q=1. The third has
    # q3(theta)=3/2(1-cos theta), theta0..2pi-theta0, cos(theta0)=1/3.
    # Relative to the -3 eps minima, Delta E/eps = 1 + V(q3)/eps.
    theta0 = np.arccos(1.0 / 3.0)
    theta = np.linspace(theta0, 2 * np.pi - theta0, 800)
    path = np.linspace(-1.0, 1.0, theta.size)
    q3 = 1.5 * (1.0 - np.cos(theta))
    v3 = np.where(q3 < 1.5, -16.0 * (q3 - 1.5) ** 2 * (q3 - 0.75), 0.0)
    dE = 1.0 + v3
    ax1.plot(path, dE, color=C1, lw=2.0, label=r"actual $s=1$ hinge path")
    ...
    ax1.scatter([-1, 1], [0, 0], s=70, color=CK, zorder=4)
    ax1.text(-1, -1.6, r"$C_+$", ha="center", fontsize=9)
    ax1.text(1, -1.6, r"$C_-$", ha="center", fontsize=9)
    ax1.set_yscale("symlog", linthresh=1.0)
    ax1.set_ylim(-2.5, 60)
    ax1.set_xlabel(r"monotone coordinate along the hinge path")
    ax1.set_ylabel(r"$\Delta E/\varepsilon$")
