...
(Master copy lives in the session scratchpad; repo copies have twice been
destroyed by a concurrently running agent session (since finished). Restore with a plain file copy.)
...
G_ACC = 9.81
LAMBDA_TRUE = K_SPRING / (4 * L_HALF**2)
MU_G = MASS * G_ACC / ELL_SUSP
DAMP_C = 0.006
WINDOW = (0.010, 0.024)
...
def section(title):
    line(); line("=" * 72); line(title); line("=" * 72)
