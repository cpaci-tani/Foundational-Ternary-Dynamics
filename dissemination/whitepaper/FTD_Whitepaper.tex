...
\date{May 1, 2026\\Framework Version: FTD v5.34\\Verification Status: 9-theorem algebraic spine (post-2026-04-19 reframe)}
...
\begin{abstract}
We present a comprehensive evaluation of Foundational Ternary Dynamics (FTD), a mathematical framework whose master quadratic $x^2 - 16 G^{*2} x + 16 G^{*3} = 0$ has a larger root numerically matching $1/\alpha$, and which provides parametric expressions for 15+ particle masses in terms of four integers $\{3, 4, 7, 13\}$ and the lemniscatic constant $G^*$.
...
\begin{enumerate}
    ...
    \item \textbf{$x_+ \leftrightarrow 1/\alpha$ (1.26 ppm) [STRONGLY MOTIVATED CONJECTURE]}: numerical match; not a dynamical derivation from FTD's update rules.
...
The polynomial itself is THEOREM (pure algebra in $G^*$, ledger row FTD-0001). The identification $x_+ \leftrightarrow 1/\alpha$ is STRONGLY MOTIVATED CONJECTURE [SMC] (ledger row FTD-0013): the evidence is the numerical match plus CM-curve uniqueness across class-number-1 fields, not a dynamical derivation from FTD's update rules.
...
\midrule
$N_c$ & Colors & 3 & Moore Layer Theorem & SU(3) gauge \\
...
\textit{Verification: 9-theorem algebraic spine}\\
