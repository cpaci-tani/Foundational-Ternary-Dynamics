...
def create_emergent_phenomena_figure():
    ...
    # CORRECTED 2026-07-01 (FTD-0347): this panel is a SYNTHETIC ILLUSTRATION
    # (S_values below are generated by formula + noise, not engine data) of the
    # RETIRED sLoop continuous-Bell mechanism -- CLOSED DECLINED under FC-1 on
    # 2026-06-10 (DERIV_OBSERVER_BELL_MECHANISM.md). The substrate is
    # local/classical (engine measures S <= 2); S = 2*sqrt(2) arises only under
    # the imported complexification/measurement reading [SELECTION].
    ax3 = axes[1, 0]
    ...
    ax3.plot(substrate_overlap, S_values, 'go-', linewidth=2, markersize=6,
            label='sLoop reading (RETIRED; synthetic)')
    ...
    ax3.set_title('(C) sLoop Bell reading [RETIRED mechanism]\nSynthetic illustration — substrate is S ≤ 2', fontweight='bold')
