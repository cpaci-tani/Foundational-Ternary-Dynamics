"""Versioned composite executor with exact private reduction and owned-Job cleanup.
...
OTHER_METADATA_CAP = 128 << 20
REDUCER_LOG_CAP = 64 << 10
REDUCER_REQUEST_CAP = 16 << 10
REDUCER_RESULT_CAP = 256 << 10
REDUCER_MODULE = ROOT / "scripts/phi_v2_lattice/composite_parallel_reduce_v1.py"
PROBE_CAP = 16 << 20
...
def source_inventory():
    ...
    paths.update((REDUCER_MODULE, Path(__file__).with_name("certify_composite_interaction_v1.py")))
    ...
    paths.update(ROOT / "engine/docs" / name for name in (
        "CONTRACT_STRICT_COMPOSITE_EXECUTION_V2.md", "CONTRACT_STRICT_COMPOSITE_EXECUTION_V2_RESOURCE_CORRECTION_2.md"))
    for name, module in tuple(sys.modules.items()):
        source = getattr(module, "__file__", None)
...
class AttemptBudget:
    ...
    def check(self, force=False):
        ...
        if not force and now - self.last_check < 0.2:
            return
        self.last_check = now
        allocated = count = other_metadata = 0
        for base, directories, files in os.walk(self.directory, followlinks=False):
            ...
            for name in files:
                ...
                count += 1
                relative = (Path(base) / name).relative_to(self.directory).parts
                scientific_data = (relative in (("catalogue.bin",), ("templates.bin",), ("actions.bin",))
                    or (len(relative) == 2 and re.fullmatch(r"range-\d{2}", relative[0])
                        and relative[1] in ("range.bin", "stdout", "stderr"))
                    or (relative[0] == "preparation" and (
                        (len(relative) == 2 and relative[1] in ("original-input.bin", "normalized-input.bin", "preparation-map.bin"))
                        or (len(relative) == 3 and relative[1] in ("original-sort", "normalized-sort")
                            and (relative[2] == "merged.bin" or re.fullmatch(r"run-\d{2}\.bin", relative[2]))))))
        ...
        if other_metadata > OTHER_METADATA_CAP:
            raise OSError("aggregate 128-MiB other-metadata quota")
...
def cleanup_job_live_descendants(cleanup_job, capacity=4096):
    """Read the owned Job, including nested Jobs; a reused foreign PID is irrelevant.

    https://learn.microsoft.com/en-us/windows/win32/api/winnt/ns-winnt-jobobject_basic_process_id_list
    The querying owner must be present. Failed or partial snapshots never pass.
    """
    import ctypes
    from ctypes import wintypes as W
    integer(capacity, "cleanup process-list capacity", 1, 4096)
    ...
    info = ProcessIds()
    length = W.DWORD()
    kernel = cleanup_job.kernel
    if not kernel.QueryInformationJobObject(cleanup_job.handle, 3, ctypes.byref(info),
                                            ctypes.sizeof(info), ctypes.byref(length)):
        raise ctypes.WinError(ctypes.get_last_error())
    if info.assigned != info.returned or info.returned > capacity:
        raise ValueError("partial cleanup Job process list")
    minimum = ProcessIds.pids.offset + int(info.returned) * ctypes.sizeof(ctypes.c_size_t)
    if not minimum <= length.value <= ctypes.sizeof(info):
        raise ValueError("cleanup Job process-list byte count")
    members = tuple(int(info.pids[index]) for index in range(info.returned))
    if len(members) != len(set(members)) or any(pid <= 0 for pid in members) or os.getpid() not in members:
        raise ValueError("cleanup Job owner/member inventory")
    return tuple(sorted(pid for pid in members if pid != os.getpid()))
...
def verified_reducer(pins):
    """Execute the owned reducer buffer whose identity the request admits."""
    digest = pins.get(str(REDUCER_MODULE))
    if digest is None:
        raise ValueError("reduction source is absent from admitted closure")
    raw = read_artifact({"path": str(REDUCER_MODULE), "sha256": digest}, 1 << 20)
    name = "_ftd_composite_parallel_reduce_v1"
    existing = sys.modules.get(name)
    if existing is not None:
        if getattr(existing, "_admitted_sha256", None) != digest:
            raise ValueError("loaded reducer differs from admission")
        return existing
    spec = importlib.util.spec_from_file_location(name, REDUCER_MODULE)
    module = importlib.util.module_from_spec(spec)
    sys.modules[name] = module
    exec(compile(raw, str(REDUCER_MODULE), "exec"), module.__dict__)
    module._admitted_sha256 = digest
    return module


def _reduction_entry(request_path, request_sha256, deadline):
    """One retained range in the same descendant Job; no transition calls."""
    if sys.pycache_prefix is None or not time.monotonic() < deadline < float("inf"):
        raise ValueError("reduction requires the bounded fresh-prefix entry")
    check_entry_cache(sys.pycache_prefix)
    raw = read_artifact({"path": str(Path(request_path).resolve()), "sha256": request_sha256}, REDUCER_REQUEST_CAP)
    row = parse_json(raw)
    pins = row["source_sha256"]
    if pins.get(str(Path(__file__).resolve())) != sha(__file__):
        raise ValueError("reduction entry is absent from admitted source closure")
    check_pins(pins)
    reducer = verified_reducer(pins)

    def check():
        if time.monotonic() >= deadline:
            raise TimeoutError("retained reduction deadline")

    check()
    reducer.run_range(raw, check)
    check_pins(pins)
    check_entry_cache(sys.pycache_prefix)
    check()
    result = Path(row["output"]) / "result.json"
    sys.stdout.buffer.write(canonical({"result": {"path": str(result), "sha256": sha(result)}}) + b"\n")
    sys.stdout.buffer.flush()


class ReductionWorkers:
    """Bounded reducer children sharing the caller's 32 worker slots."""
    def __init__(self, directory, deadline, pins, check):
        self.directory, self.deadline, self.pins, self.check = Path(directory), deadline, pins, check
        self.running, self.handles, self.inputs, self.results = {}, {}, {}, {}
        self.peak_workers = 0

    def launch(self, request):
        self.check()
        index = integer(request["expected_header"]["range"], "reducer range", 0, 63)
        if index in self.handles or len(self.running) >= 32 or request["source_sha256"] != self.pins:
            raise ValueError("duplicate/excess reducer or changed source admission")
        folder = self.directory / ("reducer-%02d" % index)
        if Path(request["output"]).resolve() != folder / "output":
            raise ValueError("reduction output outside owned range")
        raw = canonical(request)
        if len(raw) > REDUCER_REQUEST_CAP:
            raise OSError("reduction request exceeds16KiB")
        folder.mkdir()
        request_path = folder / "request.json"
        write_new(request_path, raw)
        command = [sys.executable, "-B", "-X", "pycache_prefix=" + str(sys.pycache_prefix), str(Path(__file__).resolve()),
                   "--reduce-range", str(request_path), "--request-sha256", hashlib.sha256(raw).hexdigest(),
                   "--reduction-deadline", repr(self.deadline), "--entry-sha256", self.pins[str(Path(__file__).resolve())]]
        process = subprocess.Popen(command, cwd=ROOT, env=runtime_environment(), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        self.handles[index] = process
        self.inputs[index] = raw
        write_new(folder / "launch.json", canonical({"range": index, "pid": process.pid, "argv": command}))
        self.running[index] = (process, BoundedLogs(process, folder, REDUCER_LOG_CAP), folder)
        self.peak_workers = max(self.peak_workers, len(self.running))

    def poll(self):
        completed = []
        for index, (process, logs, folder) in tuple(self.running.items()):
            self.check()
            logs.check()
            code = process.poll()
            if code is None:
                continue
            logs.finish(self.deadline)
            del self.running[index]
            if code:
                raise ValueError("retained reducer failed; range%d exit%d" % (index, code))
            output = parse_json((folder / "stdout").read_bytes().rstrip(b"\n"), {"result"})
            result_ref = output["result"]
            if Path(result_ref["path"]).resolve() != folder / "output" / "result.json":
                raise ValueError("reducer stdout names an unowned result")
            result = parse_json(read_artifact(result_ref, REDUCER_RESULT_CAP))
            if (result["request_sha256"] != hashlib.sha256(self.inputs[index]).hexdigest()
                    or result["range"] != index or result["module_sha256"] != self.pins[str(REDUCER_MODULE)]):
                raise ValueError("reducer result differs from request/module")
            self.results[index] = result_ref
            completed.append((index, result))
        return completed

    def ordered_results(self):
        if sorted(self.results) != list(range(64)) or any(p.poll() is None for p in self.handles.values()):
            raise ValueError("missing reducer result or owned worker still alive")
        for index in range(64):
            yield self.inputs[index], self.results[index]
...
def _production_coordinator(payload, barrier):
    ...
    try:
        ...
        reducer = verified_reducer(expected_sources)
        reducers = ReductionWorkers(directory, payload["deadline"], expected_sources, budget.check)
        with reducer.frozen_driver().MapIndex(mapping["path"], mapping["sha256"], descriptor_hash, descriptor["actions"]["sha256"]) as admitted_map:
            multiplicities, owned = validate_all_aliases(admitted_map, budget.check)
            next_range = 0
            native_failure = False
            peak_combined_workers = 0
            evolution_started = time.monotonic()
            while next_range < 64 or running or reducers.running:
                budget.check()
                while not native_failure and next_range < 64 and len(running) + len(reducers.running) < 32:
                    index = next_range
                    next_range += 1
                    ...
                    peak_combined_workers = max(peak_combined_workers, len(running) + len(reducers.running))
                for index, (process, logs, folder) in tuple(running.items()):
                    ...
                    del running[index]
                    native_failure = native_failure or bool(code)
                    request = {"schema": "q4-composite-parallel-reduce-request-1", "expected_header": header,
                               "map": {"path": mapping["path"], "sha256": mapping["sha256"]}, "descriptor": ref(descriptor_path),
                               "range_stream": ref(folder / "range.bin"), "native_stdout": ref(folder / "stdout"),
                               "native_stderr": ref(folder / "stderr"), "native_exit_code": code,
                               "output": str(directory / ("reducer-%02d" % index) / "output"), "source_sha256": expected_sources}
                    reducers.launch(request)
                    peak_combined_workers = max(peak_combined_workers, len(running) + len(reducers.running))
                    if len(running) + len(reducers.running) > 32:
                        raise ValueError("combined native/reducer worker ceiling")
                for index, result in reducers.poll():
                    report = result["range_receipt"]
                    write_new(directory / ("range-%02d" % index) / "reduction.json", canonical(report))
                    range_reports[index] = report
                    if result["status"] != "COMPLETE":
                        raise ValueError("incomplete retained range prevents tier completion")
                if native_failure and not running and not reducers.running:
                    raise ValueError("failed native range prevents tier completion")
                time.sleep(0.005)
            timings["native_execution_stream_reduction_seconds"] = time.monotonic() - evolution_started
            finalizing = time.monotonic()
            reduction, complete, merged_reports = reducer.merge_results(admitted_map, owned, multiplicities, reducers.ordered_results(), budget.check)
            if any(canonical(report) != canonical(range_reports[index]) for index, report in enumerate(merged_reports)):
                raise ValueError("merged receipts differ from retained worker receipts")
            globally_covered = 0
            ...
            if any(process.poll() is None for process in finished.values()):
            ...
            report = {"schema": "q4-composite-tier-result-2", "status": "COMPLETE_SCOPED_EXECUTION", "canonical_adoption": False,
                      ...
                      "support_cache_used": False, "combined_worker_limit": 32, "peak_combined_workers": peak_combined_workers,
                      "reducer_workers": len(reducers.handles), "actual_loaded_modules": verify_loaded_imports()}
...
def _scientific_entry(lock_path, lock_sha256, prefix, attempt_started):
    ...
    ranges = []
    alive = list(cleanup_job_live_descendants(parent_cleanup_job))
    for index in range(64):
        folder = directory / ("range-%02d" % index)
...
def main(argv=None):
    ...
    modes.add_argument("--reduce-range", metavar="REQUEST")
    ...
    parser.add_argument("--reduction-deadline", type=float)
    ...
    if args.guard_entry:
        ...
        if args.control_descendant == "success":
            ...
            time.sleep(30)
    elif args.reduce_range:
        if args.entry_sha256 is None or args.request_sha256 is None or args.reduction_deadline is None:
            raise ValueError("reduction entry requires owned request/source and finite deadline")
        _reduction_entry(args.reduce_range, args.request_sha256, args.reduction_deadline)
