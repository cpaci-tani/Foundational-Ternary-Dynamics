...
print()
print('  Input: G* = Gamma(1/4)/Gamma(3/4) and the integer 16 --- PLUS the four')
print('  framework integers N_c, N_base, b_3, N_eff imported from constants.py')
print('  (lines 18-19 of this file), which are NOT derived from G* and 16.')
print()
print('  READ THIS COUNT HONESTLY (clarification of record, CLAUDE.md 2026-07-01):')
print('  ~20 of the rows below are [PARAMETRIC]/[SELECTION] integer-ratio')
print('  insertions reproducing the values they were fitted to. This battery is')
print('  an internal-consistency check across the catalog --- it is NOT 50')
print('  independently-confirmed predictions. Several rows are also self-')
print('  comparisons (e.g. c_cfl = 1/sqrt(D) then t_conf = sqrt(D)/c_cfl == D),')
print('  which are tautologies, not experiment matches.')
