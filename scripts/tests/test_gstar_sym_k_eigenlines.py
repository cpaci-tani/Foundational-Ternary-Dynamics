...
def test_c_invariant_dim_sym2():
    """I4 (sharpened): the joint Z[i]-trivial-eigenvalue + c-invariant subspace
    of Sym^2(H^1) over Q[i] is 1-dimensional, generated by omega*eta.

    Z[i]-trivial means eigenvalue 1, i.e., (a - b) ≡ 0 (mod 4). For Sym^2 (a+b=2),
    only (a, b) = (1, 1) has a - b = 0.
    """
    import gstar_sym_k_eigenlines as gse
    dim = gse.c_invariant_dim(k=2, z_i_eigenvalue=1)
    assert dim == 1, f"dim_Q[i](Sym^2(H^1)^c ∩ Z[i]-trivial) = {dim}, expected 1 (H3)"


def test_c_invariant_dim_sym3():
    """Sym^3(H^1) has NO Z[i]-trivial-eigenvalue basis elements (since for k=3,
    a - b is odd, so eigenvalue is in {±i}, never 1).

    Therefore the joint Z[i]-trivial + c-invariant subspace of Sym^3 is 0-dimensional.
    """
    import gstar_sym_k_eigenlines as gse
    dim = gse.c_invariant_dim(k=3, z_i_eigenvalue=1)
    assert dim == 0, f"dim_Q[i](Sym^3(H^1)^c ∩ Z[i]-trivial) = {dim}, expected 0"
