"""derive_two_body_bion_dilation_v2.py — the interpolated-peak probe.
...
def pair(lam, u, N, sep_w=SEP_W):
    ...
    D = sep_w * w / g            # lab centers at +/-D/gamma: the PROPER
                                 # separation is 2*sep_w*w at every boost
...
def evolve(phi, dot, lam, T):
    ...
    for t in range(T):
        ...
        k = int(np.argmax(phi))
        km, kp = (k - 1) % len(phi), (k + 1) % len(phi)
        den = phi[km] - 2 * phi[k] + phi[kp]
        if den < 0:
            d = 0.5 * (phi[km] - phi[kp]) / den
...
def selftest():
    ...
    for lam in LAMS:
        phi, dot = pair(lam, 0.0, 4096)
        tr = evolve(phi, dot, lam, T_TICKS)
        ev = events(tr)
        per = np.diff(ev[N_SKIP:N_SKIP + 200])
        med, iqr = np.median(per), (np.percentile(per, 75)
                                    - np.percentile(per, 25))
        # carrier period from the same trace, FFT peak (diagnostic only)
        s = tr[6000:] - tr[6000:].mean()
        P = np.abs(np.fft.rfft(s * np.hanning(len(s)))) ** 2
        f = np.fft.rfftfreq(len(s), d=1.0)
        om = 2 * np.pi * f[np.argmax(P[3:]) + 3]
        purity = abs(med - 2 * np.pi / om) / (2 * np.pi / om)
        good = (len(ev) >= N_MIN and purity < 0.10 and iqr / med < 0.30)
        print(f"  S2 lam={lam:.2f} rest: {len(ev)} events, median period "
              f"{med:.2f} vs carrier {2*np.pi/om:.2f} (purity err "
              f"{purity:.2%}), IQR/med {iqr/med:.2f}: "
...
def main():
    ...
    with open(os.path.join(out, "two_body_bion_dilation_v2.json"), "w",
              encoding="utf-8") as f:
