"""Independent artifact certificate for FTD-0760.
...
RESULTS = ROOT / "engine" / "results" / "ftd_0760"
PROTOCOL_HASH = "681FA36CCE4479D268D37651E4CD58AA6C1D5A4809F989EA4FF2AA24B7B40722"
EXPECTED_HASHES = {
    "docs/theory/10_eft_program/preregistrations/"
    "PREREG_M3_RELATIONAL_CHART_HELD_OUT_VALIDATION_v1.md": PROTOCOL_HASH,
    "docs/theory/10_eft_program/preregistrations/"
    "PREREG_M3_SUPPORT_INVARIANT_VALIDATION_v1.md":
    ...
    "engine/tests/campaign_m3_relational_chart_held_out_validation_cuda.cpp":
        "374DE8AE67DCC69D40FF1AF8B74300497728FFE825D393429B5BF3D54B453BA5",
}
DIRECTIONS = {
    "face": "0_0_1",
    "edge": "0_1_-1",
...
REGISTERED_NAMES = {
    ("face", "energy_hostile"): "srm_s1p_s2p_r15o16",
    ("face", "graph_hostile"): "srm_s1m_s2p_r15o16",
    ("edge", "energy_hostile"): "srm_s1p_s2p_r15o16",
    ("edge", "graph_hostile"): "srm_s1p_s2m_r15o16",
    ("body", "energy_hostile"): "srm_s1p_s2p_r15o16",
    ("body", "graph_hostile"): "srm_s1m_s2p_r15o16",
}
VOLUMES = (321, 385)
...
def certify_candidate(arm: str, variant: str) -> dict[str, bool]:
    ...
    stem = f"ftd_0760_m3_candidate_v1_{DIRECTIONS[arm]}_{variant}"
    ...
    check(f"{arm}/{variant} id", metadata.get("ftd_id") == "FTD-0760")
    ...
    check(f"{arm}/{variant} shared chart selection",
          metadata.get("allow_shared_anchor_chart") is True)
    ...
    for volume in VOLUMES:
        ...
        for row in volume_rows:
            ...
            allow_shared = row["allow_shared_anchor_chart"] == "1"
            site_projection = row["site_projection_valid"] == "1"
            chart_admissible = row["chart_admissible"] == "1"
            check(prefix + " chart rule",
                  chart_admissible == (allow_shared or site_projection))
            action = action and allow_shared and chart_admissible
...
def certify_fibre(arm: str) -> dict[str, bool]:
    outcome = {"infrastructure": False, "classifier": False}
    stem = f"ftd_0760_m3_causal_fibre_v1_{DIRECTIONS[arm]}"
    csv_path, json_path = RESULTS / f"{stem}.csv", RESULTS / f"{stem}.json"
    ...
    if not csv_path.is_file() or not json_path.is_file():
        return outcome
    metadata = json.loads(json_path.read_text(encoding="utf-8"))
    check(f"{arm} fibre id", metadata.get("ftd_id") == "FTD-0760")
    ...
    check(f"{arm} fibre shared chart selection",
          metadata.get("allow_shared_anchor_chart") is True)
    data = rows(csv_path)
    grouped: dict[int, list[dict[str, str]]] = defaultdict(list)
    for row in data:
...
if failures:
    print(f"FTD-0760 {mode}: {checks - len(failures)}/{checks} checks")
    for failure in failures:
        print(f"FAIL: {failure}")
    raise SystemExit(1)
print(f"FTD-0760 {mode}: {checks}/{checks} checks")
