"""FTD-0835 exact certificate for the bilateral self-dual quartic clock.

This proves coordinate and ledger identities conditional on the selected
critical quartic Hamiltonian.  It performs no numerical search and does not
claim a native substrate realization or a neuroscience model.
"""
...
check(
    ...
    and sp.simplify(next_energy + (bath + core_energy - next_energy) - (core_energy + bath)) == 0,
)

passed = sum(1 for _, value in checks if value)
print(f"\nFTD-0835 bilateral self-dual quartic clock: {passed}/{len(checks)} PASS")
