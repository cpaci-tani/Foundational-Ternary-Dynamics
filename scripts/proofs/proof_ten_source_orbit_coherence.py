#!/usr/bin/env python3
"""Independent verifier for preregistered FTD-0593.

The verifier reconstructs every cubic mode/displacement orbit and evaluates
all N=10 removal partitions.  It performs no source-geometry, polarity,
...
PREREG = (
    ...
    "PREREG_TEN_SOURCE_ORBIT_COHERENCE_v1.md"
)
PREREG_SHA = "10EBAFCC24B0589B975BD14E3CD4FD4508942830EA7A4FB541378655F25DC348"
RESULT = ROOT / "engine/results/ftd_0593/windows_msvc_cpu.json"
ARTIFACTS = {
    "header_sha256": ROOT / "engine/include/ftd/eft/"
    "ten_source_orbit_coherence.h",
    "source_sha256": ROOT / "engine/src/eft/"
    "ten_source_orbit_coherence.cpp",
    "test_sha256": ROOT / "engine/tests/"
    "test_ten_source_orbit_coherence.cpp",
...
class Proof:
    ...
    def report(self) -> bool:
        print("=" * 79)
        print("FTD-0593 ten-source removal-time orbit coherence")
        print("=" * 79)
...
def analyze(lattice_size: int) -> dict[str, object]:
    ...
    return {
        ...
        "ten_source_bound": maximum_bound,
...
P.check("run identifier", record["identifier"] == "FTD-0593",
        record["identifier"])
...
for lattice_size, expected in zip(VOLUMES, record["volumes"]):
    ...
    P.check(f"L={lattice_size} N=10 threshold classification",
            (float(derived["ten_source_bound"]) < K_GENESIS)
            == (float(expected["margin"]) > 0.0),
            f"margin={derived['margin']:.17g}")

all_closed = all(
    float(row["ten_source_bound"]) < K_GENESIS for row in derived_rows)
expected_verdict = (
    "ARBITRARY_REMOVAL_N_LE_10_CLOSED_BY_ORBIT_COHERENCE"
    if all_closed else "TEN_SOURCE_ORBIT_BOUND_INCONCLUSIVE"
)
P.check("registered verdict", record["verdict"] == expected_verdict,
        record["verdict"])
P.check("registered ten-source bound is inconclusive", not all_closed,
        "all four registered quotients exceed the threshold")
