"""Independent run-record certificate for FTD-0724."""
...
PREREG = ROOT / "docs/theory/10_eft_program/preregistrations/constituent_complete_matter/PREREG_LOWER_ENERGY_FORMATION_CROSSOVER_v1.md"
TEST = ROOT / "engine/tests/test_lower_energy_formation_crossover.cpp"
JSON_PATH = ROOT / "engine/results/ftd_0724/ftd_0724_lower_energy_formation_crossover_v1.json"
CSV_PATH = ROOT / "engine/results/ftd_0724/ftd_0724_lower_energy_formation_crossover_v1.csv"

PREREG_SHA256 = "BCCCE4691FA5EBA22C61CB88554DC4972E07B0178C55396CA9A79ED98D3F4762"
TEST_SHA256 = "98C3D1B572B695B901C11555765CD4B5BC33F7FFCAEB5C6F638DB96113801208"
JSON_SHA256 = "068AAFAE5029C3993BF0D7ECA3A9F681A3830B0A6B9E1191011EE6C0B53CD91F"
CSV_SHA256 = "3D8BFFC1E03559CA7B14E9FA79B1EB8542B117484D0AD6C7E9BA4AC4ACB06D20"
...
def main() -> None:
    ...
    check(summary["identifier"] == "FTD-0724", "identifier", checks)
    ...
    # This failed preregistered gate is why the energetic pattern is not promoted.
    check(summary["maximum_scalar_history_spread"] > 1e-9, "locked scalar covariance gate fails", checks)
    check(close(summary["maximum_scalar_history_spread"], 1.0680766715509549e-08), "scalar spread value locked", checks)
    check(summary["maximum_scalar_history_spread"] < 2e-8, "scalar defect localized in scale", checks)
    ...
    print(f"FTD-0724 certificate: {len(checks)}/{len(checks)} checks PASS")
    print(f"verdict={VERDICT}")
    print(f"scalar_history_spread={summary['maximum_scalar_history_spread']:.17g}")
    print("raw_negative_counts=52/52/52/52/0 (NON-PROMOTABLE)")
