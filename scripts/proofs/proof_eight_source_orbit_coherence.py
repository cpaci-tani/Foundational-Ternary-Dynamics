#!/usr/bin/env python3
"""Independent verifier for preregistered FTD-0591.

The verifier reconstructs every cubic mode/displacement orbit and evaluates
all N=8 removal partitions.  It performs no source-geometry, polarity,
...
PREREG_SHA = "F6ED8183765BCCC29427DFFBCA6074D916FEDBF7D97B557F38DD3405721D4F70"
RESULT = ROOT / "engine/results/ftd_0591/windows_msvc_cpu.json"
ARTIFACTS = {
    "header_sha256": ROOT / "engine/include/ftd/eft/"
    "eight_source_orbit_coherence.h",
    "source_sha256": ROOT / "engine/src/eft/"
    "eight_source_orbit_coherence.cpp",
    "test_sha256": ROOT / "engine/tests/"
    "test_eight_source_orbit_coherence.cpp",
...
class Proof:
    ...
    def report(self) -> bool:
        print("=" * 79)
        print("FTD-0591 eight-source removal-time orbit coherence")
        print("=" * 79)
...
def analyze(lattice_size: int) -> dict[str, object]:
    ...
    partition_bounds = [
        common_step * math.sqrt(8 - removed)
        + pulse_operator * math.sqrt(
            removed + coherence * removed * (removed - 1)
        )
        for removed in range(9)
    ]
    maximizing_removed = max(range(9), key=partition_bounds.__getitem__)
    ...
    return {
        ...
        "eight_source_bound": maximum_bound,
...
P.check("run identifier", record["identifier"] == "FTD-0591",
        record["identifier"])
...
for field, path in ARTIFACTS.items():
    ...
    P.check(f"artifact hash {field}", record[field] == actual_hash,
            actual_hash)
P.check("registered source count", record["registered_source_count"] == 8,
...
for lattice_size, expected in zip(VOLUMES, record["volumes"]):
    ...
    P.check(f"L={lattice_size} strict N=8 closure",
            float(derived["eight_source_bound"]) < K_GENESIS,
            f"margin={derived['margin']:.17g}")

all_closed = all(
    float(row["eight_source_bound"]) < K_GENESIS for row in derived_rows)
expected_verdict = (
    "ARBITRARY_REMOVAL_N_LE_8_CLOSED_BY_ORBIT_COHERENCE"
    if all_closed else "EIGHT_SOURCE_ORBIT_BOUND_INCONCLUSIVE"
)
P.check("registered verdict", record["verdict"] == expected_verdict,
        record["verdict"])
P.check("uniform eight-source first-event bound", all_closed,
