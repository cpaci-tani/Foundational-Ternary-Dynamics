...
**FTD version:** 5.40 · **Engine:** 2.18.0 · **Author:** William J. Steinmetz III
...
What is missing — **MC-T4.3**, the operational readout principle — is the single load-bearing gap for upgrading FTD-0013 to `[DERIVED]`. After eleven closed-negative derivation routes and a full sweep of observable-readout attempts (ARC-A/B1 closed-negative; ARC-B2/C1 corrected `FOUND → UNDERDETERMINED`; ARC-D1 empirical readout closed-negative, commit `3bacd23c`), the obstruction is now localized precisely as **W-CRIT-2**: the master-quadratic *operator structure* `(Tr, Det) = (16G*², 16G*³)` is **imposed, not forced**. A `2×2`'s trace and determinant are independent invariants, and nothing yet compels that specific pair from the forward-derived scalars `{16, G*, G*²}`.

The current work formalizes this as a **boundary theorem** (the number-one goal's clause 2): the *Readout-Structure Independence* theorem, pre-registered in [`PREREG_READOUT_STRUCTURE_INDEPENDENCE_v1.md`](docs/theory/10_eft_program/preregistrations/PREREG_READOUT_STRUCTURE_INDEPENDENCE_v1.md). Its trace-side leg is **machine-checked**: the cube-corner module has `mult_O(E) = 0` (no `O`-symmetric 2-dimensional subspace), so the readout's complex structure `i` forces a single `C₄` axis, removing the `C₃(⟨111⟩)` the determinant's odd `G*³` term needs ([`proof_readout_multE_zero.py`](scripts/proofs/proof_readout_multE_zero.py)). The determinant-side leg and the consistency half remain open obligations, and the `FORCED` outcome (a single operator co-realizing both) stays genuinely live. **No tag moves** until a verdict lands; `x₊ = 1/α` stays `[STRONGLY MOTIVATED CONJECTURE]`.
...
The session-resume document — what's actually happening as of this commit — is [`docs/WHERE_WE_LEFT_OFF.md`](docs/WHERE_WE_LEFT_OFF.md). Recent landings of note:

- **MC-T4.3 — route-invariant BOUNDARY (2026-06-01).** The α-readout obstruction was sharpened from a per-route gap into a route-invariant boundary: α is classified **dynamical, not structural**, independent of which readout route is attempted. **No tag moved** — `x₊ = 1/α` stays `[STRONGLY MOTIVATED CONJECTURE]`; the α-status lines in [§4](#4--the-central-physics-conjecture) are unchanged.
- **Engine-flawless lifecycle / callstack / toggle audit completed (2026-06-01).** A 16-commit pass (branch `flawless-engine-2026-06-01`) added a verification harness (web specs `lifecycle-harness` / `reconcile-claims` / `toggle-coverage` / `overlay-scheduler`; C++ `test_conservation_profile` / `test_tick_phase_order` / `test_engine_lifecycle`), fixed a clean-checkout `cmake`-configure break (a dangling `_repro_gpu_empty_bridge` reference latent ~5 weeks; 242 tests now register), pinned the energy-conservation profile (the non-variational Gauss projection operator `J -= ∇φ` is the leak, with an iteration-independent stencil floor), and deprecated DagEngine clearly. Golden hash `0xcd957b601d47868a` **unchanged** (zero physics drift).
- **MC-T4.3 readout — boundary-theorem turn (2026-05-31).** The operational α-readout is being sealed as a *boundary theorem* — the master-quadratic operator structure `(Tr, Det) = (16G*², 16G*³)` is unforced (W-CRIT-2) — pre-registered in [`PREREG_READOUT_STRUCTURE_INDEPENDENCE_v1.md`](docs/theory/10_eft_program/preregistrations/PREREG_READOUT_STRUCTURE_INDEPENDENCE_v1.md). The trace-side leg is machine-checked (`mult_O(E) = 0`; [`proof_readout_multE_zero.py`](scripts/proofs/proof_readout_multE_zero.py)) and the argument survived a three-angle independent adversarial refutation pass; the determinant-side leg + consistency half remain open, and the `FORCED` outcome stays live. Sits under the **Commutativity Wall** synthesis — FTD derives the commutative period algebra (G\*, master quadratic) but not the non-commutative QM/relativistic layer. In progress; no tag moved.
...
1. **MC-T4.3 alpha readout principle** — the single load-bearing gap for promoting FTD-0013 from `[STRONGLY MOTIVATED CONJECTURE]`. Now sharpened to a **route-invariant boundary** (2026-06-01): α is dynamical, not structural, independent of readout route. Two surviving exits remain — a **6th postulate**, or the **ARC-D** route (currently closed-negative). FTD-0198 ARC-B1 was the first locked attack.
