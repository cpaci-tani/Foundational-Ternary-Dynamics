...
**Author:** William J Steinmetz III &nbsp;|&nbsp; **Version:** 5.27-neutrino &nbsp;|&nbsp; **Date:** March 2026
...
|----------|-------|-------------|
| **Genuine Derivations** | ~20 | From G\* and integers alone (α, sin²θ_W, mass ratios, mixing angles) |
...
See [`docs/theory/AUDIT_EPISTEMIC_AUDIT.md`](docs/theory/AUDIT_EPISTEMIC_AUDIT.md) for the full breakdown.
...
Three.js visualization with 23+ scenarios across three physics scales, real-time energy diagnostics, and a Lagrangian inspector. Five standalone force simulations are also available in [`dissemination/interactive/`](dissemination/interactive/).
...
All dynamics derive from six rules obtained via the action principle S[s,J]: flux wave equation, state-flux coupling, Gauss projection, manifestation/evaporation thresholds, field-mediated forces, and movement with collision handling. Written in C++17 with CMake. See [`engine/SPEC_ENGINE.md`](engine/SPEC_ENGINE.md) for the full specification.

### Three Scales

| Scale | Engine | Description |
|-------|--------|-------------|
| 0 — Voxel | `RenderBridge` | Discrete lattice dynamics, flux propagation, Gauss constraint enforcement |
...
A drop-in `GpuEngine` replacement provides GPU-accelerated simulation with cuFFT spectral Poisson solver (exact Gauss constraint, zero violation) and cuRAND stochastic genesis. Performance: **363× speedup** at 64³ lattice on RTX 5090. Build with `cmake -DFTD_ENABLE_CUDA=ON`.

### WebAssembly Dashboard

The engine compiles to WASM via Emscripten and runs entirely in the browser. The dashboard includes Three.js 3D rendering, tabbed panels (Controls, Diagnostics, Charts, Lagrangian, Inspector), and a particle catalog covering the Standard Model. No build step required for the frontend — serve static files and open in any modern browser.
...
│   ├── include/ftd/                 #   Headers (ontic.h, constants.h, voxel.h, ...)
│   ├── src/                         #   Source (render_bridge.cpp, main.cpp, ...)
│   ├── cuda/                        #   CUDA GPU kernels
│   ├── wasm/                        #   Emscripten WASM bindings
│   ├── web/                         #   Browser dashboard (HTML + JS + Three.js)
│   ├── tests/                       #   114 CTests
...
├── dissemination/
│   ├── manuscript/                  #   Quarto book (82 chapters)
│   ├── notebooks/                   #   14 Jupyter tutorials
│   └── interactive/                 #   Standalone HTML force simulations
├── simulations/                     # Mathematical verification suite (~55 scripts)
├── tests/                           # Python integration tests (184 tests)
├── evaluation/                      # Multi-domain assessment (~90 files)
├── archive/                         # Legacy components (Python engine, ImGui, Qt GUI)
...
| Assess what is genuinely derived | [`docs/theory/AUDIT_EPISTEMIC_AUDIT.md`](docs/theory/AUDIT_EPISTEMIC_AUDIT.md) |
...
The project also includes a [Quarto manuscript](dissemination/manuscript/) (82 chapters across 15 books), [14 Jupyter notebooks](dissemination/notebooks/), and [interactive HTML simulations](dissemination/interactive/).

---

## Testing

**Python** — 184 tests covering coupling constants, mass derivations, mixing matrices, cosmology, and epistemic classification:
```bash
pytest tests/ -v
```

**C++ Engine** — 114 CTests including unit tests, multi-tick physics campaigns, Maxwell equation verification, energy conservation, and GPU parity checks:
```bash
cd engine/build && ctest --output-on-failure -C Release
```
...
@book{steinmetz2026ftd,
  ...
  year      = {2026},
  version   = {5.27},
  note      = {Discrete computational physics framework; ~20 genuine
