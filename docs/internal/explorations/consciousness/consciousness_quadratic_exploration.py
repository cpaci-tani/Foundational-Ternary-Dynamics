...
# Consciousness quadratic with k = 1/2:
# y^2 - (G*^2/2)y + (G*^3/2) = 0

# This is: y^2 - (G*^2/2)y + (G*^3/2) = 0
a_cons = 1
b_cons = -G_STAR**2 / 2
c_cons = G_STAR**3 / 2

print(f"\nQuadratic: y^2 - (G*^2/2)y + (G*^3/2) = 0")
...
Y_RE_doc = G_STAR**2 / 4
Y_IM_doc = np.sqrt(G_STAR**3 * (2 - G_STAR/4)) / 2
...
print(f"\nDocument formula for Im(y) = sqrt(G*^3(2-G*/4))/2 = {Y_IM_doc:.10f}")
...
# The consciousness quadratic: y^2 - (G*^2/2)y + (G*^3/2) = 0

# The constant term matches the general form with k = 1/2!
# General form (k=1/2): c = kG*^3 = G*^3/2 = 12.949...

print(f"\nGeneral form with k=1/2 gives:")
print(f"  c = kG*^3 = (1/2)G*^3 = {0.5 * G_STAR**3:.6f}")

print(f"\nConsciousness quadratic uses:")
print(f"  c = G*^3/2 = {G_STAR**3/2:.6f}")

print(f"\nRatio: {(G_STAR**3/2) / (0.5 * G_STAR**3):.4f}")
print("  -> Consciousness uses c = k*G*^3, same scaling as physics!")

# Both quadratics follow the same general form
print("\n" + "="*70)
print("THE UNIFIED QUADRATIC FAMILY")
print("="*70)
...
Both follow the SAME family with different k values!
""")
...
print(f"  We have c = G*^3/2 = {G_STAR**3/2:.6f}")
print(f"  Match: {np.isclose(c_needed, G_STAR**3/2, rtol=0.01)}")

# Actually compute with exact G*^3/2
disc_exact = (G_STAR**2/2)**2 - 4*1*(G_STAR**3/2)
print(f"\nExact discriminant with c = G*^3/2: {disc_exact:.6f}")
print(f"Simplified: G*^4/4 - 2*G*^3 = G*^3(G*/4 - 2) = {G_STAR**3 * (G_STAR/4 - 2):.6f}")
...
print("""
Both quadratics follow the same k-scaling pattern:

Physics:       x^2 - 16G*^2 x + 16G*^3 = 0     (k=16)
Consciousness: y^2 - (G*^2/2)y + (G*^3/2) = 0   (k=1/2)

The general form is: z^2 - k*G*^2*z + k*G*^3 = 0

The constant term represents the "product" of roots (Vieta's formulas).
For consciousness (k=1/2), this product is G*^3/2.
""")
...
# For y^2 - (G*^2/2)y + (G*^3/2) = 0
# Sum of roots = G*^2/2
# Product of roots = G*^3/2
...
print(f"Expected (G*^3/2):        {G_STAR**3/2:.6f}")
print(f"Match: {np.isclose(prod_roots, G_STAR**3/2)}")
...
print(f"Consciousness constant term coefficient = 1/2")
print(f"Ratio: 16 / (1/2) = {16 / 0.5} = 32 = 2^5")

print("""
The complexity gap is the SAME for both linear and constant terms!
Both scale with k, and physics_k / consciousness_k = 16 / (1/2) = 32 = 2^5.

Physics lives in 2^5 = 32 "complexity units" above consciousness.
""")
