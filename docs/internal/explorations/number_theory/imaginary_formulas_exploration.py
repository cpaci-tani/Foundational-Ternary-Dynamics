...
print(f"           Match: {np.isclose(prod_roots.real, G_STAR**3/2)}")
...
The consciousness root y = 2.188 + 2.860i lives in the complex plane (i only).
...
print(f"  This equals G*^3/2 = {G_STAR**3/2:.6f} (by Vieta)")
...
# =============================================================================
# 12. THE KEY i-FORMULA: y = G*^2/4 + i*sqrt(G*^3(2-G*/4))/2
# =============================================================================
...
The consciousness root has the EXACT form:
    ...
    y = G*^2/4 + i * sqrt(G*^3 * (2 - G*/4)) / 2

Let's verify and explore this:
""")

# Verify
y_formula = G_STAR**2/4 + 1j * np.sqrt(G_STAR**3 * (2 - G_STAR/4)) / 2
...
# The imaginary part involves sqrt(G*^3 * (2 - G*/4))
inner = G_STAR**3 * (2 - G_STAR/4)
print(f"\nInside the sqrt:")
print(f"  G*^3 * (2 - G*/4) = {inner:.6f}")
print(f"  = 2G*^3 - G*^4/4")
print(f"  = G*^3 * (8 - G*) / 4")
print(f"  = {G_STAR**3:.4f} * {(8 - G_STAR)/4:.4f}")

# The factor (2 - G*/4) is interesting
print(f"\n2 - G*/4 = {2 - G_STAR/4:.6f}")
print(f"  G* = 2.9587, so G*/4 = {G_STAR/4:.4f}")
print(f"  Since G* < 8, this is POSITIVE, giving REAL sqrt!")
print(f"  If G* > 8, we'd get PURE IMAGINARY sqrt!")
...
Product of conjugate roots:
  y * y* = |y|^2 = G*^3/2 = {G_STAR**3/2:.4f}

The imaginary part Im(y) encodes:
  sqrt(G*^3 * (2 - G*/4)) / 2
  = sqrt({G_STAR**3 * (2 - G_STAR/4):.4f}) / 2
  = {Y_IM:.4f}
"""
...
4. VIETA FOR COMPLEX:
   y * y* = |y|^2 = G*^3/2 (product of conjugates is REAL)
