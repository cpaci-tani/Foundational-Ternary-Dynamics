...
1. A 4-dimensional sub-block of the 7-space (density plus three momentum-like
   modes) is invariant under `Λ1` and `Λ2` for all three directions.
...
- `docs/theory/07_assessment/core_ledgers/LEDGER_ROW_DRAFT_phi_hydrodynamics.md`:
  ...
  FTD-1029 and FTD-1030 are already claimed by unbooked drafts.
...
3. **Collision.** At each site and polarity, occupancy set A ⊂ V maps to
   ...
   fixed-point-free wherever the (mass, momentum) class has more than one
   member, covariant under the cube group `F(gA) = gF(A)`, and selected by a
...
- **Planning.** The writing-plans skill produces the implementation plan with
  one task per gate step. Each task's instructions are self-contained: files,
