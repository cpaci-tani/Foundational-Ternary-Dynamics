...
A project-wide sweep found ~34 sites carrying the superseded **CODATA 2018** inverse fine-structure constant, ~9 of them *mislabelled* "CODATA 2022". This document is the standard that prevents recurrence.
...
The single most common defect found in the sweep was the **compound mislabel**: a CODATA 2018 value tagged "CODATA 2022". An edition tag is only useful if checked against the digits — CODATA 2018 α⁻¹ ends `…084`, CODATA 2022 ends `…177`.
...
**Complete — 22 files / 26 α⁻¹ occurrences retrofitted to CODATA 2022:**
- 3 canonical docs — `SPEC_DOCTRINE_LEDGER.md` §5, `SPEC_PHYSICS_BRIDGE.md` §2.1, `TRACKER_ONTIC_TRUTH.md` OT-5.1.
- 11 plain derivation / exploration / benchmark files.
- 8 files verified against `REF_PREREGISTER_MANIFEST.md` as **not** pre-registered: `PAPER_A_PI_FREE_GENERATOR.tex` (its prose wrongly stated "the CODATA 2022 value is 137.035999084"), the tower cluster (`THEOREM_HARMONIC_INVARIANT_TOWER.md`, `PROTOCOL_TOWER_LEVEL_FALSIFIER.md`, `explore_tower_level_scan.py`, `proof_harmonic_invariant_tower.py`, `proof_tower_multiplier_uniqueness.py`, `tools/scan_tower_level.py`), `proof_volumetric_master_quadratic.py`.
...
**Correct as-is — 11 files retain `137.035999084`; not drift, do not change.**
- Pre-registered / hash-locked (§6): the FTD-0097 scan, `PROTOCOL_LOOK_ELSEWHERE_SCAN.md`, the `PREREG_PHASE_I` doc + its executor — the value was committed before measurement; retrofitting would break the pre-registration.
- Explicitly "CODATA 2018"-labelled historical references (`FOUND_BLIND_DERIVATION_CHAIN.md`, `DERIV_MASTER_QUADRATIC_CM_LVALUES.md`, `EXPLR_LVALUE_SPAN_CORRECTION_SEARCH.md`) — value matches label.
- Docs that quote the old value to document the change: this document.

---

*The canonical edition standard, closing a recurring CODATA-edition drift.*
