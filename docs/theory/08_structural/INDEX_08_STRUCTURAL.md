...
| [AUDIT_WEAK_SU2_PROVENANCE.md](AUDIT_WEAK_SU2_PROVENANCE.md) | [AUDIT FINDING] | Result of the Q12 audit (FTD-0192): **COUNT-MATCH** — the weak SU(2) is not a genuine derivation; its load-bearing step is the count dim{+1,−1}=2=dim(SU(2) fundamental). Genuine FTD content is only the SU(2) skeleton (Cartan + Weyl-ℤ₂). **Terminates the Q10→Q11→Q12 chain: Q10 closes negative, Q11 resolves to CLOSED-NEGATIVE** — FTD's discrete ontology determines the electroweak skeleton but not the rank-2 SU(2)×U(1); the electroweak sector is an honest effective completion (a mapped boundary, goal-clause 2). |
