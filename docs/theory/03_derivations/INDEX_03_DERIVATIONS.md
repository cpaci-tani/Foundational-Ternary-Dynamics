...
| [DERIV_RADIAL_METRIC_RESOLUTION_RETRACTED.md](archive/retracted/DERIV_RADIAL_METRIC_RESOLUTION_RETRACTED.md) | **[RETRACTED 2026-07-02]** *(was [THEOREM])* | The "Discrete Radial Metric Projection Theorem" proof is invalid — its premises force $g_{rr} = 1$ (flat), its concluded metric contradicts its premise wave speed ($C_0 f$ vs $C_0\sqrt{f}$), and its §2.2 invariance step begs the question (and implies superluminal local light speed). Frontier 3 ($g_{rr}$) reverts to [OPEN]. Archived for provenance (FTD-0360). |
