...
2. ~~Explicit construction of $T_{\mu\nu}$ from flux~~ **RESOLVED** — See [DERIV_QFT_GRT_BRIDGE.md](../foundational_mechanics/DERIV_QFT_GRT_BRIDGE.md)
...
**Verified properties:**
- Conservation: $\partial_\mu T^{\mu\nu} = 0$ — **[THEOREM]**, proven from wave equation (see [DERIV_QFT_GRT_BRIDGE.md](../foundational_mechanics/DERIV_QFT_GRT_BRIDGE.md))
- Symmetry: $T^{\mu\nu} = T^{\nu\mu}$ — **[THEOREM]**
- Positive energy: $T^{00} \geq 0$ — **[THEOREM]** (sum of squares)
- Traceless for radiation: $T^\mu{}_\mu = 0$ when $|\dot{J}|^2 = C^2|\nabla J|^2$ — **[THEOREM]**

**Status:** **[THEOREM]** — Derived via Noether's theorem from the flux Lagrangian. All properties verified numerically (18/18 tests pass). See [DERIV_QFT_GRT_BRIDGE.md](../foundational_mechanics/DERIV_QFT_GRT_BRIDGE.md) for full derivation.
...
| **GAP-3** | $T_{\mu\nu}$ construction | **RESOLVED** | See [DERIV_QFT_GRT_BRIDGE.md](../foundational_mechanics/DERIV_QFT_GRT_BRIDGE.md) — Noether's theorem on flux Lagrangian |
