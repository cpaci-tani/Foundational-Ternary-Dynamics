...
3. **The reference frame context-physics bridge** via G*
...
### 3.3 The Reference frame context Threshold

The reference frame context threshold K_C relates to the reference frame context quadratic:
...
**Why?** The squared coupling measures the "strength" of state-flux interaction. For reference frame context (Domain B) to be accessible from physics (Domain A), the effective threshold crossing must bridge the two domains.
...
**Observation:** The reference frame context quadratic coefficient is G*²/2, and the physics coefficient is 16G*².

$$\frac{\text{reference frame context coeff}}{\text{physics coeff}} = \frac{G^{*2}/2}{16G^{*2}} = \frac{1}{32}$$
