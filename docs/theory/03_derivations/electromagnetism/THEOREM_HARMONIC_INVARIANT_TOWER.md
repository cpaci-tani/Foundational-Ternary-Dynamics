...
**Corollary (transcendence of A_k for k ≥ 4) — [CONDITIONAL THEOREM given Chudnovsky 1976].** Since `G*` is transcendental over `Q` (a consequence of the algebraic independence of `π` and `Γ(1/4)` — Chudnovsky 1976; consolidated in Waldschmidt 2000 §1.4 — which gives `G* = Γ(1/4)²/(π√2)` transcendental), every nontrivial polynomial expression `2^{k−2}G*^{k−3} − 1` with `k − 3 ≥ 1` is **transcendental** (a non-rational polynomial in a transcendental over `Q` with rational coefficients is transcendental). <!-- corrected 2026-06-24 spine audit: prior "Lindemann–Weierstrass via Schneider 1941" was incoherent/insufficient — Schneider 1941 alone does not give G* transcendental; the needed result rests on Chudnovsky 1976 (alg. indep. of π and Γ(1/4)). Corollary is therefore conditional on Chudnovsky 1976, not a free-standing [THEOREM]. --> At `k = 3`, `A_3 = 1` is rational; the discriminant is `32 G*²` and `√disc(M_3) = 4 G* √2`, so the level-3 splitting field is `Q(G*, √2)` (a real quadratic extension of the field generated by `G*`). At every `k ≥ 4`, the splitting field properly contains `Q(G*, √A_k) = Q(G*, √(4G*−1))` for `k = 4`, and so on. (Heading was previously "irrationality"; the stronger transcendence claim is the natural conclusion. Tightened 2026-04-30.)

**[THEOREM]** for the algebraic discriminant factorization (proved directly above). The transcendence corollary is a **[CONDITIONAL THEOREM given Chudnovsky 1976]** (algebraic independence of `π` and `Γ(1/4)`; consolidated in Waldschmidt 2000 §1.4). <!-- corrected 2026-06-24 spine audit: was "[THEOREM] ... modulo the standard transcendence theorem of Schneider–Waldschmidt" — Schneider 1941 alone is insufficient; the corollary is conditional on Chudnovsky 1976, while the disc factorization itself stays an unconditional [THEOREM]. -->
