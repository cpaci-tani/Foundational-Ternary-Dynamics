...
**Update and speed.** Leapfrog with dt = 1 and the selected `c² = 1/3` (`c = 1/√3`; gauge_couplings.h). FTD-0407 proves this is not CFL-derived: for the production 18-point symbol, stability permits `c²≤3/4`. Exact leapfrog dispersion `sin²(ω/2) = c²σ₁₈(k)`; session verification to 5.8×10⁻¹⁴ across the Brillouin zone; engine verification <0.1% (campaign_dispersion.cpp). The free cone is massless and linear at the band bottom, ω → |k|/√3, with a nonzero dimension-six preferred-frame correction beyond leading order.
...
**FTD-0567 controlling completion:** the uniform-field counterexample now proves
the production `|J|` trigger cannot follow from the written candidate-state
action, and the exact accepted-event map proves genesis is not an amplitude
lock. The gap is closed negative for the current action rather than merely
unimplemented; a new state-conditioned action/reservoir would be added
structure.
...
3. Phantom citation: `test_variational_proof.cpp` never existed in active history; its dependent all-update tag is **RESOLVED BY RETRACTION** under FTD-0567. The separate "Montanez & Claude 2026" dual-substrate citation remains unresolved.
4. K_EVAP_RATE = 0.1 unpriced — needs an ASSUMP-ledger row or an import-ledger line.
5. The current-action genesis derivation gap is **CLOSED NEGATIVE by FTD-0567**. A conditional rest-mass/event-reservoir term is nameable added content; the chirality sign rule remains a decided `[IMPOSED]` import.
