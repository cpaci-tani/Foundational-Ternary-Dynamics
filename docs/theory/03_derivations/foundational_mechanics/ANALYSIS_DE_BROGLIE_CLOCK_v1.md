...
- **A5 (proper-time → clock): DONE** `[IMPOSED]→[SELECTION]` realized. Voxel `phase` field added (golden-safe); `accumulate_proper_time` (`transmutation_phases.cpp`) advances `dφ = ω₀·dτ`, and now runs whenever the clock is on (not only with the gravity field). Test `test_de_broglie_redshift.cpp`: a static clock's phase `= ω₀·N` **exactly**; a moving clock (v=0.3) red-shifts to ratio **`√(1−v²) = 0.95394` to machine precision** — the covariant clock *rate* is sourced by FTD's own (FTD-0252-measured) proper-time dilation, leaving only the scalar `ω₀∝M_REST` imposed.
