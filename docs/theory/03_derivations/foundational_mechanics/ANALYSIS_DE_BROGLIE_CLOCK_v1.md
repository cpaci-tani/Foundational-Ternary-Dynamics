...
Adding a Klein-Gordon rest-mass clock `∂²J/∂t² = c²∇²J − ω₀²J` (`ω₀ ∝ M_REST`) to FTD's natively-massless flux turns the substrate into a **single-particle pilot-wave theory**: a moving cluster carries a **de Broglie matter wave** (`λ ∝ 1/v`) and its non-relativistic envelope obeys the **Schrödinger equation**. This is the exact quadratic sector FTD-0270 measured as the only door out of its dispersion boundary. **The result is CONDITIONAL**: the clock is an imposed input (A0), and the de Broglie/Schrödinger consequences are textbook Klein-Gordon — so this is a derivation *given* the clock, not an unconditional FTD result. **FTD-0401 correction (2026-07-21):** the A5 argument below no longer supplies a non-circular covariant half. The raw `Voxel::velocity` is nodes/tick with `C_SPEED=1/√3`, while `voxel.tau` consumes it under an unmapped `c=1` formula. The wave-clock result FTD-0252 used `v=v_g/C_WAVE` and never read `voxel.tau`. A5 is therefore implementation self-consistency, not physical covariance; the moving-clock normalization remains imposed.
...
## 4 · A5 — historical covariance proposal, withdrawn by FTD-0401

The historical argument said that a de Broglie clock needs a rest frequency `ω₀` plus a relativistically covariant clock rate, and that `accumulate_proper_time` supplied the second. FTD-0401 invalidates that bridge as represented in the current engine. `accumulate_proper_time` passes raw `Voxel::velocity` directly to `√(1−v²)`, whereas the causal cone and force integrator use `v²/C_SPEED²`. At `v=C_SPEED`, the implemented clock still runs at `√(2/3)` instead of reaching the causal boundary. `test_de_broglie_redshift.cpp` compares the implementation to the same hardcoded raw-velocity formula, so its machine-precision pass is correctness of wiring only. It cannot upgrade any part of the clock from `[IMPOSED]` to `[SELECTION]`.
...
- **A5 (proper-time → clock): WIRING DONE; covariance claim WITHDRAWN by FTD-0401.** Voxel `phase` still advances as `dφ=ω₀·dτ`, and the test still verifies the frozen legacy formula exactly. Because raw velocity is not divided by `C_SPEED`, both the scalar `ω₀∝M_REST` and the moving-clock normalization remain `[IMPOSED]`. FTD-0252/0268 retain their independent wave-clock findings.
