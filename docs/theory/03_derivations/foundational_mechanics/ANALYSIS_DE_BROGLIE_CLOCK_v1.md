...
- **A5 (proper-time → clock): NORMALIZED IMPLEMENTATION; covariance claim remains WITHDRAWN.** Voxel `phase` advances as `dφ=ω₀·dτ` using FTD-0402's selected full budget. FTD-0403 closes the targeted exact, CPU/GPU, golden, WASM/web, and compatibility surface without altering FTD-0402's historical `PARTIAL` verdict. Both the scalar `ω₀∝K_B` and the moving-clock law remain `[IMPOSED]`. FTD-0252/0268 retain their independent wave-clock findings.
