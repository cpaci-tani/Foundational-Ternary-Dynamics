...
- **On the principal ⟨100⟩ axis, the lattice's departure from exact γ vanishes as a power law — `R = |D−√(1−v²)| ∝ L⁻¹·⁹⁸ ≈ L⁻²` (i.e. `∝ k²`, the leading UV-discretization error).** 9/9 matched groups monotone; the residual shrinks 34–94× from L=33 to L=193 (e.g. v≈0.29: 0.0024→0.00003; v≈0.67: 0.0218→0.00065). Independently reproduced by the reviewer; the velocity-drift confound is refuted (R tracks `k`, not `v`). **The established result is axis- and protocol-scoped convergence of this free-wave diagnostic. It does not prove boost covariance or a common interacting cone; FTD-0407 supplies the fully discrete preferred-frame term.** `[MEASURED — diagnostic converges in the IR on ⟨100⟩ + moderate v]`
