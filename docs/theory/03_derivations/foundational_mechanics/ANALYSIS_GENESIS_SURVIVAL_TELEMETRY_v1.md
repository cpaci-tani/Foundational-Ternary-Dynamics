...
**Platform:** canonical C++ engine, **CPU backend** (genesis/evap event counters are CPU-only; the GPU kernel path does not populate the host-side counters). **Stack: post-`c2a8f606`** (the backend-parity refactor — see §4).
...
- **Single backend, stack-pinned.** Genesis/evap event counters populate on the **CPU path only** (the GPU genesis kernel does not touch the host counters). The measurement is on the **post-`c2a8f606`** stack — the backend-parity refactor that split phase_write into a post-write genesis loop and switched Langevin noise to a per-voxel deterministic stream. The cluster magnitudes differ from the FTD-0263 staircase (stack drift: A=14 gives 14 here vs 16.4; A=30 gives ~33 vs 45), but the **qualitative finding (one-shot genesis burst, genesis-limited, high survival) is robust across 4 seeds** and is a structural property of the genesis+evaporation toggle physics, not of the RNG-stream change.
...
- **GOLDEN REGRESSION provenance.** The earlier frozen `GOLDEN_HASH = 0xc13713f0e11a96da` is **stale**: commit `c2a8f606` (backend-parity refactor — post-write genesis divergence + per-voxel `voxel_normal()` Langevin noise) **intentionally changed CPU genesis physics** without re-pinning the hash, so `test_render_bridge_golden` failed on `main` independent of this work (the canonical stack deterministically produces `0xebaa6f314f66db3f`, reproduced ×2, with and without the observation-only counters). The hash was **re-pinned to `GOLDEN_HASH = 0xebaa6f314f66db3f`** (commit `b2c1cb7c`, "perf … align golden hash"); the gate **PASSES with the observation-only counters present** — the strongest confirmation that the counters touch no physics. Two further perf commits (`a4b89e43` genesis/evap/coupling/Gauss optimization, `b2c1cb7c`) left the hash bit-identical, so this measurement reproduces exactly on the post-`c2a8f606` stack.
