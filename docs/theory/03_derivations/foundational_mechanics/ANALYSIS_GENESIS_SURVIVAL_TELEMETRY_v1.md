...
- **GOLDEN REGRESSION — surfaced here, resolved by the concurrent session.** During this arc the frozen `GOLDEN_HASH = 0xc13713f0e11a96da` was found **stale**: commit `c2a8f606` (concurrent backend-parity refactor — post-write genesis divergence + per-voxel `voxel_normal()` Langevin noise) **intentionally changed CPU genesis physics** without re-pinning the hash, so `test_render_bridge_golden` failed on `main` independent of this work (the canonical stack deterministically produces `0xebaa6f314f66db3f`, reproduced ×2, with and without this arc's counters). The concurrent session subsequently **re-pinned `GOLDEN_HASH = 0xebaa6f314f66db3f`** (commit `b2c1cb7c`, "perf … align golden hash"); the gate now **PASSES with this arc's observation-only counters present** — the strongest confirmation that the counters touch no physics. Two further concurrent perf commits (`a4b89e43` genesis/evap/coupling/Gauss optimization, `b2c1cb7c`) left the hash bit-identical, so this measurement reproduces exactly on the current stack.
