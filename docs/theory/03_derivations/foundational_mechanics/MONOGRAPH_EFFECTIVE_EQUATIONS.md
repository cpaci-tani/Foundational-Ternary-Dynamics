...
| **§4.4** | Schwarzschild Radial Component: $g_{rr}$ | **`[RETRACTED 2026-07-02, FTD-0360]`** *(was `[THEOREM]`)* | The cited proof ([DERIV_RADIAL_METRIC_RESOLUTION_RETRACTED.md](../archive/retracted/DERIV_RADIAL_METRIC_RESOLUTION_RETRACTED.md)) is invalid: its premises ($g_{00} = -(1-r_s/r)$, wave speed $C_0\sqrt{1-r_s/r}$, local light-speed constancy) force $g_{rr} = 1$, and its §2.2 invariance step begs the question. Frontier 3 (spatial metric) reverts to `[OPEN]` |
...
| **§5.4** | Compton Volume Duality | **`[DERIVED — conditional]`** *(retagged 2026-07-02, FTD-0360; was `[THEOREM]`)* | The algebra in [DERIV_COMPTON_INVERSION_RESOLUTION.md](DERIV_COMPTON_INVERSION_RESOLUTION.md) is exact and the paradox-dissolution sound (verified under FTD-0360), but it is conditional on the imported KG dispersion (FTD-0271/0270) and the extensive cluster-mass identification $m = K_B N$ (FTD-0110 linear / FTD-0250 [IMPOSED]) |
...
Four primary frontiers of the continuous emergence program were claimed resolved; **all four claims have now been adjudicated** (items 2 and 4 retracted under FTD-0348/FTD-0356; items 1 and 3 reviewed on their merits under FTD-0360, 2026-07-02 — item 1 retagged conditional with its dissolution confirmed sound, item 3 retracted):

1.  **The Compton Dimension Inversion Paradox (RETAGGED 2026-07-02, FTD-0360 — dissolution CONFIRMED sound, tag now `[DERIVED — conditional]`):**
    The **Compton Volume Duality** chain is an exact substitution identity ($\lambda_C = 1/m$, $m = K_B N$, $N = 4\pi R^3/3a^3 \Rightarrow \lambda_C \propto 1/R^3$; re-verified symbolically), and its central clarification survives scrutiny: $R$ is a real-space cluster extent while $\lambda_C$ is a spectral screening scale — different kinds of scale, hence no paradox. But it is **conditional** on the imported KG dispersion (FTD-0271/0270) and on identifying the envelope mass with the extensive cluster mass (FTD-0110 linear / FTD-0250 `[IMPOSED]`), so it is not a bare substrate `[THEOREM]`. See the dated correction in [DERIV_COMPTON_INVERSION_RESOLUTION.md](DERIV_COMPTON_INVERSION_RESOLUTION.md).
...
3.  **Strong-Field Metric Components ($g_{rr}$, RETRACTED 2026-07-02, FTD-0360 — was "RESOLVED"):**
    The claimed **Discrete Radial Metric Projection Theorem** is retracted: from its own premises ($g_{00} = -(1-r_s/r)$, coordinate wave speed $C_0\sqrt{1-r_s/r}$, locally invariant light speed) the null-cone condition forces $g_{rr} = 1$ — a flat radial metric — while the concluded metric would propagate light at coordinate speed $C_0(1-r_s/r)$ (first power), contradicting the premise; the load-bearing "physical length invariance" step ($dl \equiv a$ per coordinate tick) is unproven, equivalent to the conclusion, and implies a superluminal locally measured speed $C_0/\sqrt{1-r_s/r}$. A single scalar $C(r)$ cannot fix both metric functions (the classic light-deflection factor-2 pitfall). Frontier 3 reverts to `[OPEN]` — `DERIV_RELATIVITY_DERIVATION.md` §12's "remaining work" stands. See `docs/theory/03_derivations/archive/retracted/DERIV_RADIAL_METRIC_RESOLUTION_RETRACTED.md`.
