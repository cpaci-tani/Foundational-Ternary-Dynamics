...
| **§4.4** | Schwarzschild Radial Component: $g_{rr}$ | **`[RETRACTED 2026-07-02]`** *(was `[THEOREM]`; FTD-0361)* | The cited proof ([DERIV_RADIAL_METRIC_RESOLUTION_RETRACTED.md](../archive/retracted/DERIV_RADIAL_METRIC_RESOLUTION_RETRACTED.md)) is invalid — its concluded metric's radial null speed $C_0(1-r_s/r)$ contradicts its premised wave speed $C_0\sqrt{1-r_s/r}$, and consistent accounting of its own premises forces $g_{rr}=1$ (the single $\sqrt{1-r_s/r}$ factor is spent twice, once for $g_{00}$, once for $g_{rr}$); $g_{rr}$ remains `[GAP]` (Gap 10.1/11.1, DERIV_RELATIVITY_DERIVATION.md; FTD-0189 Frontier 4) |
...
| **§5.4** | Compton Volume Duality | **`[PARAMETRIC — conditional]`** *(retagged 2026-07-02, FTD-0361; was `[THEOREM]`)* | Algebra sound but both inputs imposed: the massive envelope equation is `[IMPOSED]` (FTD-0271 A0 — native flux is massless) and the pole-mass = $K_B N$ identification is the `[OPEN]` cluster-mass reduction (FTD-0250); see the dated correction in [DERIV_COMPTON_INVERSION_RESOLUTION.md](DERIV_COMPTON_INVERSION_RESOLUTION.md) |
...
Four primary frontiers of the continuous emergence program were claimed resolved; **all four claims have now been adjudicated (FTD-0356 items 2/4; FTD-0361 items 1/3): three are retracted, one is demoted to a conditional parametric relation**:

1.  **The Compton Dimension Inversion Paradox (DEMOTED 2026-07-02, FTD-0361 — was "RESOLVED"):**
    The claimed **Compton Volume Duality Theorem** is retagged `[PARAMETRIC — conditional]`: its algebra ($\lambda_C = 3a^3/(4\pi K_B R^3)$, re-verified sound) is a two-line substitution between two imposed inputs — the massive envelope equation ($m$ term `[IMPOSED]`, FTD-0271 A0; native flux is massless) and the identification of the spectral pole mass with the extensive cluster count $K_B N$ (exactly the FTD-0250 cluster-inertia `[IMPOSED]` / reduction-`[OPEN]` bridge). The conceptual dissolution survives at `[SELECTION]` grade: $\lambda_C$ is a propagator screening scale, not a matter radius, so $R \propto m^{1/3}$ vs $\lambda_C \propto 1/m$ was never a geometric contradiction (textbook analogue: nuclear charge radius $\propto A^{1/3}$ vs Compton wavelength $\propto 1/A$). See the dated correction in [DERIV_COMPTON_INVERSION_RESOLUTION.md](DERIV_COMPTON_INVERSION_RESOLUTION.md).
...
3.  **Strong-Field Metric Components ($g_{rr}$, RETRACTED 2026-07-02, FTD-0361 — was "RESOLVED"):**
    The claimed **Discrete Radial Metric Projection Theorem** is retracted: its conclusion contradicts its premise — the concluded metric ($g_{00} = -(1-r_s/r)$, $g_{rr} = (1-r_s/r)^{-1}$) has radial null speed $C_0(1-r_s/r)$, while the premise supplies $C(r) = C_0\sqrt{1-r_s/r}$ (at $r = 2r_s$: $0.5\,C_0$ vs $0.707\,C_0$). The single $\sqrt{1-r_s/r}$ slowdown factor is spent twice (once as the clock-rate effect yielding $g_{00}$, again as ruler stretching yielding $g_{rr}$); consistent accounting of the document's own premises forces $g_{rr} = 1$, and its premise index $n = (1-r_s/r)^{-1/2}$ is the classic half-light-deflection medium. $g_{rr}$ remains `[GAP]` (Gap 10.1/11.1 in DERIV_RELATIVITY_DERIVATION.md; FTD-0189 Frontier 4 `[OPEN]`). See `docs/theory/03_derivations/archive/retracted/DERIV_RADIAL_METRIC_RESOLUTION_RETRACTED.md`.
