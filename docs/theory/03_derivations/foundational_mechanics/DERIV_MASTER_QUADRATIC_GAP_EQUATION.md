...
This document records (i) the algebraic identity itself; (ii) the multiple finite-combinatorial routes to each of its coefficients (G\* via Watson–Chowla–Selberg, the leading 16 via two independent counts, the constant term via Vieta); (iii) the discriminant trichotomy that partitions one quadratic into bosonic, fermionic, and measurement regimes; and (iv) the empirical match plus polynomial-template structural-uniqueness evidence (FTD-0319 adversarial scan, formerly cited as FTD-0189: 0 non-G\* dual-matchers across 2.65M degree-2 polynomials over an 18-constant FTD-undesigned basket — **⚠ this uniqueness evidence was withdrawn 2026-08-03/04, FTD-0791/FTD-0802; see the correction below**) that motivates the single live physics identification x₊  1/α (FTD-0013).

What this document does **not** do: derive the master quadratic as the L → ∞ limit of any finite-L self-consistency equation. The framework is undefined-boundary (no completed-totality lattice), and the explicit finite-L gap-equation scan and the L = 2 partition-function calculation jointly close the limit-route. The polynomial is what it is — an algebraic identity — and the physical identification x₊  1/α rests on the empirical match (1.26 ppm) + polynomial-template uniqueness (FTD-0319, formerly cited as FTD-0189 — **⚠ withdrawn, see correction below**), not on a dynamical derivation.

> **Correction — 2026-08-03/04, FTD-0791 / FTD-0802.** The polynomial-template structural-uniqueness leg cited above (FTD-0319's adversarial scan: "0 non-G\* dual-matchers across 2.65M degree-2 polynomials") does not survive audit. FTD-0791 found **Leg 1 sits exactly at its own chance base rate:** at the registered `2e-6` gate the null expects **1.42–1.67** matchers and the scan found **1** — a random target near 137 is matched to 2 ppm by this family **79% of the time**. **At that gate the second (dual-matcher) leg eliminates nothing** (1 → 1), so "dual-matcher" and "matcher" coincide. FTD-0319 is retagged `[MEASURED]` → `[SELECTION]`, and the designation **"the framework's single scan-rigid identification" is withdrawn — the scan-rigid count is now zero.** FTD-0802 independently found the same defect in the separate 2,871,576-polynomial OT-3.3 extended scan (`N_null = 0.0014`, Outcome B — the zero count it reported is exactly what chance predicts), and that its published count was itself wrong (a hardcoded literal, not a tested result; corrected to 4 non-master dual-matchers). **`x₊ = 1/α` therefore retains no numerical-uniqueness support of any kind**, leaving only the structural motivation named in this document's own Part I (algebraic rigidity of the polynomial). The 1.26 ppm empirical match itself is unchanged and unretracted — only the polynomial-template uniqueness claim is withdrawn. This correction moves no tag: `x₊ = 1/α` stays `[STRONGLY MOTIVATED CONJECTURE]`.
