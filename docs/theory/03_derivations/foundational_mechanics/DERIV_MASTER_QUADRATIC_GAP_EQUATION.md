...
|---|---|---|---|---|
| **Bosonic** | $k > 4/G^*$ (e.g. $k = 16$) | $\Delta > 0$ | real, distinct | coupling constant $\alpha$ |
...
The larger root $x_+ = 137.036$ is identified with $1/\alpha$ at 1.26 ppm (FTD-0013 [STRONGLY MOTIVATED CONJECTURE]). The conjecture rests on the numerical match plus CM-curve uniqueness across class-number-1 fields (§4.2).
...
Among the nine imaginary quadratic fields with class number $1$, the discriminant $d = -4$ — corresponding to $E\colon y^2 = x^3 - x$ — is the only curve whose associated polynomial gives a larger root that matches $1/\alpha$ to permille precision; this single-curve selection is a **[NUMERICAL FACT]** of the class-number-1 scan, not a proven structural necessity. (See `scripts/exploration/scan_cm_class_number_1.py` and the Option 3 results in CONJ_ALPHA_FROM_CM.md.)
...
- That the physical identification is a [THEOREM]. The identification `x_+ = 1/α` is [STRONGLY MOTIVATED CONJECTURE], anchored on the empirical match plus CM-curve uniqueness.
...
| 10 | Physical identification $x_+ \leftrightarrow 1/\alpha$ | [STRONGLY MOTIVATED CONJECTURE] (numerical match + CM uniqueness) |
...
5. **[STRONGLY MOTIVATED CONJECTURE]** The root $x_+$ identifies with $1/\alpha$, supported by the numerical match (1.26 ppm) and CM-curve uniqueness across class-number-1 fields.
