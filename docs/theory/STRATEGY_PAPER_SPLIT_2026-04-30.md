...
1. A **rigorous mathematical core** — nine numbered results (seven
   theorem-grade + two honestly-tiered below theorem grade — Theorem 3
   at its arithmetic core only; see `SPEC_ALGEBRAIC_SPINE.md` §0 count
   convention) on `G*`, the master
...
- The look-elsewhere scan FTD-0097 results for the catalog at large
  must be cited as a methodological-hygiene caveat: the catalog at large
  has been audited as over-rich at the monomial level (FTD-0097 NULL
  REJECTED upward).
