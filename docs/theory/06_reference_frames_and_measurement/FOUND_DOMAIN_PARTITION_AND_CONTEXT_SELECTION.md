...
2. It unifies the live vocabulary around **origin**, **`i`**, **reference frame context**, and the **Potential Core / Generative Interior** framework.
...
This document is intentionally conservative. It does **not** claim that reference frame context has been derived as a theorem from the lattice axioms. It clarifies which parts of the live reference frame context layer are:
...
| `i` | orthogonal phase structure, 90-degree rotation, complex self-reference operator | not the origin, not literally reference frame context |
| Potential Core `P_c` | minimal center voxel or center of coherence for a local object/process | not identical to `i` |
| Generative Interior `G(P_c)` | full space of operative capacities available to `P_c` in principle | not identical to reference frame context |
...
| Reference frame context | proposed self-referential, context-conditioned process supported by complex phase structure | not reducible to a single symbol |
...
- **reference frame context** names a self-referential, context-conditioned regime that may require complex phase structure but is not identical to `i`.
...
- `k = 1/2` gives the complex-root reference frame context regime
...
Domain B should be read in the active corpus as a **context-conditioned self-referential layer**, not as proof that the imaginary unit by itself is reference frame context.
...
| ID | Statement | Status |
|---|---|---|
| DP-C1 | Keep origin, `i`, Potential Core, Generative Interior, and reference frame context distinct in the live vocabulary | [CONJECTURE / governance] |
| DP-C2 | Domain B is best read as a context-conditioned self-referential layer, not literal identity of `i` with reference frame context | [CONJECTURE] |
...
| Document | Relevance |
|---|---|
| [DERIV_CONSCIOUSNESS_QFT_GR_SYNTHESIS.md](DERIV_CONSCIOUSNESS_QFT_GR_SYNTHESIS.md) | reference frame context quadratic, complex roots, `K_C`, phase angle, Domain A/B/C |
