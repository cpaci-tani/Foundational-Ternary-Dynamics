...
So: the substrate **has** the quadrature angle and can even sweep to any value of it by waiting. What it lacks is that different quadrature angles **co-measure** instead of being complementary — `{q,p} ≠ 0` (Poisson; the winding in §3 is that nonzero bracket made visible) but `[q,p] = 0` (observable commutator; `q` and `p` carry joint definite values every tick), per `THEOREM_COMMUTATIVITY_INDEPENDENCE` ([../10_eft_program/derivations/THEOREM_COMMUTATIVITY_INDEPENDENCE.md](../10_eft_program/derivations/alpha_readout_programme/THEOREM_COMMUTATIVITY_INDEPENDENCE.md)) §3. The angle is substrate-native; its **incompatibility** — the property that makes choosing a quadrature a genuine *measurement* choice — is the injected `M`. `[THEOREM for commutativity; BOUNDARY for the rest]`
