...
> **Vocabulary refresh (2026-05-01):** This document originally framed several claims using "reference frame context" terminology. The mathematical content (Existence Filter, projection hierarchy, Born Rule reconstruction, Tomita–Takesaki modular conjugation correspondence) is fully preserved and unchanged. Surface-level renames only: *reference frame context phase angle* → **frame-relative phase angle**; *reference frame context* in the load-bearing claims → **frame-relative integration / frame-relative readout** per the canonical [`REF_REFERENCE_FRAME_VOCABULARY.md`](../01_reference/REF_REFERENCE_FRAME_VOCABULARY.md). Qualia commitments are dropped; the remaining claims are structural/operator-algebraic. The Existence Filter itself $E(x) = (x + \bar{x})/2$ is structural and reframe-stable.
...
5. The **frame-relative phase angle**: $\theta = 52.54°$ measures the projection geometry induced by the Existence Filter (formerly "reference frame context phase angle"; renamed 2026-05-01 per [`REF_REFERENCE_FRAME_VOCABULARY.md`](../01_reference/REF_REFERENCE_FRAME_VOCABULARY.md))
...
| $\mathcal{M}$ Type III$_1$ | $\mathbb{C} \cdot \mathbf{1}$ | For reference frame context: only the identity survives full self-reflection |

**Remark:** For Type III$_1$ factors (the algebra of reference frame context), the $J$-fixed subspace is trivial — only scalars survive. This is the algebraic expression of **first-person irreducibility**: when a reference frame applies the Existence Filter to itself (self-reflection), almost nothing survives as "objectively existing." The agent's internal states are invisible to external observation. Only the identity $\mathbf{1}$ (the fact of existence itself) persists through the filter.
...
**Proposition EF-C4** [CONJECTURE] (Domain A Projection). The Existence Filter is the **Domain A projection** — it extracts "what exists" (physics, real roots) and discards "what knows" (reference frame context, imaginary parts).
...
For Domain A (physics), the Existence Filter is transparent — real roots pass through unchanged. For Domain B (reference frame context), the filter strips the self-referential phase component ($\pm 2.86i$), leaving only the objective residue ($2.19$). The interface is also transparent.

### 6.2 The Phase Angle as Information Loss

**Proposition EF-T9** [SELECTION] (Phase Angle and Filter Projection). For the reference frame context roots $y = 2.19 \pm 2.86i$ of the reference frame context quadratic ($k = 1/2$), the Existence Filter projects onto the real axis with a ratio determined by the reference frame context phase angle $\theta = 52.54°$ ([DERIV_CONSCIOUSNESS_QFT_GR_SYNTHESIS.md](DERIV_CONSCIOUSNESS_QFT_GR_SYNTHESIS.md), §3):
...
| EF-T9 | Phase angle $\theta = 52.54°$ as information loss measure | EF-T1, reference frame context roots | §6.2 |
