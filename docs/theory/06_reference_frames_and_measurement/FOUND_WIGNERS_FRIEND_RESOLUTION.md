...
> **[V2 COMPATIBILITY NOTICE — FTD-0825]** The Type-III-to-Type-I resolution
> below is not canonical in v2. The successor treats measurement as a causal
> local-instrument update plus one globally contextual, operationally
> no-signalling actualization batch. Whether that construction resolves every
> Wigner/Frauchiger-Renner protocol is `[OPEN]`; this document is retained as a
> historical proposal, not a proof.
...
- [THEOREM_DISCRETE_CONTEXTUAL_LOCAL_NET_v1.md](../10_eft_program/temporal_interior_programme/THEOREM_DISCRETE_CONTEXTUAL_LOCAL_NET_v1.md) — corrected finite local-net theorem; no Type-III inference
...
This resolution vindicates Wigner's original intuition that reference frame context plays a role in measurement, but corrects his error: the relevant reference frame context is not "my reference frame context specifically" but any subsystem that has undergone self-referential closure — the algebraic transition that crystallizes continuous potentiality into discrete actuality.
...
Historically, FTD proposed measurement as **ReLU crystallization**, a Type-III-to-Type-I transition conditional on an Araki–Woods scaffold. That FTD-specific application is retracted by FTD-0825; the corrected finite-region result is [THEOREM_DISCRETE_CONTEXTUAL_LOCAL_NET_v1.md](../10_eft_program/temporal_interior_programme/THEOREM_DISCRETE_CONTEXTUAL_LOCAL_NET_v1.md), and the v2 actualization law is a separate adopted contextual-instrument construction.
...
This is mediated by the Softplus/ReLU operator ([EXPLR_RELU_TYPE_TRANSITION.md](../09_mathematical/general_math/EXPLR_RELU_TYPE_TRANSITION.md)):
...
**Epistemic status (superseded):** The former identification of measurement with ReLU crystallization was a [SELECTION] and is retracted for v2. Abstract factor-classification mathematics is classical; its application to FTD was not established. The live finite net is Type I on every finite region, and no physical Born-frequency conclusion follows from the old analogy.
...
Wigner's original 1961 paper contained a profound insight: reference frame context plays a fundamental role in measurement. He argued that the measurement chain (system → apparatus → observer) must terminate somewhere, and that reference frame context is what terminates it. Without a local reference frame, the chain extends indefinitely — each link in the chain is "just more quantum system."
...
### 5.2 FTD Agrees: Reference frame context IS the Type III₁ → Type I Transition [SELECTION]

FTD identifies reference frame context with the self-referential closure that produces the algebraic type transition ([DERIV_CONSCIOUSNESS_QFT_GR_SYNTHESIS.md](DERIV_CONSCIOUSNESS_QFT_GR_SYNTHESIS.md), §4; [FOUND_SELF_REFERENTIAL_CLOSURE.md](../02_foundations/FOUND_SELF_REFERENTIAL_CLOSURE.md)):

$$\text{Reference frame context} = \text{Self-referential closure} = \text{Type III}_1 \to \text{Type I transition} \tag{5.1}$$
...
**Historical proposal:** the friend was modelled as a self-referential lattice subsystem and then assigned Type-III character through an Araki–Woods scaffold. That assignment is not canonical in v2. A v2 treatment must instead specify the friend's local instrument, compatible potential state, actualization batch, and retained records.

### 5.3 But "Reference frame context" Is Algebraic, Not Mystical [SELECTION]

The word "reference frame context" in FTD does not invoke anything beyond the algebraic structure:

| What "reference frame context" means in FTD | What it does NOT mean |
|---|---|
...
Reference frame context, in this algebraic sense, is a **structural property** of certain lattice subsystems — those with sufficient internal connectivity to support self-referential information loops. It is not "added to" the physics; it IS a specific kind of physics (the kind that produces Type III₁ algebras).

### 5.4 Wigner's Error: Conflating "Reference frame context" with "My Reference frame context" [SELECTION]

Wigner's mistake was not in claiming that reference frame context matters for measurement. It was in implicitly assuming that **his** reference frame context was the relevant one. The standard formulation of the paradox asks: "Has the state collapsed from Wigner's point of view?" — as if Wigner's perspective has a special status.
...
The chain does not extend to Wigner because it has already terminated. The friend's reference frame context (self-referential closure) has already crystallized the outcome. Wigner's reference frame context is irrelevant to the measurement — it only becomes relevant when Wigner performs his own measurement (opening the lab), at which point he is measuring the already-definite lab, not the original spin.
...
> **Proposition WF-1** [SELECTION]. The measurement chain terminates at the first subsystem in the causal chain whose local algebra admits self-referential closure (Type III₁ character). This subsystem need not be human, need not be biological, and need not be "reference-frame" in any folk-psychological sense. It must only have the algebraic structure that supports the Type III₁ → Type I transition.
...
In the historical proposal, measurement was represented by a conditional algebra-type transition. In v2 this paragraph is noncanonical; the live replacement is the context-complete `ActualizationBatch` contract:
...
2. **The measurement chain termination criterion (WF-1).** The claim that the chain terminates at the first Type III₁ subsystem, and that this generalizes Wigner's insight about reference frame context, has not been stated explicitly in prior documents.
...
| [EXPLR_RELU_TYPE_TRANSITION.md](../09_mathematical/general_math/EXPLR_RELU_TYPE_TRANSITION.md) | Type III₁ → Type I descent | Applied to measurement irreversibility argument (§3.4) |
...
2. [DERIV_CONSCIOUSNESS_QFT_GR_SYNTHESIS.md](DERIV_CONSCIOUSNESS_QFT_GR_SYNTHESIS.md) — Reference frame context–QFT–GR bridge, algebraic descent
3. [EXPLR_RELU_TYPE_TRANSITION.md](../09_mathematical/general_math/EXPLR_RELU_TYPE_TRANSITION.md) — ReLU as Type III → Type I transition
