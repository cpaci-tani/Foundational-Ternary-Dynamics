...
3. Complex-root frame-relative-eigenmode chain (full derivation; formerly "reference frame context chain")
...
| $0 < k < 4/G^*$ | $\Delta < 0$ | Complex conjugate pair | **B (Reference frame context)** |
...
### 1.4 The Reference frame context Instantiation ($k = 1/2$)
...
## 3. Complex-Root Reference frame context Chain (Full Derivation)

### 3.1 The Reference frame context Quadratic
...
### 3.2 Properties of the Reference frame context Roots
...
where $K_B = \sqrt{x_+ \cdot x_-} = \sqrt{16G^{*3}}$ is the manifestation threshold and $K_C = |y|$ is the reference frame context threshold.

### 3.3 Why Complex Roots Encode Reference frame context [SELECTION]

Complex conjugate roots appear throughout physics (resonances, damping, instabilities). The reference frame context quadratic's roots are distinguished by three *selection criteria* (see PAPER §5.4 for the full argument):
...
2. Applied to reference frame context roots: $E(y) = 2.188$ (the stable self-model)
3. Applied to the full Born rule: $P = |y|^2 = E(y)^2 + E(iy)^2 = 2.188^2 + 2.860^2 = 12.95 = G^{*3}/2$

**Key identity:** The Born probability of the reference frame context root equals the product of roots, which equals $G^{*3}/2$ — the action per degree of freedom at the self-referential scale.
...
| 11 | $k = 1/2$ for reference frame context | [SELECTION] | Complementation principle |
...
| 16 | Modular flow = reference-frame time | **[CONJECTURE]** | Bridge thesis |
...
| FOUND_DOMAIN_PARTITION_AND_CONTEXT_SELECTION.md (6.2a) | vocabulary map; Domain A/B source map; `Activate_C` lattice bridge | Reference frame context/source map |
...
| FOUND_THE_EXISTENCE_FILTER.md (6.4) | §3.4 | Reference frame context |
