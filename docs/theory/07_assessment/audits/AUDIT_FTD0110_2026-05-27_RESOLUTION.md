...
- **Linear-level theorem** in [`DERIV_K_FROM_OH_A1G_MULTIPLICITY.md`](../../03_derivations/foundational_mechanics/DERIV_K_FROM_OH_A1G_MULTIPLICITY.md): under δ-localised injection at the O_h-fixed centre voxel, the mean energy fraction across the 4 A_{1g} eigenmodes of the 18-point Laplacian on the 27-block equals $1/N_\text{base} = 1/4$ exactly. **Solid `[THEOREM]`.**
- **Bridge-I global O_h-equivariance** ([`DERIV_FTD0110_NONLINEAR_BRIDGE.md`](../../03_derivations/foundational_mechanics/DERIV_FTD0110_NONLINEAR_BRIDGE.md) §§2.1–2.7): every pipeline step ($T_1$ wave, $T_2$ genesis-in-expectation, $T_3$ Langevin, $T_4$ Gauss projection, $T_5$ evaporation, $T_6$ back-reaction) commutes with $\rho_{27} \otimes T_{1u}$ in expectation, so the time-averaged flux $\langle \phi \rangle$ remains A_{1g}-isotypic globally. **Solid `[DERIVED]`.**
...
> 
> ([`DERIV_FTD0110_NONLINEAR_BRIDGE.md`](../../03_derivations/foundational_mechanics/DERIV_FTD0110_NONLINEAR_BRIDGE.md) §3.2 Route A)
...
The empirical cluster-efficiency $k(A)$ from [`EXPLR_FTD_0110_NONLINEAR_BRIDGE_ANALYSIS.md`](../../03_derivations/foundational_mechanics/EXPLR_FTD_0110_NONLINEAR_BRIDGE_ANALYSIS.md) §0:
