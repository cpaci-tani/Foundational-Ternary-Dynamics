...
| `docs/theory/10_eft_program/derivations/gravity_cosmology/DERIV_SPIN2_BOUNDARY_THEOREM_FREE_THEORY.md` | Cross-reference added distinguishing PROVEN free-theory clauses from ATTEMPTED canonical-toggle-set extension (FTD-0193) | ADD |
...
**Finding.** The canonical lattice tick was quoted everywhere as `t_phys = √3·ℓ_P/c ≈ 9.34×10⁻⁴⁴ s`. This is inconsistent by a factor of 3 with FTD's own CFL result `c_lat = 1/√3`: requiring the lattice speed `1/√3` (the Courant number used in the engine leapfrog) to reproduce physical `c` forces `c = c_lat·(a_phys/t_phys) ⇒ t_phys = ℓ_P/(√3·c) = t_P/√3 ≈ 3.11×10⁻⁴⁴ s`. Three independent derivations (Courant relation; per-axis crossing; von-Neumann stability) all give `t_P/√3`; the old `√3·t_P` silently assumed a *different* convention (tick ≡ body-diagonal light-crossing) incompatible with `c_lat=1/√3`. Origin traced to `engine/web/js/constants.js` (`FTD_TICK_S = Math.sqrt(3)·t_P`), propagated to ~30 sites. Resolution (owner-selected): **match the engine + Courant derivation** → `t_phys = ℓ_P/(√3·c)`.

**Corrected (active canon + code):** `engine/web/js/constants.js`, `units.js`, `scale-context-panel.js`; `dimensional_map.json` (+ rendered `SPEC_DIMENSIONAL_MAP.md`); `SPEC_FTD.md`; constitution `SPEC_FTD_FRAMEWORK_V1.md`; `LEDGER.md` (FTD-0041); `SPEC_IMPORT_LEDGER.md` (IMP-K2 also reclassified `[DERIVED from IMP-K1 + c_lat]` — time is **not** an independent import); `CLAUDE.md`; `FOUND_LATTICE_SPACING_GAUGE_FREEDOM.md`; `DERIV_STATE_FLUX_TO_EFT_DICTIONARY.md`; `SPEC_CLASS_B_CLUSTER_PERSISTENCE.md` (downstream muon tick-count recomputed 2.35→7.06×10³⁷); and 9 active secondary specs (energy-scales, predictions-forward, discrete-native, prediction-ledger-deviations, act-reduction ×2, wave-sectors, spin2-scope + prereg).

**New doc:** `DERIV_DIMENSIONAL_GATE.md` (META_INDEX 3.31) — the gate `Q = q̂·μ^a λ^b τ^c`, the 3-slot count, the defined/defined/derived classification, the corrected time derivation with proof, and verified worked examples.

**Provenance — intentionally NOT rewritten** (record what was believed at their date): this `CHANGELOG_REFRAME.md`, `CHANGELOG.md`, `AUDIT_CLOCK_HYPOTHESIS_v1/v2`, `AUDIT_2026-04_pre-refactor.md`, superseded `PREREG_CLOCK_HYPOTHESIS_DERIVATION_v1.md`, and the dated `scripts/exploration/audit_gn_calibration_2026-05-03.py` (which recomputes the old tick). **No dimensionless prediction changes** — the falsifiable spine is gauge-invariant; only absolute tick↔second conversions move. Zero promotions; the change is a corrected arithmetic error, not a re-derivation.

### Electron-primary adopted as the default gauge (2026-07-08)

Following `FOUND_ELECTRON_PRIMARY_GAUGE.md`, the **default calibration gauge is switched from Planck-primary to electron-primary** across the canon (SPEC_FTD calibration section; constitution SPEC_FTD_FRAMEWORK_V1 §3.3 + calibration register; LEDGER FTD-0041; SPEC_IMPORT_LEDGER gauge note; dimensional_map.json a_phys entry + regenerated map; CLAUDE.md; META_INDEX 2.16; FOUND_LATTICE_SPACING_GAUGE_FREEDOM §4.1/§4.5). **Register:** import `{ℏ, c, m_e}`; the single beyond-universal anchor is the electron mass; `a_phys = ℓ_P = ƛ_C·Kα¹¹` [DERIVED ~0.19%], `t_phys = ℓ_P/(√3·c)` [derived], and `G = ℏc(Kα¹¹)²/m_e²` (`α_G(e,e)=(m_e/m_P)²`, [SMC], 0.38%) are all derived — one beyond-universal import instead of Planck-primary's two, and `G` is an output rather than a hidden import.

**Honest tradeoff (recorded, not hidden):** the switch reclassifies `a_phys` and `G` from a clean `[IMPOSED]`/declared status to *derived-conditional* on the `[SMC]`-grade `m_e/m_P = Kα¹¹` ladder. It buys non-circularity, minimality, and operational grounding at the cost of the length/gravity scales inheriting the ladder's `[SMC]`/`[SELECTION]` tags. **Legacy Planck-primary remains a valid gauge** (FTD-0137 §4.1). Zero promotions; **no dimensionless prediction changes** (gauge-invariant spine); this is a re-anchoring of the import surface, decided by the owner.


## 2026-09-15 — Owner-requested external research archive

Removed the standalone RH/prolate-Weil exploration, six speculative application/polemic paper sources, and already-retracted Navier–Stokes/Yang–Mills artifacts from the current FTD tree. All 18 files (including related local bytecode) were moved to the external archive with verified SHA-256 digests. Existing Git history and a pre-cleanup recovery bundle preserve the tracked provenance. The canonical finitude source, independent per-voxel mass-gap proof, and engine/lattice work remain.

The active paper inventory, abstract catalog, mathematical navigation, parking lot and ledger source locations were reconciled. No mathematical or physical claim was promoted, demoted, or retracted by this scope cleanup. Former parking-lot item 3 is no longer active FTD work. See the [archive receipt](../../audits/RESEARCH_EXTERNALIZATION_2026-09-15.md). The owner requested a local checkpoint; no GitHub push is authorized.
