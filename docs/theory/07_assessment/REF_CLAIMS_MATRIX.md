...
| **WEINBERG-1** | sin²θ_W = N_c/N_eff = 3/13 = 0.2308 (0.19% error vs M_Z) | **THEOREM** | 13-axis spatial projection of Moore neighborhood | DERIV_WEINBERG_STRONG_GEOMETRY.md | Resolved via geometric derivation | `scripts/verification/verify_mixing.py` |
| **STRONG-1** | α_s = b₃/(b₃+4N_eff) = 7/59 = 0.1186 (0.3σ) | **THEOREM** | Geometric fixed point of Dirac vacuum screening on 13 axes | DERIV_WEINBERG_STRONG_GEOMETRY.md | Resolved via structural derivation of 59 denominator | `scripts/verification/verify_mixing.py` |
| **ATOMIC-1** | Helium ground state screening $\sigma_{FTD} = G^*/10$ yielding $E_0 = -79.023$ eV | **STRONGLY MOTIVATED CONJECTURE** | 10 transverse (non-Coulomb) screening axes | DERIV_DISCRETE_ATOMIC_SPECTRUM.md | >0.1% discrepancy unexplained | n/a |
| **ATOMIC-2** | Shell Capacities $(2, 8, 18)$ from Moore partition $(2 \text{ core}, 8 \text{ corners}, 18 \text{ faces/edges})$ | **STRONGLY MOTIVATED CONJECTURE** | Structural geometry of Moore bounding box | DERIV_DISCRETE_ATOMIC_SPECTRUM.md | Incompatible shell filling order observed | n/a |
