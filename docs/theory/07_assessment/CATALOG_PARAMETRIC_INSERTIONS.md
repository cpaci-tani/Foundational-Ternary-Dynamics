...
| Continuum α extrapolation (Phase F, 2026-04-19) | 4-point r_max series at consistent r/L≈0.31 and ticks=150, 3 scaling laws all agree | GPU runs at L ∈ {64, 128, 256, 384} | **[MEASURED]** | Engine-convention α_∞ ∈ [3.35×, 3.74×] α_ref (plateau at ~3.6×). Post-audit (`AUDIT_ALPHA_EXTRACTION.md`): engine's `field_energy = Σ\|J\|²` lacks the classical ½, so measured V(r) is 2× classical → classical-convention plateau is **~1.8× α_ref**. Residual ~1.8× is [OPEN] — real physics vs. kinetic normalization vs. Green's-function artefact. FTD's Coulomb-tail coupling is **not α_ref** under any convention, but "how-much-larger" depends on convention. `DERIV_DAY2_CAMPAIGN.md` §6b + `PAPER §9` + `AUDIT_ALPHA_EXTRACTION.md`. |
