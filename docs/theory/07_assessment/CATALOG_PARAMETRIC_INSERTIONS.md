...
| α (fine structure) | 1/137.036 | Master quadratic x_+ root; tree 1.26 ppm | **[STRONGLY MOTIVATED CONJECTURE]** (FTD-0013; downgraded per `AUDIT_MASTER_QUADRATIC.md`) | Polynomial layer is [THEOREM]; physical identification x_+ = 1/α conditional on curve + degree + root selections; 7-term expansion is a post-hoc fit to CODATA digits beyond experimental precision (NOT a "<0.001 ppt derivation"). *(Historical: Dual-prediction (x_- ≈ N_c) was the strongest evidence — rigidity scan in `audit_master_quadratic_rigidity.py` showed no other polynomial in 60k scan matched both roots.) The `x_-  N_c` identification is **RETIRED** per v1.4 §5 (LEDGER FTD-0014 removed in commit `ca7eb61`). The canonical structural-uniqueness evidence was FTD-0319's (formerly cited as FTD-0189) adversarial polynomial-template scan (0 non-G\* dual-matchers across 2.65 M polynomials over an 18-constant FTD-undesigned basket; rank 1 by ~130×) — **withdrawn 2026-08-03, FTD-0791** (independently confirmed by FTD-0802, 2026-08-04, on the separate OT-3.3 extended scan): the scan sits at its own chance base rate (registered gate expects 1.42–1.67 matchers, found 1), so this no longer counts as structural-uniqueness evidence and the scan-rigid count is now zero; the 1.26 ppm agreement itself is unaffected.* |
