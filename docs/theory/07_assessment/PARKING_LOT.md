# Parking Lot — Reframe Deployment Items Deferred

**Status:** items the owner has explicitly parked, not items that are silently waiting. Each item has been triaged and is **deliberately deferred** — not forgotten. Anyone resuming reframe work should consult this list to understand what was intentionally left out.

**Maintenance rule:** items move out of this list either when (a) the owner explicitly returns them to active work, or (b) a later promotion is logged via an entry in `CHANGELOG_REFRAME.md`. Items should not silently leave the parking lot.
...
- **Pre-conditions before resuming:** `dissemination/manuscript_v2/PROPAGATION_RULE.md` must be followed; the manuscript divergence audit (item 8 below) should run first to baseline the v1/v2/vol1/vol2 state.
...
- **Status:** PARKED.
- **Context:** the `α_inf` rename agent planted a TODO comment in `engine/tests/benchmark_dynamical_sm.cpp` to emit an empirical residual band for `α_largeL` once a multi-seed ensemble exists. The single-seed run produces a point estimate; the ensemble run produces the band.
