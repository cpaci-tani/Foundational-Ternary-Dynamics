...
- `speculative/FTD_Finitude_Theorem.tex` — paradoxically, this paper *is* the reframe in another voice; many uses of `\infty` are *quoting the proscribed object to deny it*. Self-reflexive — needs careful re-classification rather than simple flag-by-grep.
...
**Top passages (note: this is a self-reflexive case — paper aligns with the reframe):**
