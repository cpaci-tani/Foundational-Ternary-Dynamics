...
| **OT-1.7** | `G* via det_ζ quarter-conjugacy bridge`: $\det_\zeta D_{3/4}/\det_\zeta D_{1/4} = G^*$ where $D_a = \{n + a\}_{n\ge 0}$ are the spectra of operators on $S^1$ with quarter-twisted boundary $\psi(\phi+2\pi) = J\,\psi(\phi)$, $J^2 = -I$ | Lerch's formula: $\det_\zeta\{n+a\}_{n\ge 0} = \sqrt{2\pi}/\Gamma(a)$; $\sqrt{2\pi}$ cancels in the ratio leaving $\Gamma(1/4)/\Gamma(3/4) = G^*$ | FTD-0141 (2026-05-06); `DERIV_GSTAR_QUARTER_CONJUGACY.md`; complementary to OT-1.2 (algebraic) and FTD-0127 (L-function lens) |
