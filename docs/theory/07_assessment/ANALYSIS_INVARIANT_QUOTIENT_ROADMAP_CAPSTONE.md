...
At capstone close, the next registry ID was **FTD-0400** and this document allocated no successor. **Successor note (updated 2026-07-21):** FTD-0400 was subsequently allocated to the static confinement-energy/gravity bridge audit (`SPLIT-BOOKKEEPING`), FTD-0401 to its derived-speed/mass-role normalization prerequisite (`UNMAPPED-DUAL-NORMALIZATION`), FTD-0402 to the selected current-engine correction (historically `PARTIAL`), and FTD-0403 to the independently locked targeted dependency closure (`TARGETED-CLOSURE` v2; `§12-cnorm` closed). None was a new mass measurement. The next free id is FTD-0404, unallocated.
