...
- 🔶 **FTD-0110 nonlinear bridge — [OPEN] (stale-closure corrected 2026-06-09).** A 2026-05-27 entry here claimed the bridge "fully closed" via the `DERIV_FTD0110_NONLINEAR_BRIDGE.md` §6 Orbit-Equipartition + Timescale-Separation theorems; that closure was **reverted the same day** by [`AUDIT_FTD0110_2026-05-27_RESOLUTION.md`](../audits/AUDIT_FTD0110_2026-05-27_RESOLUTION.md) (four defects, incl. an arithmetic error at the load-bearing A²/4 step and a single-block analysis that predicts saturation at 27, not A²/4). Canonical position: linear k = ¼ `[DERIVED]` + Bridge-I O_h-equivariance `[DERIVED]`; **nonlinear coefficient origin `[OPEN]`/`[SMC]`**. **2026-06-09 update (FTD-0259):** Mechanism α's leakage-aggregation family is now quantified and `[CLOSED NEGATIVE]` as the drift mechanism (λ(r) → (2/3)/r² lemma verified; parameter-free model falsified 4/11; genesis thresholds on basis-free `|J|`, so off-center irrep re-projection is not harvest loss) — the queued "~1-week Mechanism-α projection calculation" is **retired**. Drift onset matches the untuned Langevin crossover A* = 12.8 → **Mechanism γ elevated**; the decisive next step is a pre-registered thermostat-OFF amplitude re-sweep (`EXPLR_FTD0110_MECHANISM_ALPHA_LEAKAGE_CLOSED.md` §5). **2026-06-10 update (FTD-0261/0263/0265/0266/0267) — the bridge is now sharply re-posed as a GENESIS-THROTTLING problem:** the current-stack law is a broken power (knee A≈16, k_eff≈0.05, NOT ¼; FTD-0261); the ¼ has no engine realization on the current stack `[CLOSED NEGATIVE]`. Mechanism γ-as-thermal is `[CLOSED NEGATIVE]` (thermostat = pure friction, FTD-0261); β-as-envelope and β-as-dwell-time both `[CLOSED NEGATIVE]` (engine-free linear models over-predict genesis ~4–5×; FTD-0265/0266). **The genesis-vs-survival engine telemetry (FTD-0267, `ANALYSIS_GENESIS_SURVIVAL_TELEMETRY_v1.md`) settled the mechanism by direct measurement: genesis fires in a one-shot early burst (~5 events at A=10, not ~23), evaporation ≈ 0, so cluster size ≈ genesis-firing count — the onset suppression is GENESIS-STAGE nonlinear throttling (flux consumption + state↔flux coupling + Gauss redistribution + damping quench the pulse before neighbours cross K_GENESIS), not post-genesis survival.** Converges with the concurrent β v2 model (center back-reaction shifts onset up; FTD-0263 BETA_v2_CONFIRMED). **The open question is now: derive how many voxels the nonlinear injection pushes over K_GENESIS during the burst** — a tractable nonlinear-dynamics calculation, not a survival problem. (Engine golden re-baselined to `0xebaa6f314f66db3f` @ L=17 under the backend-parity refactor `c2a8f606`; FTD-0267 counters proven observation-only.)
