...
- **[OPEN] MC-T4.3 — route-invariance boundary on the α operator-assembly** (the central foundational obstruction; LEDGER FTD-0242, `docs/theory/07_assessment/audits/AUDIT_ALPHA_OPERATOR_FORCING_ROUTE_INVARIANCE.md`, 2026-06-01). A four-route adversarial workflow (J-twisted ζ-determinant; BCC body-diagonal transfer; lemniscatic-CM arithmetic; variational/valuation/Hodge) force-attempted then adversarially refuted whether the master-quadratic $(\mathrm{Tr},\det)=(16G^{*2},16G^{*3})$ operator structure is FORCED by the substrate. **0/4 forced.** The **trace** $16G^{*2}$ and a **clean odd source** $G^*=\det_\zeta(D_{3/4})/\det_\zeta(D_{1/4})$ are forward-forced [DERIVED] (so $16G^{*3}=16G^{*2}\cdot G^*$ is assemblable), but the **operator-assembly** — that one 2×2 readout carries *both* invariants — is unforced (W-CRIT-2; Tr/det are independent for a 2×2). The wall is **route-invariant** ⇒ structural to the discrete ontology, not a one-model artifact; α is therefore classified **DYNAMICAL, not structural** (cf. $g_c$; contrast $N_c=3$ which is structural). **Stays [OPEN] research, not closed-positive.** The boundary is `[STRONGLY MOTIVATED CONJECTURE no-go]`, NOT [THEOREM] — the FORCED-escape (RSI Leg 3, a single operator co-realizing both invariants) is still live. **Two surviving exits:** (1) a 6th-postulate-class input supplying the assembly W (logically independent of P1–P5); (2) the engine-native ARC-D measurement — but **ARC-D1 already returned [CLOSED NEGATIVE]** (2026-05-30, `docs/theory/04_coupling/DERIV_ALPHA_READOUT_EMPIRICAL.md`: 0 macroscopic cluster fissions across 2000 seeds; lattice topologically rigid, count of 0 is precision-independent). Numerical precision is decoupled from MC-T4.3. No spine claim moved; FTD-0013 (`x₊=1/α`) stays [STRONGLY MOTIVATED CONJECTURE]; MC-T4.3 stays [FOUNDATIONAL OBSTRUCTION].
...
- 🔶 **FTD-0110 nonlinear bridge — [OPEN] (stale-closure corrected 2026-06-09).** A 2026-05-27 entry here claimed the bridge "fully closed" via the `DERIV_FTD0110_NONLINEAR_BRIDGE.md` §6 Orbit-Equipartition + Timescale-Separation theorems; that closure was **reverted the same day** by [`AUDIT_FTD0110_2026-05-27_RESOLUTION.md`](../audits/AUDIT_FTD0110_2026-05-27_RESOLUTION.md) (four defects, incl. an arithmetic error at the load-bearing A²/4 step and a single-block analysis that predicts saturation at 27, not A²/4). Canonical position: linear k = ¼ `[DERIVED]` + Bridge-I O_h-equivariance `[DERIVED]`; **nonlinear coefficient origin `[OPEN]`/`[SMC]`**. **2026-06-09 update (FTD-0259):** Mechanism α's leakage-aggregation family is now quantified and `[CLOSED NEGATIVE]` as the drift mechanism (λ(r) → (2/3)/r² lemma verified; parameter-free model falsified 4/11; genesis thresholds on basis-free `|J|`, so off-center irrep re-projection is not harvest loss) — the queued "~1-week Mechanism-α projection calculation" is **retired**. Drift onset matches the untuned Langevin crossover A* = 12.8 → **Mechanism γ elevated**; the decisive next step is a pre-registered thermostat-OFF amplitude re-sweep (`EXPLR_FTD0110_MECHANISM_ALPHA_LEAKAGE_CLOSED.md` §5).
...
- 🔶 **[OPEN — constrained to a six-point profile, FTD-0263] Sub-knee onset mechanism + the ¼ question.** FTD-0263 (GEOM-PARTIAL) killed the sharp 27-block-boundary reading by its own pre-registered band (fine-grid elbow at **knee_N = 14.6**, outside [19, 33]) while establishing the onset is **local** (direction-invariant; L-invariant 24–48) and **smooth** (no orbit staircase). Any future mechanism proposal must mechanically satisfy: elbow at N ≈ 15; dir-invariance; L-invariance; smooth voxel-wise growth; thermostat-free survival; T-independence. β (genesis-kink on the local envelope) is qualitatively compatible with five of six and owes a quantitative elbow-location account — the natural next derivation attempt. Also open: (ii) does any protocol/config realize the linear theorem's ¼ on the current stack? (iii, deferred) K/p/τ L-scan. *Bonus constraint from FTD-0263: the bulk branch is L-invariant too (N(30) = 45.0 exactly at L = 24/32/48) — the law is intrinsic physics.*
...
**Last audit refresh:** 2026-06-10 (FTD-0263: 27-block onset hypothesis GEOM-PARTIAL — sharp reading killed by its own band (elbow N = 14.6), local+smooth onset established, six-point constraint profile recorded; bulk law L-invariant. FTD-0262 IDENT-NULL; FTD-0261 law MEASURED; FTD-0260 resolved. Remaining [OPEN]: onset mechanism vs the profile (β next) + the ¼ question).
