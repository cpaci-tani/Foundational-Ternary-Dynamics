...
**Last full audit:** 2026-04-17 (post-engine-cleanup). **Last incremental update:** 2026-07-21 — FTD-0405 returned DOUBLE-OBSTRUCTION. The current colour pair force admits a radial potential family and closes isolated two-body particle momentum, but the actual tick has nonzero work residual and the additive strong-energy zero/local stress distribution are not selected. NCEMC-2/4 remain blocked; a production architecture now requires explicit owner authorization rather than another mass campaign. FTD-0404 remains VOLUMETRIC-NEUTRAL, FTD-0403 closes `§12-cnorm`, and FTD-0402 remains historically PARTIAL. **Live repo count:** raw regen 2026-07-12 gives 907 `[OPEN]` string-markers across 263 files (archive-excluded), but the raw count is dominated by the meta/ledger layer's own rows — see the §9 caveat; the curated math queue is `SPEC_OPEN_MATH_BY_SECTOR.md` v1.1 and the curated engine/doc items are §§1–8 here. The historical "~168 real across ~64 files" figure was the 2026-04-17 hand-filtered estimate; a fresh hand-filtered pass has not been run this arc.
...
### 1.3 Dynamical SU(3) colour force — [OPEN — NCEMC ARCHITECTURE BLOCKED ON OWNER CHOICES], reclassified by FTD-0405
...
**[OPEN / ARCHITECTURE BLOCKED] Native Confinement Energy–Momentum Contract (NCEMC-1–4):** §1.2 is closed by FTD-0403, and FTD-0405 has executed the separately locked feasibility gate. A radial `H_strong` family exists only up to an additive zero; exact work fails on the current tick; full momentum/stress localization remains absent; and gravity cannot consume a uniquely fixed strong energy. Further construction requires owner-authorized architecture. This is not authorization for another mass measurement.
