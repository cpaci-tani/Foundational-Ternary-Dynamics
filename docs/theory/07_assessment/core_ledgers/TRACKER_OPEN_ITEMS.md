...
- 🔶 **[OPEN — constrained to a six-point profile, FTD-0263] Sub-knee onset mechanism + the ¼ question.**
  - ✅ **Mechanism β envelope model (2026-06-10, FTD-0264)**: tested and evaluated as **BETA-PARTIAL** (Variant A passes T1 elbow at $\text{knee}_N = 13.1$ in target range $[9.7, 21.9]$, but fails T2 shape RMS at $0.749$). The Symplectic Euler dispersion calculation error was corrected ($0.2257$ vs $0.2253$ theory). The verdict confirms that while the linear spatial envelope successfully locates the onset boundary, the staircase shape itself requires localized charge kinetics and non-linear back-reaction. See [`ANALYSIS_BETA_ENVELOPE_MODEL_v1.md`](../../03_derivations/foundational_mechanics/ANALYSIS_BETA_ENVELOPE_MODEL_v1.md).
  - 🔶 **Remaining Open Items**: (ii) does any protocol/config realize the linear theorem's ¼ on the current stack? (iii, deferred) K/p/τ L-scan. *Bonus constraint from FTD-0263: the bulk branch is L-invariant too (N(30) = 45.0 exactly at L = 24/32/48) — the law is intrinsic physics.*
