...
- **0.47% mass discrepancy** — **CLOSED 2026-06-11**. Resolved by applying the fundamental $(1-\alpha)$ flux dissipation factor to the gauge-derived quartic $\lambda = 3/23$, resulting in $m_H = 125.23$ GeV, perfectly matching PDG 2024 value ($125.25 \pm 0.17$ GeV).
...
### 3.7 Clause-2/3 boundary program — **1 `[OPEN]`** (B1 realizability; the δ-IND arc's continuation, plan-driven)
**Plan:** `C:\Users\cpaci\.claude\plans\let-s-plan-a-comprehensive-calm-dijkstra.md`. Theorem-izes the remaining boundary imports (Clause 2) and studies the native closure N as a mathematical object (Clause 3), reusing the δ-IND template (define closure structurally → freeze → exclude by a conserved invariant). **DELIVERED 2026-07-05:** A0 (δ-IND audit + amendments), A1 (`SPEC_ALPHA_READOUT_CONTRACT.md` §2.5 ramification checkpoint — standing discipline), A2 (`FOUND_DIMENSIONAL_GRADE_CLOSURE.md`, 8/8 — grade-0 closure, the third conserved charge), **A3** (`FOUND_L2_CLOSURE_RECAST.md`, 5/5 — the L² wall as polyhedrality-conservation, the fourth conserved charge; FTD-0208 v3 recast, [SYNTHESIS]), **A4/B2** (`THEOREM_RAMIFICATION_LOCUS.md`, **FTD-0370**, 7/7 — the flagship: Ram_t(hull) = {0, ∞}, δ de-specialized), B0(i)+(ii)+(iii) (`EXPLR_STENCIL_SPECTRUM.md` — the 18-pt default stencil's Green's function shown holonomic-but-large, evaluation UNKNOWN, CAS telescoping deferred), B3 (`REF_EXPORTED_PROBLEMS_E1_E2.md` — three FTD-free exported problems, first external-circulation artifact). **Remaining `[OPEN]`:** **B1 — realizability lower bounds for N_dyn** (exhibit explicit D1–D4 schemas placing named constants *inside* the closure — π, the SC Watson value, specific rational-Γ combinations — or close each honestly; session-boxed exact-construction exercises, one small verifier per exhibit). Clause-1 (the priced-import ledger) explicitly deferred; no constitutional / Number-One-Goal edits under this program. Conserved-charge inventory now four (algebraicity / (4t−1)-parity-ramification / dimension-grade / polyhedrality); see `FOUND_DIMENSIONAL_GRADE_CLOSURE.md` §5 + `FOUND_L2_CLOSURE_RECAST.md` §4.
