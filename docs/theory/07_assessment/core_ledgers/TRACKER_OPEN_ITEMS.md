...
**Last full audit:** 2026-04-17 (post-engine-cleanup). **Last incremental update:** 2026-07-21 — FTD-0404 returned VOLUMETRIC-NEUTRAL. The D=3 unit-cell measure is now explicit in integrated energy/action diagnostics without cubing local norms or changing any production value. FTD-0403 still closes Section 1.2 / `§12-cnorm`; FTD-0402 remains historically PARTIAL. NCEMC is admissible as separate `[OPEN]` work, not completed or authorized without a fresh lock. (Previous: FTD-0403 v2 TARGETED-CLOSURE; FTD-0403 v1 INVALID instrument; FTD-0401 UNMAPPED-DUAL-NORMALIZATION; FTD-0400 SPLIT-BOOKKEEPING; FTD-0398 terminal topology UNDERDETERMINED; FTD-0399 particlehood INVALID.) **Live repo count:** raw regen 2026-07-12 gives 907 `[OPEN]` string-markers across 263 files (archive-excluded), but the raw count is dominated by the meta/ledger layer's own rows — see the §9 caveat; the curated math queue is `SPEC_OPEN_MATH_BY_SECTOR.md` v1.1 and the curated engine/doc items are §§1–8 here. The historical "~168 real across ~64 files" figure was the 2026-04-17 hand-filtered estimate; a fresh hand-filtered pass has not been run this arc.
...
### 1.3 Dynamical SU(3) colour force — [OPEN — NCEMC ADMISSIBLE, NOT STARTED], reclassified by FTD-0403
...
**[OPEN / ADMISSIBLE] Native Confinement Energy–Momentum Contract (NCEMC-1–4):** §1.2 is closed by FTD-0403, so a separately pre-registered arc may now define one backend-common `H_strong` generating the active force; exact work/energy accounting; complete total momentum; and field-only weak gravity using the same energy–momentum object with the independent `M_GRAVITATIONAL·|s|` source disabled. NCEMC has not started, and this is not authorization for another mass measurement.
