...
- 🔴 **[OPEN — NEW 2026-06-10] ic1 regression bisect (FTD-0260).** The pre-registered thermostat-OFF discriminator's validation gate found that **the engine's ic1 steady-state phenomenology changed between 2026-04-28 (`87158aef`) and 2026-06-09 (`4fa056c2`) under byte-identical config and unchanged constants**: N(A=10) went ~25 → ~4 voxels (time-stable, not transient). The FTD-0110 empirical k(A) table (and the SM-particle cluster-mass matches built on it) is therefore **provenance-pinned to the April engine**. Needed: bisect (~8 build-probe points: build + `campaign_thermostat_off_sweep --A=10 --seeds=1`, seconds per probe), then decide regression-to-fix vs intended-change-requiring-formal-rebaseline. The Mechanism-γ discriminator v2 is **blocked on this**. See [`ANALYSIS_THERMOSTAT_OFF_SWEEP_v1_INVALID.md`](../../03_derivations/foundational_mechanics/ANALYSIS_THERMOSTAT_OFF_SWEEP_v1_INVALID.md).

**Last audit refresh:** 2026-06-10 (FTD-0260 INVALID run diagnosed → ic1 regression bisect opened; prior: stale FTD-0110 closure corrected; Mechanism α closed negative — FTD-0259).
