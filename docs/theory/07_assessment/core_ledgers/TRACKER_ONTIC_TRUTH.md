...
| **OT-1.7** | `G* via det_ζ quarter-conjugacy bridge` **[CONDITIONAL DOMAIN]**: $\det_\zeta D_{3/4}/\det_\zeta D_{1/4}=G^*$ for the registered dimensionless, multiplicity-one, first-order spectra $D_a=\{n+a\}_{n\ge0}$ on the selected quarter-twisted circle. `J^2=-I` fixes the eigenphases `±i`; it does not itself fix the circle, twist, chiral half-line, origin, scale, order, or multiplicity. **Arithmetic content:** after those choices, $4D_{1/4}$ and $4D_{3/4}$ are the two non-trivial residue classes mod 4; restricted to primes these are the split and inert classes of $\mathbb Z[i]$. | Lerch's formula gives $\det_\zeta D_a=\sqrt{2\pi}/\Gamma(a)$. FTD-0839 exact controls: origin shift gives $G^*/3$; `r` copies give $(G^*)^r$; squared operator gives $(G^*)^2$; common scale gives $c^{-1/2}G^*$; orientation-blind full-line Laplacian gives ratio `1`. | FTD-0141 identity retained as a conditional theorem; scope corrected by FTD-0839 (`24/24`). `DERIV_GSTAR_QUARTER_CONJUGACY.md`; `THEOREM_I_GAMMA_QUARTIC_SQUARE_SPLIT_v1.md`. The physical spectral realization and native gearbox remain open. |
...
## TIER 3 — Numerical facts, exhaustive over stated domains (2)
...
**Honest caveat (OT-1.9)**: the arithmetic `|μ_K| = |disc(K)|` uniqueness at d=−4 stays [THEOREM] (T1, above; 2026-06-24 spine audit); but the *physics match privilege* of d=−4 (that the master quadratic's larger root matches 1/α there specifically) is only a [NUMERICAL FACT] — criterion-dependent, and flips under a rational-multiplier criterion ((d=−3, q=3) lands at +0.9077 ppm, tighter than canonical).
...
This is the load-bearing identification between FTD's algebraic structure and the inverse fine-structure constant. It is **honestly tagged as a conjecture**. No derivation chain from FTD axioms exists.

| ID | Conjecture | Empirical match | Structural evidence |
|---|---|---|---|
| **OT-5.1** | `x_+ = 1/α` | 1.26 ppm to CODATA 2022 (`α^{-1} = 137.035999177(21)`) | `x_+ = 1/α` has no numerical-uniqueness support. Remaining support is structural only: OT-1.5 (Z[i] structure unifying CM Aut count and tower level k=4) and OT-1.9's unit-group arithmetic `|μ_K| = |disc(K)|`. Separately (FTD-0792) the engine never ran on this root. **2026-06-01:** the route-invariant MC-T4.3 boundary (FTD-0242) classifies α as **DYNAMICAL, not structural** — 0/4 FTD-native routes force the `(Tr,Det)=(16G*²,16G*³)` operator assembly; the trace and a clean odd source `G*` are forward-forced but the assembly is not (W-CRIT-2). **No tier change** — remains T5 [STRONGLY MOTIVATED CONJECTURE]; the boundary *sharpens*, does not move, this entry. **2026-06-24:** FTD-0314 (carrier-narrowing theorem) extends the no-go to all native finite symmetries — the surd `√(G*(4G*−1))` is transcendental over ℚ, so no native carrier realizes it; FTD-0315 adopts the external binding law as **FC-W** (the constitution's FC-4), under which `x₊=1/α` is `[CONDITIONAL THEOREM given W]`, still `[SMC]` unconditionally (no tier change) |

**Pointer note (2026-08-13)** — nearest sibling gap, not an entry here: the clock hypothesis (CLK-1, an [AXIOM]-class dynamics posit in `SPEC_UNIFIED_AXIOM_REGISTER.md`, not raw math, hence tracked in LEDGER.md rather than this tracker) is, like OT-5.1's α wall, a "the substrate does not natively supply this" result — closed negative for substrate-derivability per FTD-0208 v3 (`AUDIT_CLOCK_HYPOTHESIS_v3_CLOSED_NEGATIVE.md`). The FTD-0899–FTD-0999 native-time-carrier / clock-carrier / temporal-interior programme (2026-08-11/12; independently adversarially audited 2026-08-13) attacked CLK-1 directly across that stretch and did not close it: every candidate still fails carrier constraint C11 by self-admission, the named next campaign (native C3 unit-strut tensegrity) was quietly set aside rather than run, and the programme's exact result (FTD-0999) is a purely scalar clock-growth conservation law with `G*` used throughout only as an exogenous cadence constant, never re-derived or fit. CLK-1 remains exactly where it was — `[AXIOM]` / `[CLOSED NEGATIVE per v3]` for substrate-derivability; no tier or tag in this tracker moves. **Addendum (2026-08-13, FTD-1000):** CLK-1 is now folded into FC-2's metric-half declaration (`SPEC_FTD_FRAMEWORK_V1.md` §5.2) as its per-worldline proper-time clause, disambiguated there from the global substrate tick (P2, unconditional, never in question) and from this same programme's separate native-carrier question — a re-homing of an already-`[AXIOM]` claim, not a promotion. Full account: LEDGER.md (search FTD-0899 through FTD-0999, and FTD-1000) and `SPEC_DOCTRINE_LEDGER.md` §17 (the FTD-1000 fold clause; §2.1 covers the native-time-carrier programme itself but not the fold).
...
2. **OT-2.3 (Q(G*) π-free) and OT-2.2 (transcendence)** depend on Chudnovsky 1976. This is a published, foundational result, but a reviewer could note that algebraic-independence proofs in this domain are not always self-evident. Cite Waldschmidt explicitly.

3. **OT-5.1 (the central conjecture)**: a reviewer who insists "you must derive α from axioms or the framework is empty" cannot be answered with current methods. The current evidence is the spine and Z[i] structural unification; the empirical match remains conjecture-grade. Three channel-limited engine diagnostics remain valid (FTD-0004 Phase G, FTD-0005 Phase J, FTD-0125 Phase I). FTD-0412 retracts FTD-0126's matter-sector result because it evolved spatial `D_W` as the real-time Hamiltonian and initialized a Dirac-basis spinor while declaring the chiral basis. Those three negatives and the FTD-0244 operator-calculus theorem exclude their stated classes; they do not establish universal structural blockage. MC-T4.3 is an unfinished search outside those classes.

4. **Pre-registration discipline**: every numerical scan in T3 is git-tagged BEFORE measurement. This is methodological hygiene the framework practices. A reviewer will check tag dates against measurement dates; they should match.
...
- **OT-1.7** G* via det_ζ quarter-conjugacy bridge (conditional-domain identity; FTD-0141, scope corrected by FTD-0839)
- **OT-1.8** G* as finite-N attractor / discharges reframe ε-L obligation (★★★★★, FTD-0142)

Plus 1 numerical fact (OT-3.1).
...
- [Paper A](../../../../dissemination/papers/PAPER_A_PI_FREE_GENERATOR.tex) — π-free generator (T1.1, 1.2, 1.3, 2.2, 2.3)
