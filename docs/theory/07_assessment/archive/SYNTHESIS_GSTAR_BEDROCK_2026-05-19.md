...
If this works, you have converted the FTD bridge from a two-arrow conjecture into a one-arrow theorem: $\chi_{-4} \to (16, G^*) \to P_{G^*}$ with all three arrows derived, only the *physical* identification of the larger root with $\alpha^{-1}$ left as conjecture. That is a dramatically stronger position. It would also dissolve the §17.6 "one bit of input" framing: there is no bit — the bit is forced by the motivic-weight constraint.
