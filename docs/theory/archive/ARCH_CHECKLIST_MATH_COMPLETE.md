# [ARCHIVED] CHECKLIST_MATH_COMPLETE — Bridge-Complete Math Roadmap

> **🗄 ARCHIVED 2026-05-08.** This document used a tier-aligned (I–IV) organisation. It has been superseded by the sector-aligned [`SPEC_OPEN_MATH_BY_SECTOR.md`](../01_reference/SPEC_OPEN_MATH_BY_SECTOR.md) (FTD-0146 [SYNTHESIS], 10 SM-aligned physics sectors). The tier-aligned content here is preserved for provenance — every MC-T*.* item maps into the sector tracker with the same effort code and dependency notation. **Do not edit this file.** New gap status updates go in `SPEC_OPEN_MATH_BY_SECTOR.md`. If a tier-aligned reading is genuinely needed, the dependency graph in §12 of the successor preserves the critical-path information.
