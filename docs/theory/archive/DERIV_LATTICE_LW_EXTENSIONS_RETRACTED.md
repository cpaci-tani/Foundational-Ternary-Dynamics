# ARCHIVED/RETRACTED — Lattice LW Extensions: Q5–Q8

> **Retraction (FTD-0558, 2026-07-26):** Q5 and Q6 inherit the wrong
> continuous-time seven-point pole and do not verify production radiation
> power.  The Q6 script used a near-pole `1/|chi|` proxy rather than the stated
> surface Jacobian, so its onset and divergence claims were not derived.  Q8's
> unrestricted continuity claim conflicts with the later exact native charge
> audit: it holds for movement histories, not genesis and weak/transmutation
> events.  Q7's Fourier form-factor algebra survives only conditionally on a
> specified source representation and corrected propagator.  This file is
> provenance; see the corrected active document at its former path.
...
convention). This is a closure of follow-up
sub-questions.
...
- **NOT a new spine theorem.** The spine count is unchanged — nine
  numbered results, seven theorem-grade + two honestly-tiered (Theorem 3
  at its arithmetic core only; see `SPEC_ALGEBRAIC_SPINE.md` §0 count
  convention); FTD-0120 is a closure of follow-up
