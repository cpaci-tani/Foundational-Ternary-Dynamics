...
**LEDGER:** maintenance-log line; content rides on FTD-0002 (Watson/G* identity), the CM-curve table (`DERIV_WATSON_GSTAR_IDENTITY.md` Part VII, `[THEOREM]`), and FTD-0013 (`x₊=1/α`, `[SMC]`). **M2's two-loop lemniscatic-retention finding is registered as FTD-0374** ([NUMERICAL FACT] exponents + [SMC] CM-type; M2 itself stays [OPEN]).
...
- **M2 — the two-loop BCC period. `[OPEN]` — attempted; precision-limited.** Compute the two-loop sunrise/sunset on the BCC propagator and identify its period. **Method:** coordinate-space form `Σ_x G_BCC(x)³` (sunset at external p=0), with `G_BCC(x)` the FFT of `1/(1−σ_BCC(k)+μ²)` on an `L³` BCC lattice; IR-regulate with a small mass `μ²`; take `μ²→0` and `L→∞`; PSLQ the extrapolated value against the basis `{Γ(1/4)-powers/π^a, Γ(1/3)-powers/π^b, weight-3 modular L-values, ζ(3), π^n}`. **Falsifier:** if the period lands on the ℤ[i]/Γ(1/4) family → the BCC regulator is *lemniscatic at two loops* (a strong new `[THEOREM]`-candidate); if it lands on Γ(1/3)/modular → even BCC climbs, and G\*'s lemniscatic character is a one-loop-only fact.
  ...
  **Holonomic route executed (2026-07-08).** The reformulation was **validated** against an independent FFT — `I_series(μ²) = Σ_N c_N/(1+μ²)^{N+3}` matches `Σ_x G_BCC(x)³` to `6×10⁻¹³` (μ²=0.3), a rigorous identity check (`lattice_two_loop_bcc_series.py`; `c_N=d_N/8^N`, `d_0..d_8 = 1,0,24,8,840,648,35368,41496,1651272`). Exact `c_N` computed to N=1200. Results:
  ...
  - **The operator is CERTIFIED (mod-p holdout).** A fast mod-p build (`lattice_two_loop_bcc_modp.py`: float64-BLAS matmul with p<2²⁰ so all intermediates are exact <2⁵³; 2m15s for N=1200 vs 6m41s exact, validated against exact `d_N`) enables large-N reconstruction over GF(p). `F(y)` satisfies a **certified order-18, degree-108 recurrence** — guessed from 1051 mod-p terms, it reproduces **150/150 held-out terms** (`bcc_sunset_guess_modp_sage.py`; a chance pass is ~p⁻¹⁵⁰), and its **minimal ODE is order 11, degree 123** (`bcc_sunset_exponents_modp_sage.py`) — both *identical at two independent primes* (1048573, 999983).
  - **Genus signature — the FTD-0372/0373 local-exponent discriminant, 2-prime-verified `[NUMERICAL FACT]`:** the order-11 ODE's local exponents at the physical point **y=1 are `{−1/4, 0, 0, 1/4, 1/2, 1, 1, 3/2, 2, 3, 4}`** — carrying **quarter-integer exponents `±1/4`**, the ℤ/4 / **j=1728 / Γ(1/4) lemniscatic (ℤ[i])** signature, with **no thirds/sixths** (the ℤ/6 / j=0 / Γ(1/3) equianharmonic signature is ABSENT). y=−1 (parity) carries only half-integers `{0,½,1,3/2,2,…,7}`; y=0 carries `{0×5,1×4,2×2}`.
  - **M2 falsifier — strongly-motivated answer: the BCC regulator appears to STAY lemniscatic (ℤ[i]/Γ(1/4)) at two loops** — the one-loop BCC period is lemniscatic (Γ(1/4), j=1728), and the two-loop period's y=1 branch structure carries the same ℤ/4/j=1728 signature (`±1/4`) with no equianharmonic signature. But it is a genuinely *higher* (order-11) period, **not** a simple Γ(1/4)-quotient (the finite part `B=0.96406222` is excluded as a low-height Γ(1/4) monomial). Tags: the exponents are `[NUMERICAL FACT, 2 primes]`; the lemniscatic CM-type *reading* is `[STRONGLY MOTIVATED CONJECTURE]` (quarter-integer exponents ⇒ j=1728 is a strong heuristic via the ℤ/4 automorphism, not a proof of the period's CM type). Like W₁₈, the y=1 exponent multiset is not symmetric about a single center ⇒ not self-dual (a diagonal, higher mixed period). **M2 stays `[OPEN]`** (no closed form, no proof); promotes no tag. The lemniscatic-retention finding is **registered as FTD-0374** (LEDGER): exponents `[NUMERICAL FACT, 2 primes]`, CM-type reading `[STRONGLY MOTIVATED CONJECTURE]`.
