...
| **L²-not-L¹ budget** | the transverse phase / orthogonal pairing of the Pythagorean norm (a continuous SO(3) choice the discrete O_h cannot make) | FTD-0208 `[stands]` — as of 2026-08-13 folded into FC-2's metric-half declaration (FTD-1000) as its per-worldline clause, rather than tracked as a separate parallel row; the modulus/argument classification below is unaffected | **PROVEN** (v3, structural incompatibility with Scale-0 primitives) | A native Scale-0 construction carrying an L²-norm/Pythagorean structure without importing it |
...
- **The four-walls-are-one forcing theorem** (T2): are FC-1, FC-2, FC-W, and the L²-not-L¹ budget the *same* import — does adopting any one force the others? A win collapses 3+ axioms to one structural deficiency and firms the frontier for FTD. *(Update 2026-07-25, FTD-0508: a proof skeleton now exists — [`DERIV_FOUR_WALLS_SECTION_SCHEMA.md`](DERIV_FOUR_WALLS_SECTION_SCHEMA.md) verifies all four walls plus the FTD-0494/0499/0502 engine obstructions instantiate one section-demand schema, proves the first directed edge (FC-2 section ⇒ FC-1 algebraic signature, via exact record-monoid non-commutativity `(m−1)(b1−b2)`), and derives that any transfer to the δ fiber must be event-data-carried. The forcing theorem itself remains `[OPEN]`.)*
