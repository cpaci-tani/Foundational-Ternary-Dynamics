...
| **direction** (which way is forward) | the update-map direction | **forced-given-FC-2** (FTD-0324): the manifestation map is many-to-one → no inverse → "backward" is not a defined dynamics, so there is no `ℤ/2` to choose. The free `±ω` orientation belongs to the *reversible wave* sector and carries **no** arrow. |
