...
| R3 | W₁₈ ≈ 1.2679 | σ₁₈: 1 − (1/6)Σcos − (1/6)Σcos·cos (**the engine default**) | all-L, unit source | 1/L-extrapolant 1.267936 (err 4×10⁻⁵ vs AUDIT_LINK8) | **UNKNOWN** (holonomic-but-large, B0) | [DERIVED — schema] |
...
**Why R3 matters.** The σ₁₈ symbol is the spec's **own default** linear operator (`AUDIT_LINK8_CLOSURE.md` §2). Its Green's constant W₁₈ is native by construction — a member of N_dyn — even though B0 showed its closed form is unknown and its annihilating operator large. So N contains a constant whose *arithmetic nature the corpus cannot yet classify*: membership is settled, evaluation is open. That is the honest shape of the boundary from the inside.
