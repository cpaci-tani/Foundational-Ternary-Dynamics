...
| R3 | W₁₈ ≈ 1.2679 | σ₁₈: 1 − (1/6)Σcos − (1/6)Σcos·cos (**the engine default**) | all-L, unit source | 1/L-extrapolant 1.267936 (err 4×10⁻⁵ vs AUDIT_LINK8) | **period of an order-4, degree-12 ODE** (FTD-0372; ½ℤ-exponents, symmetric-power/√-twist hypothesis; Γ-quotient closed form pending the P3(b) factorization) | [DERIVED — schema] |
...
**Why R3 matters.** The σ₁₈ symbol is the spec's **own default** linear operator (`AUDIT_LINK8_CLOSURE.md` §2). Its Green's constant W₁₈ is native by construction — a member of N_dyn — and its arithmetic is now **charted**: W₁₈ is the period of an explicit **order-4, degree-12** ODE (FTD-0372, `EXPLR_STENCIL_SPECTRUM.md` §2), sitting between the SC (order 3) and FCC (order 6) lattices, with every local exponent in ½ℤ and no MUM point — the signature of a symmetric-power/√-twist of a second-order elliptic operator. So N contains this constant with membership settled *and* its operator known; whether that gives it a Γ-quotient (modular/CM) value or a genuine Calabi–Yau-class value is the one remaining question (P3(b), the factorization). That is the honest shape of the boundary from the inside — no longer "evaluation open, nature unknown" but "operator in hand, factorization pending".
