...
- **S3 — the theorem.** Re-run the FTD-0353 valuation argument *relative to the defined N* (not the documented list): δ ∉ N, [THEOREM — conditional on Chudnovsky 1976]. Deliverable: theorem doc + verifier; the valuation theorem's inventory-[SELECTION] flag retires. LEDGER row minted at verdict time.
