...
δ is the right formal object: the master quadratic's discriminant is 64G\*³(4G\*−1), so x₊ = 8G\*² + 4G\*δ — reaching the α-candidate root *is* reaching δ over ℚ(G\*). A theorem δ ∉ N converts FC-W's status from *adopted convenience* to **proven necessity relative to N**: the strongest honest form of the *mark* half of the Number-One Goal's boundary clause ("rigorously mark and price which types the ontology must import", the 2026-07-05 form; amended 2026-07-12 to add the *drive* face, FTD-0383 — δ-IND delivers the marking; the priced-import ledger, FTD-0371, delivers the pricing; the drive face makes this program's v2 nonlinear rung a scheduled obligation rather than an option).
...
MC-T4.3 (`SPEC_OPEN_MATH_BY_SECTOR.md`; contract in `SPEC_ALPHA_READOUT_CONTRACT.md`) is `[OPEN — SCOPED NO-GO PACKAGES]` after the FTD-0412 scope correction. FTD-0244 is theorem-negative for its explicitly axiomatized operator calculus, while other named routes close only their declared classes. This program attempts a stronger theorem about one defended definition of the native closure `N`; success would still be relative to that definition, not an exhaustion theorem over every conceivable mechanism. The Gödel/Hölder resonance that motivated this charter is motivation only, tagged [coherent-interpretation]. **Equivocation guard (binding):** independence over a field/ring/closure is *not* logical independence; no result below Stage S4 may be announced in proof-theoretic language.
...
5. **No mid-program promotions:** MC-T4.3 stays `[OPEN — SCOPED NO-GO PACKAGES]`, x₊ = 1/α stays [SMC] (FTD-0013), and FC-W stays [AXIOM] through every stage — S3 may move only the valuation theorem's conditionality, not the physics identification or the quantified scope of MC-T4.3.
