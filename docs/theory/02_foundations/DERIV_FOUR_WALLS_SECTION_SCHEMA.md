...
| I5 | Linear quotient `V → V/W` with no canonical complement | the orthogonal complement / transverse phase | an inner product (the Pythagorean pairing); a continuous SO(3) datum the discrete `O_h` cannot supply | FTD-0208 `[stands]` (structural incompatibility, v3); as of 2026-08-13 the coordinate-level clock surrogate is co-declared inside FC-2 (FTD-1000) — one act, two data; I5's structural independence from I-instances of W2 is unchanged per `THEOREM_FOUR_WALLS_v1.md` §5.4 (FTD-1002) |
...
- *(Update 2026-07-25, FTD-0515: the FC-2 ⇒ FC-1 edge is now proven at state level as well — on the record monoid, traciality is exactly cyclic-class constancy, the quotient state is tracial, and any history-separating state is forced non-tracial; a strict tracial intermediate shelf (necklace states) exists. See [`DERIV_RECORD_STATE_DICHOTOMY.md`](DERIV_RECORD_STATE_DICHOTOMY.md), which also records the symmetric-fiber sharpening: owned = symmetric functions of each wall's fiber, imported = the ordering datum — exact for I6: `x₊+x₋`, `x₊x₋ ∈ ℚ(G*)` while `x₊−x₋ = 8G*δ`.)*
...
This corollary is the document's second substantive output: **the four-walls-are-one theorem, if true, will be proven in the event-data category or not at all.** It sharpens the search space for Front C of the Consumption Program. It does not imply a temporal phase or any other particular event variable. The former P6C-T proposal (FTD-0511) was withdrawn after FTD-0507 retracted its motivating collision-storage trilemma; its historical dossier is retained at [`SCOPE_P6C_CANDIDATE_TEMPORAL_PHASE_RETRACTED.md`](../archive/SCOPE_P6C_CANDIDATE_TEMPORAL_PHASE_RETRACTED.md).
