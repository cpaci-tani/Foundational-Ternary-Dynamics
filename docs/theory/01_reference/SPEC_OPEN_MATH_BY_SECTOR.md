...
| W-CRIT-2 | **Master quadratic operator assembly imposed not derived.** Closed theorem-negative for current substrate-native operator forcing by FTD-0244; the accepted boundary is that α remains dynamical, not structural, unless a new W-like commitment or ARC-D path is added. **Strengthened 2026-06-25 (FTD-0318):** no FTD-native ℤ/2 symmetry can supply the `δ`-selection (all are `ℚ`-entry, Galois-blind to `δ`); the W-like commitment can only be a *declaration* (proposed FC-4, drafted un-minted), not a native derivation. | [CLOSED THEOREM-NEGATIVE / ACCEPTED BOUNDARY] | closed | MC-T4.3 |
