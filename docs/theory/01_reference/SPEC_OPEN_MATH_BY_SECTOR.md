...
**Version:** v1.2 (2026-08-04 canonical reconciliation — FTD-0791 and the corrected FTD-0800/0801 C3 boundary)
**Companion docs:**
...
**Status snapshot:** Spine: nine numbered results — six theorem-grade + three honestly-tiered after the FTD-0785 audit (see `SPEC_ALGEBRAIC_SPINE.md` §0). Tier I 5/5 closed. Tier II 4/4 closed; 0 structural theorems [OPEN]. Pre-registered scan queue empty (FTD-0143 executed 2026-07-12 — uniqueness rejected). δ_c closed-form gap CLOSED scan-negative (FTD-0224); MC-T1.1-ext CLOSED (FTD-0350, [THEOREM at all L ≥ 2]).
...
**Closed (sector context):** Theorems 1–9 (FTD-0001, OT-1.1–1.8, 2.1–2.3, 4.1, 3.4 partial), MC-T1.1 (route b), **MC-T1.1-ext (CLOSED per FTD-0350 — Theorem 7 [THEOREM at all L ≥ 2], matched-stencil/Gauss-realizable scope, conditional; theorem-bucket move pending owner decision)**, MC-T1.2 (Theorem 3 retagged `[NUMERICAL FACT, h=1 only]`), MC-T1.3 (Q(G*) verification), MC-T1.4 (per-voxel mass gap), MC-T1.5 (BCC complex-structure Roles 1+3 [DERIVED]; Roles 2+4 NO-GO), MC-T2.1 + MC-T2.2 (**FTD-0319 audited by FTD-0791:** at the registered alpha gate the null expects 1.42–1.67 matches and the scan found 1; retagged `[MEASURED]` → `[SELECTION]`; scan-rigid count zero; the historical "~4×10⁵:1 Bayes" figure is retracted), MC-T2.3 items 1–3 (63-disc Γ-product null at h ≥ 2), MC-T2.3-4 (CLOSED, unit-group uniqueness proof integrated), δ_c (CLOSED scan-negative, FTD-0224).
...
| W-CRIT-1 | **Circularity in framework integer identification.** Integers `{N_c=3, N_base=4, b_3=7, N_eff=13}` selected knowing target physics values. Constraint 11 of gtca: LEDGER tagging is not resolution. After FTD-0791 there is no surviving structural-uniqueness/statistical-specialness scan to answer this objection; closure still depends on MC-T4.3 rather than match-counting | [OPEN methodological] | (closes if MC-T4.3 closes) | MC-T4.3 |
...
- **Spine-complete:** no small pure-math queue item remains here; the historical `δ_c` closed-form gap is CLOSED scan-negative (FTD-0224), not remaining work.
...
**Publication-readiness note:** this tracker does not adjudicate the owner gate. FTD-0791 withdrew the structural-uniqueness/statistical-specialness premise previously cited here for `x_+ = 1/α`; no publication-readiness argument may rely on that scan as positive evidence.
