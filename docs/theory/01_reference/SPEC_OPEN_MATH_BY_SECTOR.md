...
| NCEMC | Native Confinement Energy–Momentum Contract (FTD-0400): after FTD-0401's causal-normalization/mass-role gate, define one backend-common `H_strong` generating the force, exact work accounting, complete momentum, and field-only gravity sourced by the same object. No mass measurement before both contracts close | [OPEN — prerequisite contract] | M–RP | §12-cnorm, eng-SU3 |
...
**Status snapshot:** **Partial closure with a current-engine normalization obstruction** (FTD-0131/0401): `α_G(e,e) = (m_e/m_P)² ≈ 1.745 × 10⁻⁴⁵` remains **[STRONGLY MOTIVATED CONJECTURE]** at the FTD-0015 floor. The leading-order Schwarzschild-form chain remains conditional on the clock-hypothesis **[AXIOM]**, but FTD-0401 now adds a separate implementation gap: raw `Voxel::velocity` is not mapped to the `c=1` normalized variable assumed by the proper-time algebra, and rest-energy/inertial/gravitational `M_REST` roles are unconverted at `c_lat²=1/3`. Static `v=0` latency algebra is unaffected; moving-clock covariance and the joint mass source are not established. Arc B P2 remains **[CLOSED NEGATIVE, AXIOM-LEVEL]**. **Arc C2 spin-2 boundary theorem free-theory derivation** (`../10_eft_program/derivations/DERIV_SPIN2_BOUNDARY_THEOREM_FREE_THEORY.md` + `../10_eft_program/derivations/DERIV_J_BILINEAR_NO_SPIN2_POLE.md`); Arc C2 P3 pre-reg (`preregister-spin2-boundary-theorem-v1`, FTD-0209) hash-locked. Framework-integer `G_N = 1/(b_3 + N_c)² = 1/100` reading remains [CLOSED NEGATIVE per FTD-0131].
...
| §12-cnorm | Reconcile raw nodes/tick velocity with `c_lat=1/√3` transport and the legacy `c=1` matter-clock/Born–Infeld formulas; map rest energy, inertial mass, gravitational source, and kinetic totals; remove GPU double-`tau`/cap divergence (FTD-0401) | [OPEN — prerequisite normalization contract] | M | — |
...
| §12-mgcurv | Mass-gap to curvature source. FTD-0400 proves the current implementation is split; FTD-0401 proves its mass/rest-energy and velocity normalizations are also unmapped. Closure requires §12-cnorm, then NCEMC-1–4, before any curvature measurement | [OPEN — sharpened by scoped no-go] | RP | §12-cnorm, NCEMC |
