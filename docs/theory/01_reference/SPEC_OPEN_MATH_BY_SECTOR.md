...
**Version:** v1.2 (2026-08-04 canonical reconciliation — the corrected FTD-0800/0801 C3 boundary)
**Companion docs:**
- [`SPEC_DOCTRINE_LEDGER.md`](SPEC_DOCTRINE_LEDGER.md) — single-page status map (closed + open)
- [`SPEC_ALGEBRAIC_SPINE.md`](SPEC_ALGEBRAIC_SPINE.md) — canonical algebraic-spine reference (nine numbered results: six theorem-grade + three honestly-tiered after FTD-0785 — Theorem 3 at its arithmetic core only; see §0 count convention)
...
**Scope:** Number-theoretic and algebraic content of FTD's algebraic spine (nine numbered results: six theorem-grade + three honestly-tiered after the FTD-0785 audit, see `SPEC_ALGEBRAIC_SPINE.md` §0); independent of physics interpretation.

**Status snapshot:** Spine: nine numbered results — six theorem-grade + three honestly-tiered after the FTD-0785 audit (see `SPEC_ALGEBRAIC_SPINE.md` §0). Tier I 5/5 closed. Tier II 4/4 closed; 0 structural theorems [OPEN]. Pre-registered scan queue empty (FTD-0143 executed 2026-07-12 — uniqueness rejected). MC-T1.1-ext CLOSED (FTD-0350, [THEOREM at all L ≥ 2]).
...
**Status snapshot:** `x_+ = 137.0362` matches `1/α` to 1.26 ppm as **[STRONGLY MOTIVATED CONJECTURE]** (OT-5.1, FTD-0013); that tag is unchanged. `x_+ = 1/α` has no numerical-uniqueness support. Coefficient 16 = `\|Aut(E)\|²` is structural identification at OT-4.1 [T4]. FTD-0244 closes K-BIND theorem-negative for the current substrate-native operator calculus. FTD-0412 corrects the scope: the physical α readout remains MC-T4.3 `[OPEN — SCOPED NO-GO PACKAGES]`; the operator-calculus theorem and route closures do not exhaust every non-action observable, boundary rule, enlarged local algebra, or dynamical mechanism. Doctrine §7 bivector/Dirac/QED bridge sector remains open per FTD-0073 mode-erasure closure.

| ID | Gap | Tag | Effort | Deps |
|---|---|---|---|---|
| **MC-T4.3** | **Operational alpha-readout mechanism (CENTRAL).** A closure proof would have to derive the physical identification `x_+ = 1/alpha` from FTD structure rather than insert it. ARC-A/B1 are closed negative, ARC-B2/C1 are underdetermined or route-limited, and K-BIND is theorem-negative for the axiomatized operator calculus. These are genuine exclusions within stated classes, not an exhaustion theorem over all mechanisms. | **[OPEN — SCOPED NO-GO PACKAGES]** | **RP** | MC-T4.1, MC-T4.2 |
| **TEST4-GEN** | **Alpha arithmetic generativity test.** Pre-registers the Balmer-to-Bohr gate: the same lemniscatic CM/FQCR rigidity that produces the alpha candidate must generate one additional independent physical dimensionless observable or relation with no new tuned freedom. Target declaration must precede numerical comparison. | [PRE-REGISTRATION] | D-W | MC-T4.3; `PREREG_ALPHA_ARITHMETIC_GENERATIVITY_v1.md` |
...
| NCEMC | Native Confinement Energy–Momentum Contract (FTD-0400/0405/0406): FTD-0405 remains the double-obstruction result for the unmodified direct force. Explicit owner choices in FTD-0406 install a default-off collision-free CPU `U(1)=0` Hamiltonian, energy/momentum projection, local string T00/stress and `T00/C_SPEED²` latency source. Extend to GPU, topology changes, mixed forces and moving latency before NCEMC-5 | [OPEN — CPU v1 SELECTED; DOMAIN EXTENSION] | M–RP | eng-SU3 |
...
**Sources:** SPEC_DOCTRINE_LEDGER.md §11; `DERIV_NC_FROM_TOPOLOGY.md` (independent `N_c = 3` routes); LEDGER FTD-0020, FTD-0025, FTD-0029, FTD-0031, FTD-0093; TRACKER_OPEN_ITEMS.md §§1.3, 2.4, 2.8.
...
**Status snapshot:** **Targeted implementation reconciliation closed** (FTD-0131/0401–0403): `α_G(e,e) = (m_e/m_P)² ≈ 1.745 × 10⁻⁴⁵` remains **[STRONGLY MOTIVATED CONJECTURE]** at the FTD-0015 floor. FTD-0402 maps raw `Voxel::velocity` through `β²=|u|²/C_SPEED²`, separates `M_INERTIAL`, `E_REST`, and `M_GRAVITATIONAL`, and passes exact targeted CPU/GPU gates. FTD-0403 closes the frozen changed-surface dependency contract. The clock budget remains an **[AXIOM]**, moving-Schwarzschild exactness is retired as a current-engine claim, and the common mass/gravity source remains unestablished. Arc B P2 remains **[CLOSED NEGATIVE, AXIOM-LEVEL]**. **Arc C2 spin-2 boundary theorem free-theory derivation** (`../10_eft_program/derivations/DERIV_SPIN2_BOUNDARY_THEOREM_FREE_THEORY.md` + `../10_eft_program/derivations/DERIV_J_BILINEAR_NO_SPIN2_POLE.md`); Arc C2 P3 pre-reg (`preregister-spin2-boundary-theorem-v1`, FTD-0209) hash-locked. Framework-integer `G_N = 1/(b_3 + N_c)² = 1/100` reading remains [CLOSED NEGATIVE per FTD-0131].
...
| §12-beyond-N | Beyond-leading-order GR: Mercury perihelion, light bending, gravitational waves (full nonlinear Einstein equations beyond Deser bootstrap [SELECTION, FTD-0026]). Native radiative-holonomy Q0 **CLOSED-NEGATIVE** (FTD-1015): spatial \(\Omega\) kinematics do not isolate two TT shears. | [OPEN — GR import / Deser; native GW holonomy mapped-boundary] | RP | clock-hypothesis AXIOM |
| §12-EP | Equivalence-principle analogue from substrate. Desk Q0 **FOUND** (FTD-1013). Default engine alignment **CLOSED-NEGATIVE** (FTD-1014). Selected default-off integrator **FOUND** (FTD-1016). Sourced-then-frozen well **FOUND** (FTD-1017). CUDA port **FOUND** (FTD-1018, \(\Delta F=0\)). One-well clocks+falling **FOUND** (FTD-1019, \(\rho_\tau=1\)). Frozen-well characteristics **FOUND class 0** (FTD-1020, \(\theta_{\rm diff}=0\)). Live sourced Newton **FOUND wiring** (FTD-1021, \(r_L=0.9857\); freeze not test-body at \(1/125\)). Slow-envelope 3³ **FOUND wiring** (FTD-1022, \(r_L=0.9546\); freeze not test-body at \(27/125\)). Engine \(m_i=m_g\) remains [IMPOSED]. Class C is [SELECTION]. Lensing/GWs remain OUT. | [PARTIAL — Q0 FOUND given FC-2; default \(\nabla|J|\) CLOSED-NEGATIVE; selected geometric integrator FOUND; sourced wiring FOUND; GPU parity FOUND; one-well clocks+falling FOUND; wave sector unread of frozen \(\mathcal{L}\); live Poisson wiring FOUND, freeze not test-body at 1-voxel or 3³] | RP | clock-hypothesis AXIOM (FC-2) |
| §12-mgcurv | Mass-gap to curvature source. FTD-0406 supplies selected strong `T00/C_SPEED²` latency sourcing on an isolated collision-free CPU domain, but no target-blind gap, GPU/topology/mixed-force closure, derived stress tensor or curvature theorem. Extend NCEMC before any mass-gap/curvature measurement | [OPEN — scoped source only] | RP | NCEMC |
...
| MC-T4.4 | Native moving-source radiation: derive a conserved source/current and reciprocal recoil for general mobile matter. FTD-0558/0559 close field pole/work; FTD-0560–0562 close every fixed finite rigid linear dressing; FTD-0563 proves that finite microscopic neutrality supplies no true selected-face or restricted-native linear Gauss monopole; FTD-0564 proves a normalized-flux defect degree cannot by itself set Gauss-charge magnitude. A live nonlinear/topological/deforming carrier must supply both a protected class and a common action that locks flux magnitude, energy, recoil, and motion | [OPEN — TOPOLOGICAL CORE + NONLINEAR ACTION OR DEFORMING MATTER SIDE; TOPOLOGY-ALONE MAGNITUDE AND FIXED FINITE RIGID LINEAR CARRIERS CLOSED] | W–M | — |

**Closed (sector context):** FTD-0004 Phase G geometric Coulomb [THEOREM]; FTD-0110 cluster mass [DERIVED at linear level]; FTD-0131 leading-order Newton ([SMC] prediction floor inherited from FTD-0015; [DERIVED] chain steps conditional on clock-hypothesis AXIOM); FTD-0208 clock-hypothesis v3 [CLOSED NEGATIVE, AXIOM-LEVEL]; FTD-0113 auxiliary retarded/static identity and native zero-frequency resolvent [THEOREM]; FTD-0558 selected-drive production pole, positive speed floor, and integer-hop Floquet spectrum [THEOREM/DERIVED]; FTD-0559 exact prescribed-source field work, finite-time response, and continuum external-drive functional [THEOREM/DERIVED]; FTD-0560 point periodic-hop linear dressing, FTD-0561 rigid charged widening, FTD-0562 every fixed finite rigid linear profile, FTD-0563 the finite-neutral linear Gauss-monopole escape, and FTD-0564 topology-alone charge magnitude [CLOSED NEGATIVE in their stated source sectors].
...
| **C3-DECISION** | Decide whether the registered compact, zero-tension central-force law admits an `n = 4` carrier mechanism. A positive result requires one admissible stress-plus-flex framework and strict positivity of the reduced quartic on the **whole** nontrivial null-space unit sphere. A negative result requires universal semialgebraic realization-space coverage (including separation/support margins and rank strata) or an explicit zero/negative direction on every candidate stratum. **Bulk BCC is in scope:** the tested finite BCC block is surface-dominated. FTD-0800/0801 close only their screened configurations; correction `0cf9e635` makes the `N = 6` follow-up explicitly exploratory. No further unregistered numerical hunt is a closure path. | **[OPEN]** | RP | FTD-0789; FTD-0800/0801 |
...
**Status snapshot (scope-corrected by FTD-0412):** MC-T4.3 is a central **unfinished mechanism search**. Three channel-limited engine diagnostics remain valid (FTD-0004, FTD-0005, FTD-0125); FTD-0126 is retracted as a physical Wilson-Dirac result because it evolved spatial `D_W` as a real-time Hamiltonian and used a basis-inconsistent initial spinor. The surviving diagnostics and FTD-0244 prove meaningful exclusions only in their stated classes. They do not establish that every non-action injection mechanism fails or that ontology extension is necessary.

**FTD-0224:** the four ARC mechanism classes of `SPEC_ALPHA_READOUT_CONTRACT.md` have each been attacked. **ARC-A** (boundary-condition) and **ARC-B1** (observable-selection, catalog items 4/6/7) closed `[CLOSED NEGATIVE]`. **ARC-B2 / ARC-C1** (BCC-bridge / quantization) reach **UNDERDETERMINED** — a "FOUND-at-ARC-2" verdict would be an **overclaim**: the determinant grading `16G*³` is an *asserted* master-quadratic Vieta target, not a forward detdet_ζ identity (the J-twisted ζ-reg determinant ratio `=G*` is a genuine clean odd source, but `Det = Tr·G*` is not forced — `16G*³ = x₊x₋` is an ordinary product, and a 2×2's trace and determinant are independent). See `AUDIT_ARC_C1_B2_FOUND_INDEPENDENT_REVIEW.md` + the three pre-registered attempts (FTD-0224). **Surviving route: ARC-D** (engine-native measurement), another predeclared mechanism outside the closed classes, or a `[CONJECTURE — new postulate]`. MC-T4.3 remains `[OPEN — SCOPED NO-GO PACKAGES]`; no spine tag moved.

**FTD-0242:** the operator-forcing question (W-CRIT-2) is sharpened to a **route-invariant boundary within four attempted routes**. J-twisted ζ-determinant, BCC body-diagonal transfer operator, lemniscatic CM arithmetic of `E: y²=x³−x`, and a forced variational/period-ring/K-theory channel were each force-attempted: **0 of 4 forced** (`cleanForcedRoutes = []`). Forward-forced `[DERIVED]`: the trace `16G*²` and the existence of a clean FTD-native odd source (`det_ζ(D_{3/4})/det_ζ(D_{1/4}) = G*`, which genuinely lifts the bare parity no-go so `16G*³ = 16G*²·G*` is *assemblable*). **Not** forced: the operator assembly itself — for a 2×2, trace and determinant are independent invariants, so the det_ζ ratio supplies the odd scalar but forces neither the gluing nor that it lands in the determinant slot. The calculation excludes those routes; it does not prove that α is universally dynamical rather than structural. MC-T4.3 remains `[OPEN — SCOPED NO-GO PACKAGES]`; `x₊ = 1/α` (FTD-0013) stays `[STRONGLY MOTIVATED CONJECTURE]`; no spine tag moved. See `../07_assessment/audits/AUDIT_ALPHA_OPERATOR_FORCING_ROUTE_INVARIANCE.md` (FTD-0242).
...
| ID | Gap | Tag | Effort | Deps |
|---|---|---|---|---|
| **MC-T4.3** | **Algebraic-spine physical electromagnetic readout mechanism. Central FTD claim.** `SPEC_ALPHA_READOUT_CONTRACT.md` formalizes the closure contract: specify `(P, A_obs, O_EM, R, C)` before target-checking; avoid alpha input; survive the valid scoped diagnostics; explain why the output is an operational EM coupling rather than a distinguished number. Current action/operator/readout routes are closed or boundary-mapped only within their declared classes. | **[OPEN — SCOPED NO-GO PACKAGES]** | **RP** | MC-T4.1, MC-T4.2 |
| MC-T4.1 | Two-layer ontology axiomatization. **Reframed to documentation alignment** — substantive ontology already J-primary via SPEC_FTD.md §1.1 graded-monism table + Genesis rule | [CLOSED 2026-07-12 — the Postulate-3 J-primary textual update landed 2026-05-02 (commit `cc93c2df`: heading, Genesis-rule body, graded-monism table §1.1, "one postulate, not two" note); alignment verified this pass — no pending Postulate-3 references remain in active docs] | D (closed) | — |
...
| W-CRIT-1 | **Circularity in framework integer identification.** Integers `{N_c=3, N_base=4, b_3=7, N_eff=13}` selected knowing target physics values. Constraint 11 of gtca: LEDGER tagging is not resolution. Closure depends on MC-T4.3 rather than match-counting | [OPEN methodological] | (closes if MC-T4.3 closes) | MC-T4.3 |
...
**The three surviving empirical diagnostics for MC-T4.3, plus one invalidated historical run:**
...
| FTD-0126 (Phase II) | Matter-sector vertex | **INVALIDATED by FTD-0412:** wrong real-time operator and basis-inconsistent initial spinor; the historical `a_e` number has no physical evidentiary weight |
...
§10 foundational
   ...
   MC-T4.1      ── CLOSED 2026-07-12 (doc-alignment complete; text landed cc93c2df)
...
- **Spine-complete:** no small pure-math queue item remains here.
...
- **Bridge-complete (full):** ~~MC-T4.1~~ (CLOSED 2026-07-12) → MC-T4.2 → MC-T4.3, with MC-T4.3 carrying foundational-obstruction risk that could halt closure indefinitely. **Unbounded.**

**Publication-readiness note:** this tracker does not adjudicate the owner gate.
