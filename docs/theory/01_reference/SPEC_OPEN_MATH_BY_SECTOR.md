...
**Status snapshot:** Spine: nine numbered results — seven theorem-grade + two honestly-tiered (see `SPEC_ALGEBRAIC_SPINE.md` §0). Tier I 5/5 closed. Tier II 4/4 closed; 0 structural theorems [OPEN]. 1 pre-registered scan awaiting execution. 1 small closed-form gap.

| ID | Gap | Tag | Effort | Deps |
|---|---|---|---|---|
| MC-T1.1-ext | `L ≥ 3` ultralocality proof or disproof for matched-stencil with Gauss-constraint-allowed configurations | [Theorem 7 honest status: **[THEOREM at L=2]** (ultralocal) + **[NUMERICAL EVIDENCE at L=3]** (ultralocal per `scripts/proofs/proof_phase_j_general_L.py`) + **[OPEN for L ≥ 4]**; a blanket `[DISCONFIRMED for general L]` reading would be incorrect — see SPEC_ALGEBRAIC_SPINE.md §7] | W (partly open) | — |
...
**Closed (sector context):** Theorems 1–9 (FTD-0001, OT-1.1–1.8, 2.1–2.3, 4.1, 3.4 partial), MC-T1.1 (route b, Theorem 7 retagged `[THEOREM at L=2]`), **MC-T1.1-ext (Theorem 7 honest status: [THEOREM at L=2] + [NUMERICAL EVIDENCE at L=3 ultralocal per `proof_phase_j_general_L.py`] + [OPEN for L ≥ 4]; a blanket `[DISCONFIRMED for general L]` reading would be incorrect)**, MC-T1.2 (Theorem 3 retagged `[NUMERICAL FACT, h=1 only]`), MC-T1.3 (Q(G*) verification), MC-T1.4 (per-voxel mass gap), MC-T1.5 (BCC complex-structure Roles 1+3 [DERIVED]; Roles 2+4 NO-GO), MC-T2.1 + MC-T2.2 (extended polynomial scan; the "~4×10⁵:1 Bayes" figure is retracted to [NUMERICAL FACT] — not runner-computed, ~19× scan-size), MC-T2.3 items 1–3 (63-disc Γ-product null at h ≥ 2), MC-T2.3-4 (CLOSED, unit-group uniqueness proof integrated).
