...
| NCEMC | Native Confinement Energy–Momentum Contract (FTD-0400): after §12-cnorm closes, define one backend-common `H_strong` generating the force, exact work accounting, complete momentum, and field-only gravity sourced by the same object. FTD-0402 implements the normalization subcontracts but returns PARTIAL because its aggregate gate is incomplete. No mass measurement before both contracts close | [OPEN — blocked on §12-cnorm] | M–RP | §12-cnorm, eng-SU3 |
...
| §12-cnorm | FTD-0402 implements one raw nodes/tick map (`B=|u|²/C_SPEED²+L²`), explicit mass roles, normalized energy–momentum diagnostics, common host `tau`, and targeted CPU/GPU parity. Outstanding: complete the frozen aggregate G9 before formal closure | [OPEN — implementation subcontracts pass; aggregate incomplete] | M | — |
...
| §12-mgcurv | Mass-gap to curvature source. FTD-0400 proves the implementation is split; FTD-0402 repairs the normalization layer but does not supply common stress–energy. Closure still requires §12-cnorm, then NCEMC-1–4, before any curvature measurement | [OPEN — blocked] | RP | §12-cnorm, NCEMC |
