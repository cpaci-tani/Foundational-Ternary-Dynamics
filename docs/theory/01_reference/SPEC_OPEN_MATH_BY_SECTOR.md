...
|---|---|---|---|---|
| MC-T1.1-ext | `L ≥ 3` ultralocality proof or disproof for matched-stencil with Gauss-constraint-allowed configurations | [Theorem 7 honest status (2026-06-24 spine audit): **[THEOREM at L=2]** (ultralocal) + **[NUMERICAL EVIDENCE at L=3]** (ultralocal per `scripts/proofs/proof_phase_j_general_L.py`) + **[OPEN for L ≥ 4]**; the prior blanket `[DISCONFIRMED for general L]` reading is corrected — see SPEC_ALGEBRAIC_SPINE.md §7] | W (partly open) | — |
...
**Closed (sector context):** Theorems 1–9 (FTD-0001, OT-1.1–1.8, 2.1–2.3, 4.1, 3.4 partial), MC-T1.1 (route b, Theorem 7 retagged `[THEOREM at L=2]`), **MC-T1.1-ext (Theorem 7 honest status per 2026-06-24 spine audit: [THEOREM at L=2] + [NUMERICAL EVIDENCE at L=3 ultralocal per `proof_phase_j_general_L.py`] + [OPEN for L ≥ 4]; the prior blanket `[DISCONFIRMED for general L]` reading is corrected)**, MC-T1.2 (Theorem 3 retagged `[NUMERICAL FACT, h=1 only]`), MC-T1.3 (Q(G*) verification), MC-T1.4 (per-voxel mass gap), MC-T1.5 (BCC complex-structure Roles 1+3 [DERIVED]; Roles 2+4 NO-GO), MC-T2.1 + MC-T2.2 (extended polynomial scan; the "~4×10⁵:1 Bayes" figure is retracted to [NUMERICAL FACT] — not runner-computed, ~19× scan-size; 2026-06-24 spine audit), MC-T2.3 items 1–3 (63-disc Γ-product null at h ≥ 2), MC-T2.3-4 (CLOSED 2026-05-26, unit-group uniqueness proof integrated).
