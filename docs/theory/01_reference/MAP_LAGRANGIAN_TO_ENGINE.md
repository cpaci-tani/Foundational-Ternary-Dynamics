...
| 4 | Gauss constraint | $-\lambda_G(\nabla_L\cdot\mathbf{J} - \rho)^2$ | `gauss_term()` at `lagrangian.h:57-60`; finite $\lambda_G = 100.0$ at `lagrangian.h:55` | constraint enforced each tick by SOR Poisson on CPU at `engine/src/poisson_solvers.cpp:21-84` and cuFFT on GPU at `engine/cuda/kernels_poisson.cu` (CALLSTACK F6 RESOLVED — both solvers documented in `engine/SPEC_ENGINE.md:937-946`). Per-site violation summed in `compute_lagrangian_diagnostics()` at `lagrangian.cpp:30`. | Spec says $\lambda_G \to \infty$ exactly; engine uses $\lambda_G = 100$ as a finite penalty calibrated such that the residual is bounded by a STRUCTURAL floor on **both** backends, not by iteration count or precision. The Green's function is the 18-point Laplacian symbol while the applied correction and the residual measurement are 6-point central differences, so a per-mode factor of median ~0.62 survives projection (L=24 Brillouin-zone scan; min 0.006, max 1.0). The previously quoted $10^{-8}$ (cuFFT) belongs to the `[TOOLING]` matched-stencil CG solver, which is not the production Gauss path; the CPU floor is documented at `constants.h:331-347` and `eft/matched_poisson.h:7-19`. |
...
| 6 | Field gradient energy (18-pt) | $-\tfrac{1}{2}c^2\sum_\text{links} w_l\lvert\Delta\mathbf{J}\rvert^2$ (pairs-once link sum) with $w_\text{face}=1/3$, $w_\text{edge}=1/6$ | `field_gradient_term()` in `lagrangian.h` (per-site half-share, prefactor $-\tfrac{c^2}{4}$ on the full 18-neighbour sum, so the site-summed diagnostic is the pairs-once action) | variational $\delta/\delta J$ produces the 18-point Laplacian stencil used by `phase_read.cpp` (CPU isotropic 18-pt) and `kernels_stencil_*.cu` (GPU), at the same normalization as the kinetic term (wave speed $c$). | Pair-counting convention guarded by `test_action_stationarity.cpp` §7. Weights $\{1/3, 1/6\}$ derived variationally in `DERIV_18PT_LAPLACIAN_VARIATIONAL.md` (delivered; [THEOREM] identity, [SELECTION] weight pair via leading-order isotropy — see its §2 normalization remark for the pairs-once convention). |
...
| $C$ (lattice speed) | `constants.h` `using ontic::C_SPEED` | $1/\sqrt{3}$ **[SELECTION]** (FTD-0407) — not CFL-forced; the production 18-point stencil's exact max symbol is $16/3$, permitting $c \le \sqrt{3}/2$ |
...
│      Residual floor is STRUCTURAL (18-pt symbol vs 6-pt correction/measure),
│      not a precision limit: ~62% of each mode's residual survives projection.
│      The 10⁻⁸ figure belongs to the [TOOLING] matched-stencil CG solver,
│      NOT to the production cuFFT/SOR path.
│
