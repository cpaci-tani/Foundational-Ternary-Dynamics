...
| Field | Content |
|---|---|
| **FTD result** | Any correlation experiment fully implementable inside the commutative substrate (no measurement-map M) is bounded **S ≤ 2** — structural: the substrate satisfies realism + locality + statistical independence, so Bell's theorem applies (`[THEOREM]`-grade via FTD-0243 + Bell). Engine CHSH measurements: **S ≈ 1.95–2.00** across all tested configurations (vector flux, ternary states, wave propagation, sLoop coupling). The ternary state space produces an apparent S ≈ 3.6 at ~49 % detection efficiency — the **detection loophole**, a known artifact, not a violation. ([`AUDIT_BELL_ANALYSIS.md`](../07_assessment/quantum_foundations/AUDIT_BELL_ANALYSIS.md); [`THEOREM_COMMUTATIVITY_INDEPENDENCE.md`](../10_eft_program/derivations/alpha_readout_programme/THEOREM_COMMUTATIVITY_INDEPENDENCE.md) §4.) |
...
| **Scope + caveats** | The clock hypothesis (CLK-1) stays `[AXIOM]` (FTD-0208 `[CLOSED NEGATIVE]` for substrate derivation) — annotated with measured IR-emergent support, nothing stronger. As of 2026-08-13 (FTD-1000) CLK-1 is folded into FC-2's metric-half declaration (constitution §5.2) as its per-worldline proper-time clause, rather than tracked as an independent coordinate-level axiom; this is a re-homing, not a tag change. FTD-0208's linear single-event budget is a *different observable*; not in contest. |
...
| Field | Content |
|---|---|
| **FTD result** | **Three clean `[THEOREM]` nulls** — the ontology *forbids*: **N_monopole = 0** (`div(B) ≡ div(curl J) = 0` identity; `proof_complete_sm.py:495`); **N_SUSY = 0** (the ternary state space carries no fermionic grading; `:500`); **extra dimensions = 0** (`\|Aut(E)\|² = 2^D·(D−1)!` forces D = 3; `:503`). **τ_proton = ∞ is NOT in this set** — retagged `[SELECTION]/[BOUNDARY]` per **LEDGER FTD-0301** ([`ANALYSIS_PROTON_STABILITY_v1.md`](../10_eft_program/sm_constants_mass_flavour/ANALYSIS_PROTON_STABILITY_v1.md); LEDGER > this ledger). The substrate carries only the exact U(1) Σs charge and **no baryon/B−L current** (baryon number is an emergent 3-quark-color-singlet cluster label), so Σs conservation does *not* forbid `p → e⁺ + π⁰`; the proton is a mixed-sign uud cluster that the same-sign `triad_binding` lock structurally cannot protect; and FTD's own weak channel transmutes it (engine run of record: uud→uuu, 46/48; spontaneous evaporation 12.5 %/43.75 % at radius 1/2). A large proton lifetime is therefore a `[SELECTION]` (emergent metastability), not a forced theorem — `proof_complete_sm.py:460–476` already corrected. *Source note:* [`CATALOG_PARAMETRIC_INSERTIONS.md`](../07_assessment/CATALOG_PARAMETRIC_INSERTIONS.md) §16 still lists `τ_proton = ∞` as `[THEOREM]` — also stale under FTD-0301 (owner-flagged for the same correction). |
...
| **Discriminating protocol** | The world's existing experimental program *is* the protocol: Super-K/Hyper-K proton-decay limits, monopole searches, collider SUSY searches, short-range-gravity and collider extra-dimension searches. **For the three `[THEOREM]` nulls**, every continued null is evidence for the FTD ontology against frameworks whose structure requires the positives. **The proton asymmetry does not hold** (FTD-0301): FTD does not forbid proton decay either, so a continued decay-null is *consistent with* FTD but not specifically *favorable* to it. |
| **Scope + caveats** | The three structural nulls are absence claims — they accumulate support but a single confirmed positive kills the corresponding theorem's physical reading outright. The proton row is a `[SELECTION]/[BOUNDARY]`, not a theorem (see the FTD-result cell): an observed proton decay does **not** falsify FTD. |
| **Falsification (of the FTD side)** | An observed magnetic monopole, SUSY partner, or extra dimension — each falsifies the corresponding `[THEOREM]`'s physical identification immediately and unconditionally. **An observed proton decay does *not* falsify FTD** (FTD-0301): the substrate *licenses* baryon-number violation (emergent cluster label, no B−L current), so a measured finite τ_proton is consistent with FTD's UNFORCED-METASTABLE boundary — what such a measurement kills is the legacy `[THEOREM]` over-claim, already retired here and in `proof_complete_sm.py`. |
