# SPEC · FTD/FQCR Doctrine Ledger v1.6

**Tag:** [REFERENCE] — single-page status map. Per-element tags within (see §0 status key + §13 audit).
**Date:** 2026-06-14
**Version:** 1.6 (status reconciliation: FTD-0244 K-BIND operator-calculus closure recorded as [CLOSED THEOREM-NEGATIVE]; FTD-0208 v3 clock-hypothesis closure recorded as [CLOSED NEGATIVE, AXIOM-LEVEL]. Prior: 1.5, 2026-06-01 MC-T4.3 route-invariance boundary per FTD-0242 — α classified DYNAMICAL not structural, 0/4 FTD-native routes force the operator assembly; still [FOUNDATIONAL OBSTRUCTION], **no tag promotion**.).
...
| Identification with physical `α⁻¹` (CODATA 2022 `137.035999177(21)`)       | [STRONGLY MOTIVATED CONJECTURE]  | OT-5.1; agreement 1.26 ppm; not [DERIVED] absent a non-action injection mechanism (MC-T4.3, foundational obstruction). **2026-06-01 (FTD-0242):** the MC-T4.3 obstruction is now a **route-invariant boundary** — 0/4 FTD-native routes force the `(Tr,Det)=(16G*²,16G*³)` operator assembly (the trace and a clean odd source `G*` are forward-forced, the assembly is not — W-CRIT-2). **2026-06-10 (FTD-0244):** K-BIND is closed theorem-negative for the axiomatized substrate-native operator calculus; no current native operator forces the quadratic extension without an external W-like selection. α is classified **DYNAMICAL, not structural**. **No tag promotion.** **2026-06-24 (FTD-0314/0315):** the surviving exit is now precisely pinned — FTD-0314 (carrier-narrowing `[THEOREM]`) proves W must be an external ℤ/2 twist on a G\*-bearing analytic carrier realizing `√(G*(4G*−1))`, and no native object supplies it; FTD **adopts** it as **FC-W** (the constitution's FC-4, FTD-0315), under which `x₊=1/α` is `[CONDITIONAL THEOREM given W]`, still `[STRONGLY MOTIVATED CONJECTURE]` unconditionally. MC-T4.3 stays a `[FOUNDATIONAL OBSTRUCTION]` (FC-W is an external axiom, not a closure). |
