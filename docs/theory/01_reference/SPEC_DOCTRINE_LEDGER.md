...
At `R_N(1) = 1` and `N → ∞`, this reduces *exactly* to the spine master quadratic `x² − 16 G*² x + 16 G*³ = 0` (Theorem 2; OT-1.1; FTD-0014).
...
**QCD beta coefficient (one loop).** `b_3 = (11 N_c − 2 n_f)/3`. For `N_c = 3, n_f = 6`: `b_3 = 7`. **The formula is imported standard QCD**; the integer `b_3 = 7` follows from substituting `N_c = 3` (FTD's master-quadratic small root, [STRONGLY MOTIVATED CONJECTURE] per FTD-0014 / OT-5.2) and `n_f = 6` (SM input).
...
| Claim                                                                | Tag                              | Source                                                                                  |
|----------------------------------------------------------------------|----------------------------------|-----------------------------------------------------------------------------------------|
| `N_c = 3` from `C³` / `Z_3` triality / `Z_6` closure                 | [SELECTION] (algebraic) + [STRONGLY MOTIVATED CONJECTURE] (`x_- ↔ N_c`) | OT-5.2; FTD-0014                              |
...
| `b_3 = 7` for `N_c = 3, n_f = 6`                                     | [THEOREM once formula imported, conditional on FTD-0014 SMC + SM `n_f = 6` input] | Direct substitution                                            |
...
`α_FTD⁻¹ ≈ 137.035999177` (1.26 ppm); `x_- = 3.024 ≈ N_c = 3` (0.80%); `α_G(e,e) = (m_e/m_P)² ≈ 1.745 × 10⁻⁴⁵` (0.38%, postulate-conditional); CKM estimates from `q*` powers; structural uniqueness scans (2.87M-poly with 0 Eisenstein dual-matchers; 63-discriminant Γ-product null at `h ≥ 2`; ~4×10⁵:1 Bayes weight).
...
| §5          | SPEC_ALGEBRAIC_SPINE.md §2 (Theorem 2); SPEC_FQCR.md §2 Prop 5, §3                                  | FTD-0014 (master quadratic), FTD-0013 (`x_+ ↔ 1/α`), FTD-0143 PRE-REG | OT-1.1, OT-3.3, OT-5.1 |
