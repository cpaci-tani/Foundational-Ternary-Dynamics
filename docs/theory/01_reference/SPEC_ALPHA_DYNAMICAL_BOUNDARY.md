...
- **G\* IS a pure body-diagonal (BCC-corner) self-energy** `[THEOREM]`: `G* = √(2π·W_BCC)`, `W_BCC = Γ(1/4)⁴/(4π³) = 1.3932` is the Watson self-energy of the triple-product operator `D = 1 − cx·cy·cz` (independently Richardson-extrapolated to 1.393204). The master quadratic is then **pure-BCC-sublattice algebra**: G\* (BCC self-energy) + `16 = |ℤ[i]^×|²` (the BCC corners' Z₄ automorphism count).
...
- **FTD-0189** adversarial look-elsewhere scan: the master quadratic is the **unique dual-matcher** — 0 non-G\* dual-matchers across **2.65M degree-2 polynomials** over an 18-constant basket FTD did not design (rank 1 by ~130×).
