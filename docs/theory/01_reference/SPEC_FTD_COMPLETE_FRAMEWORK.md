...
α as a derivation · the Born rule from P1–P5 · an FC-1-consistent (M-free) account of lab Bell S>2 · QFT scattering amplitudes / interference at the observer layer · atomic spectra · any cosmological observable · confinement · a derived mass spectrum (genesis or Gaussian sector) · **final aggregate closure of the causal-normalization gate (FTD-0402 implements and verifies the scoped subcontracts but returns PARTIAL because full CTest was not completed)** · **a common confinement-energy/inertia/gravity source (FTD-0400: SPLIT-BOOKKEEPING; NCEMC remains blocked on §12-cnorm)** · **native fermions (Branch-A closed negative, FTD-0379/0380)** · **a native rest-mass gap (FTD-0362)** · **a substrate spin-2 mode in the probed regime (FTD-0193; the GR chain's graviton is imported)**. **And the F10 gap:** ~122 of the ~131 `[PARAMETRIC]` matches are *tagged but never rigidity-scanned* — the single most important methodological caveat. *Any framing that presents these as derivations is overclaim.*
...
4. **Named engine/theory residues:** PL-4 diagonals; the nonlinear-rung transcript/properness characterization left by FTD-0396's double BLOCKED-ESCAPE verdict (the scheduled v2 pre-registration is complete); M1-v2 vertex follow-up + the Program-F effective-toggle audit; **complete FTD-0402's outstanding aggregate verification before closing §12-cnorm; only then may a separately locked NCEMC arc address one strong Hamiltonian, exact work accounting, total momentum, and same-object field-only gravity before any mass readout**; the E1/E2 walls (progress in either direction re-adjudicates FTD-0369 via the frozen map).
