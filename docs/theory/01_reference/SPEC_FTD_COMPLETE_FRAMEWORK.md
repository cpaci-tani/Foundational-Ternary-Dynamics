...
**Version:** **v2 (2026-07-12; IMP-S4 carrier reconciliation 2026-07-22)** — v1→v2: claims register advanced through FTD-0382 + the finishing arc (FTD-0143 verdict, IMP-S4 pricing, FTD-0348/0350/0355 reconciliations); the priced-import ledger folded in as the boundary's quantitative face (§3); the July-2026 arc register added (§3.5); §8 replaced by the **consolidated falsifier table** (previously scattered across four document families); accounting refreshed to the FTD-0348 correction of record. FTD-0417 broadens IMP-S4 to the mutually exclusive global-projection/local-link carrier branches without changing the count. FTD-0311 id retained.
...
| **Calibrations** | `a_phys ≡ ℓ_P` · `M_INERTIAL = m_e` · `E_REST=M_INERTIAL·C_SPEED²` in raw lattice units · `t_phys = ℓ_P/(√3·c)` — **default gauge is electron-primary since 2026-07-08** (FTD-0137 §4.5, `FOUND_ELECTRON_PRIMARY_GAUGE.md`): import `{ℏ, c, m_e}`. `M_GRAVITATIONAL=M_INERTIAL` remains a separately named imposed numerical choice (FTD-0402). | `[IMPOSED]` (dimensionless predictions are calibration-independent; dimensional ones ride these) |
...
Each row is a *result* (Number-One-Goal clause 2): a rigorously-mapped limit, not unfinished work. **Each boundary is now also *priced*:** `SPEC_IMPORT_LEDGER.md` (FTD-0371, rev v1.2 2026-07-22) counts every import in a common currency with a falsifier per line — **1 adopted bit** (FC-W/δ), **4 selected types** (D=3, singlet, ℭ generator-set, and IMP-S4's gauge-connection carrier choice: global `A_μ=𝒫_TJ_μ` or FTD-0417 independent local link), **4 named results** (Chudnovsky proven; CM-h=1/E1/E\*E\*\* open), **3 calibrations**, the **empirical bridges** (x₊=1/α `[SMC]` + ~131 `[PARAMETRIC]` + ~50 external), **2 declined bets** (M, reversibility). ⚠ Reading guard: the "1 adopted bit" is the α-sector branch choice only — never the total physics import.
...
| **Native fermion/current emergence (Branch-A)** | **`[CLOSED NEGATIVE at every protocol tested]`** — DK evolution (FTD-0379, incl. the v1.1 true-DK-operator re-run, fitted speed a\*≈0) and su(2) closure noise-recoverability (FTD-0380) both closed negative under pre-registered locks. FTD-0421 additionally proves that the frozen production event algebra has no nontrivial additive charge over the preregistered discrete basis, so the native charged-pole/common-cone and native dimension-four-flow campaigns are dependency-closed. Branch-B imports Wilson–Dirac matter and the IMP-E1∘IMP-E3 coupling; its selected FTD-0417/0418 local-link regulator has a nonzero off-shell one-loop relative-cone counterterm (FTD-0419), and one fixed counterterm fails the first available off-shell multiplicity threshold (FTD-0424). Physical on-shell matching remains open. | `SCOPE_VERTEX_PROGRAM`; FTD-0417–0424 |
| **Native rest-mass / particlehood prerequisites** | **`[MEASURED — CLOSED NEGATIVE]`** for the probed native mass gap (FTD-0362); FTD-0392's local charge is zero and FTD-0398's terminal larger-shell test is **`[UNDERDETERMINED]`** with no mass evidence; FTD-0399 target-blind particlehood is **`[INVALID — G2]`** because exact A/E histories do not manifest at L=33/65. No mass observable opens on the current engine. | LEDGER FTD-0362/0392/0398/0399 |
...
| FTD-0371 | The priced-import ledger (rev v1.2 2026-07-22: IMP-S4 carrier branches reconciled) | `[SYNTHESIS]` |
...
| IMP-S4 | gauge-connection carrier: global `A_μ=𝒫_TJ_μ` or independent local-link `A_μ` | a forcing theorem derives the carrier, or matched branch protocols show inequivalent vertex phenomenology | standing (minted 2026-07-12; broadened 2026-07-22; every vertex result carries its branch conditional) |
...
α as a derivation · the Born rule from P1–P5 · an FC-1-consistent (M-free) account of lab Bell S>2 · QFT scattering amplitudes / interference at the observer layer · atomic spectra · any cosmological observable · confinement · a derived mass spectrum (genesis or Gaussian sector) · **a derived, backend-complete confinement-energy/inertia/gravity source** (FTD-0400: SPLIT-BOOKKEEPING; FTD-0405: DOUBLE-OBSTRUCTION for the unmodified direct pair-force route; FTD-0406 adds only an owner-selected collision-free CPU contract using `T00/C_SPEED²`) · **native fermions (Branch-A closed negative, FTD-0379/0380)** · **a native rest-mass gap (FTD-0362)** · **a substrate spin-2 mode in the probed regime (FTD-0193; the GR chain's graviton is imported)**. The raw-lattice causal-normalization changed surface is closed by FTD-0403, while the clock budget itself remains `[AXIOM]`. **And the F10 gap:** ~122 of the ~131 `[PARAMETRIC]` matches are *tagged but never rigidity-scanned* — the single most important methodological caveat. *Any framing that presents these as derivations is overclaim.*
