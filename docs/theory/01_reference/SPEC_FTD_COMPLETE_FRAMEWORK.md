...
4. **Named engine/theory residues:** PL-4 diagonals; the nonlinear-rung transcript/properness characterization left by FTD-0396's double BLOCKED-ESCAPE verdict (the scheduled v2 pre-registration is complete); M1-v2 vertex follow-up + the Program-F effective-toggle audit; **FTD-0403 closes §12-cnorm, so a separately locked NCEMC arc may now address one strong Hamiltonian, exact work accounting, total momentum, and same-object field-only gravity before any mass readout**; the E1/E2 walls (progress in either direction re-adjudicates FTD-0369 via the frozen map).
