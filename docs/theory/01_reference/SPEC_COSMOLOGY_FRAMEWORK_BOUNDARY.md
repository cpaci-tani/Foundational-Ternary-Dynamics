...
| **W-COSMO-4** | `Λ = α⁵⁷` | `[PARAMETRIC]` numerology | no mechanism; a constant-matching, not a derivation |
...
The Moore Layer Theorem gives **17 dark states** out of 27 (the polyhedral decomposition: octahedron + cuboctahedron + stella octangula), so a dark fraction `17/27 ≈ 0.63` is a **structural integer count** — the *most* defensible cosmology-adjacent number. **But the dark-matter DYNAMICS are not derived:** the halo profile / SPARC rotation curves were attacked directly and returned **`INDETERMINATE`** — **FTD-0290** (halo forcedness) found SPARC **NOT founded**: the lossless dark-matter halo **box-fills the periodic lattice** (`r_eff ≈ L/2`, not localized), and the §4.1 −0.69 halo exponent was **FALSIFIED** (an L=64 transient → −1.25 at L≥128). So: the dark-state *count* is structural; the dark-matter *object/dynamics* is an `[OPEN]`/mapped-negative boundary.
...
**Nothing promoted.** All §8 entries keep their existing `[SELECTION]`/`[PARAMETRIC]`/`[OPEN]` tags. The dark-state count `17/27` is structural (Moore Layer Theorem) but the dark-matter dynamics are `[OPEN]`/mapped-negative (FTD-0290). No cosmological observable is derived; FTD-0013 `[SMC]`, MC-T4.3, the spine — all unchanged. Companion accepted-boundary docs: [`SPEC_ALPHA_DYNAMICAL_BOUNDARY.md`](SPEC_ALPHA_DYNAMICAL_BOUNDARY.md), [`SPEC_FTD0110_BRIDGE_BOUNDARY.md`](../03_derivations/foundational_mechanics/SPEC_FTD0110_BRIDGE_BOUNDARY.md).
