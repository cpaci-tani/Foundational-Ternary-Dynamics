...
6. **Phase 3c: Latency Poisson (`solve_latency_poisson()`):** Solves the gravitational Poisson equation $\nabla^2 \phi_L = 4\pi G_N (\rho_{mass} - \bar\rho_{mass})$ on a mean-subtracted source, where $\rho_{mass} = K_B |s|$, then mean-subtracts $\phi_L$. The latency field is updated as $L = \sqrt{\text{clamp}(|\phi_L|, 0, 0.998)}$ — the square root of the clamped well depth, not itself a solution of $\nabla^2 L = 4\pi G\rho$.
...
| **`exchange_force`** | `false` | `poisson_coulomb`| None | `ANY` | Phase-forces: Pauli exclusion repulsion between same-spin fermions (CPU in `phase_forces.cpp`, GPU in `kernels_forces.cu` `exchange_force_kernel`). |
| **`latency_field`** | `false` | `gravity` | None | `ANY` | Solves a mean-subtracted Poisson $\nabla^2 \phi = 4\pi G_N(\rho - \bar\rho)$, mean-subtracts $\phi$, and stores $L = \sqrt{\mathrm{clamp}(-\phi, 0, 0.998)}$ — the square root of the clamped well depth, not a solution of $\nabla^2 L = 4\pi G\rho$ — as the time-dilation proxy (see `poisson_solvers.cpp`). |
...
| **`langevin`** | `false` | None | `larmor_radiation` | `ANY` | Stochastic thermalization: OU process on wave velocity (native CUDA + CPU; SplitMix64 per-voxel noise). |
