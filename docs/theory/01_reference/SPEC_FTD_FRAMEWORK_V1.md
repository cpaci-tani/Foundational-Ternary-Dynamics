...
| 4 | **Time dilation / γ-emergence** | `[MEASURED, scoped]` + `[AXIOM]` + `[OPEN]` | the independent wave-clock result normalizes `v=v_g/C_WAVE` and never reads `voxel.tau`: departure from exact γ vanishes as `R ∝ L⁻¹·⁹⁸ ≈ L⁻²` (∝ k²) on ⟨100⟩ at v ≲ 0.85; ultra-relativistic diagonals remain `[OPEN]`. **FTD-0402:** live matter-clock/BI/force/movement paths now share raw `u`, `β²=|u|²/C_SPEED²`, and `B=β²+L²`, with targeted CPU/GPU parity. This is implementation of the clock hypothesis `[AXIOM]`, not a covariance theorem; the overall result is PARTIAL pending aggregate verification. |
...
**Joint-reading guard (FTD-0400–0402, 2026-07-21):** rows 4, 7, 9, and 12 still do not form one native stress–energy account. FTD-0400 finds `SPLIT-BOOKKEEPING`: colour force, energy totals, imposed inertia, and gravity use separate paths. FTD-0401 identified the raw/c=1 mismatch. FTD-0402 corrects the live implementation with one raw causal map, explicit `M_INERTIAL`/`E_REST`/`M_GRAVITATIONAL` roles, normalized energy–momentum diagnostics, and targeted CPU/GPU parity. Its frozen result is `PARTIAL` solely because full-suite aggregation was not completed, so `§12-cnorm` remains open and NCEMC is still blocked. No framework type or mass derivation is added.
