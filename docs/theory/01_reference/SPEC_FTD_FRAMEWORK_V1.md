...
- `M_INERTIAL = m_e = K_B` (imposed calibration), `E_REST=M_INERTIAL·C_SPEED²=K_B/3` in raw lattice units, and separately named `M_GRAVITATIONAL=K_B` (imposed numerical equality, not a common native origin). FTD-0402 implements these roles; NCEMC remains open. Also `t_phys = ℓ_P/(√3·c)` (= t_P/√3 ≈ 3.11×10⁻⁴⁴ s; derived from `a_phys` + `c_lat=1/√3`, corrected 2026-07-08 from `√3·ℓ_P/c`).
...
**Joint-reading guard (FTD-0400–0403, 2026-07-21):** rows 4, 7, 9, and 12 still do not form one native stress–energy account. FTD-0400 finds `SPLIT-BOOKKEEPING`: colour force, energy totals, imposed inertia, and gravity use separate paths. FTD-0401 identified the raw/c=1 mismatch. FTD-0402 corrects the live implementation with one raw causal map, explicit `M_INERTIAL`/`E_REST`/`M_GRAVITATIONAL` roles, normalized energy–momentum diagnostics, and targeted CPU/GPU parity; its frozen verdict remains `PARTIAL`. FTD-0403 independently closes the exact changed-surface dependency contract, so `§12-cnorm` is closed and NCEMC is admissible under a separate lock. No framework type, common stress–energy source, or mass derivation is added.
