...
|---|---|---|---|
| **Postulates** (P1–P5) | Discrete space, discrete time, ternary states, Moore locality, determinism | `[AXIOM]` | **Generative** — they define the dynamics. Frozen; hundreds of documents and the results at FTD-0243 / FTD-0253 quantify over exactly these five. ⚠ **The Lean artifacts do not** (corrected 2026-07-24, Lean-validation audit): [`lean/Standalone.lean`](../../../lean/Standalone.lean) sets `Config := Nat`, an abstract index type, and in the [`lean/FtdNoGo/`](../../../lean/FtdNoGo/) rendering the postulate encoding (`Voxel`/`Fields`/`moore`/`Update`/`IsLocal`) is **never consumed by any proof** — every theorem is an instance of `commutator_zero_of_any_index`, which holds for an arbitrary index type. What Lean machine-checks is the algebra on a commutative carrier, not its faithfulness to P1–P5; that step is the `[DEFINITION]` modeling bridge (`lean/FtdNoGo/Postulates.lean` header). |
...
- **P1 — Discrete space.** A 3-dimensional cubic lattice of voxels that is **uncontained and has undefined boundary**: at every specified position, axis-adjacent sites exist. It is neither a bounded container nor a completed-infinity ℤ³ totality (`AUDIT_INFINITY_REFRAME.md`). Finitude belongs to specified or realized configurations and computations — their cloud/star-like extent or support — not to an outer wall. Finite computational windows are probes or quotients of the ontology; completed limits are not assumed.
...
- **P4 — Local causality.** Updates depend only on the 26-neighbour Moore neighborhood at the previous tick. The dependency hull advances by one coordinate step per axis per tick (Euclidean reach `1`, `sqrt(2)`, or `sqrt(3)` by direction); this is not identical to the selected flux-wave speed `1/sqrt(3)`.
...
1. **Genesis / evaporation (downward, J → s):** at a void site, `|J|` crossing the manifestation threshold projects a state (`s = sign`-rule); falling below the evaporation threshold reverts it. The kinetics scale is `K_MANIFEST`, the trigger `K_GENESIS = N_c·K_MANIFEST` (type-separated from the imposed mass calibration `M_INERTIAL=K_B`; FTD-0130/0250/0402).
...
**Nested pair — the symplectic quadratures (q, p) inside the flux layer.** The flux sector is a second-order field, so its true arena is phase space: per mode, the amplitude `q` and its rate `p = ∂_t J` (engine: `wave_vel`) form a symplectic pair. The substrate's **only native dynamical angle** is the quadrature phase `arg(q + ip)`. For an axis mode the default fully discrete frequency is `theta(k)=2 asin[C_WAVE sin(k/2)]`, with `C_WAVE=1/sqrt(3)` selected; the older `2 C_WAVE sin(k/2)` expression is spatial-only. Multi-tick winding matches the implemented eigenvalue to 0.10–0.98 % (modes n = 1, 2, 4), while the transverse spatial orientation `arg(J_x + iJ_y)` is dynamically **frozen** (leakage ≈ 1.6×10⁻¹⁶, machine zero) and the dual-substrate L/R channels stay an exact mirror (`|J_L − J_R| = 0`) ([`EXPLR_SUBSTRATE_NATIVE_ANGLE.md`](../06_reference_frames_and_measurement/EXPLR_SUBSTRATE_NATIVE_ANGLE.md), FTD-0251 `[MEASURED]`). The pair is symplectically non-trivial but observably commutative: `{q,p} ≠ 0` (Poisson) yet `[q,p] = 0` (observable commutator) — the Crucial Distinction of FTD-0243 §3. The quadrature **clock** is the substrate's native time-keeper (§5.2); identifying it with "the measurement angle" is interpretive `[SELECTION]`, and the *incompatibility* that would make choosing a quadrature a quantum measurement is exactly what FC-1 declines to import.
...
| Dual substrate L/R (chirality split) | `[IMPOSED]` — a parametrized split of `J`, not an independent field; the parity-violating coupling is hand-set | [`AUDIT_DUAL_SUBSTRATE_PROVENANCE.md`](../07_assessment/spine_gstar_cm_modular/AUDIT_DUAL_SUBSTRATE_PROVENANCE.md); FTD-0248 |
...
|---|---|---|
| `C_WAVE = 1/√3` (selected leading flux speed) | **[SELECTION] within exact stable interval `r²<=3/4`**; not the P4 dependency hull and not production CFL saturation | FTD-0407; `gauge_couplings.h` |
...
> **Correction — 2026-08-03/04, FTD-0791 / FTD-0802.** The α row above cites the FTD-0319 look-elsewhere scan ("sole dual-matcher among 2.65 M polynomials") as uniqueness evidence for `x₊ = 1/α`. That evidence has been withdrawn, refute-by-default and independently replicated. **Leg 1 sits exactly at its own chance base rate:** at the registered `2e-6` gate the null expects **1.42–1.67** matchers and the scan found **1** — a random target near 137 is matched to 2 ppm by this polynomial family **79% of the time**. **At that gate the second (dual-matcher) leg eliminates nothing** (1 → 1), so "dual-matcher" and "matcher" coincide there. FTD-0319 is retagged `[MEASURED]` → `[SELECTION]`; the designation **"the framework's single scan-rigid identification" is withdrawn, and the scan-rigid count is now zero.** FTD-0802 found the same defect in OT-3.3, the separate 2,871,576-polynomial extended scan (`N_null = 0.0014` dual-matchers expected under displaced targets, Outcome B — a zero count is exactly what chance predicts, discriminating nothing), and separately found OT-3.3's published "0 genuinely-new cubic dual-matchers" figure was itself wrong — a hardcoded literal string no code tested; the corrected runner reports 4 non-master dual-matchers. **`x₊ = 1/α` therefore retains no numerical-uniqueness support of any kind** — only the two structural legs (OT-1.9, OT-1.5) remain. The 1.26 ppm arithmetic agreement itself is unchanged and stays `[NUMERICAL FACT]`; only the uniqueness-scan evidentiary leg is withdrawn. This correction moves no tag in this document — `x₊ = 1/α` stays `[STRONGLY MOTIVATED CONJECTURE]`.
...
- **Default gauge: electron-primary** (FTD-0137 §4.5, [`FOUND_ELECTRON_PRIMARY_GAUGE.md`](../02_foundations/FOUND_ELECTRON_PRIMARY_GAUGE.md)) — import `{ℏ, c, m_e}`; the single beyond-universal anchor is `m_e`, and `a_phys = ℓ_P = ƛ_C·Kα¹¹` is computed **conditionally on the selected FTD-0015 mass relation** (≈ Planck length to 0.19%; not an independent derivation). FTD-0059/0096 forbid deriving length/mass from Axiom Zero *alone*, so one import is irreducible; electron-primary makes it the measurable `m_e` and conditionally maps `ℓ_P`/`G` from it. Legacy Planck-primary declares `a_phys ≡ ℓ_P` exactly instead (a valid gauge).
- `M_INERTIAL = m_e = K_B` (imposed calibration), `E_REST=M_INERTIAL·C_SPEED²=K_B/3` in raw lattice units, and separately named `M_GRAVITATIONAL=K_B` (imposed numerical equality, not a common native origin). FTD-0402 implements these roles. FTD-0406 additionally routes its selected strong-pair `T00` into CPU latency as mass-equivalent density `T00/C_SPEED²`; this is a scoped selected contract, not a derivation of either mass role. Also `t_phys = ℓ_P/(√3·c)` (= t_P/√3 ≈ 3.11×10⁻⁴⁴ s; derived from `a_phys` + `c_lat=1/√3`, corrected 2026-07-08 from `√3·ℓ_P/c`).
...
|---|---|---|---|
| 1 | **Local dependency hull** | `[THEOREM]` | P4 bounds every update dependency to the Moore shell per tick. This does not select `C_WAVE=1/√3` and does not prove that manifestation, matter, gravity, and flux share one operational cone (FTD-0407). The historical front demo remains an engine measurement of its protocol. |
| 2 | **Geometric Coulomb (Phase G)** | `[THEOREM]` | `α_r(r, L) = 2·r·G_L(r)` — the emergent static potential *is* the periodic lattice Poisson Green's function, zero free parameters; R² = 1.0000 at L = 384, median 0.07 % residual in the tail ([`DERIV_EMERGENT_COULOMB_GEOMETRIC.md`](../10_eft_program/derivations/alpha_readout_programme/DERIV_EMERGENT_COULOMB_GEOMETRIC.md), FTD-0004) |
| 3 | **Spatial isotropy recovery; Lorentz recovery open** | `[MEASURED]` + `[SCOPED NO-GO/OPEN]` | directional exponent `p = 4.0008 ± 0.0006` is valid for spatial rotations. The fully discrete pole has an earlier isotropic dimension-six boost term; current stable update cannot cancel it, and common-cone/radiative gates remain open ([`AUDIT_LORENTZ_RECOVERY_HARD.md`](../07_assessment/lorentz_recovery_causal_structure/AUDIT_LORENTZ_RECOVERY_HARD.md), FTD-0407). |
| 4 | **Time dilation / γ-emergence** | `[MEASURED, scoped]` + `[AXIOM]` + `[OPEN]` | the independent wave-clock result normalizes `v=v_g/C_WAVE` and never reads `voxel.tau`: departure from exact γ vanishes as `R ∝ L⁻¹·⁹⁸ ≈ L⁻²` (∝ k²) on ⟨100⟩ at v ≲ 0.85; ultra-relativistic diagonals remain `[OPEN]`. **FTD-0402/0403:** live matter-clock/BI/force/movement paths share raw `u`, `β²=|u|²/C_SPEED²`, and `B=β²+L²`, with the frozen changed surface closed by exact targeted CPU/GPU, golden, WASM/web, and compatibility gates. This is implementation of the clock hypothesis `[AXIOM]`, not a covariance theorem. |
...
| 7 | **Cluster transport inertia = N·M_INERTIAL** | `[IMPOSED]`; reduction `[OPEN]` | `M_INERTIAL=K_B` is the explicit imposed role. The CI-5 cancellation is conditional engine arithmetic; numerical equality to separately named `M_GRAVITATIONAL` is not a derived equivalence principle (FTD-0250/0402). |
...
| 9 | **Confinement signature** | `[MEASURED at an inserted coupling [SELECTION]]` | Wilson-loop area law with string tension σ ≈ 0.209 at β = x₋ (the coupling insertion is a selection, not a derivation) ([`DERIV_YANG_MILLS_CONFINEMENT.md`](../03_derivations/DERIV_YANG_MILLS_CONFINEMENT.md); LEDGER FTD-0025 — the engine `benchmark_wilson_loops` battery is historical, deleted 2026-05-03 `e8eb8e82`) |
| 10 | **Classical electrodynamics suite** | `[THEOREM/DERIVED/OPEN]` | auxiliary retarded/static identity plus native resolvent (FTD-0113/0558), Bianchi identities (FTD-0114), corrected selected-drive pole and positive speed floor (FTD-0115/0558); native Larmor/Cherenkov power remains open after FTD-0120 correction |
...
| 14 | **Bell bound** | `[THEOREM + MEASURED]` | substrate S ≤ 2 (FTD-0243 + Bell's theorem); engine S ≈ 1.95–2.00; ternary detection loophole gives S ≈ 3.6 at ~49 % efficiency — a known artifact, not a violation ([`AUDIT_BELL_ANALYSIS.md`](../07_assessment/quantum_foundations/AUDIT_BELL_ANALYSIS.md)) |
...
*(Physical-scale honesty: under the §3.3 calibration, the direct free-flux PL-4/PL-5 corrections are Planck-suppressed. Lower-dimensional radiative coefficients and inter-sector speed differences are not thereby suppressed; FTD-0407 LR-2..LR-5 are required before a whole-theory laboratory claim.)*
