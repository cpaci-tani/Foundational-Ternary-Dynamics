...
- `M_REST = m_e` (an imposed fused scalar formerly carrying the rest/inertial/gravitational labels; FTD-0400/0401 prove the common energy–momentum and `c_lat²` role conversion remain open) and `t_phys = ℓ_P/(√3·c)` (= t_P/√3 ≈ 3.11×10⁻⁴⁴ s; derived from `a_phys` + `c_lat=1/√3`, corrected 2026-07-08 from `√3·ℓ_P/c`).
...
| 4 | **Time dilation / γ-emergence** | `[MEASURED, scoped]` + `[OPEN]` | the independent wave-clock result normalizes `v=v_g/C_WAVE` and never reads `voxel.tau`: departure from exact γ vanishes as `R ∝ L⁻¹·⁹⁸ ≈ L⁻²` (∝ k²) on ⟨100⟩ at v ≲ 0.85; ultra-relativistic diagonals remain `[OPEN]`. **FTD-0401 guard:** the live matter-clock/BI path passes raw nodes/tick to a legacy `c=1` formula and is not licensed by this row. Clock hypothesis stays `[AXIOM]` ([`ANALYSIS_DYNAMICAL_TIME_DILATION.md`](../03_derivations/foundational_mechanics/ANALYSIS_DYNAMICAL_TIME_DILATION.md), FTD-0252) |
...
**Joint-reading guard (FTD-0400/0401, 2026-07-21):** rows 4, 7, 9, and 12 cannot currently be joined into one covariant energy–momentum account. FTD-0400 finds `SPLIT-BOOKKEEPING`: colour force, energy totals, imposed inertia, and gravity use separate paths ([`AUDIT_CONFINEMENT_ENERGY_GRAVITY_BRIDGE.md`](../07_assessment/AUDIT_CONFINEMENT_ENERGY_GRAVITY_BRIDGE.md)). FTD-0401 finds `UNMAPPED-DUAL-NORMALIZATION`: raw nodes/tick velocity enters both `C_SPEED=1/√3` transport and `c=1` matter-clock/BI formulas, while one unconverted `M_REST` scalar fills rest-energy, inertia, and gravity roles ([`AUDIT_C_SPEED_MASS_NORMALIZATION.md`](../07_assessment/AUDIT_C_SPEED_MASS_NORMALIZATION.md)). Resolve the causal-normalization/mass-role gate before NCEMC-1–4 or any mass readout. No framework type is added.
