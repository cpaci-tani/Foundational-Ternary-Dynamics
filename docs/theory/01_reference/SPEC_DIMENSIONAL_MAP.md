...
> **Freshness note (2026-08-13):** two later developments exist and change no entry in this map. (1) FTD-0825, `SPEC_FTD_FRAMEWORK_V2_CONTEXTUAL_ACTUALIZATION.md` ([RATIFIED SUCCESSOR CONSTITUTION], 2026-08-09), adopts a noncommutative-potentiality-net alternative to v1's FC-1 (declined measurement map M) — on the v2 branch only, with its own currency tracked separately in `contextual_actualization_register_v2.json`; v1, the branch this map describes, is unmodified and remains primary. (2) The FTD-0899–FTD-0999 native-time-carrier / clock-carrier / temporal-interior programme (independently audited 2026-08-13; fullest accounts at LEDGER.md search "FTD-0899" through "FTD-0999" and `SPEC_DOCTRINE_LEDGER.md` §2.1) is a direct attack on CLK-1 but leaves CLK-1/CLK-2 exactly where they were (AXIOM / IMPOSED) and never re-derives or fits G* or any other spine constant. All 7 spine theorems (§2), all 4 dimensionless predictions (§3), and all 3 calibration declarations (§4) below stand as they were under both developments.
...
- **Notes:** Polynomial-level theorem (x₊ is a root of a number-theoretic polynomial); the physical identification x₊ = 1/α is [STRONGLY MOTIVATED CONJECTURE] (downgraded from THEOREM) because it depends on polynomial-template uniqueness (FTD-0319 adversarial scan, formerly cited as FTD-0189: 0 non-G* dual-matchers across 2.65M degree-2 polynomials over an 18-constant FTD-undesigned basket; treat as [NUMERICAL FACT] — the historical '~4×10^5 Bayes' figure is retracted per the spine audit, runner yields only ~19x scan-size) + CM-curve uniqueness rather than a derivation chain to QED. WARNING - three withdrawals bear on this row, in date order. (1) FTD-0791 (2026-08-03): the polynomial-template-uniqueness leg is WITHDRAWN — the FTD-0319 scan sits at the chance base rate (null expects 1.42-1.67 matchers, 1 found) and is retagged [MEASURED] -> [SELECTION]; the scan-rigid count is zero. (2) FTD-0802 (2026-08-04): the second uniqueness cite also fell — OT-3.3, the separate 2,871,576-polynomial extended scan, failed its own pre-registered base-rate control (N_null = 0.0014, Outcome B) and its published count was wrong (4 non-master dual-matchers, not the reported 0); retagged [SELECTION]. (3) FTD-0321 (2026-08-04): took OT-1.9's dual-match reading off the table — the d=-4 privilege breaks by |d| <= 500,000 with 1,271 dual-matchers, so what survives of OT-1.9 carries no dual-match content. The [SMC] tag on x+ = 1/alpha is unchanged, but as of all three withdrawals it retains NO numerical-uniqueness support of any kind; remaining support is OT-1.9's non-dual-match unit-group-arithmetic content plus OT-1.5 (see TRACKER_ONTIC_TRUTH.md OT-5.1). (Historical formulation referenced 'dual-match' with x_- ↔ N_c; the x_- ↔ N_c identification is RETIRED per v1.4 §5 and the polynomial-template-uniqueness fact is independent of the historical target.) FTD-0097 look-elsewhere scan confirms the master quadratic's polynomial-root layer lives outside the monomial scan space and is unaffected by the over-richness verdict.
...
|---|---|
| FTD-0001 | `alpha_inverse`, `cm_curve_uniqueness`, `master_quadratic` |
