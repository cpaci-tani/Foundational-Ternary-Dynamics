...
**Cross-reference (FTD-0835).** Because this term is `s`-independent, a literal vacuum voxel carries `H_BI = E_REST ≠ 0` (confirmed in `engine/tests/test_born_infeld.cpp`, "H at rest = E_REST"). This does *not* reopen the cosmological-constant question in `DERIV_LAMBDA_SCALE_COVARIANT.md` §1: the canonical gravity-sourcing tensor (EFE-4, `DERIV_EINSTEIN_FIELD_EQUATIONS.md` §2.2) is derived from the free-field Lagrangian alone and structurally excludes this term; the Born-Infeld upgrade to `T_{\mu\nu}` remains an undeployed `[SELECTION]` (§2.4 of that document). `DERIV_LAMBDA_SCALE_COVARIANT.md` §1 previously cited a false `s`-gating claim for this same conclusion; that citation is corrected there.
...
**Cross-reference (2026-08-13, FTD-1000):** the "independent clock/bandwidth hypothesis" cited above is CLK-1 (LEDGER FTD-0208 v3, `[CLOSED NEGATIVE]` for substrate-derivability). As of this date CLK-1 is no longer tracked as a free-floating axiom — it is folded into `SPEC_FTD_FRAMEWORK_V1.md` §5.2 (FC-2, metric half) as that commitment's per-worldline proper-time clause, explicitly disambiguated there from the global substrate tick (P2, unconditional, forced) and from the separate native-time-carrier programme (FTD-0899–FTD-0999). This engine-level section's content (the $d\tau/dt$ implementation and its FTD-0402 parity gates) is unaffected — the tag stays `[AXIOM]`; only its constitutional home moved.
