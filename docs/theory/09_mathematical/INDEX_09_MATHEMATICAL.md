...
| [EXPLR_STENCIL_SPECTRUM.md](number_theory/EXPLR_STENCIL_SPECTRUM.md) | [NUMERICAL FACT — B0 complete; FTD-0372] | **(Clause-3 program B0 — COMPLETE)** The arithmetic of the spec's lattice symbols: BCC exact CM τ=i; SC Γ(1/24)-class (order 3); FCC Γ(1/3)-class (order 6); **the engine's default 18-pt (SC+FCC)/2 Green's function is now charted (FTD-0372): minimal ODE order 4, degree 12**, reconstructed from 171 exact moments (modular rank, two primes, surplus 90; SC-validated). Every local exponent in ½ℤ, no MUM point ⇒ symmetric-power/√-twist of an order-2 elliptic operator (not strict Calabi–Yau) — W₁₈ plausibly Γ-quotient. Between SC (order 3) and FCC (order 6). P3(a) closed; P3(b) = the factorization (CAS). |
