...
| [THEOREM_EXPONENTIAL_LATTICE_PERIODS_TRANSCENDENCE.md](number_theory/THEOREM_EXPONENTIAL_LATTICE_PERIODS_TRANSCENDENCE.md) | [THEOREM — assembled] — FTD-0378 | **Exponential lattice periods transcendental** (SC: e^{−τ}I₀(τ/3)³ via Siegel–Shidlovskii, value trdeg exactly 3; BCC: linear S–S route, SO₄ hazard recorded): E2's individual-transcendence floor CLOSED; E2-full priced behind the (e,π) wall / one F–J exp-period-conjecture instance. Corrects FTD-0376. |
| [THEOREM_WATSON_SC_TRANSCENDENCE.md](number_theory/THEOREM_WATSON_SC_TRANSCENDENCE.md) | [THEOREM — external, assembled] — FTD-0377 | **{π, W_SC} algebraically independent:** W_S = (3√6/2)Ω₋₂₄/π² exactly (disc −24 Chowla–Selberg; sin-product = 1/16) + Chudnovsky ⇒ W_SC transcendental; all three Watson constants per-constant closed; E1 wall = multi-curve Chudnovsky (< Rohrlich–Lang), marker Bertrand 1997. Corrects FTD-0376. Verifier 11/11. |
| [ANALYSIS_E1_E2_TRANSCENDENCE_SOTA.md](number_theory/ANALYSIS_E1_E2_TRANSCENDENCE_SOTA.md) | [SYNTHESIS] — FTD-0376 | **E1/E2 transcendence SOTA + boundary price:** E1 = a case of Rohrlich–Lang/GPC (proven floor trdeg 2, conjectural 4; Γ(1/24) not even individually transcendental), E2 = a case of the Fresán–Jossen exponential period conjecture; SC Watson const verified = ℚ(ζ₂₄) Γ-product (multiplicatively non-reducible, h≤10⁶). δ∉N priced = conditional on Chudnovsky (proven) + two named open conjectures. Solves neither. |
...
| [EXPLR_DYADIC_TRIGONAL_RELAY_FAMILY.md](general_math/EXPLR_DYADIC_TRIGONAL_RELAY_FAMILY.md) | [THEOREM] / [OPEN] | Family-level proof of the dyadic three-phase barycenter and alternating-chiral trigonal relay; keeps the `C_3` singularity-budget split open rather than promoted. |
| [EXPLR_DYADIC_LIFT_DESINGULARIZER.md](general_math/EXPLR_DYADIC_LIFT_DESINGULARIZER.md) | [THEOREM] / [EXPLORATORY] | Fundamental quadrature lift `z=sin(t)` embeds every finite dyadic carrier with nonzero `a_0` as an unknotted space curve; interprets visible plane nodes as readout loss, without changing the projective singularity question. |
| [EXPLR_DYADIC_OCTAVE_BIFURCATION_ATLAS.md](general_math/EXPLR_DYADIC_OCTAVE_BIFURCATION_ATLAS.md) | [THEOREM] / [OPEN] | Exact one-parameter octave-8 amplitude atlas: axis-seed chambers, tangent-axis thresholds, and all regularity-loss values for `(1,1/2,1/2,q)`; the all-node and turning-number atlas remains open. |
| [EXPLR_DYADIC_CUBIC_MASTER_QUADRATIC_BOUNDARY.md](general_math/EXPLR_DYADIC_CUBIC_MASTER_QUADRATIC_BOUNDARY.md) | [THEOREM] / [OPEN] | Exact direct-bridge boundary between the octave-8 regularity cubic and the master quadratic: degree, root-field type, Vieta structure, and a natural dyadic counter-slice differ; only a closure-polynomial analogy remains. |
| [EXPLR_DYADIC_GEOMETRIC_BYTE.md](general_math/EXPLR_DYADIC_GEOMETRIC_BYTE.md) | [THEOREM] / [EXPLORATORY] | Eight enable flags form a 256-state support byte; eight signed mode states form 6,561 ternary words, with exact balanced-ternary fingerprints and `2^h` chirality fibers over weight-`h` support masks. |
