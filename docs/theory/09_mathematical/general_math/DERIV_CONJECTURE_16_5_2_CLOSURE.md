...
is required to admit a *real ordered root pair* (x₊, x₋) with x₊ > x₋ > 0. This is a necessary condition before any admissibility criteria of Theorem 17.5 can be evaluated: Theorem 17.5 criterion (iii) (non-degeneracy; discriminant analysis) presupposes a real discriminant, and the physical identification x₊ = α⁻¹ ≈ 137 (FTD-0013 [SMC]) requires real roots. For P to have real roots, its coefficients must be real:
...
- the physical identification `x₊ = α⁻¹` (FTD-0013), which requires real roots.
