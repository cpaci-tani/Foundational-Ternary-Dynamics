...
**This document IS:** a map of one external mathematics paper — Clay Córdova, Ben Heidenreich, Alexandr Popolitov, Shamil Shakirov, *"Orbifolds and Exact Solutions of Strongly-Coupled Matrix Models"*, **Commun. Math. Phys. 361 (2018) 1235–1274**, DOI `10.1007/s00220-017-3072-x`, **arXiv:1611.03142** ("CHPS"; section/equation numbers below refer to arXiv v1 — the only arXiv version — which is the OSTI 1537659 copy) — together with the exactly-verified statements connecting its results to the field ℚ(G\*) at the center of FTD's algebraic spine. The load-bearing observation: the simplest *reflection-positive* strongly-interacting zero-dimensional QFT — the monomial quartic one-matrix model, S = Tr X⁴ (the cubic is lower-degree but positive on no contour; §6 RQ-MM-1) — has observables valued in ℚ(G\*), and CHPS in effect single out Γ(1/4)/Γ(3/4) ≈ 2.958675 as the model's irreducible transcendental (their §3.1, discussion below eq 3.7: transcendence plus "no obvious further simplifications"; the phrase "irreducible transcendental" is this document's gloss). This de-orphans G\*: the constant an ontology-first construction forces into centrality is also the value field of the observables of a mainstream exactly-solvable ensemble.
...
| 4 | **Race-constant family realized:** the N=1 conjugate-sector amplitude ratio of the Tr Xʳ model equals **R_r = Γ(1/r)/Γ(1−1/r)** for r = 3…8 — the family of `MATH_FAMILY_OF_RACES.md`, with R₄ = G\* [EXTERNAL — immediate corollary of CHPS Thm 1] | C4 |
...
CHPS's own words for row 1's consequence (their §3.1, quoted verbatim): after reducing Γ((2k+1)/4)-values, "since Γ(1/4)/Γ(3/4) ≃ 2.958675 is trancendental [sic], there are no obvious further simplifications" — **the paper in effect names G\* as the model's irreducible transcendental** (the label is ours; the transcendence-plus-no-further-reduction content is theirs). [EXTERNAL]
...
## §6 — Research questions (registered and resolved same-day 2026-07-04; falsifiers here, not in the tracker)
...
- **A0 (literature scan, 2026-07-04, two queries).** No q-analog of Watson integrals or of finite-L lattice Green's functions **was found** (a two-query scan supports "none found", not a universal negative); the lattice-Green's-function corpus (Watson, van Peype, Guttmann's all-dimensions program) lives in elliptic-integral/hypergeometric language, and the finite trigonometric sums the finite-L objects reduce to are evaluated by roots-of-unity/contour methods — a different toolkit from Γ_q. Not literature-answered.
...
**Verdict: CLOSED — gate-out at G-A, zero q-evaluations performed.** [STRUCTURAL OBSERVATION — the closure is the deliverable] The positive residue is the structural reason itself: *multiplicative/modular discreteness and FTD's uniform-toroidal discreteness are different mathematical kinds as attached to L* — the q-world's root-of-unity limit indexes symmetry sectors while FTD's L indexes volume, so "q-deform FTD's finite-L objects" was a category temptation, not a research program. **Steelman recorded (2026-07-04 adversarial review):** the G\*-world *does* carry a canonical non-tunable modular nome — the lemniscatic q = e^{−π} at the CM point, with θ₃(e^{−π}) = π^{1/4}/Γ(3/4) and hence G\* = √(2π)·θ₃(e^{−π})² exactly (verified to 40 digits; the theta-nullwert reading already registered under FTD-0132). This does **not** re-open the gate: it attaches to the *continuum* Watson/CM object, not to any finite-L candidate, and it is a fixed-nome theta representation, not a Jackson q-deformation family — Q-a and Q-c fail for it on the gate's own terms. It is recorded so no future session mistakes it for the missing q-map. **Re-open condition (not a tracker item):** a *derivation* — not a choice — of a multiplicative structure on an FTD-native finite-L object (e.g., an exact self-similar L → 2L relation with geometric weights) would create the missing forced q-map and would re-enter through a fresh pre-registration per the original gate design.
