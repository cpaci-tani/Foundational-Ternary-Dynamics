...
We recall only the properties directly relevant to this document.
...
| Physical role (FTD) | Domain A (physics) | Measurement interface | Domain B (reference frame context) |
...
**The heavy zero.** In the crossed product $\hat{\mathcal{M}} = \mathcal{M} \rtimes_\sigma \mathbb{R}$, the projection onto the vacuum spectrum $P_0 = E_{(-\infty, 0]}(K)$ has $\hat{\tau}(P_0) = \infty$. The vacuum below the manifestation threshold is not empty — it is an infinitely degenerate reservoir. One may *speculate* [CONJECTURE] that this infinite vacuum trace connects to the cosmological constant problem (the dark-energy density as a ratio of the finite physical trace to the infinite vacuum trace) — but this is not an FTD source for Λ: the canonical position is that FTD's classical vacuum is zero-energy (FC-1) and predicts $\Lambda=0$, with the small-Λ source [OPEN] and its value a [BOUNDARY] (FTD-0331). This remark is a candidate, not a resolution.
...
3. **Period-doubling onset $\leftrightarrow$ hyperfinite construction.** The existing Mandelbrot-Factor correspondence identifies the Mandelbrot cusp $c = 1/4$ as the Type II$_1$ locus via the period-doubling cascade:
...
The projection hierarchy from [FOUND_THE_EXISTENCE_FILTER.md](../../06_reference_frames_and_measurement/FOUND_THE_EXISTENCE_FILTER.md):
...
Open question VN-O1 asks: "Can the collapse map $\Phi$ be made rigorous?"
...
### 5.4 Resolution of Open Questions
...
- **RT-O1 (Non-abelian algebra classification)** & **RT-O6 (MASA physical outcomes)** — **[CLOSED DECLINED]** (FC-1).
  Under Framework Commitment 1 (FC-1), the commutative substrate algebra $A_5$ is complete and standard quantum mechanics (specifically non-commutative Hilbert space and measurement-map import $M$) is not a benchmark to be recovered. Thus, constructing and classifying non-abelian operator algebras or proving that they recover standard QM measurement outcomes is declined.
- **RT-O2 (Wilsonian RG vs Connes flow)** — **[CLOSED DECLINED]** (FC-1).
  Since standard QFT is not a benchmark, establishing a formal equivalence between FTD's numerical scale flow and the Connes flow of weights is declined.
- **RT-O3 (Jones index)** — **[CLOSED RECLASSIFIED]** (FTD-0242).
  The index of the inclusion is a purely mathematical property of the critical Jordan block and does not govern any physical coupling under the dynamic-alpha pivot.
- **RT-O4 (Warning RT-W1 Resolution)** — **[CLOSED RESOLVED]**.
  Addressed by the three-step descent chain (§2.6–2.8): crossed product + tensor decomposition + MASA selection.
- **RT-O5 (Reference frame context parameter temperature)** — **[CLOSED RECLASSIFIED]** (FTD-0242).
  The reference frame context parameter $\lambda(k) \approx 0.400$ is a structural constant of the substrate, not a physical temperature governing dynamic couplings.
...
| [FOUND_THE_EXISTENCE_FILTER.md](../../06_reference_frames_and_measurement/FOUND_THE_EXISTENCE_FILTER.md) | Projection hierarchy $E \to |\cdot| \to |\cdot|^2 \to \Phi$, modular conjugation $J$ |
| [../06_reference_frames_and_measurement/FOUND_DOMAIN_PARTITION_AND_CONTEXT_SELECTION.md](../../06_reference_frames_and_measurement/FOUND_DOMAIN_PARTITION_AND_CONTEXT_SELECTION.md) | Domain A/B/C partition, phase angle source map, and vocabulary discipline |
