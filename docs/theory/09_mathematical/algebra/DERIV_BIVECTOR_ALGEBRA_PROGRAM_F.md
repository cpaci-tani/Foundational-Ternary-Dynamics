...
**Status:** Single empirical campaign (Program F-prime / F-double-prime / Path 1) testing whether FTD flux carries a $Cl(3,0)$ bivector subalgebra. Per-stage tags: §1 Program F-prime (FTD-0086) — **[MEASURED] [STRONG POSITIVE]** (plaquette-bivector matching signature, 3/3 pairs, 40× signal/off-axis). §2 Program F-double-prime (FTD-0087) — **[MEASURED] [PARTIAL]** (1/3 closure tests pass; iterated $\mathfrak{su}(2)$ closure fails at 4-injection scale). §3 Path 1 multi-grade decomposition (FTD-0088) — **[MEASURED] [POSITIVE]** (12/12 grade-structure tests pass at 2-injection order; F-double-prime failure reinterpreted as dynamical noise).
**⚠ 2026-07-10 update (FTD-0380):** §3's noise reinterpretation was tested by its own §3.5.3 prescription — the pre-registered noise-controlled re-test (`PREREG_VERTEX_DK_CLOSURE_v1.md` §3 → `ANALYSIS_VERTEX_DK_CLOSURE_v1.md` §2: L ∈ {8,16,32}, A ∈ {1,3,10}, instant vs time-averaged readouts, 16 seeds; baseline qualitative closure outcome replicated — 0/3, negative ε signs — though the A=10 magnitudes have drifted since FTD-0087, so the refutation is measured on today's engine) and **REFUTED**: verdict CLOSURE-ROBUST-FAIL, 0/18 cells close, no criterion-level trend in either control direction. The non-closure is **structural**, not noise. §2's original reading stands, strengthened; §2.5 Path 2 (quantify the approximate closure) is what remains of the native branch; Path 3 (Branch-B selection constrained by the matching signature) is the settled accounting. The §1 matching signature itself is unaffected (re-confirmed by the FTD-0380 baseline). Read §3.4.2/§3.4.3/§3.5.3 through this update. **Additional flag:** the FTD-0088/multigrade toggle set is reported invalid by current `TermToggles` validation (three phases lack prerequisites, warning-only) — see `ANALYSIS_VERTEX_DK_CLOSURE_v1.md` §1.4; an audit of the Program-F effective toggle sets is queued.
**Net (superseding the line below):** the $Cl(3,0)$ skeleton is internally consistent at 2-injection leading order as a *kinematic* structure; the closed-Lie-algebra question is **[CLOSED NEGATIVE at accessible protocols]** (FTD-0380), and the dynamical-DK question likewise (FTD-0379). No tag promotions beyond what each per-stage row carries.
