...
- **The SC constant is worse `[OPEN]`:** Γ(1/24)-class values are **not even individually proven transcendental** — the Fermat-curve genus > 1 / higher-dimensional CM abelian-variety obstruction (the same wall that leaves Γ(1/5) open). The operative criterion is the *dimension* of the relevant CM abelian-variety factor (dim 1 = tractable), not literal Fermat genus.
...
**Verdict:** `[SYNTHESIS]` E1 admits **no unconditional progress above the trdeg-2 floor**; its full content coincides with the open Rohrlich–Lang / GPC. It is not partially closable in the sense the boundary would need.
...
- **Individual transcendence over ℚ(π, Γ(1/4)) is `[OPEN]`.** No Nesterenko-type result reaches these: those methods reach only *same-field* exponentials (eᵖ jointly with Γ(1/4)), not general lattice exponential periods. `[SYNTHESIS]`
...
- **One unconditional handle exists in principle `[OPEN — tractable sub-question]`:** `dim G_M` is an *unconditional upper bound* on the trdeg (Fresán–Jossen). If the exponential motive underlying H_σ(τ) can be identified and its `dim G_M` computed, that bound might separate H_σ(τ) from {π, Γ(1/4)} *without* the conjecture — a genuine, if hard, line of attack.

**Verdict:** `[SYNTHESIS]` E2 is further out than E1: even individual transcendence is open, and it is gated behind the exponential period conjecture.
