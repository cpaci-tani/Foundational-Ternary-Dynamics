...
The forward-looking proposal sketch lives in a separate doc — [`PROPOSAL_OBSERVER_OPERATOR_EXTENSION.md`](PROPOSAL_OBSERVER_OPERATOR_EXTENSION.md) — and is **not** consolidated here; it remains a standalone forward-looking document.
...
- [`PROPOSAL_OBSERVER_OPERATOR_EXTENSION.md`](PROPOSAL_OBSERVER_OPERATOR_EXTENSION.md) — the forward-looking proposal whose §4 prediction $c = \delta/(3\pi G^*)$ §A confirms and §B extends; §5.3 chicken-and-egg with the t-scale map. **Not consolidated here; remains a separate doc.**
...
- [`EXPLR_TOWER_MULTIPLIER_UNIQUENESS.md`](EXPLR_TOWER_MULTIPLIER_UNIQUENESS.md) — the $(m, k) = (2, 4) \Rightarrow 16$ uniqueness scan that addresses §C.9 Q2.
