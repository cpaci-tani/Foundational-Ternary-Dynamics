...
[`REF_REFLEXIVITY_VOCABULARY.md`](../01_reference/REF_REFLEXIVITY_VOCABULARY.md) (2026-05-01) explicitly disambiguates "observer." The relevant entries:

| Vocabulary term | Meaning | Operator-theoretic content |
|---|---|---|
| **Reflexive site** | A voxel/region instantiating reflexivity (canonical: 27-block center) | Spatial location; not an operator |
| **Reflexive structure** | The configuration that gives reflexivity (3³ block + Moore-26) | Combinatorial; not an operator |
| **Reflexive projection** | Operator mapping a reflexive structure's full state to its self-readout | **Operator** — corresponds to observable-algebra restriction |
| **Reflexive readout** | Output of a reflexive projection | The "observed" value |
| **Reflexive eigenmode** | Eigenstate of the reflexive projection | The master quadratic spectrum $\{x_+, x_-\}$ are *hypothesized* to be reflexive eigenmodes (MC-T4.3) |
| **Observation layer** | A subalgebra of physical observables | **Operator-algebra restriction** |
| **Agentic system** | A reflexive structure realizing agency in dynamics | Different from observer in the measurement sense |
...
| Item | Statement | Tag |
|---|---|---|
| OOE-1 | "Observer" in FTD has 3+ distinct readings; the QED-running-relevant one is observation-layer (trace-out over fermions) | [REFERENCE] (per REF_REFLEXIVITY_VOCABULARY) |
...
- [`MATH_LOG_GSTAR_IDENTITY.md`](MATH_LOG_GSTAR_IDENTITY.md) §3.5 — Beilinson-conjecture framing of $G^*$ as a regulator-irreducible-sector generating function; the right framework for understanding what "extending the operator stack" actually means structurally.
