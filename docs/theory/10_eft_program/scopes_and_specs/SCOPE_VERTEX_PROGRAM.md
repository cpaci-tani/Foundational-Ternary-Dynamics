...
**Unpriced companion selection (flagged, not silently absorbed):** the vertex's remaining physical content — the gauge-connection identification **A_μ = 𝒫_T J_μ** (the substrate's transverse flux *is* the U(1) connection the imported fermion minimally couples to), [SELECTION] per `SPEC_WILSON_DIRAC_FTD.md` §6 — has **no priced row** in `import_ledger.json` (checked: §3.2 carries exactly D=3, singlet, ℭ). It is hereby flagged as a **selected-type candidate row for the next import-ledger revision**, with the natural falsifier "an alternative flux-to-connection map producing inequivalent vertex phenomenology at matched protocol." Until priced, V2 results carry this selection as an explicit conditional.
