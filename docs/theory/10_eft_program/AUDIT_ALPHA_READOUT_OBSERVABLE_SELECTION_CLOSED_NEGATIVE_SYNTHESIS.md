...
| 7 -- reflexive projections | C4 (this doc §1.C) | CLOSED-NEGATIVE (§6 (c)) | Step 5 structural-match | Lindblad-style projection onto a public measurement channel yields characteristic equations whose coefficients are subalgebra dimensions and channel-rates -- finite-combinatorial quantities, not G\*-transcendentals |
...
### §1.C -- Reflexive projections (C4, catalog item 7)

**Catalog item 7 verbatim from pre-reg §4:** *"Projections of the finite observable algebra onto a 'public' measurement channel, per `REF_REFLEXIVITY_VOCABULARY.md` and the math-first ontology of `SPEC_MATH_FIRST_ONTOLOGY.md`."*

*Note: the reflexivity vocabulary doc was renamed during the 2026-05-22 corpus consolidation; the current reference is `docs/theory/01_reference/REF_REFLEXIVITY_VOCABULARY.md` or its successor. The 2026-05-01 sweep retired "consciousness" terminology in favour of "reflexivity = structural; agency = dynamical." For this verdict, the relevant content is the projection-onto-public-channel construction, which is Lindblad-style in standard open-quantum-systems terminology.*
...
4. The public channel `𝔄_public = span{J_α(k=0), Σ_x |J(x)|²}` is the Lindblad-style coarse-grained subalgebra. Derived by spatial averaging from the full algebra -- a standard mathematical operation (reflexive projection per item 7).
...
**Verdict at step 5 for C4:** the reflexive-projection T_O's characteristic equation has **dissipation-rate + dimension coefficients**, categorically distinct from the master quadratic's lemniscatic-curve coefficients. **CLOSED-NEGATIVE per §6 (c).**
...
- **Post-C4 count:** 14 (reflexive projections closed negative, this verdict).
...
- ARC-B1 primary catalog items 4 (plaquette bivectors -- C1, FTD-0204), 6 (boundary-to-boundary transfer -- this verdict §1.B), 7 (reflexive projections -- this verdict §1.C).
...
**Why all three primary catalog items closed negative at the same step.** The categorical structural mismatch is the same in each route: FTD-native lattice substrate arithmetic vs lemniscatic-curve arithmetic. The mismatch is not specific to bivectors / transfer / reflexive; it is structural at the level of *which mathematical category each side lives in*. This is the load-bearing finding -- and it is consistent across three independent attempts using three categorically distinct observable classes.
...
**What this verdict invites going forward.** If the user wishes to attempt closure within ARC-B1 by a variant catalog-item construction (e.g. a non-trivial subalgebra of bivectors that has not been tested, or a different boundary geometry, or a different reflexive-projection channel), a fresh §9 execution against the FTD-0198 v1 pre-reg can be run. **The categorical structural mismatch documented in §2 is the load-bearing obstruction that any such attempt must address** -- bridging lattice-substrate arithmetic to lemniscatic-curve arithmetic *without* importing M_N(t). The FTD-0122 BCC complex-structure theorem (Z[i]-module structure on Z[BCC] ⊗ ℚ) is a possible algebraic bridge that has not been operationalised as an observable readout; whether such an operationalisation is possible without firing F-j is an open question for any future Session C-variant.
