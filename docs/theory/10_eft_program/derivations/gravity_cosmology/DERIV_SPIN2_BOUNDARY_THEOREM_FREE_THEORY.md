...
>
> **PROVEN vs. ATTEMPTED, made explicit (per `AUDIT_BOUNDARY_MAP.md` §0.5, 2026-07-01):** despite the filename, only the **free-theory** clauses (C2-1, C2-2, C2-3 at free-theory + Gauss-only scope) are **PROVEN** in the structural-no-go sense — genuine group-theoretic and bubble-integral analytic-structure arguments, valid within the stated idealization (`s ≡ 0`, no interactions). The **canonical-toggle-set / real-substrate** extension of those same clauses is **ATTEMPTED**, not proven: it rests on a structural plausibility argument plus one empirical null measurement (FTD-0193, `[CLOSED NEGATIVE]`, 11/12 k-points at L∈{32,64}) — a result that raises the prior against an emergent spin-2 mode in the probed regime and proves nothing about L>64 or observables outside the §4 catalog. A reader citing this document for "FTD has a theorem that there is no graviton" is citing the free-theory idealization, not the physically interesting interacting case.
