...
- `FOUND_ALPHA_READOUT_QUANTIZATION_RESOLUTION.md` (ARC-C1, LEDGER **FTD-0216**)
- `FOUND_BCC_ALGEBRAIC_READOUT_RESOLUTION.md` (ARC-B2, LEDGER **FTD-0215**)
**Audited against:** the locked pre-regs `PREREG_ALPHA_READOUT_QUANTIZATION_v1.md` (FTD-0216) §4–§5 and `PREREG_ALPHA_READOUT_BCC_BRIDGE_v1.md` (FTD-0215); the project's own no-cheat audit `AUDIT_CHARGE_QUANTIZATION_NO_CHEAT.md` (FTD-0211); the honest companion `DERIV_BCC_ALGEBRAIC_READOUT.md` (FTD-0212); the contract `SPEC_ALPHA_READOUT_CONTRACT.md` §7.
...
2. Charge quantizes to `{−1, 0, +1}` as the topological winding index on `V_complex` — a clean, novel match to the ternary alphabet (FTD-0216 Thm 1).
...
4. The BCC self-energy is the Watson integral `G_BCC(0) = G*²/(2π)` (Spine Theorem; FTD-0215 Thm 3).
5. On any **finite block**, the readout **CLOSED-NEGATIVE** — a finite transfer operator has algebraic eigenvalues, not the transcendental `G*` (FTD-0215 Thm 2, correct and honest).
...
This is confirmed five independent ways (§2). The honest verdict per the locked pre-reg FTD-0216 §4 is **UNDERDETERMINED** ("a partial topological map is achieved, but the relation … is not fully derived"). **MC-T4.3 is therefore NOT resolved**; it remains a `[FOUNDATIONAL OBSTRUCTION]`.
...
| Charge quantization to `{−1,0,+1}` via winding index on `V_complex` | `[THEOREM]` | FTD-0216 Thm 1 |
| `16 = |μ₄|² = |Aut(E)|²` (coefficient 16, derived) | `[DERIVED]` | FTD-0212 §3 (honest) |
| BCC self-energy `G_BCC(0) = G*²/(2π)` (Watson) | `[THEOREM]` | FTD-0215 Thm 3 / Spine Thm 5 |
| Finite-block readout closes negative | `[CLOSED NEGATIVE]` | FTD-0215 Thm 2 |
...
|---|---|---|
| ARC-C1 — FTD-0216 | **FOUND (ARC-2)** | **UNDERDETERMINED** — genuine `[THEOREM]` topological-quantization + coefficient-16 map; the bridge to the full master quadratic / `x₊` is an admitted `[SELECTION]` (F-j fires per §3 admission) |
| ARC-B2 — FTD-0215 | **FOUND (ARC-2)** in `L→∞`; CLOSED-NEGATIVE finite | finite-block CLOSED-NEGATIVE **stands**; `L→∞` "FOUND" → **UNDERDETERMINED** — Watson self-energy `G*²/(2π)` is genuine, but `Det = 16G*³` (the third G\* power) is asserted, not derived |
...
1. **Downgrade the two verdict docs `FOUND → UNDERDETERMINED`** (FTD-0215, FTD-0216): retitle, fix §6 ("from first principles" → the honest partial statement), and correct the §5 "F-j PASS" to "F-j fires on the trace/determinant selection; verdict UNDERDETERMINED per §4." Preserve §1–§4 genuine content and the finite-block CLOSED-NEGATIVE.
2. **Align LEDGER** rows FTD-0215, FTD-0216 to `[UNDERDETERMINED]`, citing this review. Keep FTD-0212 at `[DERIVED]/[PARTIAL]` (already correct) and FTD-0211 (no-cheat audit) unchanged.
