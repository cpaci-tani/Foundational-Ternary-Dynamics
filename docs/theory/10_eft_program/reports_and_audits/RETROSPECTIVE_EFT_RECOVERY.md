...
Net: with A ruled out, C closed, and B open-but-not-classical, `g_c` remains `[OPEN]`. FTD's `α` is consequently an **algebraic prediction whose physical identification is `[SELECTION]`/`[STRONGLY MOTIVATED CONJECTURE]`**, not a dynamical derivation. The master quadratic's identification `x₊  1/α` (FTD-0013) lives in the motivic/algebraic structure — the Watson identity, CM curve periods, Moore-neighbourhood integers — not in the dynamical action.
...
- **Spatial anisotropy** (historical EFT-Recovery Pillar 3). The closed-form spatial-direction comparison gives phase-speed-spread exponent `p = 4.0008` (`R² = 1.000000`). FTD-0407 shows that this passed only the pre-registered **rotation** test: the fully discrete pole has an earlier isotropic dimension-six preferred-frame term. FTD-0408 supplies a slower-cone P4-local tree prototype; FTD-0409 closes the minimal fixed-`1/sqrt(3)` repairs but leaves general period-four/multi-state realization open. FTD-0410 shows that the Gauss AGM does not derive the live cone; its conditional unit-cone branch is excluded for every stable finite scalar kick period and now points to multi-state localization. FTD-0411–0413 supply a selected BCC-time/SC+FCC-space free cone shared with improved Wilson matter through q4; FTD-0414 converts its q6 mismatch into the explicit envelope `11(ka)^4/540`. Under either documented `a=ell_P` calibration the direct free-tree term is tiny. FTD-0415 proves that translations, spatial `O_h`, parity, CPT, and gauge symmetry still permit marginal preferred-frame kinetic ratios. FTD-0416 then imports the most optimistic anisotropic-QED continuum flow: the common cone is IR attractive, but simultaneous charge running limits suppression to a coupling-ratio power, no better than `1/137^3` under the stated perturbative assumptions. The selected `A=P_T J` map is spatially nonlocal. FTD-0417/0418 freeze a selected local unit-plaquette photon and one-tick Wilson regulator. FTD-0419 finds `delta_match/g²=-0.32696906(5)` in one complete off-shell step scheme. FTD-0421 proves the frozen production event algebra has no nontrivial additive charge over the preregistered native basis, so the native charged-pole/common-cone and native dimension-four-flow campaigns are dependency-closed without execution. FTD-0424 enforces one counterterm calibration and finds the existing off-shell surrogate fails at the first flavour-multiplicity threshold; the gauge-independent on-shell calculation remains open. FTD-0425 proves the free linear tick reversible but the production tick non-injective; manifested low-energy spectral unitarity remains open. The historical result is not a Lorentz-covariance pass. `[MEASURED]` / `[DERIVED, selected free-sector scope]` / `[CONDITIONAL PHENOMENOLOGY]` / `[EXTERNAL ONE-LOOP SURROGATE]` / `[SELECTED ONTOLOGY EXTENSION]` / `[SELECTED BRANCH-B REGULATOR]` / `[NUMERICAL FACT — scheme-specific]` / `[CLOSED NEGATIVE — frozen native current]` / `[OPEN — physical matching]`.
...
- **Wilson matter** (`SPEC_WILSON_DIRAC_FTD.md`, FTD-0126/0412/0413). Native FTD fermion emergence is closed-negative (FTD-0073/0076), so the sector is inserted Branch-B structure. FTD-0412 found that the historical campaign evolved spatial `D_W` as a real-time Hamiltonian and that its upper-only norm oracle was not the generic spectrum. The reported `a_e_lattice≈0.80` and relative error `684` are therefore **retracted as physical Wilson-Dirac results**. Real-time evolution now uses a Hermitian `H_W`. FTD-0413 adds a default-off SC+FCC face-diagonal kinetic prototype whose selected free pole shares the BCC-time flux cone through q4, but not q6; it is not a replacement orbit/g−2 result or an interacting common cone. The separate fact that a fixed classical magnetic background contains no photon loop remains a conceptual limitation, not a validation of the retired numbers.
...
The program **did not derive α dynamically.** It did not produce a nonlinear effective action. The central conjecture `x₊ = 1/α` stays `[STRONGLY MOTIVATED CONJECTURE]`; its evidential basis is CM-curve uniqueness, but it has no derivation chain, and the physics mechanism (MC-T4.3) remains `[OPEN]`.
...
| `AUDIT_LORENTZ_ANISOTROPY.md` | `campaign_complete/` | Spatial-direction spread `p = 4.0008`; historical Lorentz reading superseded by FTD-0407. |
...
| `PREREG_PHASE_II_WILSON_DIRAC_G2.md` | `campaign_complete/` | Historical FTD-0126 pre-registration; its physical outcome is invalidated by the FTD-0412 real-time-operator correction. Provenance retained; corrected rerun open. |
