# FTD-Native Response Tuple: First Fixed Probe

**Date:** 2026-04-22  
**Status:** [PARTIAL] native electrodynamics result, bare engine operator  
...
This probe answers the fixed linear part first:
...
## Native renormalization convention
...
The linear source/flux generator that reproduces this bare scheme is recorded
in:
...
### Blocking convention still open

A native flow measurement must use the finite-volume blocking contract in:
...
### Temporal Moore-Layer Interpretation
...
### Do we need U(1) gauge structure?
...
## Why eigenvalues are not needed here
...
4. Source-history action: define a nontrivial FTD action/measure only if the
   ...
   `g_sJ^FTD = 1`.
5. Native flow: measure how `C_L^FTD`, `K_T^FTD`, `Z_j^FTD`, and `g_sJ^FTD`
   change under a fixed, declared coarse-graining procedure.
