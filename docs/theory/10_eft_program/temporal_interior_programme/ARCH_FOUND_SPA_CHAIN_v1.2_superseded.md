...
separates the CDP reconstruction from Busch's independent trace-rule
characterization, makes the selector's response map and ensemble measure
explicit, and reconciles the FTD-facing claims with the canonical Bell and
Born records.
...
## 33. Genesis is a candidate microscopic bridge, not $\Sigma_O$

At the **substrate-transition level**, P5 makes FTD deterministic. Once
the complete engine state — including the seed used by its deterministic
pseudorandom draw — is included in $\lambda_n$, a genesis transition has
the response-map form $s_{n+1}=F_{\rm gen}(s_n,J_n,\lambda_n)$. Genesis
is not a threshold alone: eligible sites pass a probability ramp and an
index/seed/tick-keyed draw, and more than one site can manifest in a
tick; see [`SPEC_ENGINE.md`](../../../../engine/SPEC_ENGINE.md) and
[`phase_write.cpp`](../../../../engine/src/render_bridge_phases/phase_write.cpp).
Thus P5 excludes a *fundamentally primitive stochastic engine law* unless
FTD is extended, but this says nothing yet about a laboratory record map.

The FTD-native identification

$$F_{\rm gen}\stackrel{?}{\longrightarrow}F_O^{\rm cl}$$

is **[CONJECTURE] + [OPEN — BRIDGE]**, not an equality established here. It
asks only for a commutative association of laboratory preparation and
setting labels with substrate ensembles, response functions, and mutually
exclusive records $o$. Its empirical pushforward may be compared with the
externally computed quantum benchmark
$p_{\rm QM}(o\mid P,M)=\operatorname{Tr}(\rho_P E_o^M)$; neither $\rho_P$
nor $E_o^M$ thereby becomes an FTD observable. A structure-preserving map
between the substrate and the Hilbert/operator state-effect-instrument
algebra would instead be the measurement-map import $M$ that
[FC-1](../../07_assessment/core_ledgers/LEDGER.md#ftd-0255-record)
declines. It is not an open bridge inside canonical FTD; adopting it would
require an explicit framework amendment. Canonical
[FTD-0226](../../07_assessment/core_ledgers/LEDGER.md#ftd-0226-record) is
decisive: the manifestation map has a distributive Boolean event lattice
and its tested non-commutativity route is **[CLOSED NEGATIVE]**; classical
back-reaction order is not quantum complementarity.
[FTD-0187](../../07_assessment/core_ledgers/LEDGER.md#ftd-0187-record)
leaves the probability-as-energy-density step **[OPEN]**, while
[FTD-0199](../../07_assessment/core_ledgers/LEDGER.md#ftd-0199-record) and
[FTD-0200](../../07_assessment/core_ledgers/LEDGER.md#ftd-0200-record)
are **[CLOSED NEGATIVE] in their tested constructions** with the broader
empirical-frequency problem still open. Calling genesis "$\Sigma_O$" or a
quantum measurement before the allowed record map and its physical
ensemble exist would contradict the ledger.

**Bell status, without suppressing the live contradiction.**
...
The corpus then has two mutually exclusive live postures:

- **Posture A:** measurement independence. The registered FTD triangle
  correlator has $S\le2$ and is contradicted by laboratory cosine/Bell
  correlations.
- **Posture B:**
  [FTD-0329](../../07_assessment/core_ledgers/LEDGER.md#ftd-0329-record)'s
  ledger-governing measurement dependence. The
  exact allowance is $S\le\min(2+3M,4)$, but $M$ is not independently
  pinned, so PL-2 becomes unfalsifiable and the Tsirelson value is not
  derived.

This document does not resolve that contradiction. The laboratory
$2\sqrt2$ bound is imported from standard quantum operator structure
conditional on the **[SELECTION]** singlet and the rest of the Hilbert
measurement framework; complexification alone does not produce it.
...
with six separately auditable **external premises**. Under FC-1 they are
an import invoice for the SPA/quantum comparator, not six open obligations
of canonical FTD. A physical realization of their joint complex-quantum
conclusion would import $M$ and require an explicit FC-1 amendment.
Commutative protocol/readout analogues may be tested inside FTD, but they
must not be advertised as a reconstruction of the noncommutative quantum
state space:

| Principle | Relation to canonical FTD (no tags moved) |
|---|---|
| Causality | `[IMPOSED — EXTERNAL CDP PREMISE]`; a commutative no-future-signalling condition for FTD record protocols is a distinct `[OPEN]` test |
| Perfect distinguishability | `[IMPOSED — EXTERNAL CDP PREMISE]`; distinct ternary microstates do not establish its reconstructed-state meaning |
| Ideal compression | `[IMPOSED — EXTERNAL CDP PREMISE]`; no FTD analogue is asserted here |
| Local distinguishability | `[IMPOSED — EXTERNAL CDP PREMISE]`; OT-1.5's $\mathbb Z[i]^2$ seed does not prove operational local tomography |
| Pure conditioning | `[IMPOSED — EXTERNAL CDP PREMISE]`; no FTD analogue is asserted here |
| Purification | `[IMPOSED — EXTERNAL CDP PREMISE]`; its quantum role distinguishes the reconstructed theory from the classical member, so identifying $J$ as a purifier would invoke the declined $M$ unless FC-1 is amended |

A future quantitative wing may price retention of these six premises in
the SPA comparator using FTD-0371's currency, each with its falsifier. That
is owner work and must record the FC-1 amendment price if the reconstructed
quantum structures are assigned a physical FTD role; the table above moves
no canonical tag.

## 35. The Born problem split: static uniqueness, dynamic implementation

Postulate B + Busch give only a **static conditional characterization**:
normalized effect-noncontextual weights on the imported quantum effect
space are trace-form. They do not solve the dynamic pushforward (11).

The recorded toy execution and the unbooked engine-map draft narrow
possible mechanisms but do not close that gap:

- [`PREREG_BORN_DENSITY_SATURATION_v2.md`](../preregistrations/quantum_foundations/PREREG_BORN_DENSITY_SATURATION_v2.md)
  is a mechanism-level quick check with an **[IMPOSED]** ensemble. Across
  its registered grid the Born-fraction statistic rose monotonically to
  $0.8362$. Whether the true asymptote is exactly 1 or near the
  descriptive fit $0.86$ remains explicitly open. It establishes neither
  an exact Born law nor a substrate-wide necessary slow-gate theorem.
- [`PREREG_BORN_REGIME_MAP_ENGINE_v1.md`](../preregistrations/quantum_foundations/PREREG_BORN_REGIME_MAP_ENGINE_v1.md)
  is an **unbooked draft execution** reporting Outcome N: no declared
  axis reached the stated structure threshold. Its same-band signal/noise diagnosis was explicitly
  **post-hoc and non-registered**. The tested route did not succeed; the
  record does not prove that native flux noise can never succeed and
  does not close a mechanism class.

If a commutative protocol-to-record map is first constructed, FTD's
deterministic frequency problem can then be posed as an **[OPEN —
EMPIRICAL PUSHFORWARD PROBLEM]**: derive a physical $\mu$ from substrate
dynamics and test whether $(F_O^{\rm cl})_*\mu$ matches laboratory
frequencies. Equality with
$p_{\rm QM}(o\mid P,M)=\operatorname{Tr}(\rho_P E_o^M)$ is an external
benchmark, not a derivation of operator-valued observables from $A_5$.
At present even this formulation is **[CONJECTURE]** because
$F_O^{\rm cl}$ has not been derived from genesis. A physical Hilbert/effect
map would be the declined $M$, not a solution available within canonical
FTD. Non-flux slow variables, $s$-sector history, and latency remain
unscored candidates requiring separate preregistration.
