...
1. `A_obs` is built from the FTD-native non-site-local observables of
   ...
   boundary-to-boundary transfer observables, reference frame projections);
...
- **D2 -- Site-local vs non-site-local observable.** A *site-local*
  ...
  boundary-to-boundary transfer kernels, or from reference frame projections
...
7. **Reference frame projections.** Projections of the finite observable
   algebra onto a "public" measurement channel, per
   `../01_reference/REF_REFERENCE_FRAME_VOCABULARY.md` and the math-first
...
2. **Derive the observable algebra `A_obs`** from the §4 catalog
   ...
   transfer, reference frame projection), and the FTD-native source. No
