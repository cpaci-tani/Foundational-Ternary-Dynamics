# PRE-REGISTRATION — Ten-source removal-time orbit coherence v1
...
every reaction-free history of ten distinct stationary native sources stays
strictly below the production genesis threshold before a candidate first
descendant event?

This protocol evaluates one new integer count, `N=10`. It does not alter the
...
- ten distinct stationary ternary sources at arbitrary sites and with
  arbitrary signs;
...
The reported volume bound is the exhaustive maximum over these eleven removal
...
FTD-0593. Any sharper bound requires a separately locked identifier.
...
- `engine/include/ftd/eft/ten_source_orbit_coherence.h`;
...
- `engine/results/ftd_0593/windows_msvc_cpu.json`;
- `engine/results/ftd_0593/windows_msvc_cpu.csv`;
- theorem, analysis, and adversarial audit records.

The C++ implementation must recompute the orbit quantities rather than copy
the FTD-0590, FTD-0591, or FTD-0592 JSON. The Python verifier must independently
...
- If `max_r H_L^orb(10,r) < K_GENESIS` on all four volumes, verdict:
