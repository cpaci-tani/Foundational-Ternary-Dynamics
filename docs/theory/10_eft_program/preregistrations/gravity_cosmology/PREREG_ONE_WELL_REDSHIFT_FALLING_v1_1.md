# PRE-REGISTRATION — One-well redshift + falling v1.1

**Tag:** `[PRE-REGISTRATION]` — repairs v1 P1 only. Physics question, sites, \(N_\tau\), A1/A2 unchanged. Contains **no result**.
**Date:** 2026-08-19
**Hash-lock target tag:** `preregister-one-well-redshift-falling-v1-1` (pending owner commit; until the tag resolves this lock is `anchored-late` via §12 prefix SHA256).
**LEDGER reservation:** FTD-1019 (same row; v1 booked UNDERDETERMINED on P1).
**Parent:** [`PREREG_ONE_WELL_REDSHIFT_FALLING_v1.md`](PREREG_ONE_WELL_REDSHIFT_FALLING_v1.md) prefix SHA256 `EA5041998D1279DE62FF48D5EC5577451A2949B3DD1256D51AC531B4F1A6B4F6`. FTD-1017 sourced well; FTD-1016 operator; FC-2 / FTD-0402.
**Repair:** v1 P1 required \(\Gamma_{\rm near}/\Gamma_{\rm far}<0.999\) (~0.1% rate contrast). That threshold is replaced by \(<0.9999\) (~0.01%). Sites, source, \(N_\tau\), A1, A2, freeze rule, and falling path are byte-equivalent to v1 §2–§4 except this P1 line.
...
> LOCK-STD v1.1. Sections §1–§11 are frozen before any v1.1 \(\tau\) ratio or probe acceleration is observed. Post-hoc edits to §1–§11 void v1.1. The v1 hashed prefix is not edited.

---

## §1 — The question (LOCKED)

**Q-ONE-WELL-RS-FF-v1.1.** Same question as v1: after a CPU `tick()` that runs `latency_field` Poisson on a **heavy locked source alone**, freeze \(\mathcal{L}\). Do **both** of the following hold on that frozen field?
...
**Prior-favoured outcome.** FOUND. Favoured is not predetermined. A1 or A2 fails CTest if false.
...
Identical to v1 §3. Clocks: \(\rho_\tau=(\tau_{\rm near}/\tau_{\rm far})/(\Gamma_{\rm near}/\Gamma_{\rm far})\) with \(\Gamma=\sqrt{\max(1-\mathcal{L}^2,0)}\) at rest. Falling: \(r=a_x/g_{{\rm ext},x}\). No Pound–Rebka number.
...
| ID | Claim | Pass if |
|---|---|---|
| P1 | Clock contrast is non-vacuous | \(\Gamma_{\rm near}/\Gamma_{\rm far}<0.9999\) and both \(\Gamma>0\) |
...
The sole v1→v1.1 change is P1’s numeric threshold \(0.999\to 0.9999\).

---

## §5 — Outcome map (LOCKED)

**IMPROPER** (precedes): leaving `latency_field` on during clock ticks (clocks source); using EIN-4 live Poisson as this test; injecting \(F=mg\) then dividing by \(m\); retuning sites after seeing \(\rho_\tau\) or \(r\); widening ε of A1/A2; enabling colour/EM; using de Broglie **phase** as the redshift measurand; a Pound–Rebka ppm target; GPU run; CODATA retune of \(G_N\); relaxing P1 further after this v1.1 run.
...
|---|---|---|
| P1 | Yes | Periodic Poisson flatter than \(0.01\%\) in \(\Gamma\) between \(x=18\) and \(x=24\) |
...
2026-08-19 America/Chicago through 23:59, this session, CPU observer, v1.1 execution. Past window with no verdict books F10. Git tag pending; result cites §12 SHA as `anchored-late`.

---

## §11 — Reconciliation (LOCKED)

FTD-1019 remains one row. v1 is UNDERDETERMINED on P1 (over-tight contrast floor); v1.1 is the physics execution of the same question. FTD-1017 remains the sourced-falling result. FTD-1016 remains the operator. FTD-1014 remains CLOSED-NEGATIVE for default \(\nabla|J|\) (P8 re-asserts). FC-2 / FTD-0402 remain the clock law. Production default remains \(\nabla|J|\).
...
**Content SHA256 of hashed prefix:** `5DB20B6F59BA192F782772D91AB37894295A86FD15A81096D1A44EE4D8F5D0F5`

---

## §13 — Execution record (not part of the hashed prefix)

Executed 2026-08-19 America/Chicago, CPU observer, CTest `one_well_redshift_falling` **Passed**. Instrument `engine/tests/test_one_well_redshift_falling.cpp` SHA256 `4164C0DA192C9D24A8B4BB74AF885FFB6BFE59AFC06EE6430186E7500201FFC8`. Frozen classifier **FOUND**. \(\mathcal{L}_{\rm near}=3.027058\times10^{-2}\), \(\mathcal{L}_{\rm far}=0\), \(\Gamma_{\rm near}/\Gamma_{\rm far}=0.9995417\), \(\tau_n=19.99083\), \(\tau_f=20\), \(\rho_\tau=1\). Falling \(g_{{\rm ext},x}=-2.917418\times10^{-4}\), \(r_{\rm on}=0.999542\), \(r_{\rm off}=0\). Constructor logs “GPU backend active” then `force_cpu()`; not a GPU campaign (not IMPROPER). Result: [`ANALYSIS_ONE_WELL_REDSHIFT_FALLING_v1.md`](../../../03_derivations/gravity_and_cosmology/ANALYSIS_ONE_WELL_REDSHIFT_FALLING_v1.md). Anchor: **`anchored-late`** until `git rev-parse preregister-one-well-redshift-falling-v1-1` succeeds. v1 remains UNDERDETERMINED on P1. FTD-1013 / 1014 / 1015 / 1016 / 1017 / 1018 / 0131 / 0250 / 0349 / 0402 / 0208 / U-8 unmoved. Production default remains \(\nabla|J|\). No Pound–Rebka claim.
