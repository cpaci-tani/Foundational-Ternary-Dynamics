...
| 7.19 | [TRACKER_REFRAME_FLAGS.md](07_assessment/archive_session_outputs/TRACKER_REFRAME_FLAGS.md) | ⚠️ **Open work queue (2026-04-19)** — 5 inline `[FLAG: re-derivation needed]` markers in EFT-program + CM-conjecture docs (DERIV_BETA_FUNCTION_MEASURED, DERIV_DYNAMICAL_SM_EMERGENCE, DERIV_DAY2_CAMPAIGN ×2, CONJ_ALPHA_FROM_CM) where the original "engine converges to QED as L → ∞" framing was substantively load-bearing. Two acceptable restatements per item (finitary scaling claim or calibration-conditional claim). |
...
| 7.27 | [TRACKER_PDF_ONLY_PAPERS.md](07_assessment/archive_session_outputs/TRACKER_PDF_ONLY_PAPERS.md) | ⚠️ **PDF-only papers tracker (2026-04-19, Session 2)** — 13 papers in `docs/papers/` exist only as PDF (no recoverable TeX/MD source). Triage: 2 HIGHEST priority (`FTD_Thermodynamic_Limit`, `DERIV_THERMODYNAMIC_REFLEXION` — title-level evidence of proscribed framing); 2 HIGH (`FTD_KMS_Thermal_Time`, `FTD_Modular_Structure` — Type III₁/KMS); 3 likely-superseded; 6 unknown. Recovery options: git-history archaeology, PDF text extraction, archive-with-retraction, or retract entirely. |
