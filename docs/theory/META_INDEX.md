...
| 7.14 | [OPEN_A_PHYS_DERIVATION.md](10_eft_program/OPEN_A_PHYS_DERIVATION.md) | ⚠️ **[OPEN] (2026-04-19)** — load-bearing problem created by the reframe. The lattice-to-physical conversion `a_phys` (one voxel, in metres) must be derived from Axiom-Zero invariants or declared an empirical calibration. Three derivation candidates analysed: algebraic (no dimensional invariant in lattice algebra — cannot deliver), EFT-matching (calibration not derivation), gravitational fixed point (only candidate that could in principle force a value). Connects directly to the 3.6× α-plateau. |
| 7.15 | [DERIV_A_PHYS_MECHANISM_GAMMA_ATTEMPT.md](10_eft_program/DERIV_A_PHYS_MECHANISM_GAMMA_ATTEMPT.md) | ❌ **Mechanism γ closed as derivation (2026-04-19)** — explicit dimensional chain `G_N(phys) = G_N(lat) · a_phys³ / (M_unit · t_phys²)` with the framework's two existing calibrations (`c_lat = 1/√3`, `K_B = m_e`) yields `a_phys ≈ 4 × 10⁻⁵⁵ m` using the engine's toy-regime `G_N(lat) = 0.01`, or `≈ 7 × 10⁻⁷ m` using the physical `α_G ≈ 5.91 × 10⁻³⁹` — both implausible. Both `G_N(lat)` and `M_unit` are themselves calibrations rather than Axiom-Zero invariants, so the chain converts one calibration into another rather than deriving `a_phys`. **Recommended position:** declare `a_phys ≡ ℓ_P` in `SPEC_FTD.md` and quote dimensional predictions as conditional on the calibration. |
