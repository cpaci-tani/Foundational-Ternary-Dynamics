...
**Latest update:** 2026-06-13 — **THOMSON RADIATION SHELL METER PREREGISTERED (FTD-0290, locked)** — after FTD-0289 found an above-gate residual flux/wave response but no cross-section claim, the next question is frozen as `docs/theory/10_eft_program/preregistrations/PREREG_THOMSON_RADIATION_SHELLS_v1.md` plus engine artifact `engine/tests/campaign_thomson_radiation_shells.cpp` (SHA256 `a47de9c1bb52f92a6dc35471f4eba516fb76acaf9b66abd3de44dd6431d67edf`). It keeps the fixed FTD-0288/0289 setup (`L=33`, `ticks=200`, mode 4, amp 0.05) and measures residual-field Poynting flux `S_res = (-W_res) × curl(J_res)` through shells `{5,7,9,11,13,15}` after subtracting `beam_only` and `charge_only`. Outcomes are frozen as `BASELINE_RADIATION_METER_INVALIDATED`, `NATIVE_LEGACY_RESIDUAL_RADIATION_DETECTED`, `NATIVE_EMERGENT_STRUCTURED_RESIDUAL_RADIATION_DETECTED`, `NATIVE_EMERGENT_OUTWARD_RESIDUAL_POWER_DETECTED`, or `NO_BASELINE_SUBTRACTED_OUTWARD_POWER`. This is a radiation instrument only: **no alpha derivation, no Thomson cross-section, no QED amplitude, no scan.**
