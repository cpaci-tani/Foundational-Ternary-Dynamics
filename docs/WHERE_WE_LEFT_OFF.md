# Where We Left Off — 2026-06-25 (parallel act-count arc reconciled & merged — FTD-0322→0327 + FC-4 folded into FC-W; spine-audit stack already merged; golden `0xb604d81a3d79366e` green; next free id FTD-0328; CLAUDE.md v5.51)

**Latest update:** 2026-06-25 — **PARALLEL ACT-COUNT ARC RECONCILED + MERGED: FTD-0322–0327 + FC-4 fold into FC-W** (branch `reconcile/act-count-2026-06-25`; golden `0xb604d81a3d79366e` untouched, docs + 1 script; ZERO promotions). A concurrent session (`claude/inspiring-goodall-o6ow79`) had independently minted **FTD-0314→0319** for a different arc — it branched from `main`@0313 without seeing the FC-W/genesis/spine stack, a total id + FC-4 collision. The branch was fetched from origin, verified (golden + master 54/54 + index links clean), **renumbered 0314–0319 → 0322–0327**, FC-4-reconciled, and merged. **The arc — the "a √ is the one math operation that must be taken by an act of intent" lens:** **FTD-0322** act-reduction count `[SYNTHESIS]` PARTIAL (`i=√(−1)` is FTD's unique *generative* act ≡ FC-0; α's δ-selection + FC-3 + the calibration register are non-empty breakers ⇒ the universe is chosen more than once); **FTD-0323** the arrow of time as a square root `[SYNTHESIS]` (Euler-reflection **ratio** `Γ(z)/Γ(1−z)=G*` = the half-derivative `∂_t^{1/2}=√(∂_t)`, `G*` its eigenvalue; the FC-2 identification-with-time stays a declaration, NOT promoted); **FTD-0324** the arrow's **direction = forced-given-FC-2** `[SYNTHESIS]` (many-to-one manifestation map ⇒ no inverse ⇒ no ℤ/2 to choose; NOT from P1–P5 alone; reconciles a 3-way canon split — causal / thermodynamic / reversible-wave); **FTD-0325** act-count completed over the SM `[SYNTHESIS]` CLOSED (every exactly-expressible dimensionless SM quantity reduces to `{i, δ}`; lepton ratios 207/3477 + angles 3/13, 7/59, 3/10, 16/29, 1/52 are pure rationals — no act, not even δ; the third field-act is empty); **FTD-0326** no FTD-native ℤ/2 supplies δ `[DERIVED]+[SYNTHESIS]` PERMANENT-EXTENDED (all 5 native ℤ/2's are `ℚ(G*)`-entry ⇒ Galois-blind to `δ=√(G*(4G*−1))`; **corroborates main's FTD-0314 carrier-narrowing** along the orientation-symmetry axis; strengthens FTD-0242, extends FTD-0325's `i⊥δ`); **FTD-0327** the AGM place-bridge `[SYNTHESIS]` (`G*=2√π/AGM(1,√2)`, Gauss 1799; the AGM's forced-magnitude geometric means land on `G*` deg-1, **never on δ** deg-2 — the concrete witness unifying the operator/arithmetic/act faces; the `√(4G*−1)`-at-prime-3 reading left `[OPEN]`). Plus `scripts/audit/check_registry.py`, a next-free-id/FC registry guardrail. **FC-4 reconciliation:** the arc's "proposed FC-4 (δ-act)" is the **same commitment as the live FC-W** (FTD-0315 = the constitution's FC-4) — both declare the selection of `δ=√(G*(4G*−1))`, both `[AXIOM]`-class, same kill-criterion; **folded into FC-W, no separate FC minted**; `DRAFT_FC4_DELTA_ACT_DECLARATION.md` retitled as FC-W's act-of-intent reading. **ZERO promotions:** `x₊=1/α` `[SMC]` (FTD-0013); MC-T4.3 `[FOUNDATIONAL OBSTRUCTION]` (boundary corroborated, not closed); FC-W an adopted `[AXIOM]`; FTD-0244 cited not re-derived; no α derived. **Next free id FTD-0328.** *(Provenance note: the 4 pre-registration git tags were not pushed to origin, so only the committed `PREREG_*.md` files came across; re-lock if the tag provenance is needed.)*

**Prior latest update:** 2026-06-25 — **SPINE-AUDIT STACK + GENESIS-CEILING MERGED & PUSHED TO `main` (FTD-0314→0320; golden `0xb604d81a3d79366e` green; ZERO promotions; CLAUDE.md v5.50; next free id FTD-0322)** — the three-session stack that had been stranded on `audit/spine-corrections-2026-06-24` (built atop `engine/atomic-spectroscopy-helium`) was verified end-to-end and integrated to `main`. **Verification (re-run from source on the branch, not trusted from commit messages):** the golden gate was rebuilt INCLUDING the FTD-0316 `phase_write.cpp` change → computed hash `0xb604d81a3d79366e` = frozen GOLDEN_HASH **PASS** (the 3 genesis-override knobs are sentinel-guarded `≤0/false` ⇒ byte-identical default path: golden-neutral by construction AND empirically); `proof_master_verification.py` 54/54; `verify_index_links.py` 400/0 + 103/0; the 5 touched/new spine proof scripts exit 0. **Merge:** `main` fast-forwarded `3b1f20dc → 70932058` (11 deliverable commits) and pushed to `origin/main` (now in sync); the scratch WIP tip `78ded42a` ("epitaxy pre-switch": 6 helium-spectrum + session-atlas/glyph scratch scripts) was EXCLUDED (recoverable from reflog) and the `audit/…` branch DELETED. **What landed on `main`:** **(a) FTD-0316 `[EMERGENT/MEASURED — DERIVED-FROM-RULE]`** — a coherent transverse flux wave has a sharp, wavelength-invariant amplitude ceiling at `|J|=K_GENESIS=1.533` (the lattice Schwinger-limit analogue; the cliff IS the manifestation rule made visible, not a theorem; the number is `[IMPOSED]`), via 3 golden-neutral runtime genesis-override knobs; `engine/tests/campaign_genesis_amplitude_ceiling.cpp`. **(b) FTD-0317 `[MEASURED/EMERGENT]`** — at fixed energy AND fixed `|J|` histogram, a coherent disposition makes far more ORGANIZED matter than a scrambled control (R 0.97 vs 0.16, +115σ) while scrambled makes MORE matter: information buys form, not quantity (narrow operational claim; NOT "information is a creative agent"). **(c) FTD-0318 `[AUDIT/SYNTHESIS]`** — the algebraic-spine deep adversarial audit (multi-agent; every proof re-run, every load-bearing identity recomputed at dps 100–150, 3 hostile lenses/theorem): the genuine theorem-grade core reproduces to machine zero but is **smaller than the "nine theorems" headline and pervasively conditional on Chudnovsky 1976**. **CRITICAL fix:** `DERIV_LFUNCTION_GSTAR_CONNECTION.md` §3.1 carried a numerically FALSE `[THEOREM]` (`G*=4√(2/π)·L`=2.0921, inconsistent with its own §1.2 `G*=8L/√π`=2.95868) → coefficients corrected `512/π→1024/π`, `2048√2/π^1.5→8192/π^1.5`. **Demotions (demotion-only, tags reconciled to the proofs):** Thm 9 "maximal"→"a" π-free; D=3 dimension-forcing `[THEOREM]→[SELECTION]` (circular — LHS |Aut|²=16 is D-independent, RHS presupposes D=3 via |O_h|/3); CM d=−4 "proven"→`[NUMERICAL FACT]` (flips under rational-multiplier); Phase-J "DISCONFIRMED L≥3"→`[THEOREM L=2]`+`[NUMERICAL EVIDENCE L=3 ultralocal]`+`[OPEN L≥4]`; z_BCC·2=16 (FTD-0007)→`[SELECTION]`; Watson "1.3932 = I₁"→`G_BCC(0)` (the equality W₃=G*²/2π is unaffected); motivic uniqueness→`[SELECTION]`; harmonic-tower mis-citation Schneider-1941→Chudnovsky-1976. Deligne exponents are **k=9 (512) / k=13 (8192)**, NOT 2^10. New `AUDIT_ALGEBRAIC_SPINE_2026-06-24.md` + LEDGER FTD-0318. **(d) FTD-0319** — resolves the 3-way FTD-0189 ID collision: the 2.65M-polynomial look-elsewhere scan gets a dedicated row + repointed live citations; its "~4×10⁵:1 Bayes" is **unsupported** (the runner computes ~19×, asymmetric-tolerance-conditioned). **(e) FTD-0320 `[MEASURED]`** — a pre-registered rigidity-catalog scan (tag `preregister-rigidity-catalog-v1`) over the ~125 unscanned `[PARAMETRIC]` claims → sin²θ₁₂ CHANCE_LEVEL, sin²θ₂₃ MDL-dominated by 6/11, Δm²-ratio MDL-dominated → all three demoted `[SMP]→[PARAMETRIC]`. **(f) F1** — `proof_lvalue_deligne_verification.py` reproduces `L(E,1)=ϖ/4` in-repo from point-counting (rel.diff 5e-41) ⇒ the `16G*³` identity is now `[DERIVED-given-import, L(E,1) REPRODUCED]` (the Sym² value is still cited, in-repo reproduction deferred); new `SPEC_OPEN_MATH_FRONTIERS.md` maps F2 (ℚ(G*) maximality = Schanuel-class `[OPEN-external]`), F3 (coeff-16 / D=3 forcing = the MC-T4.3 wall), F5 (Chudnovsky = permanent import). **(g) FTD-0321** — the Damerell h≥2 ideal-class scan is PRE-REGISTERED (`PREREG_DAMERELL_SCAN_v1.md`), run **DEFERRED** to avoid a rushed canonical result. **ZERO promotions:** `x₊=1/α` `[SMC]` (FTD-0013), MC-T4.3 `[FOUNDATIONAL OBSTRUCTION]`, FC-W stays an adopted `[AXIOM]` (FTD-0315), the spine + linear k=¼ theorem untouched; no α derived. **Next free id FTD-0322.** **Open research (none started):** the F4 Damerell run; the Sym² in-repo reproduction; the combination-family rigidity scan v2.

**Prior latest update:** 2026-06-23 — **EARN-W: the carrier-narrowing theorem (FTD-0314) + FC-W / FC-4 declared (FTD-0315)** — the "what is W / could it be an *information field a brain connects to*" thread resolved into a real deliverable via the "earn it first" path. **(0) The information-field idea, adjudicated (7-agent workflow):** "W = a field a brain connects to" is `[CATEGORY-MISMATCH]` — W is an **arithmetic ℤ/2 Galois selector** (realize `√(G*(4G*−1))`), not a dynamical/observer field; an observer-selection field is **M-shaped** (the measurement map FC-1 *declines*; FTD-0205/0226 closed-negative), and selection *presupposes* the distinguishability W must first create. The corpus's own k=1/2 "reference-frame/consciousness" branch of the same quadratic is provably **Galois-disconnected** from the k=16 α root. Nothing adopted there. **(1) EARN-W structural attack (7-agent adversarial workflow; same-session transparent lock; `scripts/proofs/proof_w_carrier_narrowing.py` SHA `7e1e90def51e…`, 11/11 PASS dps=150):** can a SUBSTRATE-NATIVE structure realize the surd as a forced ℤ/2 invariant **without planting**? **Verdict UNDERDETERMINED-leaning-CLOSED (~85%): NO.** **Deliverable — the NARROWING THEOREM `[THEOREM]` (FTD-0314, conditional on Chudnovsky):** the surd is *transcendental over ℚ* (G\* transcendental ⇒ `Q(G*)∩Q^ab=Q`), so every finite-symmetry carrier (chirality, ±1 sign, 2O double cover, permutation parity) **and** every native operator (∈Q(G\*), FTD-0244) is excluded — **extending K-BIND from operators to all finite symmetries, geometrically explaining the wall**. The only door — a ℤ/2 twist on a G\*-bearing **analytic** carrier — closes on all three natural ones: BCC-Watson twist **degenerates exactly** (G_odd=G_even, odd-n angular integrals vanish), second Watson moot (`4G*−1∈Q(G*)`, PSLQ `[1,−4,1]`), CM period outside `Q̄(π,Γ(1/4))` (surd² degree-1 in π). Skeptic's strengthening: surd² is motivically **weight-inhomogeneous** ⇒ period of no pure graded motive (pressures the lone surviving loophole — a NEW forward-derived period — which stays `[OPEN]`, leaning CLOSED; opening it needs the banned W-CRIT-2 planting). **(2) FC-W declared (FTD-0315 = the constitution's FC-4):** because the narrowing theorem *pins exactly* what an external W must be and *proves no cheaper object suffices*, the constitution now **adopts** W: *an external order-2 ℤ/2 twist on a G\*-bearing analytic structure realizing `√(G*(4G*−1))` and breaking the root-swap.* Under FC-W, `x₊=1/α` is `[CONDITIONAL THEOREM given W]`, **explicitly NOT `[DERIVED]`**. **Honest cost (in LEDGER + constitution, not buried):** FC-W is the framework's **FIRST *adopted* import** (vs FC-1/FC-2's *declined* imports that buy the deviation spine) and does **NO work beyond the α-root** unless its carrier forces independent content (`[OPEN]`). **Files:** `docs/theory/07_assessment/audits/AUDIT_W_CARRIER_NARROWING.md` (new), `scripts/proofs/proof_w_carrier_narrowing.py` (new), LEDGER FTD-0314/0315, `SPEC_FTD_FRAMEWORK_V1.md` §3.5 + register/falsification/registry, `SPEC_ALPHA_DYNAMICAL_BOUNDARY.md` §3.2, META_INDEX 10.76a, `SPEC_FTD_COMPLETE_FRAMEWORK.md` (capstone §1/§3 α-row synced for FC-W). verify_index_links **400/0**. **ZERO promotions:** `x₊=1/α` `[SMC]` unconditional (FTD-0013 unchanged), MC-T4.3 `[FOUNDATIONAL OBSTRUCTION]` (FC-W is an external axiom, not a closure), FTD-0244 extended-not-altered, the spine + linear k=¼ theorem untouched; no α derived. Read-only / docs — golden gate untouched. **Next free id FTD-0316.** (Branch `engine/atomic-spectroscopy-helium`; concurrent-session engine WIP remains local/untracked.)

**Prior latest update:** 2026-06-20 — **CONVENTION-AUDIT + OVERCLAIM-RETRACTION + ENGINE-NATIVE ATOMIC SPECTROSCOPY** (a three-part session; **all consolidated on `main`, golden `0xb604d81a3d79366e` green throughout, ZERO promotions**). **(1) FTD-0307 — FTD-0110 Convention Audit `[MEASURED — BOUNDARY: exit (ii) CLOSED NEGATIVE]`:** attacked the never-attempted **exit (ii)** of the FTD-0110 nonlinear bridge (the FTD-0269/0276 open door) — is the engine-tuning calibration of the N(A) cluster-mass law pure CONVENTION (only the dimensionless shape physical; an affine (A,N) rescaling) or PHYSICAL? **Method correction:** a broken power law's exponents are INVARIANT under any affine (A,N) rescaling, so FTD-0269's knee-shift criterion does NOT distinguish convention from physics — **exponent-invariance** is the right discriminator. **γ = PHYSICAL** (established from the existing FTD-0276 Leg B data — exponents move 39–65% with γ). **Drain run of record** (`campaign_drain_scan`, L=32, drains {0.125..0.75}, 8 seeds, 528 rows via 12 deterministic parallel workers, **bit-identical to serial**): the clean super-knee single-power exponent (A≥20, no knee ambiguity) **DECREASES monotonically 1.91→1.59** across drain (~6σ on the seed bootstrap) — a pure rescaling would hold it exactly constant; the strict gate (exponent spread <10% AND collapse medianCV <5%) fails all three sub-tests ⇒ **drain = PHYSICAL**. **Both knobs PHYSICAL ⇒ exit (ii) CLOSED NEGATIVE; the FTD-0269 BOUNDARY is hardened** — the nonlinear calibration is irreducibly engine-emergent. Pre-reg `PREREG_FTD0110_CONVENTION_AUDIT_v1.md` (tag `preregister-ftd0110-convention-audit-v1`, lock `5023d259`); verdict `ANALYSIS_FTD0110_CONVENTION_AUDIT_v1.md`; LEDGER FTD-0307. The linear k=¼ O_h theorem is untouched mathematics. **(2) Integrity fix — 2026-06-18 overclaim RETRACTION:** an adversarial audit found two 2026-06-18 commits had promoted **8 SM claims to `[THEOREM]` via substitution identities** (m_e 16/3 = (N_eff+N_c)/N_c re-spelling; m_p 1938−102=1836 "knot"; 6 quark masses, m_t importing Z=118/Oganesson from the periodic table; sin²θ_W=3/13; α_s=7/59; G_N=1/100) and minted **2 DUPLICATE ledger ids** (a second FTD-0259 + FTD-0260), self-citing. **ALL RETRACTED** to honest motivated tags (m_e/m_p `[SMC]`, quarks `[PARAMETRIC]`, sin²θ_W/α_s `[STRUCTURALLY MOTIVATED PARAMETRIC]`, G_N `[CLOSED NEGATIVE]` per FTD-0131); the duplicate rows deleted (the real FTD-0259 Mechanism-α + FTD-0260 thermostat-sweep untouched); `AUDIT_RATIONAL_FIT_CLAIMS` + `proof_complete_sm.py` + `proof_topological_mass_generation.py` reconciled to honest tags. This is the substitution-identity move the Epistemic Discipline explicitly forbids. **(3) FTD-0308 — engine-native atomic spectroscopy `[CONDITIONAL — DERIVED-GIVEN-IMPOSED]` + `[BOUNDARY — engine FFT, instability-limited]`:** built `engine/tests/campaign_atomic_spectroscopy.cpp` (CPU+GPU, golden-neutral) — a LOCKED +1 nucleus sources the engine's OWN Gauss/Coulomb φ_C, the `de_broglie_clock` + `db_clock_coulomb` make the clocked flux ring at `ω_n=√(ω₀²+a_n)`, shell-autocorrelation `C(t)` FFT → bound levels. GPU-ported `db_clock_coulomb` (CPU↔GPU parity machine-precision; runs to **L=256 on the RTX 5090** in 1.9 GB); added `--Z` (He⁺) + `--offset` (excitation) knobs (golden-neutral). **Result — a split:** engine↔operator match at L=32 (0.53%); the **operator built from the engine's own φ_C confirms the hydrogen excited ladder binds (n_bound=6 @ L=128) + He⁺ (n_bound=8)** — the excited spectrum IS finite-size-resolvable (box-size, not structural; exactly what the Python LOBPCG couldn't reach). **But the engine-native time-domain FFT readout is dynamics-limited** — a 3-step diagnosis (He⁺ deep well resolves a sharp clean 1s → not resolution; off-center excites 2s/2p ~2500× → not excitation; yet one peak) identified **bare-wave leapfrog amplitude growth** (dt-reducible ~dt²; coupling-independent; the dt-invariance was a `set_dt` clamp artifact; C(t) grows ~13 orders, recon ρ≈1.00119/tick — absolute-ρ window-caveated; relative controls carry the verdict `[from-recon]`; the growth is a discretization artifact of the homogeneous integrator, NOT the inhomogeneous KG well). The operator-on-φ_C path is the accurate readout; a stable bare-wave integrator would fix the FFT route. Earlier "wavepacket-blending"/"excitation-limited" reads RETRACTED. ω₀ + coupling `[IMPOSED]`; FTD-0270/FC-1 ceiling stands; never "FTD derives hydrogen/helium"; neutral He stays the FTD-0279 SCF result. Verdict `ANALYSIS_ATOMIC_SPECTROSCOPY_ENGINE_v1.md`; LEDGER FTD-0308. **(4) Also:** restored the G\* paper's compilability (`PAPER_GSTAR_INTRODUCTION.tex`, 36pp, 0 undefined refs, math.NT-ready); fixed a CUDA-13 `atomicAdd(long long)` build break in `kernels_poisson.cu`. **Nothing promoted:** FTD-0013 `[SMC]`, MC-T4.3 `[FOUNDATIONAL OBSTRUCTION]`, FTD-0110 linear k=¼ / nonlinear bridge `[OPEN]`, FTD-0270/0271/0278/0279 — all unchanged. **Next free id FTD-0309.** (origin/main @ `1a45c2da`; the helium forward-model Python scripts + concurrent-session WIP remain local/untracked.)

**Prior latest update:** 2026-06-17 — **COMPREHENSIVE ENGINE PHYSICS AUDIT + "EVERYTHING ACTIONABLE" REMEDIATION** (52-agent audit workflow; 3 commits on `main`: `5b0d6b8f` C++/WASM + golden re-baseline, `21a7a3c2` JS, `6e72ffd4` CUDA-UNVERIFIED). **⚠ GOLDEN HASH RE-BASELINED `0x56fa28acb5b9fe88` → `0xb604d81a3d79366e`** — intentional diagnostic-scope fix (audit m1: `gauss_violation`/`max_gauss_error` now summed only over vacuum sites with the mean-subtracted coupling-scaled target the SOR projection actually enforces; per-voxel state/flux/wave_vel/velocity BYTE-IDENTICAL; deterministic OMP1==pool; rationale documented in `test_render_bridge_golden.cpp`). **Verdict: the canonical CPU run-of-record path is physics-sound**; defects clustered in the CUDA backend, JS/WASM mirrors, and Scale-0 viz. **Key fixes:** M2 (C++ `PROTON_RATIO`→canonical 1836.47, matches JS/Python; atom tests assert scale-invariant RATIOS so no regression); GAP1 (Langevin σ=√(2γT)→√(γ(2−γ)T) FDT-consistent — **shifts thermal-campaign equilibrium temps FTD-0272/0274**); M3 (`s0-field-plane-wave`/`photon-pulse` wave_vel→polarization axis, ω=2c·sin(k/2)); M5 (JS α_s→C++ `alpha_s_running`); M6 (dead DM-halo/genesis overlays — N³ scalar misread as N³×3, confirmed live fluxLen=N³); M7/M8 (worker-proxy parity + inspector routing); M1/M4 GPU parity (charge_coupling, color guard — **UNVERIFIED, need WSL2 GPU**). **GAP2–GAP7 follow-up (no new fixes warranted):** GAP2 MISDIAGNOSIS (flux-volume layout-conversion preserves coordinates — no X↔Z swap); GAP3/4/6/7 CLEAN; GAP5 CLEAN-by-design (weak flip intentionally violates Σ-state = Sakharov-1; pair-prod neutral). **`test_helium_scale1` fails PRE-EXISTING** (byte-identical with M2 reverted; FTD-0270 boundary). Golden GREEN @ new hash; 8/9 targeted CTests pass. **Engine code unchanged below the audit; NOTHING PROMOTED** (no theorem/LEDGER change — this is engineering-health, not physics-claims). Full record: `engine/CHECKLIST_ENGINE.md` ROUND 6. NEVER cite the old golden hash as current.

**Prior latest update:** 2026-06-14 — **WAVE SECTORS RUN OF RECORD (FTD-0299) — LIGHT-CONFIRMED + condensate-sound NULL** — pre-registered (`PREREG_WAVE_SECTORS_v1.md`, tag `preregister-wave-sectors-v1`, lock `8fff0187`; instrument+analyzer SHA256-frozen; hardened pre-lock by a 24-blocker adversarial review workflow). Executes the FTD-0298-SOUND `[OPEN]`. **(Q1) LIGHT-CONFIRMED:** the flux-wave ω(k) matches the engine's OWN 18-pt stencil eigenvalue to machine zero across ⟨100⟩/⟨110⟩/⟨111⟩ at L={24,32,48}; IR phase speed isotropic at c=1/√3 (isotropy dev converging 8.5e-3→4.8e-3→**2.1e-3** with L); light & radio = one flux-wave sector. **(Q2) NULL:** 4 seeds (all condensed m0=1.000), 5 modes, coupling ON — **0 propagating density branches distinct from the light wave**; the only response sat at ω_light (the light-wave shadow), amplitude-robust. **Engine-confirms FTD-0298: FTD has light but NO acoustic sector** (lattice IS space ⇒ no broken-translation Goldstone); **FTD-0298-SOUND closes [BOUNDARY — engine-confirmed]**. Golden-neutral (`0x56fa28acb5b9fe88`). Companion `[VISUALIZATION]`: docked Scale-0 **dispersion panel** (`dispersion-panel.js`) — analytic ω(k) + the measured atlas + no-acoustic contrast + best-effort live measure; browser-verified. Verdict `ANALYSIS_WAVE_SECTORS_v1.md`; LEDGER FTD-0299. **Nothing promoted.** Next id FTD-0300. (Commits on branch `claude/youthful-tesla-49ceed`; FTD-0298 = `b1704b9f`, pre-reg lock = `8fff0187`; NOT merged to main.)

**Prior latest update:** 2026-06-14 — **LATTICE WAVE SECTORS — SOUND, LIGHT, RADIO (FTD-0298, documentation-only)** — a synthesis + boundary analysis consolidating the lattice's wave physics around one object, the dispersion `ω(k)=2c·|sin(k/2)|` (`c=C_SPEED=1/√3`, CFL ceiling [DERIVED]; SI value calibration-dependent). **(1)** Light and radio are the **same flux-wave sector**, differing only in `k` (Maxwell-exploit FTD-0113/0114/0115/0120 [DERIVED]; FC-2 Lorentzian carrier); since every lab frequency sits at `k/k_zone ≲ 1e-28` (computed: visible 5.4e-29, FM radio 1.1e-35), FTD structurally predicts **no vacuum dispersion** (PL-4/PL-5 corrections suppressed ~1e-57/~1e-114 at visible; matches GRB/FRB null-dispersion bounds [OBSERVATION]). **(2) New mapped boundary** — FTD has light but **no sound**: acoustic phonons are Goldstone modes of spontaneously broken continuous translation, and FTD's lattice *is* space (no continuum to break) ⇒ no acoustic Goldstone [SELECTION]+[BOUNDARY]; three corpus legs (no displacement DOF in `voxel.h`; Gauss projects out longitudinal `J_L`; FTD-0272 first-order genesis = no Goldstone). One [OPEN] door: a condensate compression mode in the manifested phase. Doc: `docs/theory/03_derivations/foundational_mechanics/ANALYSIS_LATTICE_WAVE_SECTORS_v1.md`; unit-map `scripts/exploration/wave_sector_unit_map.py`. **No engine change; golden gate untouched; nothing promoted** (FTD-0013 [SMC], MC-T4.3, FC-1/FC-2, FTD-0270/0271/0272 — all unchanged). Next id FTD-0299.

**Prior latest update:** 2026-06-13 — **ALPHA ESTIMATOR VALIDATION v2: HALF-ENERGY GATE CONFIRMED MATCHED (FTD-0286 v2)** — after v1 returned `ENERGY_FUNCTIONAL_MISMATCH` (paired `energy_audit().field_energy = ½Σ|J|²` with legacy gate `2rG_L(r)`), v2 re-tests with the paired gate `α_r = r G_L(r)` via `engine/tests/campaign_alpha_estimator_validation_v2.cpp` + shared helper `engine/include/ftd/eft/lattice_coulomb_gate.h`. Pre-reg: `docs/theory/10_eft_program/preregistrations/PREREG_ALPHA_ESTIMATOR_VALIDATION_v2.md`; analysis: `docs/theory/10_eft_program/ANALYSIS_ALPHA_ESTIMATOR_VALIDATION_v2.md`. Run: `ctest --test-dir engine/build -C Release -R "^alpha_estimator_validation_v2$"`. Result: **matched static projector PASS** (mean ratio 1.002, max rel err 0.26%); **production live-tick FAIL** (mean ratio 0.88, max rel err 24% — known 18-pt/6-pt stencil mismatch). Verdict: `HALF_ENERGY_GATE_CONFIRMED_MATCHED`. `DERIV_EMERGENT_COULOMB_GEOMETRIC.md` updated with ½-convention pairing note. **No alpha derivation, no Postulate-W arm, no claim promoted.** Next honest move: FTD-0285 no-alpha discriminator v2 using the matched estimator + half-energy gate.

**Prior latest update:** 2026-06-13 — **GOLDEN DETERMINISM VERIFIED + DOC RECONCILIATION (Sprint A)** — OpenMP race fixes in `poisson_solvers.cpp` + `phase_write.cpp` (2026-06-11) confirmed green: `render_bridge_golden`, `determinism`, and `campaign_determinism_gate` all PASS; hash `0x56fa28acb5b9fe88` @ L=17 bit-identical under OMP=1 and full thread pool (3× repeat each). Stale golden-hash citations updated across ADR-0012, `engine/SPEC_ENGINE.md`, phase-file comments, `tests/README.md`, `cuda/README.md`, `DESIGN_RNG_PORTABILITY.md`. No physics change — documentation alignment only.

**Prior latest update:** 2026-06-13 — **THOMSON RADIATION SHELL METER RUN: NO BASELINE-SUBTRACTED OUTWARD POWER (FTD-0290)** — pre-registered at `8ccfee7b` / tag `preregister-thomson-radiation-shells-v1`, then run once with `ctest --test-dir engine/build -C Release -R "^thomson_radiation_shells$" --output-on-failure`. Analysis: `docs/theory/10_eft_program/ANALYSIS_THOMSON_RADIATION_SHELLS_v1.md`. Result: locked-linear residual-field shell power is machine-zero (`max_abs_power=4.84e-30`, `max_outward_power=2.42e-30`); native legacy is machine-zero (`max_abs_power=4.85e-30`, `max_outward_power=2.42e-30`); native emergent has a finite but sub-gate shell trace (`max_abs_power=2.5242201929664388e-9`, `max_outward_power=2.0869970474307537e-9`, gate `1e-8`). Verdict: `NO_BASELINE_SUBTRACTED_OUTWARD_POWER`. FTD-0289's excess residual therefore does **not** yet become above-gate outward radiation on these fixed shells. **No radiation claim, no Thomson cross-section, no QED amplitude, no alpha claim promoted.** Next honest move: derive the correct native radiation observable or pre-register a justified longer-time/continuum shell campaign; do not treat the sub-gate trace as a discovery.

**Prior latest update:** 2026-06-13 — **THOMSON RADIATION SHELL METER PREREGISTERED (FTD-0290, locked)** — after FTD-0289 found an above-gate residual flux/wave response but no cross-section claim, the next question was frozen as `docs/theory/10_eft_program/preregistrations/PREREG_THOMSON_RADIATION_SHELLS_v1.md` plus engine artifact `engine/tests/campaign_thomson_radiation_shells.cpp` (SHA256 `a47de9c1bb52f92a6dc35471f4eba516fb76acaf9b66abd3de44dd6431d67edf`). It keeps the fixed FTD-0288/0289 setup (`L=33`, `ticks=200`, mode 4, amp 0.05) and measures residual-field Poynting flux `S_res = (-W_res) × curl(J_res)` through shells `{5,7,9,11,13,15}` after subtracting `beam_only` and `charge_only`. Outcomes were frozen as `BASELINE_RADIATION_METER_INVALIDATED`, `NATIVE_LEGACY_RESIDUAL_RADIATION_DETECTED`, `NATIVE_EMERGENT_STRUCTURED_RESIDUAL_RADIATION_DETECTED`, `NATIVE_EMERGENT_OUTWARD_RESIDUAL_POWER_DETECTED`, or `NO_BASELINE_SUBTRACTED_OUTWARD_POWER`. This is a radiation instrument only: **no alpha derivation, no Thomson cross-section, no QED amplitude, no scan.**

**Prior latest update:** 2026-06-13 — **BASELINE-SUBTRACTED THOMSON FLUX-EXCESS RUN: NATIVE EMERGENT EXCESS FLUX DEFLECTION DETECTED (FTD-0289)** — pre-registered at `acb4005a` / tag `preregister-thomson-flux-excess-v1`, then run once with `ctest --test-dir engine/build -C Release -R "^thomson_flux_excess$" --output-on-failure`. Analysis: `docs/theory/10_eft_program/ANALYSIS_THOMSON_FLUX_EXCESS_v1.md`. Result: locked-linear residual stays at machine noise (`max_abs=4.35e-16`, `rel_l2=3.89e-15`); native legacy also stays at machine noise (`max_abs=4.34e-16`, `rel_l2=3.88e-15`); native emergent flux-gradient path leaves an above-gate residual after subtracting `beam_only` and `charge_only` (`l2=4.2546065759857619e-4`, `rel_l2=5.4899329705502643e-5`, `max_abs=6.3648580289611865e-5`). Verdict: `NATIVE_EMERGENT_EXCESS_FLUX_DEFLECTION_DETECTED`; frozen transverse-centroid subtype not met. Dashboard P1 Thomson/fine-structure panels now show live visual-bridge residual telemetry plus the canonical C++ FTD-0289 values; browser verification reached tick 200 with no fresh console errors, and the JS mock live residual itself remained zero while flux centroid/carrier motion were nonzero. **No Thomson cross-section, QED amplitude, or alpha claim promoted.** Next honest move: derive or measure a native radiation/angular-power observable before any continuum scattering or alpha interpretation.

**Prior latest update:** 2026-06-13 — **THOMSON DASHBOARD OBSERVATORY ADDED; RECOIL NOT OBSERVED (FTD-0287)** — added the visual scale-0 scenario `s0-field-thomson-scattering`, a live P1 "Thomson observatory" card, and native companion campaign `engine/tests/campaign_thomson_recoil_observatory.cpp`. Run: `ctest --test-dir engine/build -C Release -R "^thomson_recoil_observatory$" --output-on-failure` PASS in 0.43 s. Analysis: `docs/theory/10_eft_program/ANALYSIS_THOMSON_RECOIL_OBSERVATORY_v1.md`. Protocol: L=33, y-polarized plane wave (`mode_n=4`, amp 0.05) plus one locked negative charge, with wave propagation and state-flux coupling ON but damping/Gauss/forces/movement/Poisson OFF. Result: combined field equals `beam_only + electron_only` to machine precision (`max_abs=4.3465665094943873e-16`, `rel_l2=3.8944842137800063e-15`; repeat bit-exact). Verdict: `LINEAR_SUPERPOSITION_NO_RECOIL_OBSERVED`. The dashboard is a useful field-ingredient instrument; it is **not** nonlinear Thomson scattering, mechanical electron recoil, or an alpha derivation. Next honest recoil protocol must add a frozen unlocked-charge/force/radiation observable before interpretation.

**Prior latest update:** 2026-06-13 — **UNLOCKED THOMSON RECOIL RUN: NATIVE EMERGENT FLUX-GRADIENT RECOIL DETECTED (FTD-0288)** — pre-registered at `7260b274` / tag `preregister-thomson-unlocked-recoil-v1`, then run once with `ctest --test-dir engine/build -C Release -R "^thomson_unlocked_recoil$" --output-on-failure`. Analysis: `docs/theory/10_eft_program/ANALYSIS_THOMSON_UNLOCKED_RECOIL_v1.md`. Result: legacy native force gives no beam-induced recoil above gate (`disp_mag=1.68e-17`, `vel_mag=2.04e-19`); native emergent flux-gradient force gives deterministic recoil (`disp_mag=0.2102203195`, `vel_mag=0.0018928966`, repeat max_abs=0); explicitly imposed diagnostic qE also responds transversely (`disp_y=-0.0025053079`, `vel_y=0.0003338724`). Dashboard companion `s0-field-thomson-unlocked-recoil` added and browser-verified on `?engine=mock`: after 60 ticks the visual readout shows displacement `(-6.24e-2, 9.75e-2, 0)` and `|F_emergent|=2.66e-4`, with zero console errors. **No Thomson cross-section, QED amplitude, or alpha claim promoted.** Next honest move: separate intensity/flux-gradient recoil from direct electric qE recoil, then only after that design a radiation/angular-power observable.

**Prior latest update:** 2026-06-13 — **ALPHA ESTIMATOR VALIDATION CLOSED: ENERGY-FUNCTIONAL MISMATCH (FTD-0286)** — locked at `7cf8bd5c` / tag `preregister-alpha-estimator-validation-v1`, then run once with `ctest --test-dir engine/build -C Release -R "^alpha_estimator_validation$" --output-on-failure`. Analysis: `docs/theory/10_eft_program/ANALYSIS_ALPHA_ESTIMATOR_VALIDATION_v1.md`. Result: production live-tick fails the absolute Phase-G gate (`mean_ratio=0.43984307738805`, `max_rel_err=0.621340119549288`); matched-stencil projection converges (`worst_deep_vacuum_after=3.05175804230584e-05`) but still fails the same gate (`mean_ratio=0.501027980427946`, `max_rel_err=0.499164316784761`). Verdict: `ENERGY_FUNCTIONAL_MISMATCH`. The obstruction is not just the production Gauss stencil; the field-energy difference observable is not normalized to analytic `2rG_L(r)` in this finite protocol. **No alpha derivation, no Postulate-W arm, no tuning, no claim promoted.** Next honest move: derive or directly measure the correct finite-cell observable (potential response, source-response work, or proven energy normalization) before reopening the no-alpha-vs-Postulate-W discriminator.

**Prior latest update:** 2026-06-13 — **ALPHA ESTIMATOR VALIDATION PREREGISTERED (FTD-0286)** — after FTD-0285 invalidated its finite-protocol absolute Phase-G gate, the next move was frozen as `docs/theory/10_eft_program/preregistrations/PREREG_ALPHA_ESTIMATOR_VALIDATION_v1.md` plus engine artifact `engine/tests/campaign_alpha_estimator_validation.cpp` (SHA256 `dce6018d4ccc7565c1bab6870c9a90647f1bc4290c0fed600cac0fd3883ee570`). It compares the production live-tick Gauss estimator (`L=32`, `r={5,7,9}`, `ticks=300`, `SOR=100`) with the existing matched-stencil EFT projector (`matched_gauss_project`, tol `1e-10`, max_iter `400`) against the same analytic `2rG_L(r)` gate. Outcomes were frozen as `MATCHED_ESTIMATOR_CONFIRMED_PRODUCTION_GATE_INVALID`, `ENERGY_FUNCTIONAL_MISMATCH`, `PRODUCTION_AND_MATCHED_GATES_CONFIRMED`, or `MATCHED_PROJECTOR_FAILED_TO_CONVERGE`. This is an instrument-validation campaign only: **no alpha derivation, no Postulate-W arm, no tuning of FTD-0285, no claim promoted.**

**Prior latest update:** 2026-06-13 — **ALPHA NO-ALPHA ENGINE PROBE INVALIDATED (FTD-0285)** — locked at `cce615b0` / tag `preregister-alpha-no-alpha-engine-probe-v1`, then run once with `ctest --test-dir engine/build -C Release -R "^alpha_no_alpha_probe$" --output-on-failure`. Result: `INVALIDATED_PROTOCOL_OR_ENGINE_DRIFT`, analysis in `docs/theory/10_eft_program/ANALYSIS_ALPHA_NO_ALPHA_ENGINE_PROBE_v1.md`. The native no-alpha arm did **not** match the analytic Phase-G absolute gate (`mean_g_dyn²=0.439843`, pass=false), and the explicit Postulate-W control also failed that absolute gate (`mean_g_dyn²=0.020167`, pass=false). However, the ratio `control/native = 0.0458505607703448 = g_W²` exactly: the coupling knob scales correctly, but the finite live-engine protocol at `L=32`, `r={5,7,9}`, `ticks=300`, `SOR=100` is not calibrated to the analytic `2rG_L(r)` normalization used by the frozen gate. **No alpha derivation; no native-null verdict either; no claim promoted or demoted.** Next honest move: pre-register a v2 that first validates the live-engine estimator under the same dynamic protocol, then separately asks the no-alpha-vs-Postulate-W question. Do not tune FTD-0285.

**Prior latest update:** 2026-06-13 — **ALPHA NO-ALPHA ENGINE PROBE PREREGISTERED (FTD-0285, locked/run invalidated)** — FTD-0284 is now locked at `1584fe95` / tag `preregister-alpha-dynamical-readout-v1`; it remains a static discriminator, not a measurement. The next step was frozen as `docs/theory/10_eft_program/preregistrations/PREREG_ALPHA_NO_ALPHA_ENGINE_PROBE_v1.md` plus engine artifact `engine/tests/campaign_alpha_no_alpha_probe.cpp` (SHA256 `883a917358077f626e90f5affacabd8e565f48fd1cc00aa775ac6e4c7ffbdade`). The run fixed `L=32`, `r={5,7,9}`, `ticks=300`, `SOR=100`, CPU backend, native `coulomb_charge_coupling=1.0`, and leak guards disabling `coupling`, damping, Poisson/forces/Lorentz, and clock hooks. Arm A tested the no-alpha native Gauss-projection response; Arm B was an explicit `g_W²=2π/x_+` Postulate-W positive control and cannot count as a derivation.

**Prior latest update:** 2026-06-13 — **ALPHA CONTINUATION PREREGISTERED (FTD-0284, locked)** — after checking the latest alpha trail, the live boundary is no longer "find a prettier operator." FTD-0242 says 0/4 FTD-native routes force `(Tr,Det)=(16G*²,16G*³)`; FTD-0244 closes the substrate-native operator calculus route theorem-negative, so alpha is dynamical/not structural unless a coupling is selected by dynamics. Added `docs/theory/10_eft_program/preregistrations/PREREG_ALPHA_DYNAMICAL_READOUT_v1.md` and static verifier `scripts/proofs/proof_alpha_dynamical_readout_contract.py` (SHA256 `5a4509ad24dc9be1354b31dfa5336eb573e0eaaafa2e8607fda489748f3af390`). The verifier passes and records the key sanity check: native unit geometric Coulomb gives `1/(2π)`, which is `21.809984... × (1/x_+)`; matching the master-quadratic alpha would require `g_match²=2π/x_+` (`g_match=0.214127...`), and **g_match is not a derivation**. FTD-0284 freezes outcomes `NATIVE-NULL`, `DYNAMICAL-FOUND`, `POSTULATE-W`, and `INVALIDATED`; no engine run has been claimed, no near-miss search was run, and FTD-0013/MC-T4.3 remain unchanged.

**Prior latest update:** 2026-06-13 — **ATOMIC PARADIGM CAMPAIGN MIXED RESULT (FTD-0280–0283)** — lock commit `fe55b42f` tagged `preregister-atomic-sector-hardening-v1`, `preregister-db-clock-coulomb-spectroscopy-v1`, `preregister-atomic-exchange-correlation-wall-v1`, and `preregister-atomic-no-new-knob-ladder-v1`; canonical analysis: `docs/theory/10_eft_program/ANALYSIS_ATOMIC_PARADIGM_CAMPAIGN_v1.md`. **FTD-0280 replay CONFIRMED:** inherited locked records return `HYDROGEN-CONFIRMED` and `HELIUM-CONFIRMED`. **FTD-0281 hook CONFIRMED:** default-off `db_clock_coulomb` CPU/RenderBridge hook passes validation/sign smoke and `render_bridge_golden`; FFT spectroscopy verdict remains downstream. **FTD-0282 wall CONFIRMED:** under fixed I1+I2+I3, dynamic Pauli exchange, ortho/para splitting, and correlation energy remain unrepresented rather than fitted. **FTD-0283 no-new-knob ladder NOT CONFIRMED:** fixed `omega0=1.5`, `q_unit=0.3490`, `L=48`, hydrogenic Z={1,2,3}, helium-like Z={2,3}; non-tachyonic PASS, monotone gap PASS, helium-like SCF converged PASS, but frozen `gap12/Z²` spread FAILS (`0.3973916858 > 0.25`). Result: conditional H/He atomic sector hardened and boundary mapped, but no completed new atomic paradigm; no tuning, no lab-line matching, no promotions. FTD-0270/0271/0278/0279, FC-1, FTD-0013, and MC-T4.3 unchanged. Next honest move: preregister either the live FFT spectroscopy comparison or a v2 scale/continuum-limit ladder question; do not relax the failed FTD-0283 gate.

**⚠ Cross-cutting correction (2026-06-12, adversarial audit — `CORRECTION_FTD0278_HYDROGEN_MULTIPLET.md`):** a 4-lens audit workflow (independently re-verified) found the hydrogen **"n=2 multiplet / O_h T1u triple / Rydberg ladder"** (FTD-0278) OVERCLAIMED — only the 1s is bound in most record cells (n_bound=1 at a₀=4; only a₀=2.5/L=64 reaches n_bound=5); the "exactly degenerate T1u triple" is **torus momentum degeneracy, not bound 2p orbitals** (F-C passes even for a *repulsive* core ⇒ **F-C RETRACTED, non-probative**). Verdict narrowed to **HYDROGEN-1s-CONFIRMED** (the 1s ground state is genuinely bound + Coulombic via F-A/F-B reinterpreted as ground-state statements; the n≥2 ladder is NOT bound on these finite lattices = a mapped BOUNDARY). **This bears on the concurrent FTD-0282/0283**, which used the multiplet / `gap12` basis: `gap12` is 1s-dominated, so FTD-0283's `gap12/Z²` ladder is effectively a 1s-binding-vs-Z² test — its falsification (spread 0.397 > 0.25) STANDS, now understood as the finite-lattice 1s not scaling cleanly as Z². FTD-0279 (helium, ground-state SCF) UNAFFECTED. (My redundant Leg-2 `db_clock_coulomb` toggle was dropped in favor of the canonical FTD-0281 implementation on main.)

**Prior latest update:** 2026-06-12 — **HELIUM-CONFIRMED (FTD-0279)** — the original helium challenge (the prompt behind FTD-0270's "~0% derived") answered at its honest ceiling. **GIVEN** the FTD-0278 register + **one new declared import (I3, mode-occupancy: two electrons = two occupations of the field's bound modes, mutually sourcing via Gauss — the minimal second-quantization slice; a classical field has no L⁶ configuration space)**, restricted Hartree SCF on the engine's exact machinery — nuclear attraction AND e-e repulsion BOTH from the OT-1.4 Green's function — produces mean-field helium: pre-registered (tag `preregister-helium-lattice-scf-v1`, lock `310ad4ee`), ALL gates PASS — F-He-A dimensionless ENG≈REF (|Δσ|≤0.015, |Δion|≤0.030); **F-He-B (blind L=64 leg)** screening σ converging 0.6767→0.6929 toward the continuum 0.7154 (Z_eff=27/16 physics), trend held; F-He-C ionization ratio 0.353→0.386 toward 0.4519; F-He-D eigenpath cross-check ≤3e-14. Methodology: absolute energies are core-convention artifacts; dimensionless observables agree across conventions to ~2% — falsifiers frozen dimensionless-only. **The declared boundary beyond: correlation (1.4%) = configuration-space entanglement = the FC-1 ceiling of I3 — the conditional sector ends exactly where the framework's commitment says.** Nothing promoted; FTD-0270 unconditional verdict stands; next id FTD-0280. **Prior (same day): HYDROGEN-CONFIRMED (FTD-0278 Legs 0+1)** — the "biggest-win" arc from the emergent-physics roadmap. **GIVEN** the de Broglie clock (ω₀∝M_REST [IMPOSED], covariant rate FTD-native) + a scalar-potential coupling ω_eff²(r)=ω₀²+2ω₀V(r) (the engine's gravity-clock move applied to the Gauss φ), **the engine's exact lattice machinery produces a hydrogen-like bound-state spectrum**: pre-registered (tag `preregister-hydrogen-lattice-spectrum-v1`, lock `6be49fe9`), ALL frozen gates PASS — F-A the engine's own Gauss potential is Coulombic (lattice/1/r gap ratios 0.952–1.021, ±0.05 band); **F-B (BLIND leg)** Rydberg ratio strictly decreasing 1.975→1.447→1.138 across a₀={2.5,3,4}, endpoint in the frozen (1.0,1.40) band — the ladder converges to the continuum Rydberg; F-C the n=2 quadruple = A1g + EXACTLY degenerate T1u triple in every cell (O_h rep theory) **[⚠ F-C SINCE RETRACTED — torus momentum degeneracy, not bound 2p; see the cross-cutting correction above]**; F-E causal control reproduces FTD-0270 (s=0.944). Result class `[CONDITIONAL — DERIVED-GIVEN-IMPOSED-INPUT]` — flips "atomic dynamics ~0% derived" (FTD-0270, stands unconditionally) into a conditional sector with a one-scalar + one-coupling import surface. **Leg 0:** guidance-absence no-go formalized (`DERIV_GUIDANCE_ABSENCE_NOGO.md` — the force law has NO ∇S term [THEOREM at engine level]; Bohmian guidance = sixth-postulate-class input). **HONESTY: KG-in-Coulomb is textbook — never "FTD derives QM"; FC-1 stands; nothing promoted. Leg 2 QUEUED:** engine-native demonstration (`db_clock_coulomb` toggle + time-series FFT campaign, F-5 cross-check vs the operator ω_n; golden gate must stay `0x56fa28acb5b9fe88`). Next id FTD-0279. **Prior (same day): FTD-0276 drain scaling (Arc 2)** — Arc 2 of the 4-arc emergent-physics forward program. Executes the FTD-0269 §4 follow-ups; **drain² origin CLOSED NEGATIVE**. Infrastructure: promoted `K_GENESIS_KINETIC_DRAIN` to a runtime `TermToggles.kinetic_drain` field (CPU + GPU single path, langevin_gamma threading pattern); golden bit-identical `0x56fa28acb5b9fe88` at default 0.5. **(Leg A, pre-registered `preregister-drain-scaling-v1`, lock `1dc4db23`):** sub-knee k_eff = N̄/A² is monotone **decreasing** in drain ({0.147→0.029} across drain {0.125→0.75}), fit `k_eff ∝ drain^{−0.93}` (≈ 1/drain) — the drain is a linear calibration prefactor, NOT a squared structural quantity. R1 (exponent −0.93 ∉ [1.8,2.2]) + R2 (k_eff(0.5)=0.059 ≠ 0.25) both fail → CLOSED-NEGATIVE. (At default drain 0.5 the instrument reproduces FTD-0261 exactly: N̄(10)=4.0, k_eff≈0.05.) **(Leg B):** friction-knob map of the N(A) forward model — γ=0 → knee 14, super-knee over-predict ×1.53 (reproduces FTD-0269 framework-only); the engine's actual γ=0.02 → knee 16, p_hi 1.81, matching FTD-0261 (16, 1.86). γ is the super-knee calibrator (calibrates, doesn't derive). **(Leg C):** no framework source for the drain — quadrature equipartition (½) can't give a drain-DEPENDENT k_eff; `1−1/N_base=0.75≠0.5`. Corrects `FOUND_LATTICE_SPACING_GAUGE_FREEDOM` §12 (its "k is engine-parameter-independent" claim falsified on the current stack). The linear k=¼ theorem (FTD-0110, O_h rep theory) is mathematics, untouched. Nothing promoted; next id FTD-0277. **Prior arc (same day): thermal phase map, run of record (FTD-0275, §0.39)** — Arc 1 of the program. Promotes the FTD-0274 scout to a pre-registered run (tag `preregister-thermal-phase-map-v1`, lock `74caabec`; instrument + analyzer SHA256-frozen before the runs) and revises it on all three questions. **(Q1) RISES-CONFIRMED:** T_up = {0.0133, 0.0253, 0.0393, 0.0587} for L = {16,24,32,48} (3 seeds each) — monotone, span 5.7× pooled σ; best descriptive fit T_up ≈ 0.000324·L^1.36 (super-linear); the scout's "tighter ignites lower" quantified. (L=64 right-censored — CSV tail lost to operator-stop + un-flushed buffer; console s=0=0.064 on-trend, excluded; fixed post-lock with `setvbuf`.) **(Q2) SAFETY-VALVE-FALSIFIED:** genesis-OFF ablation and genesis-ON control BOTH stable to T=12 (T_kin ≈ 108× c²) ⇒ the high-T stability is the Langevin OU mean-reversion, NOT the manifestation rule; the scout's safety-valve `[CONJECTURE]` mechanism is withdrawn (the no-runaway *observation* stands). **(Q3) DETONATES:** the f=0.90, A=60 spark cell detonates 3/3 vs a clean control — a large local spark in a near-critical bath tips the whole lattice (the scout's open "flammable atmosphere" case); honest caveat: modest at 3 seeds (1/3 spontaneous background, non-monotone in f, p≈0.037), ≥20-seed confirm queued. Golden gate green (`0x56fa28acb5b9fe88`). Nothing promoted; next id FTD-0276. See §0.39 (detail section pending; LEDGER FTD-0275 + `ANALYSIS_THERMAL_PHASE_MAP_v1.md` are canonical).

**Prior update (same day):** 2026-06-11 — **thermal ignition scout (FTD-0274, §0.38)** [SUPERSEDED by FTD-0275 on Q1/Q2/Q3]: bracketing run mapping the temperature axis. (1) **Minimum:** void condenses first-order at T_up≈0.05 (L=24), rising with L (finite-size nucleation — tighter lattices ignite lower); self-sustaining condensate stays pinned m=1 to T=0 on cooling. (2) **No maximum found:** T_max~c² hypothesis FALSIFIED — T_set=6 drives T_kin=9.2 (27×c²), lattice stable; no ceiling found up to T_set=6 in this scout; safety-valve interpretation (genesis absorbs arbitrary heat) is `[CONJECTURE]` (mechanism not ablated). (3) **Ignition:** every A∈{2..60} at L=24 settles BOUNDED; detonation is global/thermal (crossing T_up), never local. Web: docked Thermo side panel (`thermo-panel.js`), `setLangevinTemp` WASM binding, `s0-seed-thermal-ignition` scenario. Nothing promoted; next id FTD-0275. See §0.38.

**Prior update (same day):** 2026-06-11 — **mass as flux-energy + quark quantization (FTD-0273, §0.37)**: pre-registered (verdict logic frozen by analyzer SHAs; prior-favoured BOUNDARY confirmed). Flip quantum ε=½·K_GENESIS²≈1.1750 (K_GENESIS=1.533); re-expressing mass as flux energy COLLAPSES to voxel count N (Pearson r=0.990, per-L; no per-particle plateaus; "511 quanta=electron" **FALSIFIED**; A_min=1 voxel at electron amplitude A=2). Local flux decays ≈L⁻³·⁸ toward zero on bigger boxes — radiation escapes; flux is heat, N is conserved matter-count. Whole-lattice flux is halo-dominated. Phase 0 determinism gate PASS (earlier N=2/3/10 was the Langevin thermostat, NOT a race); langevin-OFF harness bit-deterministic; golden `0x56fa28acb5b9fe88` GREEN. Color sector: quarks COLOR-PHENOMENA-IMPOSED (seeded N=1; 3-regime force profile; large-r **F=r/64, harmonic V∝r²**, NOT QCD constant tension — `phase_forces.cpp:167` comment corrected); geometric triad binding fires. Nothing promoted. See §0.37.

**Prior update (same day):** 2026-06-11 — **genesis first-order; cluster ladder NOT RG-derived (FTD-0272, §0.36)**: pre-registered (`c7371211`; prior ~55% first-order — landed). Three independent signatures: (a) maximal hysteresis loop — heating m:0→0.92 at T_up≈0.04, cooling pinned m=1 to T=0; loop persists/widens at L=32; (b) discontinuous jump m:~0→~1 across ΔT=0.005; (c) Binder cumulant U4=−0.54 (phase coexistence — U4 bounded [0,⅔] at any critical point). Consequence: no diverging correlation length, no RG fixed point, no scaling spectrum ⇒ **cluster-mass law N(A) is energy-budget / pattern formation on a first-order background, NOT RG-derived**. Combined with the Gaussian free sector (FTD-0050), FTD derives no mass spectrum via renormalization in either sector. FTD-0110 cluster law unchanged — now understood as NOT RG-derived. Nothing promoted. See §0.36.

**Prior update (same day):** 2026-06-11 — **de Broglie pilot wave; GUIDANCE-ABSENT is the key result (FTD-0271, §0.35)**: pre-registered (`3df173b1`). GIVEN an imposed KG rest-mass term `−ω₀²J` (ω₀∝M_REST; native flux massless — A0 on record), the KG envelope is Schrödinger (E_env∝L^−1.843, s≈2, exactly the sector FTD-0270 said was the only door) and a moving packet carries a de Broglie wave (λ∝v^−1.113, r≈1, vs massless r=0). Honest ceiling: `[CONDITIONAL — DERIVED-GIVEN-IMPOSED-INPUT]`. FTD-native A5: proper-time covariant rate FTD-0252-sourced; covariant half `[IMPOSED]→[SELECTION]`. **GUIDANCE-ABSENT (the key non-circular result):** a cluster in a pure-phase pilot wave acquires v_meas≈1e-19 (Coulomb control drifts +1.4e-3) — FTD's force law has NO ∇S→force term; matter moves by magnitude-derived forces; the wave does NOT guide the particle. Engine: `de_broglie_clock` toggle in `phase_read.cpp` + `phase` voxel field; golden `0x56fa28acb5b9fe88` byte-identical; `test_de_broglie_{clock,redshift}` + `campaign_de_broglie_guidance` all pass/confirm. Nothing promoted; FTD-0270 still true for unmodified FTD. See §0.35.

**Prior update (same day):** 2026-06-11 — **atomic dynamics ~0% substrate-derivable; WRONG DISPERSION (FTD-0270, §0.34)**: prompted by the helium challenge (FTD cannot compute atomic spectra). **Track A:** `AUDIT_ATOMIC_DYNAMICS_STATUS.md` + de-overclaim corrections (hydrogen benchmark = classical Kepler; orbitals.js = visualization; Lamb shift = `[PARAMETRIC]`; FC-1 declines QM recovery). **Track B** (pre-reg lock `e1c7377f`): attacked discrete-level quantization without importing ℏ. Result: the lattice DOES quantize (discrete Dirichlet-box modes — previously listed ABSENT), but the engine's 18-pt wave operator carries **linear dispersion** (ω₁∝L^−0.944, s≈1) vs Schrödinger s≈2 on the same eigenvectors; the 2nd-order-in-time leapfrog is structurally wrong for the Rydberg 1/n². Moving cluster: no de Broglie wave (r=0.00). Verdict: **QUANTIZATION EXISTS, WRONG DISPERSION** — atomic energy levels NOT substrate-derivable; the boundary IS the dispersion law. Open exit: a substrate-native 1st-order/quadratic-dispersion sector. Golden `0x56fa28acb5b9fe88` GREEN. Nothing promoted. See §0.34.

**Prior update (same day):** 2026-06-11 — **FTD-0110 nonlinear-bridge attack (FTD-0269, §0.33)**: the user asked to attack the bridge "comprehensively," recalling "we solved it." We had not — a `[DERIVED]`-tagged `DERIV_FTD0110_GENESIS_THROTTLE.md` claimed "the bridge is closed" but the LEDGER never cited it and kept the bridge `[OPEN]` (a 3-point Monte-Carlo count match, no Gauss, knee mislocated 23.5 vs 16). Built and **pre-registered** (`preregister-ftd0110-na-law-v1`, lock `192203b5`) a substrate-parameter forward model adding the two flux-injection channels the prior attempt omitted — the coupling source `G_C·∇s` and the **FFT-exact Gauss-projection boost** (lattice Poisson Green's function, symbol `M(k) = (2/3)Σcos kᵢ + (2/3)Σcos kᵢcos kⱼ − 4`) — plus a golden-neutral engine firing-geometry instrument (`campaign_genesis_geometry.cpp`, post-tick state diff). **Result (verdict BOUNDARY, mechanical adjudicator):** the framework-derived dynamics reproduce the law's **SHAPE** — broken power, fit knee = 14 ∈ [14,18], super-knee exponent 2.07 ∈ [1.6,2.1], sub-knee normalization near-exact (A≤20 ratios 0.97–1.09), sub-knee firing **geometry** matching the engine (shell-occupancy L1 = 0.18 at A=14) — and the **Gauss boost is confirmed the decisive sub-knee ingredient** (gauss-off collapses N(14) 17.9→7, the 23.5-vs-16 knee story now quantified). **But the law is ENGINE-EMERGENT:** the kinetic drain (engine-tuning) shifts the knee by 16 across {0.25,0.5,0.75}; √α-coupling is super-knee load-bearing (off/on RMS 0.118); the absent Langevin friction γ over-predicts super-knee ~1.8× (curve-RMS 0.170). The discrete substrate + O_h Laplacian + Gauss Green's function fix the geometric shape; the absolute calibration is set by non-framework constants. Bridge stays `[OPEN]` as a clean-derivation target; the boundary is mapped (clean derivation needs the 0.5 drain + γ derived from the action, or the calibration shown to be convention). `DERIV_FTD0110_GENESIS_THROTTLE.md` re-tagged `[SUPERSEDED]`; the matching `[RESOLVED]` zombie line in TRACKER_OPEN_ITEMS corrected. **Surfaced a separate flagged regression:** the 8-color-SOR Gauss optimization made the golden-hash *energy audit* non-deterministic (4 distinct hashes across runs) — the genesis *field* is bit-reproducible (verified: identical-seed firing geometry), so the campaign is sound, but the golden gate needs a deterministic-reduction fix + re-pin (background task spawned). Artifacts: `PREREG_FTD0110_NA_LAW_v1.md`, `ANALYSIS_FTD0110_NA_LAW.md`, LEDGER FTD-0269, manifest row; runs of record `scripts/exploration/results/na_law_2026-06-11/` + `engine/results/genesis_geometry_2026-06-11/`. **Nothing promoted** (FTD-0013 [SMC], MC-T4.3, SM cluster-mass identification, linear k=¼ O_h theorem — all unchanged). See §0.33 below.

**Prior update (same day):** 2026-06-11 — **forward-predictions arc (FTD-0268, §0.32)**: a predictions-on-behalf-of-the-framework session under full pre-registration discipline. **(1) Blind engine prediction CONFIRMED:** per-group 95% prediction intervals for the nine ⟨100⟩/n⊥=3 time-dilation residuals at the never-measured **L=257** were hash-locked (`ee8976b6`, tag `preregister-time-dilation-L257-blind-v1`) before the run; the run landed **7/9 inside (the frozen threshold exactly), median residual 0.002517 → 0.001415 ⇒ PREDICTION_CONFIRMED** `[MEASURED — blind extension]`; PL-4's measured domain extends to L=257; the two lowest-velocity misses (opposite directions, |R| ~ 10⁻⁵) are an `[OBSERVATION]` (signed-residual zero-crossing floor, not a stall); the frozen v2 secondary analysis over the six-point sweep **strengthens IR_CONFIRMED** (median ratio 0.153 → 0.109). Registered refinement: per-group convergence exponents drift p ≈ 2.6 (low v) → 0.7 (v → 0.94) — PL-4's "∝ L⁻²" is a low-velocity statement. **(2) Forward-predictions registry shipped** (`docs/theory/01_reference/SPEC_PREDICTIONS_FORWARD_2026.md` `[SYNTHESIS]`): FP-1 first-order EWPT ⇒ LISA-band relic GW background (SM crossover ⇒ none; cosmological identification `[CONJECTURE]`); FP-2 Higgs self-coupling κ_λ = 1.0089 from the fixed λ = 3/23 (FCC-decidable; kill = |κ_λ−1| > 0.1); FP-3 LV nulls at dimension 5/6 (a *detection* of linear/quadratic-Planck LV kills the wave sector; FTD's first anisotropy is k⁴); FP-4 structural nulls (cross-ref PL-6); EP-2 N(A) blind bands registered AND run same session after the bands committed: **N̄(35) = 66.1, N̄(60) = 192.3 — BOTH INSIDE, 2/2 CONFIRMED** (FTD-0261 rig unchanged; k_eff ≈ 0.053 both points); EP-3 k⁴ extrapolation **verified** at L ∈ {512, 768} (pred/exact 0.9997/0.9995; mpmath required — double precision underflows past L ≈ 384, a newly-noted boundary). All numbers computed by `scripts/exploration/forward_predictions_2026.py`. **(3) Honest Higgs digest:** tree-level m_H = 125.69 GeV is **+4.44σ** vs the canonical PDG 2024 combined (125.20 ± 0.11, `constants.py` `Experimental`); only the applied-not-derived (1−α) loop factor brings it to +0.27σ — the "1σ Higgs" headline is loop-factor-dependent, and the chain is `[SELECTION]` (v = M_P√(2π)α⁸) + `[PARAMETRIC]` (sin²θ_W = 3/13 → λ = 3/23); the source doc's non-canonical PDG values (125.25 ± 0.17) and its HIGGS-5 tag inconsistency (§9 [SELECTION] vs §8.3 [STRUCTURALLY MOTIVATED PARAMETRIC]) are owner-flagged; **HIGGS-5 tag fixed** (§9 → `[STRUCTURALLY MOTIVATED PARAMETRIC]`, 2026-06-12 audit). **No engine source changes; golden gate untouched; ZERO promotions** (FTD-0013 [SMC], MC-T4.3 [FOUNDATIONAL OBSTRUCTION], FC-2 [AXIOM]-class, FTD-0252 scoped [MEASURED] — all unchanged). Artifacts: prereg + `ANALYSIS_TIME_DILATION_L257_BLIND_v1.md`; runs of record `engine/results/time_dilation_L257_blind_2026-06-11/` + combined-sweep dir; LEDGER FTD-0268; manifest + INDEX_01 updated. Prior update (§0.31): —

**Prior update:** 2026-06-10 — **genesis-vs-survival engine telemetry (FTD-0267, §0.31)**: the first *direct* engine measurement of genesis/evaporation EVENTS (two observation-only counters, golden-neutral) **falsified the β arc's premise**. At A=10 the engine fires ~5 genesis events (not the β-predicted ~23); `cumulative_genesis == peak_manifested` in all 8 seed-runs ⇒ genesis is a **one-shot early burst**; evaporation ≈ 0 ⇒ survival is high (67–93%). Verdict **SURVIVAL-NULL** — the suppression is **genesis-stage nonlinear throttling** (flux consumption + coupling + Gauss + damping quench the pulse before neighbours cross threshold), NOT post-genesis survival; **cluster size ≈ genesis-firing count**, sharpening the FTD-0110 nonlinear bridge `[OPEN]`. Converges with the concurrent β v2 model (FTD-0263 `BETA_v2_CONFIRMED`). This closed the β mechanism arc (FTD-0265 envelope BETA-PARTIAL → FTD-0266 dwell-time DWELL-FAIL → FTD-0267 direct telemetry SURVIVAL-NULL). **Golden gate green at the new canonical hash `0xebaa6f314f66db3f` @ L=17** (re-pinned after the backend-parity genesis refactor `c2a8f606`; the counters are proven observation-only — bit-identical hash with/without). Engine instrument committed (`8d3ff363`); analysis `ANALYSIS_GENESIS_SURVIVAL_TELEMETRY_v1.md`; run of record `engine/results/genesis_trajectory_2026-06-10/`. **Nothing promoted.** See §0.31 below for the full arc (§0.24–0.30 cover FTD-0259→0266). Prior major milestone (§0.23): —
...
**Older:** 2026-06-01 — two-part checkpoint. **(A)** The engine-flawless lifecycle/callstack/toggle audit (16 commits, branch `flawless-engine-2026-06-01`) landed a web verification harness + three C++ lifecycle/phase-order/conservation tests, fixed a 5-week-old clean-checkout `cmake` break (`_repro_gpu_empty_bridge` dangling reference), and pinned the energy-conservation leak to the non-variational Gauss-projection **operator** (not the solver tolerance). **(B)** MC-T4.3 was sharpened to a **route-invariant boundary** (`AUDIT_ALPHA_OPERATOR_FORCING_ROUTE_INVARIANCE.md`, FTD-0242): 0/4 FTD-native routes force the master-quadratic operator assembly, so **α is dynamical, not structural**. **Nothing promoted — `x₊ = 1/α` (FTD-0013) stays `[STRONGLY MOTIVATED CONJECTURE]`; MC-T4.3 stays `[FOUNDATIONAL OBSTRUCTION]`.** See §0.18 below.

---

## 0.38 · 2026-06-11 — FTD-0274: min/max temperature + ignition map — floor at T_up≈0.05; no ceiling found up to T_kin≈27×c²

A bracketing scout (not yet a pre-registered run of record) mapping the temperature axis of the FTD lattice. **Temperature definition:** equipartition kinetic temperature of the wave field `⟨½|wave_vel|²⟩ = (3/2)T` (k_B≡1; validated by `test_langevin_equipartition`).

**(1) Minimum landmark T_up (first-order condensation).** Heating the void by ramping `langevin_T` (no injection), the manifestation fraction m=N/L³ jumps first-order at T_up ≈ 0.05–0.06 at L=24 (matches FTD-0272). T_up RISES with L (finite-size nucleation: a small box is ~one correlation volume; tighter/smaller lattices ignite at LOWER T; loop widens with L). Below T_up = metastable thermal vacuum. Self-sustaining condensate: cooling from the condensed phase stays pinned at m=1.000 down to T=0 (T_down=0 — maximal hysteresis per FTD-0272).

**(2) No maximum temperature found in this scout.** Prior `[HYPOTHESIS]` of a CFL/causality ceiling T_max~c²=1/3 (thermal velocity → signal speed) is **FALSIFIED** — T_set=6 drives T_kin=9.2 (27×c²) with the lattice still stable (m=1, no blow-up, T_kin∝T_set slope≈1.53, no saturation). Reason `[CONJECTURE]`: the Langevin OU process is unconditionally stable AND the manifestation rule caps |J| at K_GENESIS, absorbing arbitrary heat into the bounded state field (this safety-valve mechanism has not been ablated — interpretation is inferred from the finite-T scan, not confirmed by a genesis-disabled comparison). No ceiling was found up to T_set=6 in this scout.

**(3) Ignition: local injection never detonates.** Deterministic (langevin OFF) amplitude sweep A∈{2..60} at L=24: every A settles BOUNDED — no flooding. A local energy dump grows a bounded droplet, not a detonation. The autocatalytic runaway is a global/thermal phenomenon (crossing T_up), not something a local spark triggers in the cold vacuum. Open: a local spark in a near-critical bath (langevin_T just below T_up) — the "flammable atmosphere" case.

Web deliverable: Scale-0 `s0-seed-thermal-ignition` scenario + a docked **Thermo side panel** (`thermo-panel.js`, registry id 'thermo'): temperature slider across T_up, live telemetries (T_kin, m, phase, energy ledger), flux |J| heat-map slice; `setLangevinTemp` WASM binding. **Nothing promoted; next id FTD-0275.** Docs: `ANALYSIS_THERMAL_IGNITION_v1.md`; artifact `engine/tests/campaign_thermal_ignition.cpp`.

---

## 0.37 · 2026-06-11 — FTD-0273: mass as flux-energy in flip-quanta — ENERGY-COLLAPSES-TO-N; quark color phenomena IMPOSED

Pre-registered (`PREREG_CLUSTER_ENERGY_SPECTROSCOPY_v1.md`, verdict logic frozen by analyzer SHAs before the run of record; prior-favoured BOUNDARY confirmed).

**Phase 0 (determinism gate, PASS).** The earlier N=2/3/10 non-reproducibility was traced to the Langevin thermostat (`s0-seed-emergent-ic1` force-sets langevin=true,T=0.005 — Brownian fingerprint), NOT an engine race. The langevin-OFF harness is bit-deterministic per (A,seed), omp1==pool bit-for-bit. Golden `0x56fa28acb5b9fe88` GREEN/unchanged. (N still varies with seed — genesis flip probabilistic via stateless voxel_uniform — so per-seed/seed-averaged reporting.)

**Phase 1 — ENERGY-COLLAPSES-TO-N.** Flip quantum ε = ½·K_GENESIS² ≈ 1.1750 (K_GENESIS = K_MANIFEST·N_C = 0.511·3 = 1.533). The undamped flux SLOSHES flux↔wave_vel every tick; the manifested count N is the stable cluster identity, not the instantaneous flux energy. Cluster-local flux tracks N (per-L Pearson r(N,M_local) = 0.995/0.984 at L=24/32, mean 0.990); per-voxel coefficient ~0.2–0.5 with ~40–70% halo scatter, NO per-particle plateaus. M_local ≈ c(L)·N — carries no information beyond N ⇒ COLLAPSES to the FTD-0110/0269 N(A)≈k·A² law. Whole-lattice flux is halo-dominated (A=8: N=1 but ~130 un-condensed free quanta). **A_min = 1 voxel at A=2 — the "511 quanta=electron" hypothesis is FALSIFIED.** L-convergence (A=10,16; L=24,32,48,64): N is L-invariant (A=10 → N≈4 at every L); local flux energy decays ≈L⁻³·⁸ toward zero (on a bigger box, radiation escapes rather than reflects; local mean → 0). The L-invariant mass is the voxel count; flux energy is box-dependent and vanishing. Thermodynamic reading: flux energy = heat (extensive, dilutes); cluster count N = conserved matter-count (frozen at nucleation burst).

**Phase 2 — COLOR-PHENOMENA-IMPOSED.** The 6 `s0-seed-*-quark` seeds stay N=1 (amps 0.26–1.28 < K_GENESIS — quarks are seeded fixed structures). The color force `f_strong` shows a clean 3-regime profile (Coulomb 1/r² for r<3; transition ~1/r for 3≤r<8; large-r **F = α_s·cf·r/COLOR_LINEAR_DENOM ∝ r** — harmonic, V∝r², NOT QCD constant string tension). Same-color force exactly ½ diff-color at every r. Geometric triad binding fires (locked=3 ON / 0 OFF; purely geometric, color-blind; TRIAD_RADIUS=3, ratio≥0.8). **Nothing promoted.** Docs: `ANALYSIS_CLUSTER_ENERGY_SPECTROSCOPY_v1.md`, `PREREG_CLUSTER_ENERGY_SPECTROSCOPY_v1.md`.

---

## 0.36 · 2026-06-11 — FTD-0272: genesis transition is first-order; cluster-mass ladder is NOT RG-derived

Pre-registered (`preregister-genesis-criticality-v1`, lock `c7371211`, pre-run stats amendment `05caee9a`; prior ~55% first-order — landed). Direct attack on "derive the lattice spectrum via renormalization." Narrowed to: is the genesis/manifestation transition 2nd-order critical (⇒ RG-derived spectrum) or first-order/trivial (⇒ no RG spectrum)?

**Three independent first-order signatures:** (a) **Maximal hysteresis loop** — heating from void jumps m:0.0001→0.92 at T_up≈0.04 (L=24); cooling from the manifested phase stays **pinned at m=1.000 down to T=0** (self-sustaining condensate: gauss/coupling self-field latches every voxel above K_GENESIS; no damping to relax). L=32 confirms the loop persists/widens (not finite-size). (b) **Discontinuous jump** m:~0→~1 across ΔT=0.005. (c) **Negative Binder cumulant** U4 = 1−⟨m⁴⟩/(3⟨m²⟩²) = **−0.54** (FSS scout; χ_max≈844; seed bistability 0.20↔0.77) — U4 is bounded [0,⅔] at any critical point; negative value signals phase coexistence.

**Consequence:** a first-order transition has no diverging correlation length, no critical fixed point, no scaling spectrum ⇒ the cluster-mass law N(A)≈¼A² is **energy-budget / pattern formation on a first-order background, NOT an RG-derived spectrum.** Combined with the Gaussian free sector (FTD-0050 `[CLOSED NEGATIVE]`), FTD derives no mass spectrum via renormalization in either sector. FTD-0110 cluster law unchanged — now understood as NOT RG-derived. The only RG-clean spectral result is the dispersion flowing to Lorentz-invariant continuum (anisotropy ~k⁴/PL-5), carrying no derived mass scale. **Nothing promoted.** Docs: `ANALYSIS_GENESIS_CRITICALITY_v1.md`, `PREREG_GENESIS_CRITICALITY_v1.md`.

---

## 0.35 · 2026-06-11 — FTD-0271: de Broglie pilot wave — GIVEN a rest-mass clock, FTD flux is a pilot wave; GUIDANCE-ABSENT is the key non-circular result

Pre-registered (`preregister-de-broglie-clock-v1`, lock `3df173b1`; prior D1+D2 ~90% textbook-KG disclosed as weak evidence). Closes FTD-0270's open exit by ADDING a KG rest-mass term `∂²J/∂t² = c²∇²J − ω₀²J`, ω₀∝M_REST.

**A0 gate (on record):** the flux dynamics has NO restoring term (`delta_j = c²·Lap + G_C·∇s`; genesis is a dissipative drain; FTD-0251 native clock is zero at k=0) ⇒ `ω₀∝M_REST is [IMPOSED]`; native flux is massless.

**B core (run of record):** with the clock, the KG envelope is Schrödinger — `E_env = ω₁−ω₀ ∝ L^−1.843` (s≈2, exactly the sector FTD-0270 said was the only door) vs massless s=0.944; and a moving KG packet carries a de Broglie wave (λ∝v^−1.113, r≈1, vs massless r=0.00). Honest ceiling: Schrödinger + de Broglie are textbook KG consequences — `[CONDITIONAL — DERIVED-GIVEN-IMPOSED-INPUT]`.

**FTD-native A5:** FTD-0252 already measured `accumulate_proper_time` dilates as √(1−v²); wiring `dφ/dt=ω₀·dτ/dt` sources the clock's relativistic behavior from FTD's own machinery (only ω₀∝M_REST imported); covariant half `[IMPOSED]→[SELECTION]`.

**E — GUIDANCE-ABSENT (the key non-circular result):** a cluster in a pure-phase pilot wave (uniform |J|) acquires v_meas≈1e-19 at every k (Coulomb control drifts +1.4e-3) — FTD's force law has NO term converting ∇S into a force on matter; matter moves by magnitude-derived forces. **The wave does NOT guide the particle.** A working pilot wave would require ADDING the de Broglie guidance equation — it is not emergent.

Engine artifacts (committed, branch `engine/de-broglie-clock-2026-06-11`, commits `56c495af`+`3fbeafa4`): `de_broglie_clock` default-OFF toggle + `omega0` scalar + `−ω₀²·J` term in `phase_read.cpp`; `phase` voxel field + `dφ=ω₀·dτ` in `accumulate_proper_time`; `test_de_broglie_clock` (static cluster oscillates at ω₀); `test_de_broglie_redshift` (moving clock dilates to √(1−v²)=0.95394 to machine precision); `campaign_de_broglie_guidance`. Golden `0x56fa28acb5b9fe88` byte-identical. Web: `s0-seed-de-broglie-clock` + Clock card in the docked Time Observatory panel. **Nothing promoted; FTD-0270 still true for unmodified FTD.**

---

## 0.34 · 2026-06-11 — FTD-0270: lattice quantizes but with wrong dispersion; atomic spectra NOT substrate-derivable

Prompted by the helium challenge (FTD cannot compute atomic spectra). Pre-registered Track B lock `e1c7377f`.

**Track A** shipped the canonical atomic-dynamics status map `AUDIT_ATOMIC_DYNAMICS_STATUS.md` and de-overclaim corrections: (i) hydrogen benchmark = classical Kepler check (generic to any 1/r force; NOT a quantum eigenvalue derivation); (ii) orbitals.js = visualization convenience, not a substrate result; (iii) Lamb shift = `[PARAMETRIC]` (standard QED one-loop with FTD's α inserted); (iv) g-2 = full QED (not substrate); (v) FC-1 formally declines QM recovery — FTD derives ~0% of atomic quantum dynamics.

**Track B (pre-registered, verdict BOUNDARY).** Attack: discrete-level quantization from lattice standing-wave eigenmodes, without importing ℏ. The engine's exact 18-pt wave operator symbol M(k) validated against the closed-form expression to 1.33e-15. **Result: the lattice DOES quantize** (discrete Dirichlet-box modes — previously listed as ABSENT), but the ground-mode finite-size exponent is **ω₁ ∝ L^−0.944 ± 0.005** (s≈1, cavity/EM-like), vs the Schrödinger-analog diagnostic on the SAME eigenvectors: **E₁ ∝ L^−1.887 ± 0.010** (s≈2). The 2nd-order-in-time leapfrog (√(−M) operator) is precisely what makes the dispersion linear; the hydrogen Rydberg 1/n² needs quadratic Schrödinger dispersion. Moving-cluster de Broglie test: r=0.00 (wave absent at any v). **Verdict: QUANTIZATION EXISTS, WRONG DISPERSION.** Discrete atomic energy levels are NOT substrate-derivable; the boundary IS the dispersion law — sharper than "ℏ is missing." ℏ enters only as the energy scale (E=ℏω), never fixed by the substrate, and the shape is already wrong. Open exit: a substrate-native 1st-order/quadratic-dispersion sector (none at the linear level). Golden `0x56fa28acb5b9fe88` GREEN. **Nothing promoted.** Docs: `ANALYSIS_QUANTIZATION_LATTICE_MODES_v1.md`, `AUDIT_ATOMIC_DYNAMICS_STATUS.md`.

---

## 0.33 · 2026-06-11 — FTD-0269: FTD-0110 nonlinear bridge — N(A) law is engine-emergent (BOUNDARY)

The user asked to attack the bridge comprehensively, recalling "we solved it." The audit found a `[DERIVED]`-tagged `DERIV_FTD0110_GENESIS_THROTTLE.md` asserting "the bridge is closed" that the canonical LEDGER never cited and that, on its own evidence, was a 20-seed Monte-Carlo of a *reduced* automaton matching engine genesis **counts at 3 amplitudes** (failing A=9), omitting the Gauss boost, and mislocating the knee (23.5 vs 16). The decisive missing physics: two flux-injection channels from manifested voxels into the void — the coupling source `G_C·∇s` (phase_read, `G_C=√α`) and the Gauss-projection boost `flux[void] −= ∇φ` (gauss_project, the lattice Poisson Green's function = the already-theorem-grade Phase-G geometric Coulomb). Built a substrate-parameter forward model including both (FFT-exact Gauss via the 18-pt Laplacian symbol `M(k)`), an `analyze_na_law.py` mechanical adjudicator, and a golden-neutral engine instrument (`campaign_genesis_geometry.cpp`, read-only post-tick state diff), all **hash-locked** under `preregister-ftd0110-na-law-v1` (lock `192203b5`) before any run of record. **Verdict BOUNDARY:** framework-derived dynamics reproduce the law's geometric SHAPE (knee 14, super-knee exponent 2.07, sub-knee normalization near-exact, sub-knee firing geometry shell-L1=0.18 vs the engine — both center+SC+BCC dominant; FCC *does* fire ~2/seed on the current stack, contra the throttle doc) and Gauss is confirmed the decisive sub-knee ingredient (gauss-off ⇒ N(14) 17.9→7), but the calibration is engine-emergent — kinetic drain shifts the knee by 16, √α-coupling trips the super-knee RMS (0.118), and absent Langevin friction over-predicts super-knee ~1.8×. The substrate fixes the shape; non-framework constants fix the calibration. Bridge stays `[OPEN]`; boundary mapped. **Platform note:** the current canonical stack's 8-color-SOR Gauss optimization left the golden-hash *energy audit* non-deterministic (4 distinct hashes/runs); the genesis *field* is bit-reproducible (verified — identical-seed firing geometry), so the measurement holds; deterministic-reduction fix + golden re-pin spawned as a background task. **Nothing promoted.** Docs: `PREREG_FTD0110_NA_LAW_v1.md`, `ANALYSIS_FTD0110_NA_LAW.md`, LEDGER FTD-0269, manifest + TRACKER_OPEN_ITEMS corrected, `DERIV_FTD0110_GENESIS_THROTTLE.md` superseded.

---

## 0.32 · 2026-06-11 — FTD-0268: forward-predictions arc — blind L=257 extension CONFIRMED; the predictions registry; the honest Higgs digest

The owner asked for predictions made "on behalf of the framework." Execution: extend the existing prediction machinery (FTD-0258 deviations ledger + SPEC_NOVEL_PREDICTIONS) with genuinely forward rows, and prove the discipline by locking one engine-native blind prediction and measuring it the same session. **Blind leg:** `predict_time_dilation_L257.py` fit each ⟨100⟩/n⊥=3 matched group's residual `R(L)` over the five measured L (per-group log-log LSQ, dof-correct 95% prediction intervals, 0.05 dex cross-build floor) and the bands were committed + tagged (`ee8976b6`) **before** `campaign_time_dilation --L=257 --nperp-fixed=3` ran (WSL2, 12/12 sanity, first-run-is-record). Scored mechanically: **7/9 inside, median falls ⇒ PREDICTION_CONFIRMED.** The two misses are the lowest-velocity groups at the |R| ~ 10⁻⁵ floor, missing in opposite directions — the per-group *power-law model* (not IR convergence) breaks where the signed residual crosses zero; optional sign-resolved v2 queued. Secondary frozen v2 analysis over six L strengthens IR_CONFIRMED. **Registry leg:** `SPEC_PREDICTIONS_FORWARD_2026.md` (FP-1 EWPT/GW `[CONJECTURE]` identification, FP-2 κ_λ = 1.0089, FP-3 dim-5/6 LV nulls, FP-4 structural nulls, EP-2 N(A) bands registered then run post-commit — 2/2 CONFIRMED, EP-3 k⁴ verified to L=768) + §4 anti-overclaim guard (not α, not Born, not lab Bell, not absolute scales, masses at their real tags). **Digest leg:** the Higgs "1σ" headline decomposed — tree +4.44σ vs canonical PDG 2024; the (1−α) loop factor (applied as principle, underived) is what lands it at +0.27σ; two source-doc hygiene flags for the owner (PDG edition; HIGGS-5 tag inconsistency). Docs: `PREREG_TIME_DILATION_L257_BLIND_v1.md`, `ANALYSIS_TIME_DILATION_L257_BLIND_v1.md`, LEDGER FTD-0268, manifest + INDEX_01 rows. **Nothing promoted; no engine changes; golden untouched.**
...
## 0.30 · 2026-06-10 — FTD-0266: Mechanism β, dwell-time Boltzmann variant — DWELL-FAIL; suppression is entirely post-genesis survival; engine-free route exhausted

The kinetics upgrade: full per-voxel trajectory stored, `P_genesis(δ,A) = 1 − ∏_t(1 − p(t))` where `p(t) = 1 − exp(−N_c·max(0,A·|J_unit|−1))`. Dispersion self-check 0.2257 vs 0.2253 ✓. **Result: correction negligible — E[N(10)] = 23–24, virtually unchanged from the envelope's 25–33, far above measured 4.0. DWELL-FAIL; T3 passes (onset A=9∈[7,11]).** Physical reason: N_c=3 makes the Boltzmann gate a step function — one tick above threshold gives P_fire ≥ 0.82, so dwell time is irrelevant once threshold is crossed. **Survival efficiency: 13% at A=9, rising to 40% at A=14 — nucleation signature.** Conclusion: suppression is 100% post-genesis. Engine-free route exhausted. Required next: engine telemetry (per-tick genesis + evaporation). Eighth constraint on FTD-0263 profile: pre-genesis kinetics ≲ 10%. Two bugs in FTD-0265's runner corrected (display-only; FTD-0265 verdict unaffected). LEDGER FTD-0266. Nothing promoted.
...
## 0.25 · 2026-06-10 — Thermostat-OFF discriminator v1: run of record INVALID (V-1) — and the diagnosis found an engine-evolution reproducibility break (FTD-0260)
...
## 0.24 · 2026-06-09 — Open-items arc: Mechanism α quantified and CLOSED (FTD-0259); the FTD-0110 drift redirected to Mechanism γ

The first execution arc against the constitution's §6.4 queue, run under the Number-One Goal's second clause. Target chosen by triage: the FTD-0110 nonlinear bridge via **Mechanism α** (multi-block irrep leakage) — the audit-designated "route that needs the actual ~1-week perturbation calculation." The calculation was done this session (quick-check platform; in-session pre-registration: predictions P1–P3 stated before compute in the runner docstring).

- **P1 [VERIFIED]:** the per-block non-A₁g leakage of the lattice Green profile obeys `λ(r) → (18/27)/r² = (2/3)/r²` (pure 27-block geometry — the T₁ᵤ gradient term; continuum λ·r² = 0.6642 at r = 24, monotone). Exact lattice λ(r) table computed (18-pt 2:1 stencil, L = 96).
- **P2 [CLOSED NEGATIVE — family]:** the parameter-free shell-dephasing model `k(A) = ¼·exp(−Σ λ(r))` scores **4/11 within σ = 0.018 (pre-stated bar ≥ 9/11), RMS 0.039** — and the failure brackets the whole leakage-aggregation family: the computed λ(r) is too front-loaded to match a drift that is flat through A = 10 and gentle at A = 30. **Structural diagnosis:** genesis thresholds on basis-free `|J|` (`voxel.h:169`) — energy re-projected into off-center-block irreps is not lost to harvest; projections that don't enter the dynamics don't cost energy. **The queued Mechanism-α projection calculation is retired.**
- **P3 [METHODOLOGICAL]:** log vs small-power drift forms differ by **0.0008 at A = 2000** — operationally undecidable by curve-fitting at this drift size; only mechanism-killing experiments and shape features discriminate.
- **[OBSERVATION] — Mechanism γ elevated:** the drift onset (flat at A = 2, 10; first drop at A = 15) matches the untuned Langevin thermal crossover `A* = √(L³·T_L) = 12.8` (γ_L = 0.02, T_L = 0.005; the k(A) campaign ran thermostat-active per FOUND_LATTICE_SPACING_GAUGE_FREEDOM §6.5 — the "T=0" line there describes only the WASM viz scenarios). **Decisive next experiment (design only, NOT run): pre-registered thermostat-OFF amplitude re-sweep** — outcome A (drift vanishes) ⇒ substrate-native k = ¼ exactly and the bridge reduces to linear theorem + thermal correction; outcome B ⇒ γ dies, β/front-energetics survive.
- **Zombie cleanup (cleanup discipline):** `TRACKER_OPEN_ITEMS.md` and `INDEX_03_DERIVATIONS.md` still carried the audit-**reverted** "nonlinear bridge CLOSED 2026-05-27" claim as live — both corrected to the canonical post-audit position (bridge **[OPEN]**; linear [DERIVED]; identification [SMC]).
- **Artifacts:** `docs/theory/03_derivations/foundational_mechanics/EXPLR_FTD0110_MECHANISM_ALPHA_LEAKAGE_CLOSED.md` + `scripts/exploration/explore_ftd0110_mechanism_alpha_leakage.py`; LEDGER FTD-0259 + maintenance row. **FTD-0110 tags unchanged; nothing promoted.**

---

## 0.23 · 2026-06-09 — Framework Spec v1: the constitution (FC register declared; deviation spine registered; nothing promoted)

The framework-completion arc. The owner locked three decisions (via explicit question round): **(1)** the "two orthogonal fields" intuition = the **flux ⊥ state** primary pair *plus* the nested **symplectic (q,p)** quadrature pair; **(2)** **native-postulate posture** — where P1–P5 provably don't force an import, declare the FTD-native choice and let the deviations be predictions; **(3)** **constitution-first** deliverable, derivation arcs behind it. Three exploration agents swept the corpus (ontology/foundations; QM-boundary/observer-epistemics; computational-EFT/open-program) and confirmed readiness: the forks were already proven open (FTD-0243, FTD-0253), the deviations already measured (FTD-0199/0200, Bell S≤2, FTD-0251, FTD-0252), and no single document declared the completed ontology. Plan: `.claude/plans/you-are-a-leading-fluffy-candle.md`.

- **`SPEC_FTD_FRAMEWORK_V1.md` (FTD-0254 `[SYNTHESIS]`)** — the constitution, §0 Charter → §1 Ontology → §2 Logic → §3 Math → §4 Philosophy → §5 Physics → §6 Science → §7 Governance. **Three registers:** Postulates P1–P5 (frozen; the machine-checked theorems quantify over exactly these) / **Framework Commitments** (the new register — [AXIOM]-class declarations on proven-open forks; explicitly **not** "P6/P7", which would invert the corpus's "6th-postulate-class" usage and collide with FTD-0248's "Postulate Six") / Calibrations (`a_phys ≡ ℓ_P`, `M_REST = m_e`, `t_phys` — FTD-0059/0096-enforced declarations).
- **The commitments** (each: "the theorem proves the fork; the commitment picks the branch — a declaration, not a derivation"): **FC-0** ℤ[i]-reading-as-choice (canonized from the FTD-0249 red-team honesty correction). **FC-1 (FTD-0255 [AXIOM-class])** A₅ complete, measurement map **M declined** — licensed by FTD-0243's independence [THEOREM] + the FTD-0225/0226/0228 route closures; measurement re-located to frame-relative readout (binding [THEOREM] / sharpness [OPEN], FTD-0227). **FC-2 (FTD-0256 [AXIOM-class])** arrow native + reversibility declined + **metric emergent-IR, sector-scoped** + **space ⊥ time fundamental** — licensed by FTD-0253 (reversibility absent from P1–P5; finiteness opposes it) + FTD-0252 (the scoped `L⁻²` IR approach); the Born-Infeld action term keeps its [AXIOM] tag, now sector-scoped; the clock hypothesis stays [AXIOM]; FTD-0208 stands.
- **FTD-0257 `[SYNTHESIS+SELECTION]`** — the two-orthogonal-fields formalization: J ⊥ s orthogonal at ontological grade, interacting through **exactly two channels** (genesis/evaporation down; Gauss sourcing up — the engine's `phase_write`/`gauss_project` architecture as canon); nested (q,p) with `{q,p} ≠ 0` Poisson / `[q,p] = 0` observable; latency, L/R, wave_vel = decompositions over D=3, never dimensions; closed generator inventory {s, J, wave_vel, L}.
- **`SPEC_PREDICTION_LEDGER_DEVIATIONS.md` (FTD-0258 `[SYNTHESIS]` registry)** — six structural deviations from the QM/SR *formalism* (division of labor vs `SPEC_NOVEL_PREDICTIONS.md` declared), each row: measured numbers + source tag + protocol + scope caveats + kill condition. The **PL-2 honesty block** is mandatory reading: the substrate bound S ≤ 2 vs laboratory S > 2 is an **accepted open burden** ([SELECTION]+[OPEN] observer-layer account) — the framework's most likely break point, stated as such.
...
## 0.22 · 2026-06-07 — QM's place → the measurement-angle → time → spacetime (measurement + boundary arc; nothing promoted)

A long conceptual + engine arc from "where is the FTD physics?" to a measured/bounded account of how spacetime emerges. **Nothing promoted; FTD-0013, MC-T4.3, and the algebraic spine all unchanged.** Three new LEDGER rows (FTD-0251/0252/0253), two pre-registered campaigns, **three independent RedTeamAuditor reviews**, golden gate `0xc13713f0e11a96da` unchanged throughout (all engine work read-only). All uncommitted.

- **QM's place (clarification, not new physics):** wave-face = the classical flux; statistical-face = a partial-access observer's epistemics; quantum-core (non-commutativity) = provably absent from the commutative substrate, an injected 6th postulate (re-states FTD-0225/0226/0243). The session also separated three things people conflate — order-dependence (non-commutativity), irreversibility (the arrow), and the √-structure — as independent axes; and reframed GPS time dilation honestly (a per-entity-bookkeeping view *predicts* it; universality across muons/clocks kills "it's something else").
- **FTD-0251 — substrate measurement-angle** (`docs/theory/06_reference_frames_and_measurement/EXPLR_SUBSTRATE_NATIVE_ANGLE.md`; `engine/tests/test_substrate_angle_probe.cpp`, 10/10): the substrate's only native dynamical angle is the symplectic **quadrature clock** — winds at ω(k) but is strictly **commutative** (`[q,p]=0`); the transverse spatial orientation is a frozen, observer-set label; L/R mirrors. The "measurement angle" exists as a commutative clock; its incompatibility is the injected M. Independent of the W-CRIT-2 α-readout. `[MEASURED]+[SELECTION]+[BOUNDARY]`.
...
## 0.21 · 2026-06-06 — Unified FTD Mass arc (Phase 0–3): the engine honors the EP [THEOREM]; cluster transport inertia [IMPOSED] (FTD-0250)

**Latest update (2026-06-06):** the **Unified FTD Mass arc** (branch `engine/unified-mass-2026-06-06`, 8 commits `4028bc1f`→`f627126d`) closed the long-standing disconnect between the FTD action's equivalence-principle [THEOREM] and what the simulation actually did. **No spine claim moved** — `x₊ = 1/α` (FTD-0013) stays `[STRONGLY MOTIVATED CONJECTURE]`, MC-T4.3 stays a `[FOUNDATIONAL OBSTRUCTION]`, and **α is not derived anywhere**. The one new claim — cluster transport inertia = N·M_REST — ships `[IMPOSED]` (FTD-0250) with its reduction `[OPEN]`. Plan: `.claude/plans/let-s-plan-for-and-eager-tide.md`.

**The gap it closed.** The action says one constant `K_B` is simultaneously the field inertial scale (`SPEC_FTD_LAGRANGIAN.md` §4.1) and the gravitational source (§4.2) — that identity *is* the EP, a [THEOREM] of the action. The engine did not honor it: gravity sourced from rest mass (`poisson_solvers.cpp`) while transport used unit mass (the momentum integrator never felt FTD-0110's "mass = cluster size N" — it was a label, not a dynamical quantity). This arc made the engine honor the EP at the engine level — an **engine-fidelity** result, **not** a new theorem (the EP was already a theorem of the action).

### Phase 0 — disentangle K_B (`4028bc1f`)
`K_B` split into `M_REST` (rest = inertial = gravitational mass quantum) + `K_MANIFEST` (genesis/Boltzmann kinetics scale), with `K_GENESIS = N_c·K_MANIFEST`. Every consuming site repointed across CPU + CUDA + constants (kinetics → `K_MANIFEST`; mass/gravity → `M_REST`). All values remain 0.511 ⇒ pure no-op: golden bit-exact, GPU parity 70/0, constants-sentinel CLEAN. **This resolves FTD-0130 finding (1)** (the K_B mass-anchor vs kinetics-threshold conflation) at the type level.

### Phase 1 — massive-body gravity from real mass (`61f0b5f2`)
New `s0-seed-massive-body` scenario: a dense ball of LOCKED rest mass sources a real latency-Poisson well (`ρ = M_REST·|state|`, **not** the `|J|²` flux proxy). The `SCALE0_MASS_GRAVITY_SCENARIOS` set drives the scenario-loader to enable `latency_field` *without* `field_energy_gravity` and with genesis OFF (else the locked ball's Gauss self-field triggers runaway manifestation → saturated black hole); `R = min(2, N/16)` keeps it sub-horizon. Golden bit-exact, 27/27 web regression.

### Phase 2 — cluster inertia, the dynamical core (`0e21b8f5` + `c19525be` + `ab287f3f` + `0a79bfec`)
Default-off `cluster_inertia` toggle (requires `forces`, `bulk_managed=false`). `phase_forces_integrate_clusters` does an additive 26-Moore flood-fill of LOCKED same-sign clusters, sums the exact per-voxel force `F_cluster = Σ force_diag` (= f_em + f_grav + f_lorentz + f_color), applies the same γ_FTD scheme with a per-mass force `F_cluster/(N·M_REST)`, and writes `V_COM` to every member — **making FTD-0110's mass = N dynamical** (equal total force ⇒ equal acceleration regardless of N, where the old unit-mass integrator gave equal-force ⇒ equal-a for any N). The **a∝1/N falsifier** (`engine/tests/test_cluster_inertia.cpp`): under a FIXED injected total force, `a_COM·N` is constant to ~6 ppm across N∈{1,8,27} and `a_COM = F/(N·M_REST)` to sub-ppm (residual = γ_FTD momentum correction). CUDA mirrors via a **host-side reduction** (sync state/velocity/force-diag to host, run the same CPU flood-fill, upload velocity) — bit-exact by construction; an on-device atomic Σ would break the bit-exact golden/parity gate. LEDGER **FTD-0250** added `[IMPOSED]`.

### Phase 3 — EP demonstrated; translation is the boundary (`de7bb24d` + `f627126d`)
**EP DEMONSTRATED** — test CI-5: two unequal-N locked clusters under the SAME uniform per-voxel force produce **bit-identical** `a_COM` (universal free-fall) while `F_cluster ∝ N`, the textbook EP discriminator. Tagged `[DEMONSTRATION, conditional on the [IMPOSED] cluster-inertia claim — not a derivation]`. The rigid lattice **TRANSLATION** (the falling *visual*) is **`[BOUNDARY — blocked in the current engine]`**: it needs a new cluster-aware translation phase, because the existing movement phase same-sign-bounces a packed locked cluster, has an x-major visitation race, and resolves GPU collisions via nondeterministic `atomicCAS` (breaking bit-exact parity). The velocity-EP is the complete EP demonstration; translation is only the visual.
...
## 0.20 · 2026-06-02 — Construction Monograph + Corpus Consolidation + Sympathetic Red-Team

A capstone consolidation arc: built the canonical bottom-up **construction story** of FTD's mathematics, consolidated the theory corpus to back it, aligned the paper family, and bulletproofed it with a sympathetic-but-incorruptible red-team. **Nothing promoted; FTD-0013 (`x₊=1/α`) and MC-T4.3 unchanged; the spine count (7 theorem-grade + 2 honestly-tiered) unchanged.** Commits deferred (working-tree only, per owner posture).

- **The monograph** ([`MONOGRAPH_FTD_CONSTRUCTION.md`](theory/01_reference/MONOGRAPH_FTD_CONSTRUCTION.md), `[SYNTHESIS]`, ~11.3k words, for a philosophy-of-physics / interdisciplinary audience): **Part 0** (discrete ontology + the epistemic-tag contract) → **Part I** (the theorem-grade spine, strict dependency order: `i/ℤ[i]` → G\* four ways → master quadratic → the 7 theorem-grade results + 2 tiered) → **Part II** (the route-invariant α boundary: the `√(G*(4G*−1))` / K-BIND **"discrete limit of the square root"**, α dynamical-not-structural, FTD-0242/0243) → **Part III** (physics at honest tags) → **Coda**. Re-states canonical claims at their canonical tags. Navigation companion [`INDEX_CONSTRUCTION_SPINE.md`](theory/01_reference/INDEX_CONSTRUCTION_SPINE.md) (section→source map); registered in `META_INDEX` as 1.16a; construction-DAG figure wired to the existing `spine_only` node-map view.
...
- **Owner action items** (deferred during live concurrent-session FTD-id contention): add the monograph's `[SYNTHESIS]` LEDGER row at next-free **FTD-0249**; repoint `LEDGER` FTD-0238 / FTD-0079 paths for the archived docs; fix the propagated `det=G*`→140.04 mislabel in the `AUDIT_RSI_LEG3_CONDITIONAL_THEOREM` §5 / `AUDIT_ALPHA_OPERATOR_FORCING_ROUTE_INVARIANCE` §5 source audits; resolve the known FTD-0189 ID collision; `CLAUDE.md`/README `m_p/m_e` "174 ppm" (accurate 173 ppm). Paper-compile build artifacts in `dissemination/papers/` should not be staged.
...
- **[CLOSED THEOREM-NEGATIVE under FTD-0244] K-BIND:** the irreducible remaining obligation — natively realize `√(G*(4G*−1))`, the squarefree generator of `Q(G*)(√disc)/Q(G*)` that first distinguishes x₊ from x₋. Resolved as a theorem-negative via the substrate-native operator calculus axiomatization in FTD-0244 showing a degree-2 Galois obstruction. ARC-D1 already [CLOSED NEGATIVE].
...
## 0.18 · 2026-06-01 — Engine-Flawless Audit + MC-T4.3 Route-Invariance Boundary

**Latest update (2026-06-01):** a two-part checkpoint that hardened the engine's verification surface and mapped one more route-class of the central α obstruction as a boundary. **No spine claim moved** — `x₊ = 1/α` (FTD-0013) stays `[STRONGLY MOTIVATED CONJECTURE]`, MC-T4.3 stays a `[FOUNDATIONAL OBSTRUCTION]`, and **α is not derived anywhere**. New audit doc: [`07_assessment/audits/AUDIT_ALPHA_OPERATOR_FORCING_ROUTE_INVARIANCE.md`](theory/07_assessment/audits/AUDIT_ALPHA_OPERATOR_FORCING_ROUTE_INVARIANCE.md) (FTD-0242).

### Part A — Engine-flawless lifecycle / callstack / toggle audit (16 commits, branch `flawless-engine-2026-06-01`)

- **Web verification harness added:** `engine/web/tests/lifecycle-harness.spec.js`, `reconcile-claims.spec.js`, `toggle-coverage.spec.js`, `overlay-scheduler.spec.js` — Playwright specs pinning bridge lifecycle, claim reconciliation, toggle coverage, and the overlay scheduler.
- **C++ tests added:** `engine/tests/test_conservation_profile.cpp`, `test_tick_phase_order.cpp`, `test_engine_lifecycle.cpp` — pin the energy-conservation profile, the `phase_read → phase_write → gauss_project → phase_forces → phase_movement` tick-phase order, and engine construct/run/teardown lifecycle.
- **Found + fixed a 5-week clean-checkout break:** a dangling `_repro_gpu_empty_bridge` reference in `engine/CMakeLists.txt` broke `cmake` from a clean checkout for ~5 weeks; removed. Documented that `DagEngine::entity_count()` returns `0` and marked **DagEngine deprecate-clearly**.
- **Energy-conservation finding (pinned):** the conservation leak is the **non-variational Gauss-projection operator** itself — a structural property of the projection step — **not** the Poisson-solver tolerance. Tightening the solver does not close the leak; this is the real conservation profile.

### Part B — MC-T4.3 sharpened to a route-invariant boundary (FTD-0242)

- **The audit:** `AUDIT_ALPHA_OPERATOR_FORCING_ROUTE_INVARIANCE.md` examines whether any FTD-native route *forces* the master-quadratic operator assembly `(Tr, Det) = (16G*², 16G*³)`. Verdict: **0/4 routes force it** — the operator structure is an imposed selection at every route, route-invariantly.
- **Honest reading:** **α is dynamical, not structural.** The discrete ontology does not, via any of the four routes examined, fix the EM coupling. This sharpens the prior 2026-05-28 UNDERDETERMINED verdicts (§0.15) from "this particular readout is unforced" to "no FTD-native route forces the readout" — a route-invariant boundary.
- **Retraction:** `DERIV_ALPHA_READOUT_RESOLUTION.md` is **retracted** (it claimed a resolution the route-invariance scan refutes); archived at `docs/theory/04_coupling/archive/retracted/DERIV_ALPHA_READOUT_RESOLUTION.md`.
- **Tag discipline:** the boundary is a `[STRONGLY MOTIVATED CONJECTURE no-go]` (a boundary-mapping deliverable per the Number-One Goal's second clause), **not** a `[THEOREM]`. **Nothing was promoted.** `x₊ = 1/α` (FTD-0013) stays `[STRONGLY MOTIVATED CONJECTURE]`; MC-T4.3 stays `[FOUNDATIONAL OBSTRUCTION]`. Surviving MC-T4.3 exits: a 6th-postulate-class input that *forces* the operator assembly, or a fresh engine-native ARC-D measurement (ARC-D1 already returned `[CLOSED NEGATIVE]`, 2026-05-30).
