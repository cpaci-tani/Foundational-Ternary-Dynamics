# Where We Left Off — 2026-06-12 (FTD-0275 thermal phase map, run of record: **T_up(L) RISES-CONFIRMED** (≈L^1.36 across L=16–48, seed statistics — tighter ignites lower, quantified); **safety-valve FALSIFIED** (genesis-OFF ablation equally stable to T=12 ⇒ the Langevin OU thermostat's mean-reversion bounds the energy, NOT genesis — the FTD-0274 scout's mechanism `[CONJECTURE]` withdrawn; the no-runaway *observation* stands); **near-critical spark DETONATES** (A=60 at f=0.90·T_up → 3/3 vs clean control — the "spark in a flammable atmosphere" the scout left open; modest effect at 3 seeds, ≥20-seed confirm queued). Pre-registered (tag `preregister-thermal-phase-map-v1`, lock `74caabec`); nothing promoted; next id FTD-0276. Started the 4-arc emergent-physics forward program (plan: thermal run-of-record → drain/N(A) v2 → FTD-0110 bridge → native-clock); this is Arc 1 of 4. Prior arc, FTD-0274 thermal-ignition scout: floor at T_up≈0.05 (first-order condensation, L-dependent — tighter lattices ignite lower); no temperature ceiling found up to T_kin≈27×c² (T_max~c² hypothesis FALSIFIED); local injection never detonates; safety-valve mechanism `[CONJECTURE]`. Prior arc, FTD-0273 mass as flux-energy: ENERGY-COLLAPSES-TO-N (Pearson r=0.990; "511 quanta=electron" FALSIFIED; A_min=1 voxel; local flux decays ≈L⁻³·⁸); quarks COLOR-PHENOMENA-IMPOSED; large-r F∝r harmonic (phase_forces comment corrected). Prior arc, FTD-0272 genesis first-order: hysteresis loop (grows with L), Binder U4=−0.54; cluster ladder N(A) NOT RG-derived. Prior arc, FTD-0271 pilot-wave: **GIVEN a rest-mass clock, FTD's flux is a single-particle pilot wave**. Closing the FTD-0270 dispersion boundary by ADDING a Klein-Gordon mass term `−ω₀²J` (ω₀∝M_REST). A0 gate (on record): the flux has NO restoring term ⇒ the clock is **[IMPOSED]**, native flux massless. B core (pre-reg lock `3df173b1`, run of record): gates pass (G-2 causal control = massless reproduces FTD-0270 s=0.944); with the clock, the KG **envelope is Schrödinger** (E_env∝L^−1.843, s≈2 — the exact sector FTD-0270 said was the only door) and a moving packet carries a **de Broglie wave** (λ∝v^−1.113, r≈1, vs massless r=0). **Honest ceiling: `[CONDITIONAL — DERIVED-GIVEN-IMPOSED-INPUT]`** — Schrödinger+de Broglie are textbook KG; confirming them on the exact lattice is correctness, not an unconditional FTD derivation. **The FTD-native half (A5):** FTD-0252 ALREADY measured the proper-time clock dilates as √(1−v²) — exactly the covariant rate a de Broglie clock needs — so wiring dφ/dt=ω₀·dτ/dt sources the clock's relativistic behavior from FTD's own machinery, shrinking the import to the scalar ω₀∝M_REST. **[OPEN] downstream:** engine mass-term toggle (A), τ→phase wiring (A5 demo), and the guidance test E (does the cluster follow v∝∇S? — moves by forces today, ~40%). Nothing promoted; FTD-0270 still true for unmodified FTD. Prior arc, atomic-dynamics audit + FTD-0270: the helium challenge exposed that FTD derives **~0% of atomic quantum dynamics** from the substrate (no ℏ, no quantum kinetic operator, no discrete levels, no dynamical Pauli/exchange; FC-1 declines QM recovery). **Track A** shipped the canonical status map `AUDIT_ATOMIC_DYNAMICS_STATUS.md` + de-overclaim corrections (hydrogen benchmark = classical Kepler; orbitals = visualization; Lamb shift [PARAMETRIC]). **Track B (FTD-0270, pre-reg lock `e1c7377f`)** attacked the one derivable piece — discrete-level quantization from lattice standing-wave modes, no ℏ — and landed a sharp **BOUNDARY**: the lattice DOES quantize, but the engine's exact 18-pt wave operator carries **linear dispersion** (ground-mode ω₁∝L^−0.944, s≈1) vs the Schrödinger s≈2 on the same eigenvectors; the 2nd-order-in-time √ is what makes it linear, structurally wrong for the Rydberg 1/n². de Broglie failed (r=0). "QUANTIZATION EXISTS, WRONG DISPERSION" — atomic spectra NOT substrate-derivable; the boundary IS the dispersion law. Nothing promoted. Prior arc, FTD-0110 nonlinear-bridge attack FTD-0269: pre-registered substrate-parameter forward model with the FFT-exact Gauss-projection boost reproduces the N(A) law's SHAPE (knee, super-knee exponent, sub-knee firing geometry) but the law is **ENGINE-EMERGENT — verdict BOUNDARY** (kinetic drain + Langevin friction + √α set the calibration); bridge stays [OPEN], boundary mapped; the over-tagged `DERIV_FTD0110_GENESIS_THROTTLE.md` SUPERSEDED. Same-day prior arc: forward-predictions FTD-0268 (blind L=257 CONFIRMED). Nothing promoted anywhere; FTD-0013/MC-T4.3 unchanged. Note: golden hash floats (energy-audit reduction non-determinism from the 8-color-SOR optimization — genesis field bit-reproducible, separate fix flagged).)
