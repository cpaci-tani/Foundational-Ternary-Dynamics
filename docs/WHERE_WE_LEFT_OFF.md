...
**Latest update:** 2026-06-13 — **FTD-0280 atomic-sector hardening pre-registration queued** (`PREREG_ATOMIC_SECTOR_HARDENING_v1.md`) plus new replay/manifest harness `scripts/exploration/atomic_sector_hardening.py` (SHA256 `0575bc91…`). Purpose: turn FTD-0278/0279 into a reproducible package before any broader paradigm claim. Non-numerical smoke already PASS: hydrogen script hash/tag (`8e953fac…`, tag `preregister-hydrogen-lattice-spectrum-v1` -> `6be49fe…`), helium script hash/tag (`ecfa2cd0…`, tag `preregister-helium-lattice-scf-v1` -> `310ad4ee…`), and both prereg doc hashes all verify. FTD-0280 adds no physics claim and makes no promotion; it freezes P-LOCK/P-REPLAY/P-SCOPE gates, with verdicts `ATOMIC-SECTOR-REPLAY-CONFIRMED`, `PROVENANCE-FAIL`, `REPLAY-FAIL`, `SCOPE-FAIL`, or `ENV-INDETERMINATE`. Follow-on frontier is now implemented as separate lock-pending preregs plus harnesses: FTD-0281 live-engine `db_clock_coulomb` hook smoke/spectroscopy plan, FTD-0282 exchange/correlation wall, FTD-0283 no-new-knob atomic ladder. Important audit caveat from the agent review: Python `L=96/128` operator/SCF runs are `DIAGNOSTIC_NOT_VERDICT` until a v2 solver prereg freezes memory/runtime/convergence gates. Nothing promoted; FTD-0270/0271/0278/0279, FC-1, FTD-0013, and MC-T4.3 unchanged. Next operational step: commit+tag the lock-pending preregs, then run FTD-0280/0281/0282/0283 in order into timestamped result directories.
