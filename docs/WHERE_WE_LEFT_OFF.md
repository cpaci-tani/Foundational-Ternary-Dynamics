...
**Latest update:** 2026-06-13 — **ALPHA ESTIMATOR VALIDATION PREREGISTERED (FTD-0286)** — after FTD-0285 invalidated its finite-protocol absolute Phase-G gate, the next move is now frozen as `docs/theory/10_eft_program/preregistrations/PREREG_ALPHA_ESTIMATOR_VALIDATION_v1.md` plus engine artifact `engine/tests/campaign_alpha_estimator_validation.cpp` (SHA256 `dce6018d4ccc7565c1bab6870c9a90647f1bc4290c0fed600cac0fd3883ee570`). It compares the production live-tick Gauss estimator (`L=32`, `r={5,7,9}`, `ticks=300`, `SOR=100`) with the existing matched-stencil EFT projector (`matched_gauss_project`, tol `1e-10`, max_iter `400`) against the same analytic `2rG_L(r)` gate. Outcomes are frozen as `MATCHED_ESTIMATOR_CONFIRMED_PRODUCTION_GATE_INVALID`, `ENERGY_FUNCTIONAL_MISMATCH`, `PRODUCTION_AND_MATCHED_GATES_CONFIRMED`, or `MATCHED_PROJECTOR_FAILED_TO_CONVERGE`. This is an instrument-validation campaign only: **no alpha derivation, no Postulate-W arm, no tuning of FTD-0285, no claim promoted.**
