...
**Latest update:** 2026-06-13 — **ALPHA NO-ALPHA ENGINE PROBE PREREGISTERED (FTD-0285, lock pending)** — FTD-0284 is now locked at `1584fe95` / tag `preregister-alpha-dynamical-readout-v1`; it remains a static discriminator, not a measurement. The next step is now frozen as `docs/theory/10_eft_program/preregistrations/PREREG_ALPHA_NO_ALPHA_ENGINE_PROBE_v1.md` plus engine artifact `engine/tests/campaign_alpha_no_alpha_probe.cpp` (SHA256 `883a917358077f626e90f5affacabd8e565f48fd1cc00aa775ac6e4c7ffbdade`). The run is fixed at `L=32`, `r={5,7,9}`, `ticks=300`, `SOR=100`, CPU backend, native `coulomb_charge_coupling=1.0`, and leak guards disabling `coupling`, damping, Poisson/forces/Lorentz, and clock hooks. Arm A tests the no-alpha native Gauss-projection response; Arm B is an explicit `g_W²=2π/x_+` Postulate-W positive control and cannot count as a derivation. Frozen outcomes: `NATIVE_NULL_WITH_POSTULATE_W_CONTROL`, `DYNAMICAL_FOUND_CANDIDATE_REQUIRES_LEAK_AUDIT`, `INVALIDATED_PROTOCOL_OR_ENGINE_DRIFT`, or `UNCLASSIFIED_NATIVE_RESPONSE`. No run-of-record yet; next honest move is to commit/tag FTD-0285, build, then run `ctest --test-dir engine/build -C Release -R "^alpha_no_alpha_probe$" --output-on-failure` exactly once.
