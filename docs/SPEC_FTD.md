...
**Rationale.** `a_phys` is not derivable from Axiom-Zero invariants alone — see `docs/theory/10_eft_program/archive/resolved/OPEN_A_PHYS_DERIVATION.md` and `docs/theory/10_eft_program/archive/closed_negative/DERIV_A_PHYS_MECHANISM_GAMMA_ATTEMPT.md` for the analysis closing Mechanisms α, β, and γ as candidate derivations, and `docs/theory/10_eft_program/archive/closed_negative/DERIV_A_PHYS_MECHANISM_DELTA_ATTEMPT.md` for Mechanism δ. The Planck-length declaration is one defensible **gauge choice** retained as the framework default for backward compatibility. **Per FTD-0137 (`docs/theory/02_foundations/FOUND_LATTICE_SPACING_GAUGE_FREEDOM.md`)**, the lattice spacing is a *gauge degree of freedom* undetermined by the FTD axioms; the no-go theorem (`docs/theory/10_eft_program/THEOREM_A_PHYS_NO_GO.md`, FTD-0059) is read not as forcing the Planck-length value but as confirming the gauge freedom — no derivation from Axiom Zero exists, so a declaration is required. Three other gauge choices are documented at FOUND_LATTICE_SPACING_GAUGE_FREEDOM §4: cluster-primary (FTD-0130 path-(b)), hadronic-primary (`a_phys ≡ 1` fm), and dimensionless-only (no dimensional calibration; framework purely dimensionless). Dimensionless predictions (the falsifiable spine) are *gauge-invariant* under all four choices.
