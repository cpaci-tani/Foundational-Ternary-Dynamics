...
\title{The $\Gs$ opus: a three-paper overview\\
...
the same arithmetic structure. The opus consists of three papers, with the master quadratic exponent
constraints (see Erratum below) and the value-algebra polynomial-ring
statement as the principal structural results.

\subsection*{Erratum (2026-07-02, FTD-0356, propagating FTD-0351)}

The ``master quadratic uniqueness theorem'' referred to in this overview
(Paper A \S 16.5, formerly Theorem 16.5.1: $(a,b)=(2,3)$ as the
\emph{unique minimal-$a$} exponent pair) was \textbf{retracted} on
2026-07-01 (LEDGER FTD-0351): its Case-A/Case-C discriminant analysis was
notationally vacuous --- the roots are identically
$8\,{\Gs}^{a} \pm 4\sqrt{4\,{\Gs}^{2a} - {\Gs}^{b}}$ in every case --- and
the pair $(1,3)$ in fact survives every substantive criterion
($\Delta(1,3) = 64\,{\Gs}^{2}(4-{\Gs}) > 0$, roots $35.746/11.593$), making
$(1,3)$, not $(2,3)$, the minimal-$a$ element. What stands as a theorem is
the corrected \emph{constraint set} $\{a < b < 2a\} \cup \{b = 2a+1\}$;
the $(2,3)$ choice is a \emph{selection}, conditional on the Watson trace
($a = 2$) and the $\mathrm{Det} = \mathrm{Tr}\cdot{\Gs}$ Vieta ansatz
(the underdetermined W-CRIT-2 assembly, FTD-0235). Paper A \S 16.5 carries
the corrected statement with dated remarks; every mention of the
``uniqueness theorem'' in this overview should be read against this
erratum. Conjecture 16.5.2, restated as an admissible-set conjecture,
remains open.
...
All three are LaTeX-compiled, internally cross-referenced, and verified to
...
quadratic with its $(2, 3)$ exponent constraints --- formerly a
``uniqueness theorem,'' retracted per the Erratum above). Papers C and D are short
...
\paragraph{The master quadratic exponent constraints (Paper A \S 16.5;
corrected 2026-07-02 --- see Erratum).}
Among polynomials of the form $x^{2} - 16\,{\Gs}^{a}\,x + 16\,{\Gs}^{b}$
($a < b$ positive integers), the criteria (roots not \emph{constant}
multiples of any single period ${\Gs}^{k}$; positive discriminant)
constrain $(a,b)$ to $\{a < b < 2a\} \cup \{b = 2a+1\}$ [theorem]. The
former claim that $(2,3)$ is the unique minimal-$a$ choice is retracted
(FTD-0351): $(1,3)$ survives all criteria and is the minimal-$a$ element;
$(2,3)$ is a selection conditional on the Watson trace and the
$\mathrm{Det}=\mathrm{Tr}\cdot{\Gs}$ ansatz (W-CRIT-2). (Restriction:
leading-period polynomials. The general case where coefficients range
over $\mathrm{Sym}^{a}H^{1}$ remains as Conjecture 16.5.2, restated as an
admissible-set conjecture.)
...
Paper A is the trunk. Papers C and D are independent companions, each
...
The three papers collectively pose six open mathematical questions,
...
the corrected \S 16.5 constraint-set theorem (formerly the ``uniqueness
Theorem 16.5.1,'' retracted per the Erratum) from leading-period
polynomials to polynomials with arbitrary coefficients in $\mathrm{Sym}^{a}H^{1}$
and $\mathrm{Sym}^{b}H^{1}$, in the admissible-set restatement. \emph{Estimated:} 2--4 months, requires
...
\paragraph{Submission order.} Submit A first (it is cited by C, D as
the mathematical input). Submit C, D in parallel after Paper A's
status is known; each is self-contained relative to the published Paper A.
C, D can also be posted as a single arXiv preprint together with A
...
substantive claim in the three papers carries one of:
...
isolated --- and Theorem 16.5.1's uniqueness component was subsequently
\emph{retracted} (2026-07-01, FTD-0351; see Erratum), leaving the
corrected constraint-set theorem: the tagging discipline working in both
directions.
...
\section*{The three-paper bundle}
