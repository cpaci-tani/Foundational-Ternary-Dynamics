...
scope of the present paper; the companion note \cite{paper-gstar-B}
treats those questions. The transcendence and algebraic-independence
...
order $4$. The real period of the identity component is
$\Omega^+ = 2\int_1^\infty dx/\sqrt{x^3 - x} = \Gs\sqrt{\pi} = 2\varpi = 2\pi\GG$;
under the Cremona convention, the period that enters the BSD formula
includes the contributions of all connected components, so we use
$\Omega^+$ together with $c_\infty = 2$ but without separately
multiplying by both. Equivalently, the full real period is
$\Omega = c_\infty \cdot (\text{component volume}) = 2\varpi$.
The Birch--Swinnerton-Dyer formula for the rank-$0$ CM case, known
unconditionally by Rubin \cite{rubin1991} (building on Coates--Wiles
\cite{coates-wiles}), gives
\[
L(E, 1) \;=\; \frac{\Omega \cdot \prod_p c_p \cdot |\Sha|}{|E(\Q)_{\mathrm{tors}}|^2}
...
$L(E_{\mathrm{lemn}}, 1) = \varpi/4 = \pi\GG/4$ & Thm.~\ref{thm:Lvalue} (NEW) \\
...
bookkeeping convenience and emerged as the algebraic--analytic shadow of
the Kronecker character $\chi_{-4}$ of $K = \Q(i)$. The compendium of
\S\ref{sec:compendium} makes the discipline operational; \S\ref{sec:character}
makes it structural. The two principal new results --- the polynomial-ring
value algebra $\Q[\pi^{-1}, \GG^{4}]$ for quasi-modular forms at $\tau = i$
(Theorem~\ref{thm:QM-algebra}) and the $m \mid k$ vanishing principle at
class-number-one CM points with extra automorphisms (Theorem~\ref{thm:rho-vanishing}
and its lemniscatic counterpart) --- are uniform statements that survive
in the only two non-trivial cases over $\Q$, and reduce the bookkeeping
of dozens of identities to a single character. What remains open is
whether the polynomial-assembly question --- the source of the exponent
pair $(2, 3)$ in the master quadratic $P_{\Gs}$ --- can be settled inside
CM theory proper, or whether it requires the period-algebra completion
conjectured in \S\ref{sec:not-CM-derived}. We leave that question to
future work.
