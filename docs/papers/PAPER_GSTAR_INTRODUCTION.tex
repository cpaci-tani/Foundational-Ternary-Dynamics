...
\begin{theorem}[Uniqueness of $(2, 3)$ among leading-period polynomials]\label{thm:sym-23-uniqueness}
The pair $(a, b) = (2, 3)$ is the unique exponent pair with $a < b$
such that the leading-period polynomial $P_{(a,b)}(x) = x^{2} - 16\,{\Gs}^{a}\,x + 16\,{\Gs}^{b}$
satisfies all three of the following:
\begin{enumerate}[label=\textup{(\roman*)}, leftmargin=*, itemsep=2pt]
\item the integer prefactor on each non-leading coefficient equals
$|\Aut_\C(E_{\mathrm{lemn}})|^{2} = 16$;
...
\end{enumerate}
Among the family of all leading-period polynomials with integer
prefactor $16$, the pair $(2, 3)$ is uniquely minimal in the sense of
minimising $a$, and given $a = 2$ the value of $b$ is uniquely
determined.
\end{theorem}
...
multiples of any single ${\Gs}^{k}$. Criterion (ii) holds.
...
scalar multiples of ${\Gs}^{a}$. Criterion (ii) \emph{fails}.

Hence criterion (ii) forces $2a > b$. Combined with $a < b$, this gives
\[
a < b < 2a.
\]
...
Criterion (iii) at $(2, 3)$: $\Delta(2, 3) = 64\,{\Gs}^{3}(4\,\Gs - 1) > 0$
since $\Gs \approx 2.959 > 1/4$. Criterion (i) is the definitional
integer prefactor $16$. \qedhere
\end{proof}
