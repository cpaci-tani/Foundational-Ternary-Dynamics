...
\begin{proposition}[Family-wide master-quadratic root asymptotic]\label{prop:Rn-asymptotic}
For the master-quadratic family
$x^2 - 16 R_n^2 x + 16 R_n^3 = 0$, the smaller root satisfies
\[
x_-(R_n) \;=\; R_n \;+\; \frac{1}{16} \;+\; O(1/R_n) \qquad \text{as } n \to \infty.
\]
The larger root then satisfies $x_+(R_n) = 16 R_n^2 - R_n - 1/16 + O(1/R_n)$ by Vieta.
\end{proposition}

\begin{proof}
Apply the quadratic formula and rationalise:
$x_- = 4 R_n^{3/2}/[2 R_n^{1/2} + \sqrt{4 R_n - 1}]$.
Expanding $\sqrt{4 R_n - 1} = 2 R_n^{1/2}(1 - 1/(8 R_n) + O(R_n^{-2}))$,
the denominator becomes $4 R_n^{1/2}(1 - 1/(16 R_n) + O(R_n^{-2}))$, so
$x_- = R_n \cdot (1 + 1/(16 R_n) + O(R_n^{-2})) = R_n + 1/16 + O(1/R_n)$.
\end{proof}
...
\item \textbf{Strict KZ-period status of $\Gs$ and $\GG$.}
Proposition~\ref{prop:KZ-conditional} reduces this to whether $\sqrt{2\pi}
\in \mathcal{P}$, which is conjecturally false \cite[Problem~3]{kontsevich-zagier}
but not proven. An unconditional resolution would settle the status of all
three of $\{\Gs, \GG, \sqrt{2\pi}\}$ jointly.

\item \textbf{Closed form for $W^{(4)}_{\mathrm{BCC}}$ in terms of
$\Gamma$-values.} Theorem~\ref{thm:gen-watson} expresses
$W^{(4)}_{\mathrm{BCC}}$ as ${}_4F_3(\tfrac12{}^4; 1^3; 1)$, but no
elementary-function or finite-$\Gamma$-product reduction is known.
Determine whether
$\{W^{(4)}_{\mathrm{BCC}}, \Gs, \pi, \Gamma(1/4)\}$ are algebraically
independent over $\overline{\Q}$, or exhibit a non-trivial algebraic relation.
PSLQ at 30-digit precision finds no relation with coefficient bound $10^{12}$.

\item \textbf{Selection mechanism for $R_4$ within the family $R_n$.}
Theorem~\ref{thm:R4-distinguished} records that $R_4 = \Gs$ uniquely
generates a master quadratic with roots numerically matching $(\alpha^{-1},
N_c)$. The algebraic-geometric reason is the unique CM-elliptic-curve
structure $|\Aut|^2 = 16$ with $D = 3$ (from $2^D(D-1)! = 16$). A direct
proof that this structure forces $R_4$ over $R_3, R_5, R_6$ \emph{without
using the empirical match} is open.

\item \textbf{Catalan's constant lies outside the lemniscatic neighbourhood.}
The Dirichlet $L$-value $L(\chi_{-4}, 1) = \pi/4$ (Leibniz) is a period of
$\mathbb{Q}(i)$, and the $s = 1$ critical value of the lemniscatic $L$-function
equals $\varpi/2 = \pi\GG/2$ (Theorem~\ref{thm:Lvalue}). However, the
non-critical value $L(\chi_{-4}, 2) = G_{\text{Catalan}} \approx 0.91596\ldots$
appears to be algebraically independent of $\{\GG, \pi\}$: a PSLQ
search at 80 decimal-digit precision over the basis
$\{1, G_{\text{Catalan}}, \GG^k, \pi^k, G_{\text{Catalan}}/\pi, G_{\text{Catalan}} \cdot \GG\}$
finds no integer relation with coefficient bound $10^{12}$. This is
consistent with the Beilinson--Deligne conjectures: critical $L$-values are
periods, but non-critical $L$-values such as
$L(\chi_{-4}, 2) = G_{\text{Catalan}}$ are conjecturally not period-ring
elements. Determine whether $G_{\text{Catalan}}$ is in
$\overline{\mathbb{Q}}(\GG, \pi)$ or properly outside; prove or disprove
algebraic independence.

\item \textbf{Higher-dimensional generalisation of the dichotomy.}
Observation~\ref{obs:dichotomy} treats the lemniscatic CM point $\tau = i$
($K = \Q(i)$). For the equianharmonic CM point $\tau = \rho$ ($K = \Q(\rho)$,
where $\rho = e^{2\pi i/3}$), the analogous algebraic constant is
$R_3 = \Gamma(1/3)/\Gamma(2/3)$, and the analytic constant is presumably
some function of $\AGM(1, \omega_3)$ for an appropriate $\omega_3$.
Formulate the equianharmonic dichotomy and the bridge identity.
