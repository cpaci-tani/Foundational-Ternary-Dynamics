...
In Foundational Ternary Dynamics (FTD), a discrete lattice gauge theory built on ternary states $s \in \{-1, 0, +1\}$, the manifestation operator $\Man$ governs the transition $0 \to \pm 1$. This transition is the discrete event by which void substrate becomes observable matter \cite{Anon2026_Zenodo}. The operator acts on the shifted flux potential $z = |J(v)| - K_B$, where $K_B$ is the mathematical manifestation threshold. This paper establishes four primary results:
...
The master quadratic of FTD admits a natural spectral interpretation. While the quadratic itself is geometrically derived elsewhere \cite{Anon2026_Zenodo}, this section represents the equation as a formal characteristic polynomial governing a dynamical state matrix.
...
\label{cor:vieta}
The spectral decomposition dictates:
\begin{align}
    \lambda_+ + \lambda_- &= 16(G^{*})^2 & \text{(Sum Rule)} \label{eq:sum_rule}\\
    \lambda_+ \cdot \lambda_- &= 16(G^{*})^3 & \text{(Product Rule)} \label{eq:product_rule}\\
...
For $n = 16$ and $c = G^{*}$, $k_{\text{crit}} = 1/(4G^{*}) \approx 0.0845$. Since the physical vacuum ($k = 1$) is overwhelmingly greater than $0.0845$, the baseline universe rests safely deep inside the $\PT$-unbroken phase.
...
At $\beta = 0$ (infinite temperature), all modes are thermally active, corresponding to the bare lattice theory at the UV cutoff $\Lambda$. At arbitrarily large $\beta$ (low temperature), all modes above threshold are frozen out, corresponding to the deep IR regime: at any specified $\varepsilon > 0$ there exists a scale $\mu_{\varepsilon}$ such that for all $\mu < \mu_{\varepsilon}$ the running coupling satisfies $|g(\mu) - g_{*}| < \varepsilon$, where $g_{*}$ is the value characterizing the RG-flow behaviour at arbitrarily-low finite scale $\mu$.
\end{proof}
