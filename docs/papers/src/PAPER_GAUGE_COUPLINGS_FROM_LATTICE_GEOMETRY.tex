...
\end{equation}
or 1.26 parts per million. The polynomial identity~\eqref{eq:master} is itself a [THEOREM]: its coefficient $k = 16 = |\mathrm{Aut}_{\bar{\mathbb{Q}}}(E)|^{2}$ for $E\colon y^{2} = x^{3} - x$ is fixed by classical CM theory, and $G^{*} = \Gamma(1/4)/\Gamma(3/4)$ is a fixed transcendental constant. The \emph{physical identification} $x_{+} = 1/\alpha$ is, however, a [STRONGLY MOTIVATED CONJECTURE], not a derivation. (Correction 2026-08-05: an earlier version cited ``structural-uniqueness scans, combined Bayes factor $\sim 4\times10^{5}$:1'' here. The Bayes figure was retracted 2026-06-24 --- it was never runner-computed --- and the scans themselves were subsequently withdrawn as evidence: FTD-0319 sits at its chance base rate (FTD-0791), the extended scan failed its pre-registered base-rate control with a miscounted result (FTD-0802), and the CM-uniqueness leg is a range artifact (FTD-0321). The identification presently has no numerical-uniqueness support; remaining support is structural.) No derivation chain from FTD axioms to the value of $\alpha$ exists at present.
