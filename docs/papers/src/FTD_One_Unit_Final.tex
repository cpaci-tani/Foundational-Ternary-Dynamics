...
\noindent\textbf{Physical identification.} \etag{Conjecture} $x_+ = \alpha^{-1}$, the inverse fine-structure constant. The CODATA 2022 value $\alpha^{-1} = 137.035\,999\,177(21)$ agrees at 1.26\,ppm~\cite{codata}.
...
The gauge symmetry is selected by the lattice geometry. \etag{Theorem}
...
This is a \emph{deterministic, non-local} hidden variable theory---structurally analogous to the quantum potential in Bohmian mechanics. The lattice dynamics are deterministic (no randomness), but the Gauss solver creates non-local correlations between distant sites within each tick. The $S = 2\sqrt{2}$ figure is measured on the \emph{complexified readout} $\psi = J_x + i\,J_y$ (the imported \etag{Selection} of the next paragraph), verified with $2 \times 10^6$ Monte Carlo samples; the substrate-native readout gives $S \le 2$ (the engine measures $S \approx 1.95$--$2.00$; per the framework constitution, a native $S > 2$ would falsify FC-1). Its source is the global constraint acting on the complexified observable, not a local integral. (Corrected 2026-07-01, FTD-0347: an earlier version of this sentence called $S = 2\sqrt{2}$ ``a computational fact of the engine'' without the complexification qualifier.)
...
\item \textbf{Fourth fermion generation.} Discovery of a fourth generation with standard gauge couplings falsifies $N_{\mathrm{gen}} = 3$.
...
A cubic lattice with ternary states and local causality, combined with the geometry of the lemniscate of Bernoulli, produces a master quadratic whose larger root determines the fine-structure constant. From this number and four framework integers, the gauge groups, the Higgs quartic, the Bell violation bound, and the hydrogen spectrum follow---conditionally on five stated selection principles of explicitly different types and weights.
