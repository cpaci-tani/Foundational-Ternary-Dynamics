...
the finite algebraic value parameterizing the sum at zero spacing-parameter
$q = 0$, which the sum approaches at any specified precision $\varepsilon$
once the lattice spacing $a$ is chosen small enough relative to the period
(the explicit formula gives $a_{\varepsilon}$ for any $\varepsilon$).
...
$D = 3$ member produces a self-consistent framework.
