...
\]
then recast the result entirely in terms of the lemniscate constant~$\vpi$
and the bridge constant $\Gstar=2\vpi/\sqrt{\pi}$ of Foundational Ternary
Dynamics~(FTD).  In the FTD rewriting, the ontic content of the identity
reduces to the single ratio $\ell=\vpi^2/\Gstar^2=\pi/4$, revealing
...
$E_4(i)=3\,\Gstar^8/(256\,\vpi^4)$, and prove the Epstein factorisation
$Z_{\Z[i]}(2)=4\,\zeta(2)\,L(2,\chi_{-4})=32\,\vpi^4\,G/(3\,\Gstar^4)$
...
the Eisenstein series $G_4$ encodes the period of~$\Z[i]$ through~$\vpi$;
the Watson integral encodes the period of~$\Z^3$ through~$\Gstar$.
Each dimension has its own ``circle constant,'' and only $D = 3$
produces a self-consistent framework.

This paper is one of five companion papers presenting Foundational Ternary
Dynamics; see~\cite{chain} for the derivation chain,
\cite{lifecycle} for the physical lifecycle, \cite{constructor} for
the ontological constructor, and \cite{engine} for the computational
implementation.
\end{abstract}
...
The lemniscate constant~$\vpi$ is defined by
\[
\vpi \;=\; 2\int_0^1\frac{dt}{\sqrt{1-t^4}}
\;=\;\frac{\Gamma(1/4)^2}{2\sqrt{2\pi}}
\;\approx\;2.62206\,.
\]
It is the half-perimeter of the lemniscate of Bernoulli and satisfies
$\vpi=\pi/M(1,\sqrt{2})$, where $M$ denotes the arithmetic--geometric mean.
\end{definition}

\begin{definition}[Bridge constant]\label{def:Gstar}
The FTD bridge constant is
\[
\Gstar \;=\; \frac{\Gamma(1/4)^2}{\sqrt{2}\;\Gamma(1/2)^2} \;=\; \frac{2\vpi}{\sqrt{\pi}} \;\approx\; 2.95868\,.
\]
...
\ell \;=\; \frac{\vpi^2}{\Gstar^2}\,.
\end{equation}
\end{definition}

\begin{lemma}\label{lem:pi}
$\pi = 4\ell = 4\vpi^2/\Gstar^2$.
\end{lemma}
\begin{proof}
From $\Gstar=2\vpi/\sqrt{\pi}$ we obtain $\Gstar^2=4\vpi^2/\pi$,
hence $\pi=4\vpi^2/\Gstar^2=4\ell$.
\end{proof}

\begin{remark}\label{rem:ontic-priority}
In the FTD framework, $\vpi$ and $\Gstar$ (equivalently $\vpi$ and
...
$\vpi^2/\Gstar^2$.  This reflects the claim that the lemniscatic lattice
...
of $4\vpi^2/\Gstar^2$.  The hyperbolic functions $\coth$ and $\csch^2$
depend on a single transcendental argument: $4\ell=4\vpi^2/\Gstar^2$.
\end{remark}
...
\[
e^{4\ell}\;=\;e^{4\vpi^2/\Gstar^2}\;=\;e^{\pi}\;\approx\;23.1407\,,
\]
...
e^{-8\vpi^2/\Gstar^2}\;\approx\;1.87\times 10^{-3}\,.
\]
The smallness of~$q$ (controlled entirely by $\vpi^2/\Gstar^2$) is
...
E_4(i)\;=\;3\!\left(\frac{\vpi}{\pi}\right)^{\!4}
\;=\;\frac{3\,\Gstar^8}{256\,\vpi^4}\,.
\end{equation}
\end{theorem}
\begin{proof}
...
$\pi=4\vpi^2/\Gstar^2$:
\[
3\!\left(\frac{\vpi}{\pi}\right)^{\!4}
= 3\!\left(\frac{\vpi\,\Gstar^2}{4\vpi^2}\right)^{\!4}
= 3\!\left(\frac{\Gstar^2}{4\vpi}\right)^{\!4}
= \frac{3\,\Gstar^8}{256\,\vpi^4}\,.\qedhere
\]
\end{proof}

\begin{remark}\label{rem:2d-ontic}
The full two-dimensional lattice structure at the Gaussian point is
controlled entirely by $\vpi$ and~$\Gstar$.  Our one-dimensional sum~$S$
...
Z_{\Z[i]}(2) \;=\; 4\,\zeta(2)\,G
\;=\;\frac{32\,\vpi^4}{3\,\Gstar^4}\;\cdot\; G\,,
\end{equation}
where $G=L(2,\chi_{-4})\approx 0.915966$ is Catalan's constant.
\end{corollary}
\begin{proof}
Substitute $\zeta(2)=\pi^2/6$ and $\pi=4\vpi^2/\Gstar^2$:
\[
\zeta(2)=\frac{\pi^2}{6}=\frac{16\vpi^4}{6\,\Gstar^4}
=\frac{8\vpi^4}{3\,\Gstar^4}\,.
\]
Then $Z_{\Z[i]}(2)=4\cdot\dfrac{8\vpi^4}{3\,\Gstar^4}\cdot G
=\dfrac{32\,\vpi^4\,G}{3\,\Gstar^4}$.
\end{proof}
...
lattice and cannot be expressed in terms of $\vpi$ and~$\Gstar$ alone.
...
Z_{\Z[i]}(2) = \frac{2^8\,\vpi^8}{45\,\Gstar^8}
+ 2\sum_{n=1}^{\infty}\left[
\frac{8\vpi^4\csch^2(4n\ell)}{n^2\,\Gstar^4}
+ \frac{2\vpi^2\coth(4n\ell)}{n^3\,\Gstar^2}\right],
\end{equation}
where $\ell=\vpi^2/\Gstar^2$ and the leading term uses
$\zeta(4)=\pi^4/90=2^8\vpi^8/(90\,\Gstar^8)$.
\end{proposition}
...
terms of $\vpi$ and~$\Gstar$ (via $\pi=4\vpi^2/\Gstar^2$).  Catalan's
...
smooth function controlled by the single period~$\vpi$.
...
determined by the lattice periods $\vpi$ and $i\vpi$ alone.  The norm
...
In both projections, the lemniscatic constants $\vpi$ and $\Gstar$ control
...
(``what are the periods?'').  The bridge constant $\Gstar$ mediates the
...
Linear in $\ell$ & $4\ell\coth(4\ell)/4$  & 60.3\% & Bridge term ($\Gstar$ mediation)\\
...
answer and is dominated by the ratio $\vpi^2/\Gstar^2$.  This is the
...
set by $\ell=\vpi^2/\Gstar^2$ alone.
...
periods are $\vpi$ and~$i\vpi$, and whose invariants are expressible purely
in terms of $\vpi$ and~$\Gstar$.  (For the square lattice, $g_3=0$ by
four-fold symmetry, and $g_2$ is determined by~$\vpi$ alone.)
...
The ontic content is $\ell=\vpi^2/\Gstar^2$.  Everything else is
projection.
\end{quote}
...
Now substitute $\pi = 4\ell$, where $\ell = \vpi^2/\Gstar^2$ is the
ontic ratio:
\[
...
\midrule
1 & $\Z$ & $\pi = 4\ell$
  & $\displaystyle\zeta(2) = \frac{\pi^2}{6} = \frac{8\vpi^4}{3\Gstar^4}$
...
3 & $\Z^3$ & $\Gstar$
  & $\displaystyle W_3 = \frac{\Gstar^2}{2\pi} = \frac{\Gamma(1/4)^4}{4\pi^3}$
  & Watson integral\\
\bottomrule
\end{tabular}
\end{center}
...
applied to the Gaussian lattice $\Z[i]$ in $D = 2$ discovers $\vpi$:
the Weierstrass $\sigma$-function of $\Z[i]$ is the doubly-periodic
analogue of $\sin(\pi z)$, and its periods are $\vpi$ and $i\vpi$.  In
$D = 3$, the lattice Green's function of $\Z^3$---the Watson
integral---discovers $\Gstar$, which absorbs the geometric scaling
...
\item $\vpi$ measures how far you walk around a lemniscate (the
  fundamental domain boundary of $\Z[i]$).
\item $\Gstar$ measures the lattice Green's function of $\Z^3$---the
...
computed over it.  When we compute over $\Z[i]$, we find $\vpi$.  When
we compute over $\Z^3$, we find $\Gstar$.

The connection to FTD's dimension selection is immediate.  The master
quadratic $x^2 - 16\Gstar_D^2\,x + 16\Gstar_D^3 = 0$, applied to the
...
periods $(\pi,\; \vpi,\; \Gstar,\; \Gstar_4,\; \ldots)$, only the
...
2 & $\Z[i]$ & $\vpi$ & $G_4(i) \propto \vpi^4$ & \textbf{No} ($8 \neq 2$) \\
3 & $\Z^3$ & $\Gstar$ & $W_3 = {\Gstar}^2/(2\pi)$ & \textbf{Yes} ($3 = 3$) \\
\bottomrule
\end{tabular}
\end{center}

\noindent Each dimension has its own ``circle constant.'' $\pi$ is the period of $\Z$. $\vpi$ is the period of $\Z[i]$. $\Gstar$ is the period of $\Z^3$. The relation $\pi = 4\vpi^2/{\Gstar}^2$ expresses the lowest-dimensional constant through the higher ones---$\pi$ is the projection of lemniscatic geometry onto a line.
...
\midrule
Argument of $\coth/\csch$ & $\pi$ & $4\vpi^2/\Gstar^2$\\
Exponential scale & $e^{\pi}$ & $e^{4\vpi^2/\Gstar^2}$\\
Lattice nome & $e^{-2\pi}$ & $e^{-8\vpi^2/\Gstar^2}$\\
$E_4(i)$ & $3(\vpi/\pi)^4$ & $3\Gstar^8/(256\vpi^4)$\\
Prefactor $\pi^2$ & $\pi^2$ & $16\vpi^4/\Gstar^4$\\
$\zeta(2)$ & $\pi^2/6$ & $8\vpi^4/(3\Gstar^4)$\\
Euler product & $\sin(\pi z)/(\pi z) = \prod(1-z^2/n^2)$ & $\sin(4\ell z)/(4\ell z) = \prod(1-z^2/n^2)$\\
$Z_{\Z[i]}(2)$ & $4\,\zeta(2)\,G$ & $32\vpi^4 G/(3\Gstar^4)$\\
...
The claim that $\vpi$ and $\Gstar$ are ontologically prior to~$\pi$.
...
The identification of $\Gstar$ as the mediator of all
...
\bibitem{chain}
``The Ontic Derivation Chain: Twenty-Six Physical Constants from Two Inputs,''
