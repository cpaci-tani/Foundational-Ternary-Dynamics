...
Each tick processes $E_{\text{tick}}=16\,G^{*2}=x_++x_-$ energy per degree of freedom~\cite{ftdtime}.
...
Given the FTD coupling Lagrangian $\mathcal L_{\text{coupling}}=+g_c\,s\,(\nabla\!\cdot\!\mathbf J)$ (sign per the Term~2 amendment of 2026-07-18), define the effective field as the path-weighted expectation:
\[
...
It is the algebraic backbone from which $\alpha$ and the manifestation threshold $K_B$ are derived.
\end{cautionbox}
