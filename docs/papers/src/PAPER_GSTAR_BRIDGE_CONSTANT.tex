...
\begin{enumerate}[label=\textup{(\roman*)}]
  ...
  \item The conductor is $N=32$ \textup{(}LMFDB label \texttt{32.a3},
    Cremona label \texttt{32a2}\textup{)}
...
\subsection{The exact \texorpdfstring{$L$}{L}-value and BSD normalisation}\label{ssec:bsd}

The curve corresponds to the normalised newform
$f_{32}(\tau)=\eta(4\tau)^2\eta(8\tau)^2$.
Its special value follows directly from a Mellin-transform evaluation
\cite[Lemma~3.1]{LiLongTu}:
\begin{equation}\label{eq:LE1}
  L(E,1)=L(f_{32},1)
  =\frac18 B\!\left(\frac12,\frac14\right)
  =\frac{\sqrt{\pi}}8\,
    \frac{\Gamma(\frac14)}{\Gamma(\frac34)}
  =\frac{\varpiup}{4}\ne0.
\end{equation}
This evaluation does not assume the BSD conjecture.

For the N\'{e}ron differential $\omega=dx/(2y)$, the least positive
real period is
\[
  \Omega_{+}=\int_1^\infty\frac{dx}{\sqrt{x^3-x}}
  =2\int_0^1\frac{dt}{\sqrt{1-t^4}}=\varpiup,
\]
where $x=t^{-2}$.  Since $E(\mathbb R)$ has two components, the total
real period used in the BSD formula is
$\Omega_E=\int_{E(\mathbb R)}|\omega|=2\Omega_{+}=2\varpiup$.
The rank-zero BSD expression is
\begin{equation}\label{eq:bsd}
  L(E,1) \;\stackrel{\mathrm{BSD}}{=}\;
  \frac{\Omega_E\cdot\abs{\text{III}}\cdot
  \prod_{p}c_{p}}{%
  \abs{E(\QQ)_{\mathrm{tors}}}^{2}}
\end{equation}
where $c_2=2$, $c_p=1$ for odd~$p$, and
$\abs{E(\QQ)_{\mathrm{tors}}}=4$~\cite{Cremona}.
Thus its right-hand side is $\varpiup\abs{\text{III}}/4$;
the exact value~\eqref{eq:LE1} gives analytic order~$1$ for the
Tate--Shafarevich group in this formula.  A general rank-zero CM
BSD theorem is not needed for the identity used here.
The representation $\Gstar = 8\,L(E,1)/\gamma_{2}$
(Theorem~\ref{thm:seven},~\eqref{eq:D7}) thus links $\Gstar$ directly to
the arithmetic of this specified curve through its exact $L$-value.
...
% ================================================================
\section{Seven Representations of \texorpdfstring{$\Gstar$}{G*}}\label{sec:derivations}
% ================================================================
...
The fifty-two-entry catalogue~\cite{Paper2} includes these representations,
algebraic restatements, and an explicitly marked approximation.
\end{theorem}
...
\subsection{Why degree~\texorpdfstring{$2$}{2}?}\label{ssec:degree}
...
\subsection{Why coefficient~\texorpdfstring{$16$}{16}?}\label{ssec:coeff}
...
\subsection{Why Sum~\texorpdfstring{$=$}{=}~Product?}\label{ssec:sep}
...
\subsection{The \texorpdfstring{$k$}{k}-family}\label{ssec:kfamily}
...
\begin{itemize}
  \item $\lfloor x_{-}\rfloor = 3$, the number of spatial dimensions.
...
See also \url{https://www.lmfdb.org/EllipticCurve/Q/32/a/3}.
