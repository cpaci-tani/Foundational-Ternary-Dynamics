...
Every result in this paper is conditional on the physical identifications $x_+ = 1/\alpha$ and $\lfloor x_- \rfloor = N_c = 3$. Given these identifications, all 26 constants follow as theorems of algebra from the master quadratic and the framework integers $\{3, 4, 7, 13\}$. Without the identifications, the results are mathematical facts about specific algebraic expressions, not physical predictions.
\end{remark}
...
\begin{remark}
The harmonic mean identity $G^{*} = x_+\,x_-/(x_+ + x_-)$ shows $G^{*}$ is half the harmonic mean of the two physics roots.
\end{remark}
...
From $x_- = 3.024\ldots$ and $D = 3$, the following integers are fixed:
\end{proposition}
...
\midrule
$N_c$ & $\lfloor x_-\rfloor$ & 3 & Color charges & \epi{Conjecture} \\
...
24 & $N_\text{gen}$ & $\lfloor x_-\rfloor$ & 3 & 3 & exact & \epi{Conjecture} \\
25 & $N_c$ & $\lfloor x_-\rfloor$ & 3 & 3 & exact & \epi{Conjecture} \\
