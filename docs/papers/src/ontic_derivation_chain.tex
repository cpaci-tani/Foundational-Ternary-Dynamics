...
\setstretch{1.15}
% pdfstringdef commands for removed macros no longer needed
\titleformat{\section}{\large\bfseries}{\thesection.}{1em}{}
\titleformat{\subsection}{\normalsize\bfseries}{\thesubsection}{1em}{}

% ----- Math Commands -----
% G^{*} macro removed — using inline G^{*} for canonical style
% x_+, x_- macros removed — using inline x_+ and x_- for canonical style
...
Every result in this paper is conditional on the physical identification $x_+ = 1/\alpha$. Given this identification, all 26 constants follow as theorems of algebra from the master quadratic and the framework integers $\{3, 4, 7, 13\}$. Without the identification, the results are mathematical facts about specific algebraic expressions, not physical predictions.
\end{remark}
...
$\alpha$ & $1/x_+$ & $7.297\times10^{-3}$ & 5 \\
$N_c$ & Moore Layer Theorem (color charges) & $3$ & 4 \\
...
\item $kG^{*} > 4$ $(k=16)$: $\Delta > 0$, real roots $\to$ \textbf{physics}.
\item $kG^{*} = 4$ $(k=4/G^{*})$: $\Delta = 0$, degenerate $\to$ Born rule boundary.
\item $kG^{*} < 4$ $(k=1/2)$: $\Delta < 0$, complex conjugate roots (no physical interpretation in this paper).
\end{enumerate}
\end{lemma}
...
$x_+\,x_-/(x_+ + x_-) = kc^3/(kc^2) = c = G^{*}$.
That is, $G^{*}$ is half the harmonic mean of the two roots.
...
with roots
\begin{align}
x_+ &= 8G^{*2}\!\left(1 + \sqrt{1 - \tfrac{1}{4G^{*}}}\right) = 137.0362\ldots \label{eq:xplus}\\
x_- &= 8G^{*2}\!\left(1 - \sqrt{1 - \tfrac{1}{4G^{*}}}\right) = 3.0240\ldots \label{eq:xminus}
\end{align}
\end{theorem}
...
\begin{remark}
The harmonic mean identity $G^{*} = x_+\,x_-/(x_+ + x_-)$ shows $G^{*}$ is half the harmonic mean of the two roots.
\end{remark}
...
From $D = 3$, the following integers are fixed:
\end{proposition}
...
\midrule
$N_c$ & Moore Layer Theorem & 3 & Color charges & \epi{Conjecture} \\
...
\noindent Four coupling constants follow from the integers and $x_+$.

\begin{equation}
\boxed{\alpha = \frac{1}{x_+} = 7.297\times 10^{-3}}
...
The proton mass combines the EM contribution ($N_\text{eff}\cdot x_+$: valence quarks
...
\noindent The tree-level $1/\alpha = x_+$ is refined by a four-term correction using
the modular deviation $\varepsilon = e^\pi - \pi - (b_3 + N_\text{eff})
= e^\pi - \pi - 20 \approx -9.00\times 10^{-4}$.

\begin{equation}
\boxed{\frac{1}{\alpha} = x_+ - c_1|\varepsilon| + c_2|\varepsilon|^2
...
1 & $1/\alpha$ (tree) & $x_+$ & 137.036 & 137.036 & 1.3\,ppm & \epi{Conjecture} \\
...
9 & $m_p$ & $N_\text{eff}\cdot x_+\!+\!T_{10}$ & 938.4 & 938.3 & 0.02\% & \epi{Selection} \\
...
24 & $N_\text{gen}$ & $N_c$ & 3 & 3 & exact & \epi{Conjecture} \\
25 & $N_c$ & Moore Layer Theorem & 3 & 3 & exact & \epi{Conjecture} \\
...
\item Precision $\alpha$ measurement deviating from $x_+ = 137.036\ldots$ by $> 10$\,ppm
...
with no imports from the project's own constants module. The computational engine implements these constants as \texttt{constexpr} values derived at compile time.
