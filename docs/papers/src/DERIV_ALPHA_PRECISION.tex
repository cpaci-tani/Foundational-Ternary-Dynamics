...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[LE]{\small\textit{Anonymous et al.}}
...
Foundational Ternary Dynamics (FTD) is a discrete computational framework that postulates a three-dimensional cubic lattice with ternary states $\{-1, 0, +1\}$ and a continuous flux field $\mathbf{J} \in \mathbb{R}^3$~\cite{Anon2026}.
...
\noindent \selection: The Gauss constraint counting follows from the FTD action~\cite{Anon2026}.
