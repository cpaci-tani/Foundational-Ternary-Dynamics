...
%
%  Authors: William J. cpaci-tani III, et al.
%  February 2026
% ======================================================================
...
\begin{center}
    ...
    {\large William J.\ cpaci-tani~III$^{1}$}\\[0.3cm]
...
In the Standard Model, $\alpha$ is a free parameter determined by measurement.

Foundational Ternary Dynamics (FTD) is a discrete computational framework that postulates a three-dimensional cubic lattice with ternary states $\{-1, 0, +1\}$ and a continuous flux field $\mathbf{J} \in \mathbb{R}^3$~\cite{cpaci-tani2026}.
...
The near-integer relation $e^{\pi} \approx \pi + 20$ is documented in number theory~\cite{Weisstein}.
Within FTD, the connection to the framework is twofold:
\begin{enumerate}[label=(\roman*), nosep]
    \item $e^{\pi} = 1/q_L$ where $q_L = e^{-\pi}$ is the nome of the lemniscate elliptic curve underlying the master quadratic;
    \item the integer~20 decomposes as $b_3 + N_{\mathrm{eff}} = 7 + 13$, the sum of the topological coefficient and effective mode count from the FTD integer set.
\end{enumerate}

\noindent Whether this decomposition has deeper significance or is coincidental remains \openq.
...
\noindent \selection: The Gauss constraint counting follows from the FTD action~\cite{cpaci-tani2026}.
...
\begin{enumerate}[nosep]
    \item The coefficients are not arbitrary---they are constrained to ratios of specific FTD combinatorial quantities.
    \item The expansion parameter is not a free real number---it is a fixed mathematical constant.
