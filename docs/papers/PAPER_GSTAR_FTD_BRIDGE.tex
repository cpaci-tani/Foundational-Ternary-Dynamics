...
structural realist because the entities are nodes in the structural
network rather than independent objects; specialized because the
particular structure is the maximal order of $\Q(i)$ rather than an
arbitrary mathematical structure.
\end{remark}
...
\cite[\S 06\_consciousness]{ftd-spec}, the same arrow applies at three
nested scopes:
\[
...
\cite[Theorem 16.x]{paper-gstar-A}) that $\chi_{-4}$ generates the
...
\begin{observation}[Joint-matching uniqueness, restated from {\cite[\S 17.2]{paper-gstar-A}}]\label{obs:joint-matching-bridge}
...
Combining \cite[\S 16.6]{paper-gstar-A} (no CM-internal source for $(2, 3)$)
...
\Big\uparrow \text{\quad(2, 3) exponents: uniquely forced by joint root match \cite[\S 17.2]{paper-gstar-A}} \\
\underbrace{\{16,\; \Gs\}}_{\text{building blocks}} \\
\Big\uparrow \text{\quad four functorial projections \cite[Thm 16.x]{paper-gstar-A}} \\
...
candidate framing in \cite[\S 16.6, Conjecture (Sym\textsuperscript{2}--Sym\textsuperscript{3})]{paper-gstar-A}
...
C. Pacitanimulli,
\textit{The algebraic and analytic faces of the lemniscatic CM constant
$\Gs = \Gamma(1/4)/\Gamma(3/4)$: a $\chi_{-4}$-unified survey},
manuscript, 2026.

\bibitem{ftd-spec}
C. Pacitanimulli et al.,
\textit{Foundational Ternary Dynamics: specification and theoretical framework},
project documentation, 2026.
\url{https://github.com/[repository]/ftd/blob/main/docs/SPEC_FTD.md}.
