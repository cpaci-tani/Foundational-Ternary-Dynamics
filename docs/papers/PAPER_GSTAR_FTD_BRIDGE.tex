...
This companion note draws the consequences of those results for the
math--physics bridge of the Foundational Ternary Dynamics (FTD)
framework \cite{ftd-spec}, where the same set $\{-1, 0, +1\}$ appears
as the ternary voxel alphabet of a discrete physical substrate. Three
structural observations are recorded, all explicitly tagged as synthesis
rather than theorem:
\begin{enumerate}[label=\textup{(\roman*)}, leftmargin=*, itemsep=2pt]
\item the FTD voxel alphabet coincides as a set with the value set of
$\chi_{-4}$, sourcing a candidate \emph{Gaussian-integer-module ontology};
\item the Born rule $\psi \mapsto |\psi|^{2}$ and the character
$\chi_{-4}: i^{n} \mapsto \mathrm{Im}(i^{n})$ are two instances of a
general ``complex object $\to$ single real coordinate'' schema, sharing
ternary or near-ternary output but differing in domain, codomain, and
coordinate read;
\item the FTD identification $(x_{+}, x_{-}) \approx (\alpha^{-1}, N_{c})$
of the roots of the master quadratic $P_{\Gs}(x) = x^{2} - 16\Gss x + 16\Gsss$
with the fine-structure constant and the number of QCD colours requires
a single discrete integer-pair selector --- the exponent pair $(2, 3)$
--- which is over-determined by joint root-matching.
\end{enumerate}
All mathematical content is taken from \cite{paper-gstar-A}; this
companion contributes only the bridge interpretation, presented under
the project's \texttt{[SYNTHESIS]} tag.
\end{abstract}
...
\emph{Pythagorean structural realism, specialised to the Gaussian-integer
module}: Pythagorean because number is taken as the substance from which
the physical emerges; structural realist in attitude (entities are nodes
in the structural network rather than independent objects --- compare
\cite{ladyman-ross}, though our specialisation to a single algebraic
structure goes beyond their naturalistic stance); specialised because the
particular structure is the maximal order of $\Q(i)$ rather than an
arbitrary mathematical structure. This is a more specific, and therefore
more falsifiable, restriction of an MUH-style position
\cite{tegmark2008}: not ``all mathematical structures are physical'' but
``the physical is the $\Z[i]$-module slice of the mathematical.'' Under
this specialisation Wigner's ``unreasonable effectiveness''
\cite{wigner1960} becomes a definitional consequence rather than a
mystery.
\end{remark}
...
\section{The Born rule and $\chi_{-4}$: a schema and a conjecture}\label{sec:born-rule-arrow}

The $\chi_{-4}$-unification of \cite{paper-gstar-A} makes available a
structural schema that connects two otherwise unrelated operations.

\begin{observation}[Born rule and $\chi_{-4}$ as two instances of a real-coordinate schema]\label{obs:born-arrow}
\textbf{[Synthesis.]} The Born rule of quantum mechanics and the
character $\chi_{-4}$ are two instances of a general schema
``complex object $\to$ single real coordinate $\to$ $\R$,'' chosen
independently in QM and in number theory for unrelated structural reasons:
\begin{itemize}[leftmargin=*, itemsep=4pt]
\item \textbf{Born rule}: $\psi \in \C \;\mapsto\; |\psi|^{2} \in \R_{\geq 0}$,
the squared modulus, a continuous map whose load-bearing content is the
Gleason-theorem link between an inner-product-space amplitude and a
probability measure.
\item \textbf{Character $\chi_{-4}$}: $i^{n} \in \langle i\rangle \subset \C \;\mapsto\; \mathrm{Im}(i^{n}) \in \{-1, 0, +1\}$,
the imaginary-part trace on a finite cyclic group, a discrete map whose
content is the detection of quadratic residues mod $4$.
\end{itemize}
The two operations differ in their domain (continuous Hilbert space vs.
finite cyclic group), their codomain (positive half-line vs.\ signed
three-point set), the coordinate they read (modulus vs.\ imaginary part),
and their non-linearity (squared modulus vs.\ linear imaginary part).
The observation worth recording is therefore not that they are ``the
same arrow,'' but that both produce ternary or near-ternary outcomes
when restricted to relevant subdomains, by virtue of the imaginary-unit
orbit $\langle i\rangle$ having exactly three distinct imaginary
projections.
\end{observation}

\begin{remark}[Reflexive projection at two identical scopes and one analogous scope]\label{rem:three-scopes}
...
\cite{ftd-spec}, two scopes are \emph{literally the same operation} and
a third is \emph{conjecturally analogous}:
\begin{itemize}[leftmargin=*, itemsep=2pt]
\item (Identical) The FTD voxel state field as the imaginary-part readout
of $\Z[i]^{\times} \cup \{0\}$, and the character $\chi_{-4}$ as
$\mathrm{Im} \circ (i^{\bullet})$ on the angular index, are literally
the same map; their coincidence is a definitional consequence of the
$\Z[i]$-substrate position rather than an independent identification.
\item (Analogous) The Born rule of quantum mechanics is conjecturally
related to this same map at a derived scope (amplitude measurement of
$\Z[i]$-structured quantum states). This is the load-bearing physical
conjecture, tagged \texttt{[STRONGLY MOTIVATED CONJECTURE]}; it is not
established by any mathematical theorem and the differences enumerated in
Observation~\ref{obs:born-arrow} mean it should not be conflated with the
...
\emph{a single discrete integer-pair selector} from the physical side --- the exponent-pair
...
candidate framing in \cite[\S 16.5, Theorem (Sym\textsuperscript{2}--Sym\textsuperscript{3} uniqueness, leading-period)]{paper-gstar-A}
...
A second-tier programme suggested by the parallel equianharmonic structure
of \cite[\S 16.4]{paper-gstar-A} is the identification of the
