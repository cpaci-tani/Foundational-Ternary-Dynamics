...
\]
Adding the spatial gradient term $\frac{1}{2}|\Delta_{\mu}\mathbf{J}|^{2}$ from the continuum limit of the lattice Laplacian, the Euler--Lagrange equation is~\eqref{eq:kg}.
\end{proof}
...
\item \textbf{Lorentz invariance.} The cubic lattice breaks continuous rotation symmetry to the discrete hypercubic group $H(4)$. Restoration of $O(3,1)$ invariance in the continuum limit is expected on universality grounds for matter fields~\cite{Wilson} but has not been established for the gravitational sector.
