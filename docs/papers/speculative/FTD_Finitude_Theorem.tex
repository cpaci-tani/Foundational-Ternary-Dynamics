...
%===================================================================
\section*{Preamble: Canonical Status}
%===================================================================
\addcontentsline{toc}{section}{Preamble: Canonical Status}

The framework's foundational ontology has formally adopted the undefined-boundary cubic-graph stance described in this paper (see \texttt{docs/theory/07\_assessment/AUDIT\_INFINITY\_REFRAME.md} and \texttt{reframe\_deployment/CANONICAL\_REFRAME.md} for the canonical statement). This paper, previously speculative, now describes the canonical position: completed infinity is rejected; undefined boundary replaces it. The arguments below should be read as canonical-development rather than as exploration. The notational convention $\Z^3$ used throughout this paper denotes the undefined-boundary cubic substrate---at every specified position the substrate exists with 26 Moore neighbors, and no specified position is the boundary---rather than the totalized integer lattice as a completed object.
...
(electric-source sign per the Term~2 amendment of 2026-07-18; the bound below uses
$|{\cdot}|$ throughout and is unaffected by it).
...
\textbf{Response:} Under the canonical undefined-boundary stance (see Preamble), $\Z^3$ is not claimed to be a completed-infinite totality; at every specified position, the substrate exists with 26 Moore neighbors, and no specified position is the boundary. Independent of that ontological point, no observer can access an unboundedly large number of voxels.
...
The substrate has no defined boundary, but every observable quantity is computed from a finite causal region.
\end{statusbox}
