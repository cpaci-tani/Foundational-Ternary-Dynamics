...
(Part~I). And the \emph{same} ontology \textbf{provably does not force} the
electromagnetic coupling $\alpha$ without an additional binding law: the boundary
is route-invariant, built from theorem-grade masonry around one
precisely-located open kernel, and its honest verdict is that $\alpha$ is
\emph{dynamical, not structural} (Part~II). \textbf{Both halves are the
deliverable.} This is the point that is easiest to miss and most important to
hold: the second finding is not the first finding's failure. A mapped
boundary (\emph{this} is what the ontology reaches, and \emph{this} is the
precise place it stops) is a result in its own right, a different and
non-trivial kind of result, one easy to mistake for a failure. A program that
exhibited only the spine would be a curiosity; one that maps the wall, in both
directions and at honest tags, is doing the thing the goal was set to do.
...
and can exhibit, also with proofs, that the same ontology does not force a
specific physical coupling. The effectiveness is real on the forced side and
absent on the unforced side, and the boundary between them is mapped rather than
asserted. That is the most the project claims, and it claims it at the tags the
claim can bear.
