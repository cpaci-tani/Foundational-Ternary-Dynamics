...
$1/\alpha$ to $\ppmval$~ppm) is real, and it is a \emph{match}: it
carries the tag \etag{smc}, not a derivation. (An earlier version of this
paragraph called the look-elsewhere scan ``the strongest structural evidence the
framework holds --- a unique dual-matcher across millions of candidate
polynomials.'' That claim is withdrawn: the scan sits at its chance base rate
and its uniqueness reading did not survive audit --- FTD-0791/0802/0321; see
Part~III \S\ref{sec:uniqueness} for the corrected state.) Part~III presents the
match at its honest tags, alongside the \etag{par} physics insertions.
