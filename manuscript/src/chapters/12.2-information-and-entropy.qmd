...
::: {.callout-note}
## Key Revelation
Information is physical. Erasing a bit releases heat. Entropy measures what we do not know. The second law drives the universe toward equilibrium: but pockets of order can persist, borrowing from the cosmic entropy budget.
:::
...
For quantum systems, Shannon entropy is insufficient. We need the **von Neumann entropy**:
$$
S_{vN} = -\text{Tr}(\rho \ln \rho)
$$

where $\rho$ is the **density matrix** describing the quantum state.

### Density Matrix in FTD

In the FTD Hilbert space H_TRD = L²(Lattice, ℂ), the density matrix for a pure state |ψ⟩ is:
$$
...
For entangled systems A ⊗ B, the **entanglement entropy** measures quantum correlations:
$$
...
::: {.callout-note}
## FTD Connection
In FTD, entangled pairs created via pair production have:
- **Pure total state**: S_vN(AB) = 0
- **Mixed subsystems**: S_vN(A) = S_vN(B) = ln(2)
...
For ground states of local Hamiltonians, entanglement entropy scales with boundary area:
$$
...
At room temperature: ~$3 \times 10^{-21}$ J per bit.

Information is physical. Computation has thermodynamic cost.

### Maxwell's Demon (Standard Resolution)

![Maxwell's demon attempts to sort fast and slow molecules, but the second law is preserved because storing and erasing information about each molecule requires entropy production.](../../media/images/fig-maxwells-demon.png){#fig-maxwells-demon}

The resolution: the demon must store information about each molecule's velocity, and erasing that information (Landauer's principle) costs entropy. The second law remains inviolate.
...
The universe begins in the all-Void state (s = 0 everywhere), the minimum entropy configuration. As time progresses, genesis creates particles that clump and form structures, increasing local complexity while global entropy rises. Eventually the universe returns toward void, approaching maximum entropy in the heat death.
...
1. **Anthropic**: Only universes with low initial entropy produce observers. We could not be asking this question otherwise.
...
## Societal Noetics: Information as Perturbation

The principles of entropy and information extend to the societal level.

### Cultural Objectivity
Societies maintain a **Shared Epistemic Model** ($\Phi_{culture}$) based on accumulated information. This "Cultural Objectivity" behaves like a physical object with **Noetic Inertia**: it resists change.

### Propaganda as Perturbation
"Propaganda" is the injection of **Informational Entropy** designed to destabilize $\Phi_{culture}$.
1.  **Perturbation:** High-intensity flux (narrative) disrupts the existing coherence of the cultural model.
2.  **Restructuring:** The model re-condenses around the new attractor (the propagated truth).
3.  **The Limit:** This manipulates the **Epistemic Domain** (meaning), but cannot alter the **Ontic Domain** (physics). No amount of propaganda can change the value of $G^*$.

This distinguishes **Political Truth** (mutable/informational) from **Physical Truth** (immutable/geometric).
...
- **shannon-entropy**: Measure of uncertainty in classical information
- **boltzmann-entropy**: Thermodynamic entropy from microstate counting
- **von-neumann-entropy**: Quantum entropy: S = -Tr(ρ ln ρ)
- **density-matrix**: Quantum state representation for pure and mixed states
- **entanglement-entropy**: Measures quantum correlations in bipartite systems
